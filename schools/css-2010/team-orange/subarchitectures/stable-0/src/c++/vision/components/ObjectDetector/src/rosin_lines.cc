...
void LineSegmenter::RosinFitLinesToSegment(unsigned seg)
{
  const int MAX_LINES = 100;
  /* note: list indexing goes from 1 (not 0!) to SIZE-1 */
  /* original list of pixels */
  float x_c[Z::Segments(seg)->edgels.Size() + 1];
  float y_c[Z::Segments(seg)->edgels.Size() + 1];
  ...
  if(segment_lines(x_c, y_c, Z::Segments(seg)->edgels.Size(),
      x_start, y_start, x_end, y_end, line_start, line_end, sigs, &n_lines))
  {
    for(int i = 0; i < n_lines; i++)
    {
      ...
      if(line_end[i] - line_start[i] + 1 >= 3)
        Z::NewGestalt(new Z::Line(seg, line_start[i] - 1, line_end[i] - 1));
    }
  }
}
