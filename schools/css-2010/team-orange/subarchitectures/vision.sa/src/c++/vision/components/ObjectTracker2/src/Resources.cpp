...
Resources::~Resources(){
	...
	if(m_showlog) g_Messages->log("Resources released", "Resources::~Resources()");
}
...
SDL_Surface* Resources::InitScreen(int width, int height){
	if(!m_screen){
		...
		SDL_GL_SetAttribute(SDL_GL_ALPHA_SIZE, 8);	// activate alpha channel
		//glPixelStorei(GL_PACK_ALIGNMENT, 8);		// supposed to be faster
		//glPixelStorei(GL_UNPACK_ALIGNMENT, 8);		// supposed to be faster
...
OrthoSearch* Resources::InitOrthoSearch(float width, float height){
	if(!m_ortho_search)
		m_ortho_search = new OrthoSearch(width, height);
		
	return m_ortho_search;
}
...
void Resources::ReleaseOrthoSearch(){
	if(m_ortho_search)
		delete(m_ortho_search);
	m_ortho_search = 0;
}
...
OrthoSearch* Resources::GetOrthoSearch(){
	if(!m_ortho_search){
		printf("[Resources::GetOrthoSearch] Error OrthoSearch not initialised\n" );
		return 0;
	}
	return m_ortho_search;
}
