...
public class DiscriminativeLearner {
	...
	public void onlineLearning() {	
		...
		for (int i = 0; i < nbIterations; i++) {
			...
			String alteredUtterance = example.getUtterance(); 
			    //GenerationUtils.produceASRErrors(example.getUtterance());
			example.setRecognizedUtterance(alteredUtterance);

			log("\tInitial utterance: \"" + example.getUtterance() +"\"");
			if (!example.getRecognizedUtterance().equals(example.getUtterance())) {
				log("\tUtterance + random ASR errors: \"" + example.getRecognizedUtterance() +"\"");
			}
		...
		for (int i = nbexistingfiles; i < nbIterations + nbexistingfiles ; i++) {
			...
			if (parser.verifyExample(example)) {
					...
					try {
						...
						tts.saveToFile(example.getRecognizedUtterance(), wavfilesDir+"utt"+i+".wav");
					...
					transcriptionsText += wavfilesDir+"utt"+i+".wav " + example.getUtterance() + "\n";
	...
	public void batchLearning() {	
		...
		for (Enumeration<String> e = lines.elements(); e.hasMoreElements(); ) {
			...
			if (colonIndex >0) {
				...
				String semantics = cfg.getSemantics(GenerationUtils.modifyCase2(example.utterance));
				if (semantics.length() == 0 | 
					...
					log("Warning: no correct semantics found for \"" + example.utterance +"\"");
				}
				else {
		...
		for (Enumeration<TrainingExample> e = examples.elements(); e.hasMoreElements(); ) {
			...
			log("\tInitial utterance: \"" + example.getUtterance() +"\"");
			log("\tRecognized utterance: \"" + example.getRecognizedUtterance() +"\"");
			log("\tVerifying semantics using CCG grammar...\t" );
			if (parser.verifyExample(example)) {	
				PackedLFs plf = parser.getLastParse();
				params = LearningUtils.incrInitPV(example, params, plf);
	...
	public void storeFinalOutput() {
		System.out.println("");
		log("Number of examples examined: " + bold(nbGoodExamples + "/" + nbIterations));
