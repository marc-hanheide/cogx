 ...
 * @version $Revision: 1.26 $, $Date: 2005/10/20 18:49:42 $
 */
public class CrossValidateRealizer implements ScorerMaker {
    ...
    public void prepScorers() throws IOException {
        ...
        // do each fold
        for (int i = 0; i < numFolds; i++) {
            // make training/test URLs for fold
            URL trainURL = new File(tmpDir, trainingFileName(i)).toURL();
            URL testURL = new File(tmpDir, testFileName(i)).toURL();
            // prep scorer
            scorerMaker.prepScorer(tmpDir, i, trainURL, testURL);
        }
        // summary
    ...
    public SignScorer loadScorer(File tmpDir, int foldNum, URL trainURL) throws IOException {
        RegressionInfo trainingItems = new RegressionInfo(tester.grammar, trainURL.openStream());
    ...
    public void runTest() throws IOException {
        ...
        for (int i = 0; i < numFolds; i++) {
            ...
            URL testURL = new File(tmpDir, testFileName(i)).toURL();
            // load scorer
            tester.scorer = scorerMaker.loadScorer(tmpDir, i, trainURL);
            // run test
            tester.runTest(testURL);
        }
    ...
    public static void main(String[] args) throws IOException { 
        ...
        // set-up inputs
        URL testbedURL = new File(testbedfile).toURL();
