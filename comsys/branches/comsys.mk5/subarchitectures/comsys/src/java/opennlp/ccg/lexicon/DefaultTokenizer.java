 ...
 * @version     $Revision: 1.24 $, $Date: 2005/10/20 17:30:30 $
 **/
...
public class DefaultTokenizer implements Tokenizer {
     ...
     */
    public String getOrthography(List words) {
     ...
     */
    public String getOrthography(List words, boolean semClassReplacement) {
    ...
    public String getOrthography(Word w, boolean semClassReplacement) {
        ...
        for (Iterator it = w.getAttrValPairs(); it.hasNext(); ) {
            Pair p = (Pair) it.next();
    ...
    public String format(Word w, boolean semClassReplacement) {
        ...
        String semClass = w.getSemClass();
        if (semClassReplacement && semClass != null && replacementSemClasses.contains(semClass)) {
            form = semClass.toUpperCase(); stem = form; 
        }
        if (form != null) sb.append(escape(form));
            ...
            if (c == '&') {
                ...
                if (endPos < 0) {
                    throw new RuntimeException(
                        "Unable to find matching ';' following '&' at position " + i + " in: " + s
                    );
                }
