...
public abstract class AbstractCompositionRule extends AbstractApplicationRule {
    ...
    protected List<Category> apply (Category xyCat, Category yzCat)
        throws UnifyFailure {
        ...
        if (xyCat instanceof ComplexCat && yzCat instanceof ComplexCat) {
            ...
            if (xyOuter instanceof BasicArg) {
                ...
                if (xyOuterCat instanceof AtomCat) {
                    ...
                    Category outcome =
                        createResult(xyCC.getResult(), zStack, xySlash, sub);
		    ...
		    ArgStack zStack =
			composeComplexY((ComplexCat)xyOuterCat, xySlash, yzCC, sub);
		    xySlash = (Slash)xySlash.fill(sub);
		    xySlash.unifyCheck(_functorSlash);
		...
		if (targetIndex > -1) {
			...
			if (yInSet.getCat() instanceof ComplexCat) {
			    ...
			    ArgStack zStack =
				composeComplexY((ComplexCat)yCat, xySlash, yzCC, sub);
			    xySlash = (Slash)xySlash.fill(sub);
			    xySlash.unifyCheck(_functorSlash);
			    Category result = xyCC.copy();
    ...
    private Category createResult (Category result,
                                   ArgStack zStack,
                                   Slash xySlash,
                                   Substitution sub) throws UnifyFailure 
    {
        ...
        if (!_isHarmonic
            && (!xySlash.sameDirAsModality()
                || zStack.containsContrarySlash())) {
    ...
    /**
     * A function that tries to unify the value ant=+ into feature
     * structures.
     */
    ...
    private ArgStack composeComplexY (ComplexCat xyOuterCC, Slash xySlash,
				      ComplexCat yzCC, Substitution sub)
	throws UnifyFailure {
	
	GUnifier.unify(xyOuterCC.getTarget(),
		       yzCC.getTarget(),
		       sub);
	ArgStack zStack = yzCC.getArgStack();
	if (xyOuterCC.containsDollarArg()) {
	    // e.g. s$/(s$\n) s\n/n
	    xyOuterCC.getArgStack().unifyPrefix(zStack,
						zStack.size()-1,
						sub);
    ...
    //private void composeComplexY (ComplexCat xyOuterCC, ComplexCat yzCC,
    //				    Substitution sub)
    //throws UnifyFailure {
    //	  
    //	  GUnifier.unify(xyOuterCC.getTarget(),
