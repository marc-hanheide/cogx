 ...
 * @author  Gunes Erkan
 * @version $Revision: 1.36 $, $Date: 2005/10/20 18:49:42 $ 
 */
public class Grammar {
    ...
    public synchronized void serializeXml(Document doc, OutputStream out) throws IOException {
        try {
            initializeTransformers();
            // nb: OutputStreamWriter nec. to get indenting to work with Java 1.5
            transformer.transform(new JDOMSource(doc), new StreamResult(new OutputStreamWriter(out)));
        } catch (TransformerException exc) { 
            throw (IOException) new IOException().initCause(exc);
        }
    }
