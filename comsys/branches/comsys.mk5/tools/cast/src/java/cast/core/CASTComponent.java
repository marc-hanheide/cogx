...
public abstract class CASTComponent implements _CASTComponentOperations,
	 ...
	 */
	protected <Type extends Ice.Object> void registerIceServer(
			Class<Type> _interface, Type _servant) {
	 ...
	 * @param _servant
	 */
	protected <Type extends Ice.Object> void registerIceInterface(String name,
			String category, Type _servant) {
	 ...
	 * @throws CASTException
	 */
	protected <Type extends Ice.Object, TypePrx extends Ice.ObjectPrx> TypePrx getIceServer(
		...
		int port = CASTUtils.languageToPort(desc.language);
		ObjectPrx prx = getIceServer(_componentID, CASTUtils
				.toServantCategory(_cls), host, port);
		...
		try {
			...
			TypePrx checkedPrx = _prxCls.cast(checkedCastMethod.invoke(null,
					prx));
			if(checkedPrx == null) {
				throw new CASTException("Unable to cast returned proxy to " + _prxCls); 
			}
			return checkedPrx;
		} catch (Exception e) {
	 ...
	 * @return
	 */
	protected ObjectPrx getIceServer(String _componentID, String _category,
			String _host, int _port) {
		Identity id = new Identity(_componentID, _category);

		StringBuilder details = new StringBuilder(getCommunicator()
				.identityToString(id));
		...
		ObjectPrx prx = getCommunicator().stringToProxy(details.toString());
		return prx;
	}
