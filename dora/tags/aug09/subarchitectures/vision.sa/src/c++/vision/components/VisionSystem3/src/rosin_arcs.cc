...
void FitArcsToSegment(unsigned seg)
{
printf("rosin_arcs.cc:FitArcsToSegment(): Possible division by zero!\n");
    int j;
    float sig;    /* not really used - just dummy param for segment */
    unsigned new_arc, prev_arc = Z::UNDEF_ID;

    number_pixels = 0;
    for(unsigned i = 0; i < Z::Segments(seg)->edgels.Size(); i++)
    {
      // note Rosin/West start arrays at 1, I start at 0
      number_pixels++;
      x_c[number_pixels] = (int)Z::Segments(seg)->edgels[i].p.x;
      y_c[number_pixels] = (int)Z::Segments(seg)->edgels[i].p.y;
    }
    ...
    for (j = 1;j <= number_arcs;j++)
        if (arc_status[j] == KEEP) {
            if (!arc_line[j]) 
            {
              ...
              {
                Z::Vector2 cent(arc_centre_x[j], arc_centre_y[j]);
                // note Rosin/West start arrays at 1, I start at 0
                new_arc = Z::NewGestalt(new Z::Arc(seg,
                        (unsigned)arc_start[j] - 1,
                        (unsigned)(arc_start[j] - 1 + arc_finish[j] - 1)/2,
                        (unsigned)arc_finish[j] - 1,
                        cent, (double)radii[j]));
                if(prev_arc != Z::UNDEF_ID)
                {
                  Z::Arcs(new_arc)->prev_on_seg = prev_arc;
                  Z::Arcs(prev_arc)->next_on_seg = new_arc;
                }
              }
            }
        }
