...
{
  if( Arcs(*(unsigned*)a)->sig > Arcs(*(unsigned*)b)->sig )
    return -1;  // a is first
  else
    return 1 ;  // b is first
}

FormArcs::FormArcs(Config *cfg)
: GestaltPrinciple(cfg)
{
  // RANSAC is the default method of: SPLIT, GROW, RANSAC
  config->AddItem("ARC_FIT_METHOD", "RANSAC");
  done = false;
}

void FormArcs::Reset(const Image *img)
{
  done = false;
}
...
void FormArcs::Operate(bool incremental)
{
  ...
  if(!done)
  {
    if(config->GetValueString("ARC_FIT_METHOD") == "SPLIT")
      FIT_METHOD = FIT_ARCS_SPLIT;
    else if(config->GetValueString("ARC_FIT_METHOD") == "GROW")
      FIT_METHOD = FIT_ARCS_GROW;
    else if(config->GetValueString("ARC_FIT_METHOD") == "RANSAC")
      FIT_METHOD = FIT_ARCS_RANSAC;
    else
    {
      fprintf(stderr, "* arcs: unkown fit method: %s\n",
          config->GetValueString("ARC_FIT_METHOD").c_str());
      return;
    }
    Create();
    Rank();
...
void FormArcs::Create()
{
  for(unsigned rank = 0; rank < NumSegments(); rank++)
  {
    ...
    {
      FitArcRANSAC(seg, 0, Segments(seg)->edgels.Size() - 1);
    }
  }
}
 ...
 */
bool FormArcs::FitArc(unsigned seg, unsigned *cur_pos)
{
  Array<Edgel> &edgels = Segments(seg)->edgels;
  unsigned i = *cur_pos;
    ...
    try
    {
      ...
      if(k-i+1 >= 6)
      {
        ...
        if(radius > MIN_RADIUS && radius < MAX_RADIUS)
          NewGestalt(new Arc(seg, i, j, k, center, radius));
      }
    }
    catch(Except &e)
    {
 ...
 */
void FormArcs::FitArcRANSAC(unsigned seg, int l, int u)
{
...
/*  if(seg == 11)
  ...
  if(sup_max > 0)
  {
    ...
    if(k - i + 1 >= 6 && radius_opt > MIN_RADIUS && radius_opt < MAX_RADIUS)
      NewGestalt(new Arc(seg, i, j, k, center_opt, radius_opt));
    if(i > l)
