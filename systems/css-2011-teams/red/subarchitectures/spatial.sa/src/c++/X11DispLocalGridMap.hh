...
public:
     ...
     */
    X11DispLocalGridMap(LocalGridMap<MAPDATA> &lm,
                       int magnification = 1);
    ...
    void updateDisplay(Cure::Pose3D *robPose = 0,
                       NavGraph *graph = 0,
                       std::list<FrontierPt> *fPts = 0, int m_samplesize = 0, 
                       int* samples = 0, std::vector<int> tpoints = 0,
                       std::vector<std::vector<int> > ViewConePts = 0,
...
template <class MAPDATA>
void
X11DispLocalGridMap<MAPDATA>::setRobotPose(double x, double y, double a) {
    Cure::Pose3D p;
    p.Position.X[0] = x;
    p.Position.X[1] = y;
    p.setTheta(a);
...
template <class MAPDATA>
void
X11DispLocalGridMap<MAPDATA>::setRobotPose(const Pose3D &p) {
    m_Xr = p;
...
template <class MAPDATA>
void
X11DispLocalGridMap<MAPDATA>::updateCoverageDisplay() {
...
template <class MAPDATA>
void
X11DispLocalGridMap<MAPDATA>::updateDisplay(Pose3D *robPose,
        NavGraph *graph,
        std::list<FrontierPt> *fPts, int m_samplesize, int* samples,
        std::vector<int> tpoints,std::vector<std::vector<int> > ViewConePts,
        int highestVCindex, std::vector<Cure::Pose3D> plan,
        std::vector<int> planindex) {
    ...
    for (int i= 0; i < m_samplesize; i++) {
      for (int mx = 0; mx < m_Magnification; mx++) {
        for (int my = 0; my < m_Magnification; my++) {
          ...
          gridPt.y = 2 * msize - (samples[2*i +1]*m_Magnification+my + msize);
          XDrawPoints(disp, pixmap, gcGreen, &gridPt, 1, CoordModeOrigin);
        }
      }
    }
    ...
    if (fPts) {
      ...
      for (std::list<FrontierPt>::iterator fi = fPts->
        ...
        if (m_Map.worldCoords2Index(fi->getX(), fi->getY(), i, j) == 0) {
          ...
          double halfLen = 0.5 * fi->m_Width / m_Map.getCellSize()*m_Magnification;
