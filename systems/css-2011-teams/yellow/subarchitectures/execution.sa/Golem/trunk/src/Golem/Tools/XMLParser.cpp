...
const std::string& XMLContext::getAttribute(const std::string& attribute) const {
	...
	if (pos == attributeMap.end())
		throw MsgXMLParserAttributeNotFound("XMLContext::getAttribute(): Attribute not found: %s %s", getName().c_str(), attribute.c_str());
	return pos->second;
}
...
XMLContext* XMLContext::createContext(const char* name) {
	if (name == NULL || *name == '\0')
		throw MsgXMLParserInvalidName("XMLContext::createContext(): Invalid name at: %s", getName().c_str());
...
XMLContext* XMLContext::getContextFirst(const char* name, bool create) const {
	if (name == NULL)
		throw MsgXMLParserInvalidName("XMLContext::getContextFirst(): Invalid name at: %s", getName().c_str());
	...
	for (;;) {
		...
		if (p0 == p1)
			throw MsgXMLParserInvalidName("XMLContext::getContextFirst(): Invalid name at: %s", result->getName().c_str());
		...
		if (i != result->contextMap.end())
			...
			throw MsgXMLParserNameNotFound("XMLContext::getContextFirst(): Name not found: %s %s", result->getName().c_str(), key.c_str());
...
bool XMLParser::create(const Desc &desc) {
	if (!desc.isValid())
...
void XMLParser::load(const Stream &stream) {
	...
	for (;;) {
		...
		switch (XML_ParseBuffer(parser, bytesRead, isFinal)) {
