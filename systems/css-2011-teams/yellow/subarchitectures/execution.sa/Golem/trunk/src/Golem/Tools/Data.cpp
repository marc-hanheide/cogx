...
void golem::XMLData(Twist &val, Real& theta, XMLContext* context, bool create) {
	...
	XMLData("theta", theta, context, create);
}
...
template <> void Stream::read(BoundingPlane::Desc& desc) const {
	*this >> (Bounds::Desc&)desc >> desc.normal >> desc.distance;
}

template <> void Stream::write(const BoundingPlane::Desc& desc) {
	*this << (const Bounds::Desc&)desc << desc.normal << desc.distance;
}

template <> void Stream::read(BoundingSphere::Desc& desc) const {
	*this >> (Bounds::Desc&)desc >> desc.radius;
}

template <> void Stream::write(const BoundingSphere::Desc& desc) {
	*this << (const Bounds::Desc&)desc << desc.radius;
}
...
void golem::XMLData(Bounds::Desc::Ptr &val, XMLContext* context, bool create) {
	...
	if (create) {
		...
		if (!type.compare("plane")) {
			...
			throw MsgXMLParser("XMLData(): unknown bounds type: %s", type.c_str());;
	}
}
...
void golem::XMLData(BoundingConvexMesh::Desc &val, XMLContext* context, bool create) {
	ASSERT(context)
	XMLData((Bounds::Desc&)val, context, create);
	XMLData(val.vertices, context, "vertex");
	val.bCook = true;
	try {
		XMLData(val.triangles, context, "triangle");
		val.bCook = false;
	}
...
void golem::XMLData(MessageStream::Desc::Ptr& desc, XMLContext* context, bool create) {
	...
	desc->filter.reset(
		level=="debug"		? new LevelFilter(Message::LEVEL_DEBUG)		:
		level=="info"		? new LevelFilter(Message::LEVEL_INFO)		:
		level=="warning"	? new LevelFilter(Message::LEVEL_WARNING)	:
		level=="error"		? new LevelFilter(Message::LEVEL_ERROR)		:
		level=="crit"		? new LevelFilter(Message::LEVEL_CRIT)		:
		level=="alert"		? new LevelFilter(Message::LEVEL_ALERT)		:
		level=="emerg"		? new LevelFilter(Message::LEVEL_EMERG)		: NULL
	);
}
