		...
		do {
			...
			for (Threads::iterator t = threads.begin(); t != threads.end(); t++) {
				...
				if (lastest->second < time - threadTimeOut) {
					bOK = false;
					pContext->getLogger()->post(Message::LEVEL_DEBUG, "ThreadWatch: thread: %u, point: %d, time out: %f",
			...
			//if (bOK)
			//	pContext->getLogger()->post(Message::LEVEL_DEBUG, "ThreadWatch: OK");

		} while (!evStop.wait(MSecTmU32(1000*threadTimeOut)));
	}

public:
	ThreadWatch(SecTmReal threadTimeOut = SecTmReal(15.0), MSecTmU32 exitTimeOut = 5000) : threadTimeOut(threadTimeOut), exitTimeOut(exitTimeOut), pContext(NULL) {
	}
	virtual ~ThreadWatch() {
		evStop.set(true);
