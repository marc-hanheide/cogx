...
class Extremum {
public:
	typedef shared_ptr< Extremum<REAL> > Ptr;
	...
	virtual bool findLocal(REAL &x, REAL &y, const Function<REAL> &function, REAL a, REAL b, bool min = true) const = 0;

	/** Finds function global extremum within specified interval [left, right] */
	virtual bool findGlobal(REAL &x, REAL &y, const Function<REAL> &function, REAL left, REAL right, bool min = true) const = 0;
};

template <typename REAL> const REAL Extremum<REAL>::GAIN_DFLT = REAL(100.0);
template <typename REAL> const REAL Extremum<REAL>::EPS_DFLT = REAL(1.0e-6);

//------------------------------------------------------------------------------

template <typename REAL>
class ExtremumBracket : public Extremum<REAL> {
protected:
	const U32 partitions;
	const REAL gain;

	ExtremumBracket(U32 partitions, REAL gain) :
		partitions(partitions), gain(gain)
	{}

	/** Brackets the extremum starting with the initial interval [a, b] */
	bool searchLocal(REAL &a, REAL &b, REAL &c, const Function<REAL> &function, bool min = true) const {
	...
	bool searchGlobal(REAL &a, REAL &b, REAL &c, const Function<REAL> &function, bool min = true) const {
		const REAL sign = min ? REAL(1.0) : -REAL(1.0);
	...
	virtual bool find(REAL &x, REAL &y, const Function<REAL> &function, REAL a, REAL b, REAL c, bool min = true) const = 0;

public:
	/** Finds function extremum within specified interval */
	virtual bool findLocal(REAL &x, REAL &y, const Function<REAL> &function, REAL left, REAL right, bool min = true) const {
	...
	/** Finds function extremum within specified interval */
	virtual bool findGlobal(REAL &x, REAL &y, const Function<REAL> &function, REAL left, REAL right, bool min = true) const {
...
template <typename REAL>
class ExtremumGold : public ExtremumBracket<REAL> {
protected:
	const REAL epsilon;

	bool find(REAL &x, REAL &y, const Function<REAL> &function, REAL a, REAL b, REAL c, bool min = true) const {
		const REAL sign = min ? REAL(1.0) : -REAL(1.0);
...
public:
	ExtremumGold(U32 partitions = Extremum<REAL>::PARTITIONS_DFLT, REAL epsilon = Extremum<REAL>::EPS_DFLT, REAL gain = Extremum<REAL>::GAIN_DFLT) :
		ExtremumBracket<REAL>(partitions, gain), epsilon(epsilon)
	{}
};
