...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict, mutex_ranges, implied_facts):
    ...
    replancond = translate_strips_conditions(operator.replan, dictionary, ranges, mutex_dict, mutex_ranges)
    ...
    return sas_tasks.SASOperator(operator.name, prevail, replan, pre_post, operator.cost)
...
def pddl_to_sas(task, mutex_file):
    ...
    with timers.timing("Computing fact groups", block=True):
        groups, mutex_groups, translation_key = fact_groups.compute_groups(
            task, atoms, mutex_file, partial_encoding=USE_PARTIAL_ENCODING)
...
#    with timers.timing("Inverting task"):
#        invert_sas_task(sas_task, translation_key)
...
def invert_sas_task(task, translation_key):
    required_axioms = {}
    def compat_axiom(var, value):
        if (var,value) in required_axioms:
            return required_axioms[var, value][0]
        task.variables.ranges.append(2)
        task.variables.axiom_layers.append(task.variables.axiom_layers[var]+1)

        key = translation_key[var][value]
        translation_key.append(["not compatible: %s" % key, "compatible: %s" % key])
        
        newvar = len(task.variables.ranges)-1
        axioms = [sas_tasks.SASAxiom([(var, value)], (newvar, 1)), sas_tasks.SASAxiom([(var, task.variables.ranges[var]-1)], (newvar, 1))]
                           
        required_axioms[var, value] = (newvar , axioms)
        return newvar
    
    assert not task.axioms

    varcount = len(task.variables.ranges)
    
    #step 1: add "unknown" to each variable
    #for i, val in enumerate(task.variables.ranges):
    #    task.variables.ranges[i] +=1

    #translate init to goal
    goalpairs = []
    for i, val in enumerate(task.init.values):
        goalpairs.append((compat_axiom(i, val), 1))
    ...
    for op in task.operators:
        ...
        for var, val in op.prevail:
            prevail.append((compat_axiom(var, val), 1))
            pre_post.append((var, -1, val, []))
        if len(op.pre_post) == 1:
            var, pre, post, cond = op.pre_post[0]
            if pre == -1:
                pre_post.append((var, post, task.variables.ranges[var]-1, []))
            else:
                pre_post.append((var, post, pre, []))
        else:
            for var, pre, post, cond in op.pre_post:
                assert not cond
                prevail.append((compat_axiom(var, post), 1))
                if pre == -1:
                    pre_post.append((var, -1, task.variables.ranges[var]-1, []))
                else:
                    pre_post.append((var, -1, pre, []))
        operators.append(sas_tasks.SASOperator(op.name, prevail, [], pre_post, op.cost))

    #new initial state
    init = [0] * len(task.variables.ranges)
    for var, range in enumerate(task.variables.ranges[:varcount]):
        init[var] = range-1
    for var, val in task.goal.pairs:
        init[var] = val

    task.goal = sas_tasks.SASGoal(goalpairs)
    task.init = sas_tasks.SASInit(init)
    task.operators = operators
    for var, axioms in required_axioms.itervalues():
        task.axioms += axioms
