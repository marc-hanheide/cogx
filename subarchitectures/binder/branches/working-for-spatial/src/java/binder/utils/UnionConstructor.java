...
/**
 * Library to construct new binding unions, and compute their related
 * probability distributions (based on the information contained in the included
 * proxies, and the bayesian network to compute the internal coherence of the
 * whole)
 * 
 * @author Pierre Lison
...
public class UnionConstructor {
	...
	private static Logger m_logger = ComponentLogger
			.getLogger(UnionConstructor.class);
	...
	/**
	 * Construct a new union constructor, bayed on the specification of a
	 * bayesian network
	 * 
	 * @param bayesiannetworkfile
	 *            the file containing the bayesian network
	 */
	...
	/**
	 * Set the alpha parameter to control the binder greediness (higher -->
	 * greedier binder)
	 * 
	 * @param alpha
	 *            the alpha parameter
	...
	/**
	 * Construct an initial, single-proxy union with a new data ID, and a
	 * timestamp
	 * 
	 * @param proxy
	 *            the proxy to include in the initial union
	 * @param newDataID
	 *            the entity ID of this new union
	 * @param timestamp
	 *            the timestamp
	 * @return the new union
	 */

	public Union constructInitialUnion(Proxy proxy, String newDataID,
			CASTTime timestamp) {
	...
	/**
	 * Construct a new basic union (i.e. not a relation union) based on the
	 * merge of several perceptual entities (which may be proxies or unions
	 * themselves)
	 * 
	 * @param includedEntities
	 *            as vector of entities (with size > 0 )
	...
	private Union constructNewBasicUnion(
		...
		Vector<Proxy> includedProxies = BinderUtils
				.getProxies(includedEntities);
		union.includedProxies = new Proxy[includedProxies.size()];
		union.includedProxies = includedProxies.toArray(union.includedProxies);

		// TODO: check correctness of prob. exists computation, especially for
		// single-proxy unions!
	...
	private Feature cloneFeature(Feature feat) {
		...
		for (int i = 0; i < feat.alternativeValues.length; i++) {
			newFeat.alternativeValues[i] = FeatureValueUtils
					.cloneFeatureValue(feat.alternativeValues[i]);
		}
		return newFeat;
	}
	...
	/**
	 * Computes the probability distribution of the union, in three steps: (1)
	 * computes the prior distribution of the union using the bayesian network,
	 * (2) compute the proxy distributions for each proxy, and multiply them,
	 * (3) finally, multiply the prior and the proxy distributions, and combines
	 * the result with a multiplication factor to get the final distribution
	 * 
	 * @param union
	 *            the union
	...
	public ProbabilityDistribution computeUnionDistribution(Union union) {
		...
		DiscreteProbabilityDistribution priorDistrib = BNManager
				.getPriorDistribution(union);
		...
		for (int i = 0; i < union.includedProxies.length; i++) {
			...
			// create a new combined probability distribution to specify the
			// full proxy distribution
			...
			// define the first subdistribution to be the posterior distribution
			// of the proxy
			proxyDistrib.distributions[0] = proxy.distribution;

			// computes the prior distribution of the proxy
			DiscreteProbabilityDistribution priorDistribForProxy = BNManager
					.getPriorDistribution(proxy);

			// inverts the result, and define it as the second subdistribution
			proxyDistrib.distributions[1] = ProbabilityUtils
					.invertDistribution(priorDistribForProxy);

			// adds the proxy distribution to the list of all proxy
			// distributions
		...
		finalDistrib.distributions = new ProbabilityDistribution[proxiesDistrib
				.size() + 1];
		...
		float multiplicationFactor = 1.0f / ((float) Math.pow(ALPHA_CONST,
				(union.includedProxies.length - 1)));
		finalDistrib.distributions[0] = ProbabilityUtils
				.multiplyDistributionWithConstantValue(priorDistrib,
						multiplicationFactor);

		// and define all the other subdistributions to be the proxy
		// distributions
	 ...
	 * 
	 * @param includedEntities
	 *            the perceived entities
	 * @return the list of features
	 */

	private Collection<Feature> getFeatures(
			Vector<PerceivedEntity> includedEntities) {

		HashMap<String, Feature> features = new HashMap<String, Feature>();

		for (Enumeration<PerceivedEntity> e = includedEntities.elements(); e
				.hasMoreElements();) {
			...
			for (int i = 0; i < prox.features.length; i++) {
				...
				for (int j = 0; j < prox.features[i].alternativeValues.length; j++) {
					feat.alternativeValues[j] = FeatureValueUtils
							.cloneFeatureValue(prox.features[i].alternativeValues[j]);
				}

				if (!features.containsKey(feat.featlabel)) {
					features.put(feat.featlabel, feat);
				} else {
					Feature mergedFeature = mergeFeatures(features
							.get(feat.featlabel), feat);
	...
	private static Feature mergeFeatures(Feature feat1, Feature feat2) {
		...
		if (feat1.alternativeValues.length == 1
				&& feat2.alternativeValues.length == 1) {
			mergedFeature.alternativeValues = new FeatureValue[1];
			mergedFeature.alternativeValues[0] = FeatureValueUtils
					.mergeFeatureValues(feat1.alternativeValues[0],
							feat2.alternativeValues[0]);
		} else {
	 ...
	 * { Feature feat = new Feature(); feat.featlabel = "boundPhantom";
	 * feat.alternativeValues = new AddressValue[1]; feat.alternativeValues[0] =
	 * new AddressValue(1.0f, phantom.timeStamp, phantom.entityID); return feat;
	 * }
	 */

	private static void log(String s) {
		// m_logger.info("[UnionConstructor] " + s);
		m_logger.info(s);

	}

	private static void errlog(String s) {
		// m_logger.debug("[UnionConstructor] " + s);
		m_logger.debug(s);
	}
}
