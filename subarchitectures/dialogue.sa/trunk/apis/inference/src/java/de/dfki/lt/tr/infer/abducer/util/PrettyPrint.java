...
import de.dfki.lt.tr.infer.abducer.lang.ConstAssumabilityFunction;
...
public abstract class PrettyPrint {
	...
	/**
	 * Assuming the function term is a list, convert it to string.
	 * Returns the contents of the list, i.e. without the square brackets,
	 * or null if the function term is not a list.
	 *
	 * @param f the term
	 * @return contents of the list, or null if f is not a string
	 */
	public static String tryListTermToString(FunctionTerm f) {
		if (f.args.size() == 2 && f.functor.equals("[|]")) {
			Term head = f.args.get(0);
			Term tail = f.args.get(1);
			if (tail instanceof FunctionTerm) {
				FunctionTerm ftail = (FunctionTerm) tail;
				String tailStr = tryListTermToString(ftail);
				if (tailStr != null) {
					String headStr = termToString(head);
					if (tailStr.equals("")) {
						return headStr;
					}
					else {
		...
		else if (f.args.isEmpty() && f.functor.equals("[]")) {
			return "";
		}
		return null;
	}
	...
	public static String assumabilityFunctionToString(AssumabilityFunction f) {
		if (f instanceof NullAssumabilityFunction) {
			return "false";
		}
		if (f instanceof ConstAssumabilityFunction) {
			return Double.toString(((ConstAssumabilityFunction) f).cost);
		}
		if (f instanceof NamedAssumabilityFunction) {
			return ((NamedAssumabilityFunction) f).name;
		}
		return "UNK";
	}
	...
	public static String markedQueryToMarkingString(MarkedQuery q) {
		if (q instanceof UnsolvedQuery) {
			return "[UNSOLVED(" + assumabilityFunctionToString(((UnsolvedQuery) q).f) + ")]";
		}
		...
		if (q instanceof AssumedQuery) {
			...
			return "[assumed(" + assumabilityFunctionToString(((AssumedQuery) q).f) + ")]";
		}
