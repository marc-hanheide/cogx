 ...
 * @version $Revision: 1.7 $, $Date: 2007/06/20 21:16:04 $
 */
		...
		if (xyCat instanceof ComplexCat && yzCat instanceof ComplexCat) {
			...
			if (xyOuter instanceof BasicArg) {
				...
				if (xyOuterCat instanceof AtomCat) {
					...
					Category outcome = createResult(xyCC.getResult(), zStack,
							xySlash, sub);
					...
					ArgStack zStack = composeComplexY((ComplexCat) xyOuterCat,
							xySlash, yzCC, sub);
					xySlash = (Slash) xySlash.fill(sub);
					xySlash.unifyCheck(_functorSlash);
					Category outcome = createResult(xyCC.getResult(), zStack,
				...
				if (targetIndex > -1) {
					...
					xySlash.unifyCheck(_functorSlash);
					Substitution sub = new GSubstitution();
					GUnifier.unify(xyOuterSet.getCat(targetIndex), yzTarget,
							sub);
					Category result = xyCC.copy();
					((ComplexCat) result).setOuterArgument(xyOuterSet
							.copyWithout(targetIndex));
					ArgStack zStack = yzCC.getArgStack();
					zStack.slashesUnify(_argSlash);
					Category outcome = createResult(result, zStack, xySlash,
							sub);
					appendLFs(xyCat, yzCat, outcome, sub);
					results.add(outcome);
				} else {
					...
					for (int i = 0; i < xyOuterSet.size(); i++) {
						...
						if (yInSet.getCat() instanceof ComplexCat) {
							...
							ArgStack zStack = composeComplexY(
									(ComplexCat) yCat, xySlash, yzCC, sub);
							xySlash = (Slash) xySlash.fill(sub);
							xySlash.unifyCheck(_functorSlash);
							Category result = xyCC.copy();
							((ComplexCat) result).setOuterArgument(xyOuterSet
									.copyWithout(i));
							Category outcome = createResult(result, zStack,
									xySlash, sub);
							appendLFs(xyCat, yzCat, outcome, sub);
							results.add(outcome);
							success = true;
