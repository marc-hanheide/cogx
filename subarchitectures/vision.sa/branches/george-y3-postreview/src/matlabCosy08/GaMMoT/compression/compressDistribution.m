...
if isfield(f1, 'pars2')
    scaleErrorThreshold = f1.pars2.scaleErrorThreshold ;
    hellErrorGlobal = f1.pars2.hellErrorGlobal ;
end
...
%     tic
 % remove components weights
    ...
    [f0,alpha] = removeUnits( pruning, f0, f1, f_ref_data_X, f_ref_data_Y, reportProgress, hellErrorGlobal ) ;

    
%     H =[ H, uHellingerJointSupport( f0, f1 ) ] ;
...
% ----------------------------------------------------------------------- %
function [f0,alpha]= removeUnits( pruning, f0, f1, f_ref_data_X, f_ref_data_Y, reportProgress, hellErrorGlobal )
% f0 ... target
% f1 ... reference
