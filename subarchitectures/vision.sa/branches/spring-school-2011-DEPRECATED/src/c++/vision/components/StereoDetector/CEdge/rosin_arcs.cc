...
void FitArcsToSegment(P::Segment *seg, P::Array<P::Arc*> &arcs)
{
    int j;
    ...
    for (j = 1;j <= number_arcs;j++)
        if (arc_status[j] == KEEP) {
            if (!arc_line[j]) 
            {
              ...
              {
                P::Vector2 cent(arc_centre_x[j], arc_centre_y[j]);
                // note Rosin/West start arrays at 1, I start at 0
                unsigned a = (unsigned)arc_start[j] - 1;
                unsigned b = (unsigned)(arc_start[j] - 1 + arc_finish[j] - 1)/2;
                unsigned c = (unsigned)arc_finish[j] - 1;
                if (cent!=seg->edgels[a].p && cent!=seg->edgels[b].p && 
                    cent!=seg->edgels[c].p){
                  new_arc = new P::Arc(seg, a, b, c, cent, (double)radii[j]);
                  arcs.PushBack(new_arc);
                  if(prev_arc != 0)
                  {
