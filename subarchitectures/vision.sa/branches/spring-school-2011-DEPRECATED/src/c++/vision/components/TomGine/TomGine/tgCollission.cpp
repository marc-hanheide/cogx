...
bool tgCollission::IntersectRayTriangle(vec3& p, vec3& n, double& z, const Ray& ray, const vec3& t1, const vec3& t2, const vec3& t3)
{
...
bool tgCollission::IntersectRayModel(std::vector<vec3> &pl, std::vector<vec3> &nl, std::vector<double> &zl, const Ray& ray, const tgModel& model)
{
	bool hit = false;
	pl.clear();
	zl.clear();
