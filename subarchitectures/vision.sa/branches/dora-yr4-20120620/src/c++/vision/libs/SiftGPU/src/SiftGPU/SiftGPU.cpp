...
inline void SiftGPU::InitSiftGPU()
{
	...
	if(GlobalUtil::_UseCUDA)
	{
		...
		std::cerr	<< "---------------------------------------------------------------------------\n"
					<< "CUDA is not supported by this version of MSVC\n"
					<< "----------------------------------------------------------------------------\n";
	}
#elif !defined(CUDA_SIFTGPU_ENABLED)
	if(GlobalUtil::_UseCUDA)
	{
		GlobalUtil::_UseCUDA = 0;
		std::cerr	<< "---------------------------------------------------------------------------\n"
...
#else
    if(GlobalUtil::_UseCUDA &&  !PyramidCU::CheckCudaDevice(GlobalUtil::_UseCUDA - 1)) GlobalUtil::_UseCUDA = 0;
#endif
...
void SiftGPU::PrintUsage()
{
	...
	<<"-tc <int>   *     : Threshold for limiting the overall number of features\n"
	...
	<<"-winpos <X>x<Y> * : Window coordinate used in Win32 for select monitor/GPU.\n"
...
void SiftGPU::ParseParam(int argc, char **argv)
{
	...
	for(int i = 0; i< argc; i++)
	{
		...
		}else if(_stricmp(opt, "display")==0)
		{
		...
		{
			int num = 0;
			if(sscanf(param, "%d", &num) && num >=0 && num <=5)
			{
				SetVerbose(num);
			}
	...
	if(setMaxD == 0) GlobalUtil::_texMaxDim = (GlobalUtil::_usePackedTex || GlobalUtil::_UseCUDA) ? 3200 : 2560;
...
int SiftGPU::CreateContextGL()
{
	...
	if(!GlobalUtil::CreateWindowEZ()) return 0;
