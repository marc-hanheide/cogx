 ...
 */
KinectPclLines::KinectPclLines(KinectCore *kc, VisionCore *vc) 
               : KinectBase(kc, vc)
{
  numLines = 0;
  tgEngine = NULL;
 ...
 */
void KinectPclLines::ClearResults()
{
  numLines = 0;
}
 ...
 */
void KinectPclLines::Process()
{
  double fx, fy, cx, cy;
  kcore->GetScaledCameraParameters(fx, fy, cx, cy);
printf("KinectPclLines::Process: time to implement\n");
//   pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcl_pc2;
...
//   pclA::Dilation(pcl_pc2, pcl_pc, kcore->fx/rgb2depthScale, kcore->fy/rgb2depthScale, kcore->cx/rgb2depthScale,kcore->cy/rgb2depthScale, 1);
  ...
  // zMin, zMax, weightColorEdge, weightDepthEdge
  // weightCurvatureEdge, weightMaskEdge, segDist, segSigma
  // clusteringThreshold, clusteringMinPoints, downsampleRate
  float zMin = 0.3;
  float zMax = 2.3;
...
//   cv::Mat_<float> edges1 = cv::Mat_<float>(rows, cols);
//   rgbdSegment->DetectEdges(kcore->GetPointCloud(), normals, edges1);
//   cv::Mat_<cv::Vec3b> edge_image = cv::Mat_<cv::Vec3b>(rows, cols);
//   pclA::ConvertCvMat2Image(edges1, edge_image, true);
//   cv::imshow("Grey-level edge image", edge_image);
...
//   edges.setTo(0);
  ...
  pclA::ConvertPCLCloud2Mask(pcl_pc/*kcore->GetPclCloud()*/, mask, false, false);       /// TODO TODO TODO TODO Warum funkt. Makse von pcl_pc aus nicht?
