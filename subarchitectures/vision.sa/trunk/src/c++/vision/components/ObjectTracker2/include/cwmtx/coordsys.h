...
// CWTSquareMatrix utilities for use in coordinate systems
...
namespace CwMtx
{
  ...
  // (roll angle)
  template < class T >
  CWTSquareMatrix<T> smatFromEuler321Angles(T sclrAbout3,
					    T sclrAbout2,
					    T sclrAbout1)
  {
    CWTSquareMatrix<T> smat(3U);
  ...
  // CWTQuaternion utilities for use in coordinate systems
  ...
  template < class T >
  CWTQuaternion<T>
  qtnFromEulerAxisAndAngle(const CWTSpaceVector<T> &svecEulerAxis,
			   const T &sclrEulerAngle)
  {
    return CWTQuaternion<T>(1, svecEulerAxis, 0.5*sclrEulerAngle);
  }
  ...
  CWTSpaceVector<T> eulerAxisFromQtn(const CWTQuaternion<T> &qtn)
  {
    ...
    return CWTSpaceVector<T>(qtn[0]/sclrTmp,
			     qtn[1]/sclrTmp,
			     qtn[2]/sclrTmp);
  }
  ...
  {
    return
      CWTQuaternion<T>(1, CWTSpaceVector<T>(0, 0, 1), 0.5*sclrAbout3)
      *CWTQuaternion<T>(1, CWTSpaceVector<T>(0, 1, 0), 0.5*sclrAbout2)
      *CWTQuaternion<T>(1, CWTSpaceVector<T>(1, 0, 0), 0.5*sclrAbout1);
  }
  ...
  CWTQuaternion<T> qtnFromAxisAngle( const CWTVector<T> &vAxis,
				     const T sAngle )
  {
    return qtnFromEulerAxisAndAngle(CWTSpaceVector<T>(vAxis), sAngle);
  }
