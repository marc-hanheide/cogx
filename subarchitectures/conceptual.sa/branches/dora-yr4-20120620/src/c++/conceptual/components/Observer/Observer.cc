...
// -------------------------------------------------------
Observer::Observer() : _lastWsUpdateTime(0.0), _mapLoadingMode(false),
                       _worldStateUpdatesUnlocked(false)
{
...
void Observer::start()
{
  ...
  // Global filter on MapLoadStatus
  addChangeFilter(createGlobalTypeFilter<SpatialData::MapLoadStatus>(cdl::ADD),
                  new MemberFunctionChangeReceiver<Observer>(this,
                                                             &Observer::mapLoadStatusChanged));
  addChangeFilter(createGlobalTypeFilter<SpatialData::MapLoadStatus>(cdl::OVERWRITE),
                  new MemberFunctionChangeReceiver<Observer>(this,
                                                             &Observer::mapLoadStatusChanged));
...
{
  // Check if updating is not locked (since map is loading)
  if (!_worldStateUpdatesUnlocked)
    return;
...
void Observer::placeChanged(const cast::cdl::WorkingMemoryChange &wmChange)
{
  ...
  case cdl::ADD:
  {
    ...
    // If we are not in the map loading mode, make sure we unlock world state updates
    if (!_mapLoadingMode)
    {
...
void Observer::shapePlacePropertyChanged(const cast::cdl::WorkingMemoryChange &wmChange)
{
  ...
  case cdl::ADD:
  {
    ...
    if (!shapePlacePropertyPtr)
    {
      log("ShapePlaceProperty disappeared from WM.");
      return;
    }
...
void Observer::sizePlacePropertyChanged(const cast::cdl::WorkingMemoryChange &wmChange)
{
  ...
  case cdl::ADD:
  {
    ...
    if (!sizePlacePropertyPtr)
    {
      log("SizePlaceProperty disappeared from WM.");
      return;
    }
...
void Observer::appearancePlacePropertyChanged(const cast::cdl::WorkingMemoryChange &wmChange)
{
  ...
  case cdl::ADD:
  {
    ...
    if (!appearancePlacePropertyPtr)
    {
      log("AppearancePlaceProperty disappeared from WM.");
      return;
    }
...
// -------------------------------------------------------
void Observer::mapLoadStatusChanged(const cast::cdl::WorkingMemoryChange &wmChange)
{
  // Get the status
  SpatialData::MapLoadStatusPtr mapLoadStatusPtr;
  try
  {
    mapLoadStatusPtr = getMemoryEntry<SpatialData::MapLoadStatus>(wmChange.address);
  }
  catch(CASTException &e)
  {
  ...
  // Set map loading mode
  _mapLoadingMode=true;
