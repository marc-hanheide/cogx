        ...
        for var, val in dictionary[fact]:
            ...
            if temporal: 
                already_undefined = (condition[cond_time].get(var) == none_of_those or 
                                     eff_condition_dict[cond_time].get(var) == none_of_those)
            else:
                already_undefined = (condition.get(var) == none_of_those)
            if found_matching_add_effect or already_undefined:
                ...
                if temporal:
                    if (condition[cond_time].get(var) != val and 
                       eff_condition_dict[cond_time].get(var) != val):
                        # Need a guard for this delete effect.
                        assert (var not in condition[cond_time] and 
                               var not in eff_condition[cond_time]), "Oops?"
