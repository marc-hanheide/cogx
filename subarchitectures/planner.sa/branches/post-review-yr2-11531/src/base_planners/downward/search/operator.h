...
class Operator {
    ...
    std::vector<Prevail> replan;      // var, val
...
public:
    ...
    bool is_assertion() const {return replan.size() > 0;}

    const std::vector<Prevail> &get_prevail() const {return prevail;}
    const std::vector<Prevail> &get_replan() const {return replan;}
    ...
    bool is_expandable(const State &state) const {
	for(int i = 0; i < replan.size(); i++)
	    if(!replan[i].is_applicable(state))
		return false;
	return true;
    }
