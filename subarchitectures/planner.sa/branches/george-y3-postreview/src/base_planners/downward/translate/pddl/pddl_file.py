...
def open(task_filename=None, domain_filename=None, mutex_filename=None):
  if task_filename is None:
    if "-m" in sys.argv:
      mutex_filename = sys.argv[sys.argv.index("-m")+1]
      sys.argv = sys.argv[:sys.argv.index("-m")]
      
    if len(sys.argv) not in (2, 3):
      raise SystemExit("Error: Need exactly one, two or three command line arguments.\n"
  ...
  return tasks.Task.parse(domain_pddl, task_pddl), mutex_filename
