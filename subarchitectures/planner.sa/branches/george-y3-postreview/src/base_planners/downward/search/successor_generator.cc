...
public:
    ...
    virtual void remove_assertions();
};
...
void SuccessorGeneratorSwitch::remove_assertions() {
    immediate_ops->remove_assertions();
    for(int i = 0; i < g_variable_domain[switch_var]; i++) {
        generator_for_value[i]->remove_assertions();
    }
    default_generator->remove_assertions();
}
...
void SuccessorGeneratorGenerate::remove_assertions() {
    for (int i=0; i < op.size(); i++) {
        if (op[i]->is_assertion() && op[i]->is_expandable(*g_initial_state)) {
            op.erase(op.begin() + i);
            i--;
        }
    }
