...
#define SYLVIETIME( val ) val = ( float ) ( ( gsylvie_end.tms_utime - gsylvie_start.tms_utime + \
					 gsylvie_end.tms_stime - gsylvie_start.tms_stime  ) / 100.0 )
...
struct _command_line {
  ...
  char heuristic_plan_file_name[MAX_LENGTH];
  char action_descriptions_file_name[MAX_LENGTH];
  char *sylvie_file_name;
  char *plan_output_fn;
  int prediction_level;
  int display_info;
  int debug;
  float sylvie_time;
};
...
typedef struct _PlNode {
  ...
  /* MB: replanning conditions of assertions
   */
  Bool isReplanCond;
...
extern struct tms gsylvie_start, gsylvie_end;
