...
//		unionsWithRoomId = binderFacade.findFeaturesInUnion("roomID");
	...
	private PlanningTask newPlanningTask() {
		return new PlanningTask(0, null, true, null, null, Completion.PENDING, 0, Completion.PENDING, 0);
	}
...
//									+ "/"
//									+ binderFacade
//											.getBelief(em.correspondingUnion).getId());
//
...
//									.getBelief(crm.correspondingUnion).getId(),
//									getAgentUnion());
//			} else if (m instanceof HomingMotive) {
//				HomingMotive crm = (HomingMotive) m;
//				crm.correspondingUnion = resolveMotive(crm);
//				conjunctiveGoal = GoalTranslator.motive2PlannerGoal(crm,
//						binderFacade.getBelief(crm.correspondingUnion).getId(),
//						getAgentUnion());
			if (m instanceof GeneralGoalMotive) {
...
//						binderFacade.getBelief(pm.correspondingUnion).getId(),
//						getAgentUnion());
...
//								.getBelief(m.correspondingUnion).getId(),
//						getAgentUnion());
//			}
		...
		if (countGoals > 0) {
			...
			plan.goals[0]=new Goal(-1, goalString, false);
			return plan;
		} else {
			...
			while (continueWaiting) {
				...
				case SUCCEEDED:
					...
					component.log("we have a plan right now: "
							+ ((PlanningTask) pt.getEntry()).goals[0].goalString);
