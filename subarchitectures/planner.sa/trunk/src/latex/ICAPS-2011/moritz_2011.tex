...
the validity of the assumptions made so far.
...
visited. The starting state, $s_0$, for sequential planning is the
abstract state with probability $1.0$ -- E.G., for $\pp{R2D2}$ $s_0
\equiv \{(= (is-in R2D2) kitchen)\}$.
...
for these actions is as follows. $\actions(p_i;(T_i))$ can be executed
