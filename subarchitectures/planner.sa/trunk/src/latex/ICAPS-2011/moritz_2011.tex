...
power is achieved by incorporating perceptual analogues of fluent and
predicate signatures, and of action definitions. In detail, we have
declarations of state characterising predicate and fluent symbols as
usual. In addition, we allow two other declarative blocks, for
perceptual predicates and fluents respectively. In detail, suppose
...
formula cannot contain negation. Moreover, and without a loss of
generality, we find it convenient to separate the actional
precondition from the state precondition. For example, our mobile
robot is able to look for a visual object, such as a box of
cornflakes, at a given place. We can model that deterministic action
...
$\pp{look-for-object}$ has an effect effect on the state. That is,
...
interpreted as accumulated rewards and penalties.}  
...
\poss_\stochActions(\pp{vision}~\pp{R2D2}~\pp{box}~\pp{desk})$. We
also include a redundant state precondition, $(=(\pp{is-in}~?r)?l)$,
in order to fully demonstrate the syntax. Interpreting the schema, if
action
$(\pp{look-for-object}~\pp{robot}~\pp{cornflakes}~\pp{bedroom})$ is
executed, there is a $0.8$ chance of perceiving the cornflakes if they
are in the $\pp{bedroom}$, and otherwise a $0.1$ of perceiving them.
