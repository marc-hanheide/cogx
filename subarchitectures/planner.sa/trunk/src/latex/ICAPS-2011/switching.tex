...
sequential and decision-theoretic sessions. Continual planning and
execution proceeds in a POMDP described in DTPDDL. During sequential
sessions, planning is performed by a ``classical'' system,\footnote{By
``classical'', we mean a system the is designed to solve fully
observable deterministic tasks.}  and execution proceeds according to
the {\em trace} computed by that system. In the format of a classical
plan, that {\em trace} specifies a sequence of POMDP actions that
achieves the agent's objectives for a deterministic approximation,
i.e., {\em determinisation}, of the problem at hand. More precisely,
the trace is an interleaved sequence of POMDP actions and {\em
assumptive} actions. The latter give some of the assumptions the
planner makes about the truth value of {\em runtime state variables}
-- e.g. that a box of cornflakes is located in the kitchen at the
third plan step. Another kind of assumption, called an {\em
executability} assumption, is made when the trace includes an action
$\action$ that is not applicable with probability $1$ at the
belief-state
...
A sequential session always proceeds first. Planning and execution are
interleaved according to the continual paradigm until an executability
assumption about an action, $\action$, scheduled for execution,
occurs. At this point our approach switches to using a
decision-theoretic solver which tailors sensory processing by the
agent to test the validity of the assumptions made in the trace,
including the truth value of $\poss(\action)$.
