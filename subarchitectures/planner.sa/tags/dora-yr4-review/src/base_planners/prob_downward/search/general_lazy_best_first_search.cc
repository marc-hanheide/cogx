...
int GeneralLazyBestFirstSearch::fetch_next_state() {
    ...
    current_g = search_space.get_node(current_predecessor).succ_info(current_operator).get_g();
    ...
    if(node.is_new() || reopen) {
        ...
        if(!open_list->is_dead_end()) {
            ...
            if(check_goal_and_set_plan(current_state)) {
                search_progress.print_h_line(node.get_g(), node.get_p());
                return SOLVED;
            }
            if (search_progress.check_h_progress(current_g, node.get_p())) {
                reward_progress();
            }
            generate_successors();
