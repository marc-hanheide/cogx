...
public class GenerateWavFiles {
	...
	public static void main(String[] args) {
		...
		try{
		FileReader l_CSVreader = new FileReader(args[0]);
		...
		while((l_str= l_CSVbfr.readLine())!= null && l_ctr<=48){
			++l_ctr;
			...
			*/
			...
			//if (l_ctr == 5)break; //Use this break for trial
	...
	private static void WriteWebExpResource(){
		 ...
		 int i_list = 0;
		  for (Enumeration<LinkedHashMap> e=l_allImageRes.elements(); e.hasMoreElements();) {
			  ...
			  if(!l_tmp.isEmpty()) {
				  WriteWebExpImageResource(l_tmp, l_imgres);
				  }
			 }	  
		  ...
		  for (Enumeration<LinkedHashMap> e=l_allAudioRes.elements(); e.hasMoreElements();) {
			  ...
			  //If the LinkedHashMap has content, continue
			  if(!l_tmp.isEmpty()) {
				  WriteWebExpImageResource(l_tmp, l_audres);
			  }
		  }	  
		  System.out.println("All Resources written.");
	  }
	  
	  private static void WriteWebExpImageResource(LinkedHashMap<String, String> i_resName, String i_resourcefile) {
		 ...
		 // System.out.println(l_xmlheader);
		  ...
		  try {
			  BufferedWriter l_resourceFile = new BufferedWriter(new FileWriter(l_outputloc_rsrc.concat(i_resourcefile)));
			
			  l_resourceFile.write(l_xmlheader);
			  l_resourceFile.newLine();
			  l_resourceFile.newLine();
			  
			  l_resourceFile.write(l_xmlresource);
			  l_resourceFile.newLine();
			  l_resourceFile.newLine();
			  

			  l_resourceFile.write(l_xmlblock);
			  l_resourceFile.newLine();
			  l_resourceFile.newLine();
			 ...
			 //}
			  //for( Enumeration<String> e = i_resName.keys(); e.hasMoreElements();) {
				...
				//  System.out.println("Get from LinkedHashMap: "+l_resLine);
				  l_resourceFile.write(l_resLine.toString());
				  l_resourceFile.newLine();
				  
			  }
			  
			  l_resourceFile.write(l_xmlblockend);
			  l_resourceFile.newLine();
			  l_resourceFile.newLine();
			 
			  l_resourceFile.write(l_xmlresourceend);
			  l_resourceFile.newLine();
			  l_resourceFile.close();
			  
			System.out.println("Resource written ->"+ l_outputloc_rsrc.concat(i_resourcefile));
		  }
		  catch (Exception fx) {
			System.out.println("Error in writing files: " + fx.toString());
		}
