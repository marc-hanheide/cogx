 ...
 * @version     $Revision: 1.7 $, $Date: 2005/10/20 18:49:42 $
 */
public class GenTargets
{
     ...
     */
    private GenTargets(URL grammarURL, URL testbedURL) throws IOException {
        ...
        System.out.println("Loading target phrases from URL: " + testbedURL);
        RegressionInfo rinfo = new RegressionInfo(grammar, testbedURL.openStream());
    ...
    public static void main(String[] args) throws IOException {
        ...
        URL grammarURL = gFile.toURL();
        File tbFile = new File(testbedfile);
        if (!tbFile.exists()) {
            tbFile = new File(gFile.getParentFile(), testbedfile);
        }
        URL testbedURL = tbFile.toURL();
        GenTargets gen = new GenTargets(grammarURL, testbedURL);
