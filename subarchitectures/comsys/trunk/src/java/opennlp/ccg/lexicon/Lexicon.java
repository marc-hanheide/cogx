///////////////////////////////////////////////////////////////////////////////
// Copyright (C) 2004-8 Jason Baldridge, Gann Bierner and 
 ...
 * @version     $Revision: 1.63 $, $Date: 2008/08/26 05:24:57 $
 */
public class Lexicon { 
    ...
    private GroupMap<String,Family> _families;
    ...
    public void init(URL lexiconUrl, URL morphUrl) throws IOException {
        ...
        // index families by supertag+pos, for supertagging
        _families = new GroupMap<String,Family>();
        ...
        for (Family family : lexicon) {
        	...
        	_families.put(family.getSupertag()+family.getPOS(), family);
        ...
        throws LexException 
    {
    	...
    	Set<String> supertags = null;
    	Set<String> supertagsFound = null;
    	if (_supertagger != null) {
    		supertags = _supertagger.getSupertags(mi);
        ...
        if (_stems.containsKey(stem+pos)) {
            ...
            for (Iterator<Object> I=stemItems.iterator(); I.hasNext();) {
                ...
                if (item instanceof EntriesItem) {
                    ...
                    // check supertag
                    if (supertags != null) {
                    	if (!supertags.contains(entry.getSupertag())) continue;
                    	else supertagsFound.add(entry.getSupertag());
                    }
                    // do lookup
                    getWithEntriesItem(w, mi, stem, stem, entry, macAdder, result);
                } 
                ...
                // an EntriesItem[]
                    ...
                    // check supertag
                    if (supertags != null) {
                    	if (!supertags.contains(entries[0].getSupertag())) continue;
                    	else supertagsFound.add(entries[0].getSupertag());
                    }
                    // do lookup
        ...
        // XXX tmp
//        System.out.println("stem: " + stem + " pos: " + pos + " targetPred: " + targetPred);
//        if (supertagsFound != null) {
// 	        System.out.print("supertags found: ");
// 	        for (String tag : supertagsFound) System.out.print(tag + " ");
// 	        System.out.println();
//        }
//        else {
// 	        System.out.println("no supertags");
//        }
//        if (supertags != null) {
// 	        System.out.println("supertags predicted: " + supertags.size());
//                 if (supertags.size() < 10) {
// 	        for (String tag : supertags) System.out.print(tag + " ");
// 	        System.out.println();
//                 }
//        }
        ...
        // finally do families for any remaining supertags
        if (supertags != null) {
        	for (String supertag : supertags) {
        		if (supertagsFound.contains(supertag)) continue;
        		Set<Family> families = _families.get(supertag+pos);
        		if (families == null) continue; // nb: could be a POS mismatch
	            // get entries with pred = targetPred, or stem if null
        		for (Family family : families) {
		            String pred = (targetPred != null) ? targetPred : stem;
		            getWithDataItem(w, mi, new DataItem(stem, pred), family.getEntries(), targetRel, macAdder, result);
        		}
        	}
        }
    }
