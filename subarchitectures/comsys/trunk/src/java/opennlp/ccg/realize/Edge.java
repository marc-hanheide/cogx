 ...
 * @version     $Revision: 1.29 $, $Date: 2005/11/29 21:45:12 $
 */
public class Edge extends Tracker
{
    ...
    /** Returns a hash code for this edge based on the surface words, and ignoring the score. */
    public int surfaceWordHashCode() {
    	return sign.surfaceWordHashCode() + 31 * bitset.hashCode() + indices.hashCode();
    }
    ...
    /** Returns whether this edge equals the given object based on the surface words, and ignoring the score. */
    public boolean surfaceWordEquals(Object obj) {
        if (obj == this) return true;
        if (!(obj instanceof Edge)) return false;
        Edge edge = (Edge) obj;
        return bitset.equals(edge.bitset) && indices.equals(edge.indices) &&
               sign.surfaceWordEquals(edge.sign);
    }
