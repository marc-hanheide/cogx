...
# Generated by convert-ply.py at Tue Apr 15 09:59:53 2008.
#
...
word_tokens = ('NUMBER',) + word_no_number_tokens
    ...
    for (key, value) in my_sorted(rules.items()):
        if type(key) is tuple and key[0] in rules_to_xml_mapping:
        	...
        	if lf:
                    ...
                    lf = []
                rulename = 'typechange-%d' % unique
                
                if "x2du" in str(arg):
                    rulename = "x2du-%d" % unique
                
                if "disclevelcomp" in str(arg):
                    rulename = "disclevelcomposition"
                    
                if "correction" in str(arg):
                    rulename = "correction-%d" % unique
                    
                if "repetition" in str(arg):
                    rulename = "repetition"
                if "completion" in str(arg):
                    rulename = "completion"
                    
                xml.append(['typechanging', [('name', rulename)],
                            ['arg', [], arg],
                            ['result', [], result + lf]])
        else:
