...
/**
 * Utility class to update testbed files.
 *
...
public class UpdateTestbed
{
    ...
    /** Adds full words for each test item, if missing, based on the parse. */
    private void addFullWords(File tbFile) throws IOException {
        ...
        RegressionInfo tbInfo = new RegressionInfo(grammar, testbedURL.openStream());
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (testItem.fullWords == null) {
                List<Word> words = grammar.getParsedWords(testItem.sentence);
