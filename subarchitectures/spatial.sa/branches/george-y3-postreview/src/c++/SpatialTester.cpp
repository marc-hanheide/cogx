...
NavTester::NavTester(const string &_id) :
  ...
  AbstractBinder(_id),
  NavGraphEventListener("NavTester"),
  m_test(-1),
  m_handler(*this),
...
void 
NavTester::configure(map<string,string> & _config) {
...
void 
NavTester::bindingProxyUpdated(const cast::cdl::WorkingMemoryChange & _wmc) {
...
void 
NavTester::bindingProxyDeleted(const cast::cdl::WorkingMemoryChange & _wmc) {
  string id(_wmc.m_address.m_id);
...
void 
NavTester::statusUpdated(const cast::cdl::WorkingMemoryChange & _wmc) {
...
NavTester::testCompleteness() {
  if (m_test != -1) {
	...
	if (m_testFinished && allBound()) {
	  ...
	  assert(true_for_all(proxies,
		(hasFeature<ProxyPtr,BindingFeatures::Concept>() ||
		 hasFeature<ProxyPtr,BindingFeatures::AreaID>() ||
		 hasFeature<ProxyPtr,BindingFeatures::RelationLabel>())));
	  assert(true_for_some(proxies,
		hasFeature<ProxyPtr,BindingFeatures::AreaID>() &&
		featureCheck<ProxyPtr,BindingFeatures::AreaID>(AreaIDMatcher(0))));
	  assert(!true_for_some(proxies,
	...
	if (m_testFinished && allBound()) {
	  ...
	  assert(true_for_some(proxies,
		hasFeature<ProxyPtr,BindingFeatures::AreaID>() &&
		featureCheck<ProxyPtr,BindingFeatures::AreaID>(AreaIDMatcher(2))));
	  assert(!true_for_some(proxies,
		(InportsCountCheck<ProxyPtr>(0)) &&
		(OutportsCountCheck<ProxyPtr>(0))));
	  assert(true_for_some(proxies,
...
{  
  ...
  if ( !m_addSignalledProxyIDs.empty() && 
      ...
      for(std::set<std::string>::iterator it = m_addSignalledProxyIDs.begin();
	  it != m_addSignalledProxyIDs.end();
	  it++) {
	log(*it);
      }
      ProxySet proxies;
...
void NavTester::checkAndAddNewArea(int aid)
{
  ...
  if (aIter != m_Areas.end()) {
    ...
    addToWorkingMemory<NavData::Area>(newArea.m_WMid,
				      getNavSubarchID(),
				      new NavData::Area(newArea.m_data),
				      cdl::BLOCKING);
  }
}
...
void NavTester::AddFreeNode(long nodeID, 
  ...
  m_pNavGraph->m_FNodes[numFNodes].m_areaType = CORBA::string_dup(areaType.c_str());  
  ...
  ///////////////////////
  addToWorkingMemory<NavData::FNode>(newDataID(),
				     getNavSubarchID(),
                                     new NavData::FNode(m_pNavGraph->m_FNodes[numFNodes]),
                                     cdl::BLOCKING); // sync!
  
  log("added free node to working memory");
}

void NavTester::MovePerson(long personID, double x, double y,
    double theta, long areaID)
{
  log("NavTester::MovePerson");
  vector< shared_ptr< const CASTData < NavData::Person > > > vec;
  getWorkingMemoryEntries<NavData::Person>(getNavSubarchID(),
      0,
      vec);
...
void NavTester::AddPerson(long personID, double x, double y,
    double theta, double speed, double visibility, long areaID)
{
  log("added person");
  NavData::Person person;
  person.m_id = personID;
  ...
  addToWorkingMemory<NavData::Person>(newDataID(),
      getNavSubarchID(),
      new NavData::Person(person),
      cdl::BLOCKING);
}
...
void NavTester::AddObjectNode(long nodeID, 
                                      ...
                                      long objectID) 
{
  ...
  m_pNavGraph->m_ONodes[numONodes].m_areaType = CORBA::string_dup(areaType.c_str());  
  
  m_pNavGraph->m_ONodes[numONodes].m_category = CORBA::string_dup(category.c_str());  
  ...
  m_pNavGraph->m_Objects[numObjects].m_category = CORBA::string_dup(category.c_str());
  ...
  ////////////////////////////
  addToWorkingMemory<NavData::ONode>(newDataID(),
				     getNavSubarchID(),
                                     new NavData::ONode(m_pNavGraph->m_ONodes[numONodes]),
                                     cdl::BLOCKING); // sync!
...
{  
  ...
  //////////////////////
  addToWorkingMemory<NavData::AEdge>(newDataID(),
				     getNavSubarchID(),
                                     new NavData::AEdge(m_pNavGraph->m_AEdges[numAEdges]),
                                     cdl::BLOCKING); // sync!
  
  log("added access edge to working memory");
}

void NavTester::AddVisibilityEdge(long startNodeID, long endNodeID, double weight) 
{
  ...
  //////////////////////
  addToWorkingMemory<NavData::VEdge>(newDataID(),
				     getNavSubarchID(),
                                     new NavData::VEdge(m_pNavGraph->m_VEdges[numVEdges]),
                                     cdl::BLOCKING); // sync!
...
void NavTester::CreateNewNavGraphIDL() 
{
  ...
  if(m_pNavGraph) delete m_pNavGraph;
...
void NavTester::ConvertCureNavGraphToIDL()
{
  ...
  for (it1= m_cureNavGraph.m_Nodes.begin();it1!=m_cureNavGraph.m_Nodes.end();it1++) { 
    ...
    log(buf);
    AddFreeNode(nodeID, x, y, z, theta, areaID, areaType, gateway, 
    maxSpeed, width, node->getName());
  ...
  for (it2= m_cureNavGraph.m_Edges.begin();it2!=m_cureNavGraph.m_Edges.end();it2++) {
...
void NavTester::WriteNavGraphToWorkingMemory() // cp: done
{
  ...
  if(m_NavGraphWMid == ""){
    ...
    overwriteWorkingMemory<NavData::NavGraph>(m_NavGraphWMid,
					      getNavSubarchID(),
                                              new NavData::NavGraph(*m_pNavGraph), cdl::BLOCKING);
...
void NavTester::WriteTopologicalPosToWorkingMemory(int aid)
{
  ...
  if(m_TopRobPosWMid == ""){
    ...
    NavData::TopologicalRobotPos *newRobotPos = new NavData::TopologicalRobotPos(m_TopRobPos);
...
void NavTester::updatePeopleInWM()
{
  ...
  for (std::vector<PersonHyp>::iterator pi = m_People.begin();
    ...
    if(pi->m_Status == PersonHyp::STATUS_CREATE) {
      pi->m_WMid = newDataID();
      addToWorkingMemory<NavData::Person>(pi->m_WMid, 
				     getNavSubarchID(),
      ...
      overwriteWorkingMemory<NavData::Person>(pi->m_WMid,
					      getNavSubarchID(),
...
void NavTester::setRobotPose(double x, double y, double theta)
{
  ...
  if(m_RobotPoseWMid == ""){
    ...
    addToWorkingMemory<NavData::RobotPose>(m_RobotPoseWMid, 
					   getNavSubarchID(),
...
void NavTester::updateTrackedPersonInWM(long id)
{
  ...
  if (m_CurrPersonWMid == "") {
    ...
    addToWorkingMemory<NavData::PersonFollowed>(m_CurrPersonWMid,
				     getNavSubarchID(),
                                                new NavData::PersonFollowed(p), cdl::BLOCKING); // sync!
    ...
    if (id != m_CurrPerson) {
      ...
      overwriteWorkingMemory<NavData::PersonFollowed>(m_CurrPersonWMid,
						      getNavSubarchID(),
                                                      new NavData::PersonFollowed(p), cdl::BLOCKING);
...
{
  addToWorkingMemory(newDataID(), "binding.sa", new BindingData::TriggerDotViewer, cast::cdl::BLOCKING);
...
shared_ptr<AbstractPredicate<ProxyPtr> > NavTester::getPredicate(istream &str)
{
  ...
  if (name == "INPORTCOUNT") {
    ...
    debug(buf);
    return (featureCheck<ProxyPtr,BindingFeatures::RelationLabel>(RelationLabelMatcher(expected))).clone();
  }
    ...
    sprintf(buf, "%s %s", name.c_str(), expected.c_str());
    debug(buf);
    return (featureCheck<ProxyPtr,BindingFeatures::Concept>(ConceptMatcher(expected))).clone();
  }
    ...
    sprintf(buf, "%s %i", name.c_str(), expected);
    debug(buf);
    return (featureCheck<ProxyPtr,BindingFeatures::AreaID>(AreaIDMatcher(expected))).clone();
  }
...
{
  shared_ptr<const CASTData<TestingData::TestingCommand> >command = getWorkingMemoryEntry<TestingData::TestingCommand>(_wmc.m_address);
  ...
  if(temp == "SETROBOTAREA"){
    ...
    double x, y, z, radius;
    str >> nodeID >> x >> y >> z >> areaID >> areaType >> category >> radius >> objectID;
    ...
    str >> nodeID >> x >> y >> z >> theta >> areaID >> areaType >> gateway >> maxSpeed >> width >> name;
    AddFreeNode(nodeID, x, y, z, theta, areaID, areaType, gateway, maxSpeed, width, name);
  }
  else if (temp == "REMOVEPERSON") {
    long personID;
    str >> personID;
  ...
  string logStr = string("Marking command ") + CORBA::string_dup(_wmc.m_address.m_id) + " as completed.";
  log(logStr);
  addToWorkingMemory(newDataID(), "binding.sa", new BindingData::TriggerDotViewer, cast::cdl::BLOCKING);
  overwriteWorkingMemory<TestingData::TestingCommand>(CORBA::string_dup(_wmc.m_address.m_id), owtCmd);
}
