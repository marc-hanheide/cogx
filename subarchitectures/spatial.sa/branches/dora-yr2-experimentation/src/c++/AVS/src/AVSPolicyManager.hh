...
class AVSPolicyManager {
  ...
  public:
    ...
    void allocate(long numofconfigs, long numofpolicies, 
		  int steps){
	...
	m_NumberSteps=n;
      ...
      for (int i=n;i<m_NumberSteps;i++)m_Index[i]=-1;
      n=p.m_NumberProbs+p.m_NumberCosts;
      if ((m_NumberProbs!=p.m_NumberProbs)||(m_NumberCosts!=p.m_NumberCosts)){
  ...
  public:
    ...
    Policy * addNew(std::string &cfg, bool room=false){
      if (m_Next)return m_Next->addNew(cfg);
      m_Next=new Policy();
      m_Policy.push_back(cfg);
      if (room)m_Room=cfg;
    }    
  ...
  public:
    ...
    void clearChildren(){
      if (m_NumberOfChildren==0)return;
     ...
     * more.
     */
    void completeList(std::list<std::string> &configurations,
  ...
  void setCostCredit(double d){m_CostCredit=d;}
  void setCurrentRoom(long r){m_CurrentRoom=r;}
  void setConfigurationsFilename(std::string s){m_ConfigurationsFilename=s;}
  void setRoomsFilename(std::string s){m_RoomsFilename=s;}
  /**
   ...
   *
   */
  void preComputeExclusive(std::list<std::string> &strings, double threshold=0){
    Cure::Matrix probabilities;
    std::list<std::string> configurations;
    getConfiguration(strings,configurations,probabilities);
  ...
  bool nextNearlyBestPolicy(std::list<std::string> &pol, 
			    ...
			    double threshold,
			    double percentage);
 bool next1StepPolicy(std::list<std::string> &pol, 
		      std::string &currentCfg,
		      double threshold,
		      double percentage); 
 bool next2StepPolicy(std::list<std::string> &pol, 
		      std::string &currentCfg,
		      double threshold,
		      double percentage); 
  bool next3StepPolicy(std::list<std::string> &pol, 
		       std::string &currentCfg,
		       double threshold,
		       double percentage); 
  bool next4StepPolicy(std::list<std::string> &pol, 
		       std::string &currentCfg,
		       double threshold,
		       double percentage); 

  bool multiRoomSearch(std::list<std::string> &pol, 
		       short *stepsInRoom,
		       PolicyPath *bestPaths,
		       PolicyPath &restorePath,
		       double percentage=.99);
  void iterate(PolicyPath *paths,
  ...
  void update(std::string &currentCfg,
	      double percentage); 
  void update(double percentage); 
    
  double expectedCost(double &q,Cure::LongArray policies,
		      double percentage=.99, double threshold=.01);
  /**
  ...
  void getFileConfigurations(std::list<std::string> &configurations,
			     Cure::Matrix &probabilities,
			     std::string searchTarget);
  void getFileRoomCosts(std::list<std::string> &rooms,
			Cure::Matrix &costs);
