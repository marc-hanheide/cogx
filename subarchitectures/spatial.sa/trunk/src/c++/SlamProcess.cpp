...
void SlamProcess::configure(const map<string, string>& _config) {
  ...
  if (m_DontWriteFiles)
    log("Will NOT output files");
  else
    log("Will output files with robotpose and map");
  ...
  if (m_MapFilename != "" && tmp.is_open()) {
    ...
    if (loc->config(configfile)) {
      printf(
          "SlamThread::configure Failed to config localization with \"%s\"\n",
          configfile.c_str());
      m_PP = 0;
  ...
  if (_config.find("--output-raw-data") != _config.end()) {
    if (m_odomfile.openWriteFile("odom.tdf"))
      return;
    if (m_scanfile.openWriteFile("scans.tdf"))
      return;
...
void SlamProcess::receiveOdometry(const Robotbase::Odometry &castOdom) {
  ...
  if ((m_LastOdom.getTime() > 0) && (odom.getDoubleTime()
      - m_LastOdom.getDoubleTime() > timeDiffToWarn)) {
  ...
  if (m_OdomTimer.isRunning()) {
    if (m_OdomTimer.stop() > timeDiffToWarn) {
      log(
          "WARNING: receiveOdomData called very seldom (%fs since last), bad sign!!",
          m_OdomTimer.stop());
    }
  }
  m_OdomTimer.restart();
  ...
  if (m_LastUsedOdom.getTime() == Cure::Timestamp(0) || hypot(
      m_LastUsedOdom.getX() - odom.getX(), m_LastUsedOdom.getY() - odom.getY())
      > 1e-3 || fabs(Cure::HelpFunctions::angleDiffRad(
      m_LastUsedOdom.getTheta(), odom.getTheta())) > 1e-2) {
  ...
  debug("addOdom(x=%.2f y=%.2f a=%.2f t=%ld.%06ld)", odom.getX(), odom.getY(),
      odom.getTheta(), odom.getTime().Seconds, odom.getTime().Microsec);
  {
    IceUtil::Mutex::Lock lock(m_Mutex);
  ...
  if (m_usePeekabot) {
    m_SLAMPoseProxy.set_pose(m_PP->getPose().getX(), m_PP->getPose().getY(), 2,
        m_PP->getPose().getTheta(), 0, 0);
  }
}
...
void SlamProcess::processScan2d(const Laser::Scan2d &castScan) {
  ...
  log("Got scan n=%d, r[0]=%.3f, r[n-1]=%.3f t=%ld.%06ld",
      ...
      castScan.ranges[castScan.ranges.size() - 1], (long) castScan.time.s,
      (long) castScan.time.us);
  ...
  if (m_MaxScanRate > 0)
    timeDiffToWarn += 1.0 / m_MaxScanRate;
  if (m_ScanTimer.isRunning()) {
    if (m_ScanTimer.stop() > timeDiffToWarn) {
      log(
          "WARNING: receiveScanData called very seldom (%fs since last), bad sign!!",
          m_ScanTimer.stop());
    }
  }
  m_ScanTimer.restart();
  ...
  if ((m_NotRunningYet || m_RobotIsMoving || m_UpdateWithoutMotion)
    ...
    if (m_NotRunningYet) {
      ...
      if (m_NotRunningYet) {
        ...
        println(
            "Fake measurement kick start finished x=%.2f y=%.2f a=%.4f t=%.6f",
            m_PP->getPose().getX(), m_PP->getPose().getY(),
            m_PP->getPose().getTheta(), m_PP->getPose().getTime().getDouble());
      ...
      debug("addXXXX(#=%d a0=%.2f aS=%.4f r0=%.2f rEnd=%.2f t=%ld.%06ld)",
          ...
          cureScan.getAngleStep(), cureScan.getRange(0), cureScan.getRange(
              cureScan.getNPts() - 1), cureScan.getTime().Seconds,
          cureScan.getTime().Microsec);
...
int SlamProcess::extractMeasSet(Cure::LaserScan2d &cureScan,
    Cure::MeasurementSet &measSet) {
  m_Seg.segment(cureScan);
  m_Lsq.doFit(m_Seg.m_Segments, cureScan);
  debug("Extracted %d lines with RANSAC+LSQ", m_Lsq.m_Lines.size());

  Cure::RedundantLine2DRep::makeMeasurementSetFromLines(m_Lsq.m_Lines, measSet,
      &m_Seg.m_Segments, &cureScan);
...
void SlamProcess::updateRobotPoseInWM() {
  ...
  if (m_RobotPoseIdString == "") {
    ...
    addToWorkingMemory<NavData::RobotPose2d> (m_RobotPoseIdString, pRP); // sync!
    debug("Added RobotPose to WM");
  } else {
    overwriteWorkingMemory<NavData::RobotPose2d> (m_RobotPoseIdString, pRP);
    char buf[256];
    sprintf(buf,
        "Overwriting RobotPose in WM x=%.3f y=%.3f theta=%.3f t=%ld.%06ld",
        pose.getX(), pose.getY(), pose.getTheta(), pose.getTime().Seconds,
        pose.getTime().Microsec);
    log(buf);
  }
}
...
int SlamProcess::writeLineMapToWorkingMemory(bool overwrite) {
  if (m_PP == 0)
    return -1;
  ...
  if (walls.size() > 0) {
    int i = 0;
    for (std::list<Cure::Line2D>::iterator w = walls.begin(); w != walls.end(); w++) {
  ...
  if (overwrite) {
    overwriteWorkingMemory<NavData::LineMap> (m_LineMapIdString, lineMap); // synch!
    char buf[128];
...
void SlamProcess::connectPeekabot() {
  try {
    ...
    m_SLAMPoseProxy.add(m_PeekabotClient, "SLAM_Pose",
