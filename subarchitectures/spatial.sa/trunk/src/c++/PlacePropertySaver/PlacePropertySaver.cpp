...
void PlacePropertySaver::configure(const map<string, string> &config) {
	...
	if (_doSave) {
		...
		_saveInterval = parseOptionLexicalCast<unsigned int> ("--save-interval",
				1000);
	...
	if (_doLoad) {
		...
		_waitBeforeLoading = parseOptionLexicalCast<unsigned int> (
				"--wait-before-loading", 3000);

		_waitBetweenLoading = parseOptionLexicalCast<unsigned int> (
				"--wait-between-loading", 1);
	}
	...
	log("-> wait for map load status: %s", (_waitForMapLoadStatus ? "true"
			: "false"));
...
void PlacePropertySaver::start() {
	addChangeFilter(
			...
			new MemberFunctionChangeReceiver<PlacePropertySaver> (this,
					&PlacePropertySaver::mapLoadStatusAdded));
	addChangeFilter(createGlobalTypeFilter<SpatialData::MapLoadStatus> (
			cdl::OVERWRITE), new MemberFunctionChangeReceiver<PlacePropertySaver> (
			this, &PlacePropertySaver::mapLoadStatusOverwritten));
	addChangeFilter(createGlobalTypeFilter<SpatialData::MapLoadStatus> (
			cdl::DELETE), new MemberFunctionChangeReceiver<PlacePropertySaver> (this,
			&PlacePropertySaver::mapLoadStatusDeleted));
...
bool PlacePropertySaver::checkWMMapLoadStatus() {
	...
	try {
		SpatialData::MapLoadStatusPtr statusStruct = getMemoryEntry<
				SpatialData::MapLoadStatus> (_mapLoadStatusAddress);
...
// ------------------------------------------------------
void PlacePropertySaver::mapLoadStatusOverwritten(
		const cdl::WorkingMemoryChange &wmc) {
...
// ------------------------------------------------------
void PlacePropertySaver::mapLoadStatusDeleted(
		const cdl::WorkingMemoryChange &wmc) {
