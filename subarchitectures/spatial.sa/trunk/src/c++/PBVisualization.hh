...
public:
  VisualPB_Bloxel(std::string PB_Hostname, int PB_Port, int PB_xSize,
...
public:
  ...
  void DisplayMap(const SpatialGridMap::GridMap<MapData> &map,
      std::string name = "boxmap");
  template<class MapData>
  void DisplayPCMap(const SpatialGridMap::GridMap<MapData> &map,
      double cellsize);

  void Display2DCureMap(const Cure::LocalGridMap<unsigned char>* curemap,
      std::string name = "og2d", bool drawNew = false);
  void Display2DBinaryMap(const Cure::BinaryMatrix &binmap,
      const Cure::LocalGridMap<unsigned char>* curemap, std::string name =
          "og2d_binarymatrix");
  void Add3DPointCloud(std::vector<Cure::Pose3D> point, bool ishidden = true,
      std::string name = "extrapoints");
...
void VisualPB_Bloxel::Display2DCureMap(
    const Cure::LocalGridMap<unsigned char>* curemap, std::string name,
    bool drawNew) {
  ...
  if (drawNew) {
    ...
    temp2DOccGridProxy.add(client, "combined_placemap2D",
        peekabot::AUTO_ENUMERATE_ON_CONFLICT);
    local2DOccGridProxy = temp2DOccGridProxy;
  }

  og2d.add(local2DOccGridProxy, name, curemap->getCellSize(),
...
void VisualPB_Bloxel::Display2DBinaryMap(const Cure::BinaryMatrix &binmap,
    const Cure::LocalGridMap<unsigned char>* curemap, std::string name) {
  peekabot::OccupancyGrid2DProxy og2d;
  peekabot::OccupancySet2D cells;
  og2d.add(client, name, curemap->getCellSize(), peekabot::REPLACE_ON_CONFLICT);
  og2d.set_unoccupied_color(0.3, 0.3, 0);
  og2d.set_occupied_color(1, 0, 0);
...
VisualPB_Bloxel::VisualPB_Bloxel(std::string PB_Hostname, int PB_Port,
    int PB_xSize, int PB_ySize, double PB_CellSize, double PB_scale,
    bool replaceMap) :
...
bool VisualPB_Bloxel::connectPeekabot() {
  try {
    ...
    m_2DOccGridProxy.add(client, "combined_placemap2D",
        peekabot::REPLACE_ON_CONFLICT);
...
/*
 ...
 cells.add(coord.first,coord.second, z,1);
 }
 }
...
void VisualPB_Bloxel::Add3DPointCloud(std::vector<Cure::Pose3D> point,
    bool ishidden, std::string name) {
  try {
    peekabot::OccupancyGrid3DProxy ogproxy;
...
int fallsWhere(std::vector<std::pair<double, double> > thresholdvalues,
    double value) {
  ...
  try {
    ...
    if (drawNew) {
      ...
      for (int i = 0; i < maxLevels; i++) {
        ...
        char buf[256];
        sprintf(buf, "distribution_%4.5f_%4.5f", thresholds[i], thresholds[i
            + 1]);
        std::string s(buf);
        std::replace(s.begin(), s.end(), '.', ',');
        strcpy(buf, s.c_str());
    ...
    std::cout << "Map Size: " << map.getMapSize().first << " "
        << map.getMapSize().second << endl;
    typedef SpatialGridMap::Bloxel<MapData> MapBloxel;
    typedef SpatialGridMap::Bloxel<SpatialGridMap::GridMapWrapper<char> >
        VisData;
    ...
    for (int x = 0; x < map.getMapSize().first; x++) {
      for (int y = 0; y < map.getMapSize().second; y++) {
        for (typename std::vector<MapBloxel>::const_iterator it =
            map(x, y).begin(); it != map(x, y).end(); it++) {
    ...
    if (cutLevel < 0) {
      cerr << "Over " << maxBloxels << " (" << counters[0]
          << ") bloxels in first layer, no layers will be visualized";
      return;
    }
        ...
        if (levelChanged || map.isDirty(x, y)) { // If column or level has been changed
          ...
          if (col.size() != cache.size()) {
            ...
            for (unsigned int i = 0; i < col.size(); i++) {
              if (col[i].data.data != cache[i].data.data || col[i].celing
                  != cache[i].celing) {
                unchanged = false;
                break;
              }
          ...
          if (!unchanged) {
            ...
            int number = 0;
            for (std::vector<VisData>::const_iterator it = cache.begin(); it
                != cache.end(); it++) {
              ...
              if (it->data.data >= 0) {
                ...
                peekabot::CubeProxy cube;
                cube.add(localpdflevels[it->data.data],
                    nameBuilder.str().c_str(), peekabot::REPLACE_ON_CONFLICT);
                cube.remove();
            ...
            number = 0;
            for (std::vector<VisData>::const_iterator it = col.begin(); it
                != col.end(); it++) {
              // Add all bloxels that have an associated layer
              if (it->data.data >= 0) {
                nameBuilder.str(""); //reset name
                ...
                cube.add(localpdflevels[it->data.data],
                    nameBuilder.str().c_str(), peekabot::REPLACE_ON_CONFLICT);
                cube.set_position(coord.first, coord.second, (bloxel_floor
                    + it->celing) / 2);
...
template<class MapData>
void VisualPB_Bloxel::DisplayMap(const SpatialGridMap::GridMap<MapData> &map,
    std::string name) {
  typedef SpatialGridMap::Bloxel<MapData> MapBloxel;
    ...
    for (int x = 0; x < pairs.first; x++) {
      for (int y = 0; y < pairs.second; y++) {
        ...
        for (int i = 0; i < obsCount[x * ySize + y]; i++) {
          ...
          cube.add(pb_map, nameBuilder.str().c_str(),
              peekabot::REPLACE_ON_CONFLICT);
          cube.remove();
        }
        ...
        int number = 0;
        for (typename std::vector<MapBloxel>::const_iterator it =
            map(x, y).begin(); it != map(x, y).end(); it++, number++) {
