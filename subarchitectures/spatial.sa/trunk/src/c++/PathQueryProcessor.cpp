...
void PathQueryProcessor::start() {
  ...
  addChangeFilter(
      createLocalTypeFilter<SpatialData::PathTransitionProbRequest> (cdl::ADD),
      new MemberFunctionChangeReceiver<PathQueryProcessor> (this,
          &PathQueryProcessor::newPathTransitionProbRequest));

  addChangeFilter(
      createLocalTypeFilter<SpatialData::PathTransitionCostRequest> (cdl::ADD),
      new MemberFunctionChangeReceiver<PathQueryProcessor> (this,
...
void PathQueryProcessor::stop() {

}

void PathQueryProcessor::configure(
...
void PathQueryProcessor::runComponent() {
}

void PathQueryProcessor::newNavGraph(
...
void PathQueryProcessor::newPathTransitionProbRequest(
    const cast::cdl::WorkingMemoryChange &objID) {
  debug("newPathTransitionProbRequest called");
  CASTData<SpatialData::PathTransitionProbRequest> oobj =
      getMemoryEntryWithData<SpatialData::PathTransitionProbRequest> (
          objID.address);
  ...
  if (startHyp != 0) {
    ...
    if (ptr->noSuccessors < 0) {
      ...
      for (std::list<Cure::NavGraphNode*>::iterator i =
        ...
        SpatialData::PlacePtr place = m_placeInterface->getPlaceFromNodeID(
            nodeID);
        if (place != 0) {
          pp.placeID = place->id;
          pp.prob = 0;
      ...
      for (std::vector<SpatialData::NodeHypothesisPtr>::iterator i =
        ...
        if ((*i)->hypID == startHyp->hypID) {
          ...
          SpatialData::PlacePtr place = m_placeInterface->getPlaceFromHypID(
              hypID);
          if (place != 0) {
            pp.placeID = place->id;
            pp.prob = 0;
      ...
      NavData::FNodePtr startNode = m_placeInterface->getNodeFromPlaceID(
          ptr->startPlaceID);
      if (startNode == 0) {
    ...
    if (startNode == 0) {
      ...
      if (goalHyp != 0) {
        ...
        if (ptr->noSuccessors < 0) {
          ...
          for (std::list<Cure::NavGraphNode*>::iterator i =
            ...
            if ((*i)->getId() == startNode->nodeId) {
              ...
              SpatialData::PlacePtr place =
                  m_placeInterface->getPlaceFromNodeID(nodeID);
              if (place != 0) {
                pp.placeID = place->id;
                pp.prob = 0;
                ptr->successors.push_back(pp);
          ...
          for (std::vector<SpatialData::NodeHypothesisPtr>::iterator i =
            ...
            if ((*i)->hypID == goalHypID) {
              ...
              unsigned int hypID = (*i)->hypID;
              SpatialData::PlacePtr place =
                  m_placeInterface->getPlaceFromHypID(hypID);
              if (place != 0) {
                pp.placeID = place->id;
                pp.prob = 0;
                ptr->successors.push_back(pp);
        ...
        NavData::FNodePtr goalNode = m_placeInterface->getNodeFromPlaceID(
            ptr->goalPlaceID);
        if (goalNode == 0) {
          ...
          int ret = m_NavGraph.findPath(startNodeID, goalNodeID, path, &cost);
  ...
  if (ptr->status == SpatialData::QUERYPENDING)
    ptr->status = SpatialData::QUERYCOMPLETED;
  overwriteWorkingMemory<SpatialData::PathTransitionProbRequest> (
      objID.address, ptr);

  m_GraphMutex.unlock();
}

void PathQueryProcessor::newPathTransitionCostRequest(
    const cast::cdl::WorkingMemoryChange &objID) {
  CASTData<SpatialData::PathTransitionCostRequest> oobj =
      getMemoryEntryWithData<SpatialData::PathTransitionCostRequest> (
          objID.address);
    ...
    if (startNode == 0) {
      ...
      if (goalHyp != 0) {
        ...
        if (directlyReachable) {
          double distSq = (goalHyp->x - startNode->x) * (goalHyp->x
              - startNode->x) + (goalHyp->y - startNode->y) * (goalHyp->y
              - startNode->y);
          ptr->cost = sqrt(distSq);
          log("found direct connection... can break here");
          ptr->status = SpatialData::QUERYCOMPLETED;
        } else {
          goalNode = m_placeInterface->getNodeFromPlaceID(
              goalHyp->originPlaceID);
        }
        //	else
        //	{
        ...
        goalNode = m_placeInterface->getNodeFromPlaceID(ptr->goalPlaceID);
      } //goalHyp != 0
  ...
  overwriteWorkingMemory<SpatialData::PathTransitionCostRequest> (
      objID.address, ptr);
  m_GraphMutex.unlock();
  //}
}
