...
NewNavController::NewNavController(NavGraph &graph, LocalMap &lmap,
		cast::ManagedComponent* sc) :
	m_PP(0), m_Graph(graph), m_LMap(lmap), m_Turn(sc, 0.05), m_ND(8), m_TaskType(
			TASKTYPE_STOP), m_TaskID(-1), m_sc(sc), m_LastProgressTime(-1),
			m_ProgressTimeout(-1) {
...
int NewNavController::config(const std::string &configfile) {
...
void NewNavController::setMinPathLengthForTrim(unsigned int len) {
	if (len >= 1) {
...
void NewNavController::setMinNonzeroSpeeds(double minV, double minW) {
	m_MinNonZeroV = minV;
	m_MinNonZeroW = minW;
}

void NewNavController::setTurnAngleIntoSpeed(bool doit, double gain) {
	m_TurnAngleIntoSpeed = doit;
	m_AngleGain = gain;
}

void NewNavController::setGotoMaxSpeeds(double maxV, double maxW) {
	m_MaxGotoV = maxV;
	m_MaxGotoW = maxW;
}

void NewNavController::setTurnMaxSpeeds(double maxV, double maxW) {
	m_MaxTurnV = maxV;
	m_MaxTurnW = maxW;
}

void NewNavController::setGatewayMaxSpeeds(double maxV, double maxW) {
	m_MaxGatewayV = maxV;
	m_MaxGatewayW = maxW;
}

void NewNavController::setFollowMaxSpeeds(double maxV, double maxW) {
	m_MaxFollowV = maxV;
	m_MaxFollowW = maxW;
}

void NewNavController::turnOn() {
	m_sc->debug("Turning on NewNavController\n");
	m_TaskType = TASKTYPE_STOP;
}

void NewNavController::turnOff() {
...
void NewNavController::addEventListener(NewNavControllerEventListener *l) {
...
void NewNavController::delEventListener(NewNavControllerEventListener *l) {
...
void NewNavController::setProgressTimeout(double t) {
	m_ProgressTimeout = t;
	m_LastProgressTime = -1;
}

int NewNavController::prepForTask(const std::string &task) {
	if (m_TaskType == TASKTYPE_OFF) {
...
void NewNavController::startProgressMonitoring() {
...
void NewNavController::stopProgressMonitoring() {
	m_LastProgressTime = -1;
}

int NewNavController::stop() {
	if (m_TaskType == TASKTYPE_OFF)
		return RETVAL_OK;
...
int NewNavController::setDirectSpeed(int taskID, double v, double w,
		double timeout) {
	if (m_TaskType == TASKTYPE_OFF)
		return RETVAL_OK;
...
int NewNavController::gotoNode(int taskID, int id) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("gotoNode"))
		return RETVAL_ERROR;
...
int NewNavController::approachXY(int taskID, double x, double y) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("approachXY"))
		return RETVAL_ERROR;
...
int NewNavController::approachXYA(int taskID, double x, double y, double a) {
	if (prepForTask("approachXYA"))
		return RETVAL_ERROR;
...
int NewNavController::gotoXY(int taskID, double x, double y) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("gotoXY"))
		return RETVAL_ERROR;
...
int NewNavController::gotoXYA(int taskID, double x, double y, double a) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("gotoXYA"))
		return RETVAL_ERROR;
...
int NewNavController::gotoPolar(int taskID, double a, double r) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("gotoPolar"))
		return RETVAL_ERROR;
...
int NewNavController::rotateRel(int taskID, double da) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("rotateRel"))
		return RETVAL_ERROR;
...
int NewNavController::rotateAbs(int taskID, double a) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("rotateAbs"))
		return RETVAL_ERROR;
...
int NewNavController::backOff(int taskID, double dist) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}

	if (prepForTask("backOff"))
		return RETVAL_ERROR;
...
int NewNavController::followPerson(int taskID, double x, double y, double dir,
		double speed, double visibility) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}
...
int NewNavController::gotoArea(int taskID, int areaId) {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		return RETVAL_ERROR;
	}
	...
	for (int i = 0; i < 2; i++) {
		...
		for (std::list<NavGraphNode*>::iterator ni = m_Graph.m_Nodes.begin(); ni
				!= m_Graph.m_Nodes.end(); ni++) {
			...
			if ((*ni)->getAreaId() == areaId) {
				...
				if (m_Graph.findPath(cp.getX(), cp.getY(), cp.getTheta(), *ni, path,
						&cost) == 0) {
...
int NewNavController::execPath(int taskID, const std::list<NavGraphNode> &path,
		bool finalTurn) {
	if (finalTurn) {
		if (prepForTask("gotoXYA"))
			return RETVAL_ERROR;
...
void NewNavController::initTrimPath(const Cure::Pose3D &cp, std::list<
		NavGraphNode> &path) {
	if (!m_UsePathTrimming)
		return;
	...
	for (ni++; ni != path.end(); ni++) {
		...
		if (!m_CanTrimGateways && ni->getType() == NavGraphNode::NODETYPE_GATEWAY) {
...
void NewNavController::trimPath(const Cure::Pose3D &cp,
		std::list<NavGraphNode> &path) {
	if (!m_UsePathTrimming)
		return;

	if (path.size() <= m_MinPathLengthForTrim)
		return;

	if (!m_CanTrimGateways && path.front().getType()
	...
	for (ni++; ni != path.end(); ni++) {
		...
		if (!m_LMap.goalReachable(ni->getX(), ni->getY(), 0.5 * getRobotWidth(),
				m_MaxTrimDist + 1.0)) {
		...
		for (std::list<NavGraphNode>::iterator ni2 = path.begin(); ni2 != ni; ni2++) {
			double d = HelpFunctions::distPt2LinePts(cp.getX(), cp.getY(),
					ni->getX(), ni->getY(), ni2->getX(), ni2->getY());
			if (d > m_InterGoalTol) {
...
int NewNavController::updateCtrl() {
	...
	if (m_TaskType == TASKTYPE_DIRECT && HelpFunctions::getCurrentTime()
			> m_DirectTimeout) {
		m_TaskType = TASKTYPE_STOP;
		reportDone( m_TaskID);
	}
	...
	if (m_TaskType == TASKTYPE_STOP) {
		...
		if (m_TaskType == TASKTYPE_GOTOXY || m_TaskType == TASKTYPE_GOTOXYA
				|| m_TaskType == TASKTYPE_FOLLOWPERSON) {
			...
			do {
				...
				m_sc->log("Heading for node %d of type %d m_PrevWasGateway=%d d=%f\n",
						m_Path.front().getId(), m_Path.front().getType(), m_PrevWasGateway,
						d);
				...
				if (m_Path.size() > 1) {
					...
					if (m_TaskType == TASKTYPE_FOLLOWPERSON) {
						...
						if (pd < m_FollowBackupThreshold || ((m_FollowV < slowPersonSpeed
								|| fabs(Cure::HelpFunctions::angleDiffRad(cp.getTheta() + M_PI,
										m_FollowDir)) < M_PI_4) && (pd < m_FollowDist
								- m_FollowTolDist))) {
							// If we are too close we should back up
							followBackup = true;
							m_sc->error("FOLLOW: Too close back up, pd=%f\n", pd);

						} else if ((m_FollowV < slowPersonSpeed || (fabs(
								Cure::HelpFunctions::angleDiffRad(cp.getTheta() + M_PI,
										m_FollowDir)) > M_PI / 3.0 || fabs(
								Cure::HelpFunctions::angleDiffRad(cp.getTheta(), m_FollowDir))
								> M_PI / 3.0)) && fabs(pd - m_FollowDist) < m_FollowTolDist) {
				...
				if (goalReached) {
					...
					if (m_Path.size() == 1) {
						if (m_TaskType == TASKTYPE_GOTOXYA) {
							...
							// Point to the target
							cmd.dir = atan2(m_FollowY - cp.getY(), m_FollowX - cp.getX());
			...
			if (m_TaskType != TASKTYPE_ROTATE && !turnMode && !followBackup) {
				...
				double altGX, altGY;
				int err = m_ND.calcMotionCommand(m_LMap, goalX, goalY, m_MaxGotoV, cmd,
						largeTurn, true, altGX, altGY);
				if (err == 0) {
					// We limit the speed based on the distance to the goal or
					// so depending on the mode
					if (m_TaskType == TASKTYPE_FOLLOWPERSON) {
						double distErr = (hypot(m_FollowY - cp.getY(), m_FollowX
								- cp.getX()) - m_FollowDist);
						...
						if (forwVel > 0.2) {
							Cure::HelpFunctions::limitAndSetValue(cmd.v, 0, forwVel + 0.5
									* distErr);
						} else if (distErr > 0) {
		...
		if (m_TaskType == TASKTYPE_ROTATE || largeTurn) {
			...
			if (m_ProgressTimeout > 0 && m_LastProgressTime > 0) {
				...
				// progress
				if (Cure::HelpFunctions::getCurrentTime() - m_LastProgressTime
						> m_ProgressTimeout) {
			...
			if (m_TaskType == TASKTYPE_ROTATE && fabs(da) < m_FinalGoalTolRot) {
				...
				m_TaskType = TASKTYPE_STOP;
				reportDone( m_TaskID);
			} else if (m_TaskType == TASKTYPE_FOLLOWPERSON && fabs(da)
					< m_FollowTolRot) {
				cmd.w = 0;
			} else {
		...
		if (m_TaskType == TASKTYPE_APPROACH_POINT || m_TaskType
			...
			if (distance < m_ApproachTolDist) {
				...
				if ((m_TaskType == TASKTYPE_APPROACH_POINT) || (fabs(da)
						< m_ApproachTolRot)) {
		...
		if (m_TaskType == TASKTYPE_FOLLOWPERSON) {
			if (followBackup) {
				...
				if (m_LMap.obstacleFreeRegion(m_LMap.getX(), m_LMap.getY(),
						m_LMap.getX() - d * cos(m_LMap.getA()), m_LMap.getY() - d * sin(
								m_LMap.getA()), w)) {
		...
		if (cmd.type == Cure::MotionAlgorithm::CMD_TYPE_VA) {
			if (m_TurnAngleIntoSpeed) {
				cmd.type = Cure::MotionAlgorithm::CMD_TYPE_VW;
				cmd.w = (m_AngleGain * Cure::HelpFunctions::angleDiffRad(cmd.dir,
						cp.getTheta()));
		...
		m_sc->log("Speed command now type=%d v=%f w=%f dir=%f", cmd.type, cmd.v,
				cmd.w, Cure::HelpFunctions::rad2deg(cmd.dir));

		if (cmd.v > 0 && cmd.v < m_MinNonZeroV)
			cmd.v = m_MinNonZeroV;
		if (cmd.v < 0 && cmd.v > -m_MinNonZeroV)
			cmd.v = -m_MinNonZeroV;
		if (cmd.w > 0 && cmd.w < m_MinNonZeroW)
			cmd.w = m_MinNonZeroW;
		if (cmd.w < 0 && cmd.w > -m_MinNonZeroW)
			cmd.w = -m_MinNonZeroW;
...
void NewNavController::reportAbort(int taskID) {
	std::list<NewNavControllerEventListener*>::iterator li;
...
void NewNavController::reportDone(int taskID) {
	std::list<NewNavControllerEventListener*>::iterator li;
	for (li = m_EventListeners.begin(); li != m_EventListeners.end(); li++) {
		(*li)->doneTask(taskID);
	}
...
void NewNavController::reportFail(int taskID, int error) {
	std::list<NewNavControllerEventListener*>::iterator li;
	for (li = m_EventListeners.begin(); li != m_EventListeners.end(); li++) {
...
bool NewNavController::getCurrentNode(Cure::NavGraphNode &n) const {
	if (m_Path.empty()) {
...
bool NewNavController::getTargetNode(Cure::NavGraphNode &n) const {
	if (m_Path.empty()) {
		m_sc->log("Path is empty, no node to return\n");
		return false;
	}
...
Cure::Pose3D NewNavController::getPose(bool usePrediction) const {
	if (m_PP == 0) {
		m_sc->error("Must call setPoseProvider first\n");
		Cure::Pose3D ret;
		ret.setTime(0);
...
int NewNavController::findPath(const Cure::Pose3D &goal,
		std::list<NavGraphNode> &path) {
	...
	Cure::Pose3D cp;
	cp = getPose();
	return m_Graph.findPath(cp.getX(), cp.getY(), cp.getTheta(), goal.getX(),
			goal.getY(), goal.getTheta(), path, 0);
}

int NewNavController::findPath(NavGraphNode *goal,
		std::list<NavGraphNode> &path) {
	if (goal == 0) {
...
int NewNavController::findPath(double x, double y, double a, std::list<
		NavGraphNode> &path) {
	Cure::Pose3D g;
	g.setX(x);
	g.setY(y);
