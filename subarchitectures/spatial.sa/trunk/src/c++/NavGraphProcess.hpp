...
protected:
  ...
  void addFreeNode(long nodeID, double x, double y, double z, double theta,
      long areaID, const std::string &areaType, long areaTypeNo, short gateway,
   ...
   *
   */
  void addObject(double x, double y, double z, double camX, double camY,
      long areaID, const std::string &category, long objectID,
      double probability);
...
private:
  class NavGraphServer: public NavData::NavGraphInterface {
    virtual double getPathLength(double xS, double yS, double aS, double xG,
        double yG, double aG, const Ice::Current &_context);
    virtual int getAreaId(double x, double y, double a, double maxDist,
        const Ice::Current &_context);
    virtual int getClosestNodeId(double x, double y, double a, double maxDist,
        const Ice::Current &_context);
    NavGraphProcess *m_pOwner;
    NavGraphServer(NavGraphProcess *owner) :
      m_pOwner(owner) {
    }
  ...
  void checkAndAddArea(int aid, const std::string &areaClass = "",
      long areaClassNo = -1);
  ...
  enum GraphEventType {
    NEW_OBJ_OBS, NEW_VISUAL_OBJECT, NEW_DOOR_HYPTOHESIS
  };
