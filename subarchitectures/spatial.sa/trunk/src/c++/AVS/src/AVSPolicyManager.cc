...
void policyPrint(std::list<std::string> &lst) {
  for (std::list<std::string>::iterator pi = lst.begin(); pi != lst.end(); pi++) {
    std::cerr << (*pi) << " -> ";
  }
...
void AVSPolicyManager::Configuration::print() {
  ...
  std::cerr << m_Final << " " << m_Room << " " << m_Index << " "
      << m_Probability << " " << m_NumberOfSubsets << " "
      << m_NumberOfSupersets << "\n";
...
bool AVSPolicyManager::Configuration::addToSupersets(Configuration *cfg) {
  if (cfg == this)
    return false;
  if (cfg->m_Configuration == m_Configuration)
    return false;
...
bool AVSPolicyManager::Configuration::addToSubsets(Configuration *cfg) {
  ...
  if (moreOrEqualGeneral(m_Configuration, cfg->m_Configuration)) {
    ...
    for (int i = 0; i < m_NumberOfSubsets; i++) {
      if (m_Subsets[i] == cfg)
        return true;
      if (m_Subsets[i]->addToSubsets(cfg))
        return true;
    }
...
void AVSPolicyManager::Policy::print() {
  ...
  for (std::list<std::string>::iterator pi = m_Policy.begin(); pi
      != m_Policy.end(); pi++) {
    std::cerr << (*pi) << " -> ";
  }
  std::cerr << "\n";
...
void AVSPolicyManager::PolicyTree::completeList(
    std::list<std::string> &configurations, std::list<std::string> &more) {
  std::istringstream sst(m_Configuration);
  ...
  if (cnt > 1) {
    ...
    for (long i = 0; i < combin.rows(); i++) {
      ...
      tok = osst.str();
      bool ok = false;
      for (std::list<std::string>::iterator pi = configurations.begin(); pi
          != configurations.end(); pi++) {
        if (tok == (*pi)) {
          ok = true;
          break;
        }
      ...
      if (!ok) {
        for (std::list<std::string>::iterator pi = more.begin(); pi
            != more.end(); pi++) {
          if (tok == (*pi)) {
            ok = true;
            break;
...
void AVSPolicyManager::getConfigurations(
    std::list<std::string> &configurations, Cure::Matrix &probabilities,
    std::string searchTarget) {
  configurations.clear();
  probabilities.grow(0, 1);
...
void AVSPolicyManager::addProbabilities(Cure::Matrix &probabilities, std::list<
    std::string> &more) {
  for (std::list<std::string>::iterator pi = more.begin(); pi != more.end(); pi++) {
...
void AVSPolicyManager::configure(std::list<std::string> &configurations,
  ...
  while (p->m_Next) {
    if (p->m_Room != room) {
      room = p->m_Room;
      roomindex = 0;
      for (std::list<std::string>::iterator pi = rooms.begin(); pi
          != rooms.end(); pi++, roomindex++) {
        if (room == (*pi))
          break;
      }
    }
    sizep = 0;
    for (std::list<std::string>::iterator pi = p->m_Policy.begin(); pi
        != p->m_Policy.end(); pi++) {
      str1 = (*pi);
      k = getConfig(str1);
  ...
  for (i = 0; i < m_NumberOfConfigs; i++) {
    ...
    roomindex = 0;
    for (std::list<std::string>::iterator pi = rooms.begin(); pi != rooms.end(); pi++, roomindex++) {
      if (room == (*pi))
        break;
    }
...
void AVSPolicyManager::configureExclusive(
    std::list<std::string> &configurations, Cure::Matrix &probabilities) {
...
bool AVSPolicyManager::nextNearlyBestPolicy(std::list<std::string> &pol,
  ...
  for (int room = 0; room < m_NumberOfRooms; room++) {
    ...
    for (int k = 0; k < m_NumberOfRooms; k++)
      if (k != room)
        eliminate(m_Rooms[k]->m_Index, 1.0);
    save(prevPaths[0]);
      ...
      for (int step = 0; step < m_MaxSteps; step++) {
            ...
            if (1) {//((step>0)||(getProb(poly)>m_Threshold)){
              ...
              if ((c > 0) && (prob > 1E-6)) {//m_RoomProbability(1,room))){	     
                ...
                if (m_Type & 1)
                  ...
                  c /= (m_ProbabilityItsHere - prob * (prob - 1
                      + m_ProbabilityItsHere));
                }
        ...
        for (long i = 0; i < numnext; i++) {
          ...
          for (int j = 0; j < m_PolicyDepend.columns(m_CurrentPolicy); j++)
            nextPaths[i].m_Dependent. addUnique(0, m_PolicyDepend(
                m_CurrentPolicy, j));
          update(percentage);
          save(nextPaths[i]);
        }
...
void AVSPolicyManager::iterate(PolicyPath *paths, short step, long &bestpol,
    double &best, long &poly, short *prevroom, short *roomcnt,
    short *stepsInRoom, PolicyPath *bestPaths, double percentage) {

  for (int room = 0; room < m_NumberOfRooms; room++) {
    if (roomcnt[room] < stepsInRoom[room]) {
      restore(paths[step]);
      m_CurrentRoom = prevroom[step];
      m_CurrentPolicy = bestPaths[room].m_Index[roomcnt[room]];
      roomcnt[room]++;
      if (step == 0)
        poly = m_CurrentPolicy;
      ...
      if ((c > 0) && (prob > 1E-15)) {
        ...
        c /= 2.0;
        good = true;
        c = paths[step].m_Q * (c + (1.0 - prob) * c + m_RoomCosts(room,
            prevroom[step]));
...
bool AVSPolicyManager::multiRoomSearch(std::list<std::string> &pol,
    short *stepsInRoom, PolicyPath *bestPaths, PolicyPath &restorePath,
    double percentage) {
  bool ok = true;
  if (m_MaxSteps < 1)
    ok = false;
  ...
  iterate(paths, step, bestpoly, best, poly, prevroom, roomcnt, stepsInRoom,
      bestPaths, percentage);
  restore(restorePath);
...
void AVSPolicyManager::update(std::string &currentCfg, double percentage) {
  long currentstep = -1;
  long bestrow = -1;
  if (m_CurrentPolicy > -1) {//figure out where we are and what happened
...
void AVSPolicyManager::getFileConfigurations(
    std::list<std::string> &configurations, Cure::Matrix &probabilities,
    std::string searchTarget) {
  configurations.clear();
...
void AVSPolicyManager::getConfiguration(std::list<std::string> &strings,
    std::list<std::string> &configurations, Cure::Matrix &probabilities) {
  probabilities.reallocateZero(strings.size(), 1);
  long k = 0;
  std::string cmd = "";
  for (std::list<std::string>::iterator pi = strings.begin(); pi
      != strings.end(); pi++, k++) {
    CureCERR(80) << (*pi) << "\n";
...
void AVSPolicyManager::getFileRoomCosts(std::list<std::string> &rooms,
    Cure::Matrix &costs) {
  rooms.clear();
  Cure::ConfigFileReader cfg;
