...
public:
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(NavGraphNode *from, int toNodeId, std::list<NavGraphNode> &path,
			double *cost);
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(int fromNodeId, NavGraphNode *to, std::list<NavGraphNode> &path,
			double *cost);
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(int fromNodeId, int toNodeId, std::list<NavGraphNode> &path,
			double *cost);
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(NavGraphNode *from, double xG, double yG, double aG, std::list<
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(double xS, double yS, double aS, double xG, double yG,
			double aG, std::list<NavGraphNode> &path, double *cost);
	 ...
	 * @return o if ok, else error code
	 */
	int findPath(double xS, double yS, double aS, NavGraphNode *to, std::list<
			NavGraphNode> &path, double *cost);
	 ...
	 */
	int addNodeToNodeList(int id, int areaId, double x, double y, double a,
			const std::string &name = "-", double maxspeed = 2);
	 ...
	 * @param maxspeed the max speed the robot is allowed to drive near this node
	 */
	int addDoorToNodeList(int id, int areaId, double x, double y, double a,
			double width, const std::string &name = "-", double maxspeed = 1);
	...
	void displayRL(RoboLookProxy *rlp, int env = RL_ENV_EST, bool clearStars =
			false, bool clearLines = false, bool showDoorFrames = false);
	 ...
	 */
	void configRLStars(double sizeCurrent = -1, double sizeNormal = -1,
			double sizeNamedGoal = -1, double sizeGateway = -1,
	 ...
	 * be found to it
	 */
	void setMaxDistTargetFromNode(double d) {
		m_MaxDist = d;
	}

	double getMaxDistTargetFromNode() const {
		return m_MaxDist;
	}
protected:
	 ...
	 * happened, else error code <0
	 */
	int maybeAddNewNodeAt(double x, double y, double a, bool forceInNode,
			int type);
	 ...
	 * @return 0 if ok, else error code
	 */
	int makeGatewayOutOfNode(NavGraphNode *node, double x, double y, double a,
			double width);
