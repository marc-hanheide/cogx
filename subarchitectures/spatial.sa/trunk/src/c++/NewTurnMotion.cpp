...
int NewTurnMotion::calcMotionCommand(LocalMap &m, double aGoal, double vMax,
    MotionAlgorithm::MotionCmd &cmd) {
  robX_ = m.getX();
  robY_ = m.getY();
  robA_ = m.getA();
  ...
  clearBackward_ = m.obstacleFreeRegion(robX_ + frontDist_ * cos(robA_), robY_
      + frontDist_ * sin(robA_), robX_ - ((backDist_ + freeZoneFB_)
      * cos(robA_)), robY_ - ((backDist_ + freeZoneFB_) * sin(robA_)), 2.0
      * (halfWidth_ + freeZoneS_));

  // Check if the obstacles are only behind the robot
  clearForward_ = m.obstacleFreeRegion(robX_ + ((frontDist_ + freeZoneFB_)
      * cos(robA_)), robY_ + ((frontDist_ + freeZoneFB_) * sin(robA_)), robX_
      - backDist_ * cos(robA_), robY_ - backDist_ * sin(robA_), 2.0
      * (halfWidth_ + freeZoneS_));
  ...
  m_sc->log("mode=%d robA=%f dir=%f vel=%f err=%d type=%d\n", mode_, rad2deg(
      robA_), rad2deg(cmd.dir), cmd.v, err, cmd.type);

  return err;
}

bool NewTurnMotion::findCloseObstacles(LocalMap &m) {
  nClosePts_ = 0;
  ...
  if (minObstIndex >= 0) {
    double a = angleDiffRad(atan2(m.obstRef(minObstIndex).y - robY_, m.obstRef(
        minObstIndex).x - robX_), robA_);
...
bool NewTurnMotion::inStuckSituation() {
  ...
  for (int i = 0; i < nClosePts_; i++) {
    ...
    double fdist = distPt2Line(closePts_[i].o->x, closePts_[i].o->y, robX_,
        robY_, cosFront, sinFront);
    ...
    if (fabs(fdist) < hlen && fabs(fdist) > 0.05) {
      ...
      double sdist = distPt2Line(closePts_[i].o->x, closePts_[i].o->y, robX_,
          robY_, cosSide, sinSide);
  ...
  if ((closeFrontLeft && closeRearLeft) || (closeFrontRight && closeRearRight)) {
...
bool NewTurnMotion::canTurnOnTheSpot() {
  ...
  for (int i = 0; i < nSteps; i++) {
    if (findViolations(robX_, robY_, robA_ + stepSize * i))
      return false;
  }
...
bool NewTurnMotion::findViolations(double rX, double rY, double rA) {
  nViolationPts_ = 0;
  ...
  for (int i = 0; i < nClosePts_; i++) {
    ...
    double a = angleDiffRad(atan2(closePts_[i].o->y - rY, closePts_[i].o->x
        - rX), rA);
    int s = getSector(a);
    if (d < E_[s] + ds_) {
      m_sc->log("Found violation at x=%f y=%f d=%f dd=%f s=%d a=%f",
          closePts_[i].o->x, closePts_[i].o->y, d, d - E_[s], s, rad2deg(a));
  ...
  m_sc->log("violation regions rl=%d fl=%d rr=%d fr=%d", violRearLeft_,
      violFrontLeft_, violRearRight_, violFrontRight_);

  m_sc->debug("Found %d violation point(s)\n", nViolationPts_);
  return (nViolationPts_ > 0);
}

bool NewTurnMotion::changeToMoveMode() {
  if (nViolationPts_ == 0) {
    m_sc->error("No violation points, cannot change to move mode\n");
    return false;
  }

  if ((violFrontLeft_ || violFrontRight_) && (violRearLeft_ || violRearRight_)) {
...
bool NewTurnMotion::calcTranslationCommand(double &vel) {
  if (mode_ == MODE_BACKWARD) {
...
bool NewTurnMotion::configRobotShape(int type, const std::string &params) {
  ...
  if (type == 0) {
    ...
    if (!(str >> radius)) {
      m_sc->error(
          "Failed to parse robot radius, assuming radius=%f coffset=%f",
          radius, centerOffset);
      ret = false;
    } else {
    ...
    if (!(str >> length >> width >> centerOffset)) {
      m_sc->error(
          "Failed to parse rect shape params, using length=%f width=%f coffset=%f\n",
  ...
  m_sc->debug("frontDist_=%f backDist_=%f halfWidth_=%f\n", frontDist_,
      backDist_, halfWidth_);

  return ret;
}

void NewTurnMotion::setCircularShape(double radius, double centerOffset) {
  frontDist_ = 0.5 * radius + centerOffset;
  backDist_ = 0.5 * radius - centerOffset;
  halfWidth_ = 0.5 * radius;

  maxRadius_ = hypot(HelpFunctions::max(frontDist_, backDist_), halfWidth_);

  MotionAlgorithm::setCircularShape(radius, centerOffset);
}

void NewTurnMotion::setRectangularShape(double robotLength, double robotWidth,
    double centerOffset) {
...
void NewTurnMotion::setPolygonShape(int np, double *lx, double *ly) {
...
void NewTurnMotion::setShapeFunction(double *E) {
...
int NewTurnMotion::getSector(double a) {
...
void NewTurnMotion::displayRL(RoboLookProxy *rlp) {
  if (rlp == NULL)
    return;
