...
public:
  ...
  /** Writes image. */
  bool writeImage(const PlaceData::ImagePtr image, Format format);

  /** Copies an image from the source file. The file name is recreated based on the PlaceData::Image. */
  bool copyImage(std::string sourceImage, const PlaceData::ImagePtr image);

  /** Writes scan. */
  bool writeLaserScan(const PlaceData::LaserScanPtr scan);

  /** Writes odometry. */
  bool writeOdometry(const PlaceData::OdometryPtr odom);
...
private:
  ...
  bool writePgmImage(const PlaceData::ImagePtr image, std::string path);
