...
public:
  ...
  /** Reads an image. */
  bool readImage(const PlaceData::ImagePtr image);

  /** Reads info about an image. Image data are not loaded. */
  bool readImageInfo(std::string &iPath, PlaceData::ImagePtr image, bool checkFrameNo=true);

  /** Reads a scan. */
  bool readLaserScan(PlaceData::LaserScanPtr scan, bool checkFrameNo=true);

  /** Reads an odometry. */
  bool readOdometry(PlaceData::OdometryPtr odom, bool checkFrameNo=true);

  /** Reads target. */
  bool readTarget(PlaceData::TargetPtr target, bool checkFrameNo=true);

  /** Reads a PGM image. */
  bool readPgmImage(PlaceData::ImagePtr image, std::string filePath);
