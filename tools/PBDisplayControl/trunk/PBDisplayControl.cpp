...
ControlWindow::ControlWindow(PeekabotController &_ctrl) :
  m_controller(_ctrl), follow_robot_button("follow the robot") {
  ...
  m_refTreeSelection->signal_changed().connect(sigc::mem_fun(*this,
      &ControlWindow::on_selection_changed));
  // The final step is to display this newly created widget...
  thread
      = Glib::Thread::create(sigc::mem_fun(*this, &ControlWindow::run), true);
...
void PeekabotController::ApplyView(int vid) {
  ...
  if (s.succeeded()) {
    if (!ps.follow_robot) {
      pb_camera.set_position(ps.views[vid].camera.x, ps.views[vid].camera.y,
          ps.views[vid].camera.z);
    }
    pb_camera.set_rotation(ps.views[vid].camera.yaw,
        ps.views[vid].camera.pitch, ps.views[vid].camera.roll);
    pb_camera.set_orthographic(ps.views[vid].camera.orthographic);
  }

  std::map<std::string, bool>::const_iterator iter1;
  for (iter1 = ps.views[vid].objects.begin(); iter1
      != ps.views[vid].objects.end(); ++iter1) {
    peekabot::ObjectProxy pb_object;
...
void printHelp() {
  cerr
      << "Usage: PBDisplayControl [--exec {View_name | [+|-]Element_to_display} ...]\n";
}
...
void ControlWindow::setActivationFromView(int vid) {
  std::map<std::string, bool>::const_iterator iter1;
  for (iter1 = m_controller.ps.views[vid].objects.begin(); iter1
      != m_controller.ps.views[vid].objects.end(); ++iter1) {
    if (iter1->second) {
