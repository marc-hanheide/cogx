...
public abstract class ContentProxy<T extends ProbDistribution> implements Proxy<T> {
	public abstract T getContent();
	public static ConditionallyIndependentDistributionProxy asConditinallyIndependentDistributionDelegate(ProbDistribution pd) {
		assert (pd instanceof CondIndependentDistribs);
		return new ConditionallyIndependentDistributionProxy(pd);
	}

	public static BasicProbDistributionProxy asBasicDistributionDelegate(ProbDistribution pd) {
		assert (pd instanceof BasicProbDistribution);
		return new BasicProbDistributionProxy(pd);
	}
	
	public static ContentProxy<?> createDelegate(ProbDistribution pd) {
		if (pd instanceof BasicProbDistribution) {
			return asBasicDistributionDelegate(pd);
		} else {
			return asConditinallyIndependentDistributionDelegate(pd);
		}
	}
