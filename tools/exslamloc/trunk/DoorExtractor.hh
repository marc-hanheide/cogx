...
#ifndef Cure_DoorExtractor_hh
#define Cure_DoorExtractor_hh
...
public:
   ...
   */
  Cure::Timestamp timestamp() const { return m_Timestamp; }
   ...
   */
  std::list<RedundantLine2DRep>& doors();
   ...
   */
  int extract(Cure::SICKScan &scan, int algs = -1);
...
private: 
  ...
  Cure::LsqLineExtractor *m_Lsq;

  /** list of extracted doors */
  std::list<RedundantLine2DRep> doors_;
  ...
  std::list<RedundantLine2DRep>& lsqLines();

  /** Does init stuff, like creating the lineExtarctor, it is called
      form the constructor */
  void init();

  bool linesParallel(std::list<RedundantLine2DRep>::iterator &wallLineR, 
                     std::list<RedundantLine2DRep>::iterator &wallLineL);
