...
#ifndef Cure_LsqLineExtractor_hh
#define Cure_LsqLineExtractor_hh

#include <Sensory/RedundantLine2DRep.hh>
...
/**
 * Class used to extract lines using a least squares algorithm. Its
 * main use is in the LineServer
 *
...
public:
   ...
   * determine if the points is an outlier or not. If this is data
   * that comes from some source where the range readings are not well
   * defined just supply a 0 pointer.
   */
  int extract(double *x, double *y, double *r, int nData);

  /** @return a reference to a list of extracted lines */
  std::list<Cure::RedundantLine2DRep>& lines();
...
private:
  ...
  std::list<RedundantLine2DRep> lines_;
   ...
   */
  void fillInLine(int nUsed, int* used, RedundantLine2DRep& line);

}; // class LsqLineExtractor //

inline std::list<RedundantLine2DRep>&
...
#endif // Cure_LsqLineExtractor_hh
