...
void displayPeekabot(Cure::SICKScan &scan, Cure::PoseProvider *pp,
                     Cure::LocalMap &lmap)
{
  if (g_subsampleDisplay > 0 &&
      g_dispCounter < g_subsampleDisplay) return;
  g_dispCounter = 0;
  ...
  if (g_dispMask & DISP_WALLS) {
    ...
    while( g_pps.size() < walls.size() ) {
      ...
      char buf[32];
      sprintf(buf, "poly%03d", g_pps.size());
    ...
    if (walls.size() > 0) {
      ...
      for (std::list<Cure::Line2D>::iterator w = walls.begin();
        ...
        vs.add_vertex(w->StartPoint.getX(), w->StartPoint.getY(), 0);
        vs.add_vertex(w->StartPoint.getX(), w->StartPoint.getY(), 1.0);
        vs.add_vertex(w->EndPoint.getX(), w->EndPoint.getY(), 1.0);
        vs.add_vertex(w->EndPoint.getX(), w->EndPoint.getY(), 0);
  ...
  if (g_dispMask & DISP_GRAPH) {
    ...
    for (iter = g_graph.m_Nodes.begin();
      ...
      sp.add(gp, name);
      sp.set_pose((*iter)->getX(),(*iter)->getY(),0,(*iter)->getTheta(),0,0);
  ...
  if (g_dispCounterGlobal == 1 && g_pauseAfterFirstDisplay) getchar();
  else if (g_pauseAfterDisplay) getchar();
}
                 ...
                 Cure::NavGraph &graph)
{
  ...
  if (g_saveData && !g_loadData) g_odomfile.write(odom);
...
void processScan(Cure::SICKScan &scan, Cure::PoseProvider *pp,
                 Cure::NavGraph &graph)
{
  if (g_saveData && !g_loadData) g_scanfile.write(scan);
  ...
  if (g_extractDoors) {
    ...
    for (std::list<Cure::RedundantLine2DRep>::iterator i = 
           g_doorExtractor.doors().begin();
         i != g_doorExtractor.doors().end(); i++) {
      DoorMeas dm;
      dm.x = i->xC();
      dm.y = i->yC();
      Cure::HelpFunctions::transfPtL2W(dm.x, dm.y, 
    ...
    if (g_writeouput) {
      ...
      if (g_odombuf.read(scan_odom, scan.getTime())) {
        ...
        char tbuf[32];
        sprintf(tbuf, "%ld.%06ld", 
                scan.getTime().Seconds, scan.getTime().Microsec);
        fs_out << tbuf << " " 
               << scan_odom.getX() << " "
               << scan_odom.getY() << " "
               << scan_odom.getTheta() << " "
  ...
  // detected
    ...
    double xR = lpW.getX() + (doorDetector.m_RangeR * cos(lpW.getTheta() + doorDetector.m_AngleR));
    double yR = lpW.getY() + (doorDetector.m_RangeR * sin(lpW.getTheta() + doorDetector.m_AngleR));
    double xL = lpW.getX() + (doorDetector.m_RangeL * cos(lpW.getTheta() + doorDetector.m_AngleL));
    double yL = lpW.getY() + (doorDetector.m_RangeL * sin(lpW.getTheta() + doorDetector.m_AngleL));
...
int scanCallback(Cure::TimestampedData &td)
{
  ...
  */
  ...
  if (!g_supressDisplay) displayPeekabot(scan, g_poseprov, g_lmap);
...
void runFromFile(Cure::ConfigFileReader &cfg)
{
  ...
  if (g_writeSlamPoses) {
    if (posefile.openWriteFile("slampose.tdf")) return;
  }

  Cure::FileAddressManager fam;
  if (fam.addOdomFile(cfg, &odomFcn)) return;
  if (fam.addSensorFile(cfg, 3, &scanFcn)) return;
