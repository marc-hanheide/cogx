...
private final Class<Via> m_viaCls;
	...
	public double match(WorkingMemoryChange wmc, From from, To to) {
		logger.debug("Matching is errelevant for verified beliefs");
		return 0.0;
	/*
		if (beliefTypes != null)
			if (!beliefTypes.contains(from.type))
...
// 		// if the current key is in the set of keys to ignore, we ignore it!
	...
	protected final CASTIndependentFormulaDistributionsBelief<Via> createViaBeliefProxy(
			Via via) {
		return CASTIndependentFormulaDistributionsBelief.create(m_viaCls, via);
	}
	/*
	 * (non-Javadoc)
	 */
	@Override
	public void update(WorkingMemoryChange wmc, From from, Via via, To to)
			throws IncompatibleAssignmentException {
		if (beliefTypes != null) {
			if (!beliefTypes.contains(from.type))
				throw (new IncompatibleAssignmentException(
						"cannot update with from type " + from.type));
			if (!beliefTypes.contains(via.type))
				throw (new IncompatibleAssignmentException(
						"cannot update with via type " + via.type));
	...
	protected void mergeBeliefs(WorkingMemoryChange wmc,
			CASTIndependentFormulaDistributionsBelief<From> fromBelief,
			CASTIndependentFormulaDistributionsBelief<Via> viaBelief,
			CASTIndependentFormulaDistributionsBelief<To> toBelief) {

		try {
			toBelief.setTime(toBelief.getStartTime(), fromBelief.getEndTime());
			mergeDist(fromBelief.getContent(), viaBelief.getContent(), toBelief.getContent());
			propagatePointers(toBelief, ignoreForPointerPropagationSet);
		} catch (InterruptedException e) {
			logger.error("interrupted", e);
		}
	...
	@Override
	public WorkingMemoryAddress bestMatch(WorkingMemoryChange wmc, From from,
				WMView beliefView) {
