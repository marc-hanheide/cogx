...
#define CHESSBOARD_BOXES_WIDTH 30
#define CHESSBOARD_BOXES_HEIGHT 30
...
int main ( int argc, char *argv[] ) {
    ...
    if ( argc < 3 ) {
        printf ( "usage: %s < folder regx> [ -c -i -m ] \n", argv[0] );
    ...
    for ( int i = 3; i < argc; i++ ) {
        if ( strcmp ( argv[i], "-c" ) == 0 ) {
            ...
            fBoardBoxWidth = atof ( argv[i + 3] );
            fBoardBoxHeight = atof ( argv[i + 4] );
    ...
    cvPrint ( pIntrinsic, "intrinsicDistored\n");
    cvPrint ( pDistortions, "Distortions\n\n");

    CvMat *pImgSize = cvCreateMat ( 2,1,CV_64FC1 );
    cvSetReal1D(pImgSize, 0, width);
    cvSetReal1D(pImgSize, 1, height);
    cvSave ( "intrinsicDistored.xml", pIntrinsic );
    cvSave ( "distortions.xml", pDistortions );
    cvSave ( "imgsize.xml", pImgSize );
    cvReleaseMat(&pImgSize);

    CvMat *pImagePoints2 = cvCreateMat ( image_points.size(),2,CV_64FC1 );
    CvMat imagePointsSrc64 = cvMat ( image_points.size(),1,CV_64FC2, pImagePoints->data.db );
    CvMat imagePointsDes64 = cvMat ( image_points.size(),1,CV_64FC2, pImagePoints2->data.db );
    cvZero(pImagePoints2);
    CvMat *pIntrinsic2 = cvCreateMat ( 3,3,CV_64FC1 );
    CvMat *pDistortions2 = cvCreateMat ( 4,1,CV_64FC1 );

    CvPoint2D64f pDes;
    for ( unsigned int i = 0; i < image_points.size(); i++ ) {
        undistort ( pIntrinsic->data.db, pDistortions->data.db, ( double* ) &image_points[i], ( double* ) &pDes );
        CV_MAT_ELEM ( *pImagePoints2, double, i, 0 ) = pDes.x;
        CV_MAT_ELEM ( *pImagePoints2, double, i, 1 ) = pDes.y;
    }
    
    CV_MAT_ELEM ( *pIntrinsic2, double, 0, 0 ) = 1.0f;
    CV_MAT_ELEM ( *pIntrinsic2, double, 1, 1 ) = 1.0f;

    printf("calibrating camera, please wait ...\n");
    cvCalibrateCamera2 ( pObjectPoints, pImagePoints2, pPointsCounts, cvGetSize ( pImgGray ), pIntrinsic2, pDistortions2, pRotVecs, pTraVecs, 0 );

    printf ( "Paramter of the undistored image using : cvUndistortPoints\n");
    cvPrint ( pIntrinsic2, "intrinsicUndistored\n");
    cvPrint ( pDistortions2, "Distortions after undistoring\n");
    cvSave ( "intrinsicUndistored.xml", pIntrinsic2 );
    cvSave("rvecs.xml", pRotVecs);
    cvSave("tvecs.xml", pTraVecs);
    cvSave("imagePoints.xml", pImagePoints2);
    cvSave("objectPoints.xml", pObjectPoints);
    
    printf("\nparameters saved to file:\n");
    printf("  intrinsicUndistored.xml\n");
    printf("  distortions.xml\n");
    printf("  imgsize.xml\n");
    printf("  intrinsicUndistored.xml\n");
    printf("  rvecs.xml\n");
    printf("  tvecs.xml\n");

//     cvNamedWindow ( "cvUndistortPoints", 1 );
//     IplImage *pImgUndistort = cvCreateImage ( cvGetSize ( pImgGray ), IPL_DEPTH_8U, 3 );
//     std::cout << "Files: ";
//     for ( unsigned int i = 0; i < usedFiles.size(); i++ ) {
//         CvPoint2D64f pSrc, pDes;
//         CvPoint des;
//         path filePath ( usedFiles[i].c_str() );
//         std::cout << filePath.filename() << ", ";
//         pImg = cvLoadImage ( usedFiles[i].c_str(), 1 );
//         cvZero ( pImgUndistort );
//         CvScalar value;
//         for ( int x = 0; x < pImgGray->width; x++ ) {
//             for ( int y = 0; y < pImgGray->height; y++ ) {
//                 // undistors every point in the image
//                 value = cvGet2D ( pImg, y, x );
//                 pSrc.x = x;
//                 pSrc.y = y;
//                 undistort ( pIntrinsic->data.db, pDistortions->data.db, ( double* ) &pSrc, ( double* ) &pDes );
//                 des.x = round ( pDes.x );
//                 des.y = round ( pDes.y );
//                 if ( ( des.x >= 0 ) && ( des.x < pImg->width ) && ( des.y >= 0 ) && ( des.y < pImg->height ) ) {
//                     cvSet2D ( pImgUndistort, des.y, des.x, value );
//                 }
//             }
//         }
//         cvShowImage ( "Camera", pImg );
//         cvShowImage ( "cvUndistortPoints", pImgUndistort );
//         //cvMoveWindow ( "Camera", 0, 0 );
//         //cvMoveWindow ( "Undistort1", 100, 0 );
//         //cvMoveWindow ( "Undistort2", 200, 0 );
//         cvWaitKey ( 100 );
//         cvReleaseImage ( &pImg );
//     }
// 
//     cvReleaseImage ( &pImgUndistort );
