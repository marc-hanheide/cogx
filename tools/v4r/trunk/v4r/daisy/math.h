...
#include <v4r/daisy/kutility.def>
#include <v4r/daisy/matrix_manipulation.h>
#include <v4r/daisy/general.h>

namespace kutility
{
   /// creates a 1D gaussian filter with N(mean,sigma).
   REAL* gaussian_1d(int fsz, REAL sigma, REAL mean = 0.0 );

   /// creates a 2D gaussian filter with N(mean,sigma).
   REAL* gaussian_2d(int fsz, REAL sigma, REAL mn=0);
   ...
   /// Same as ConvBuffer, but implemented with loop unrolling for
   /// increased speed. This is the most time intensive routine in
   /// keypoint detection, so deserves careful attention to
   /// efficiency. Loop unrolling simply sums 5 multiplications at a
   /// time to allow the compiler to schedule operations better and
   /// avoid loop overhead. This almost triples speed of previous
   /// version on a Pentium with gcc.
   template<class T1, class T2> inline
   void conv_buffer_fast(T1 *buffer, T2 *kernel, int rsize, int ksize)
   {
      int i;
      double sum;
      T1 *bp;
...
//       #pragma omp parallel for private( bp, kp, sum, endkp )
      for (i = 0; i < rsize; i++)
      {
         sum = 0;
         bp = &buffer[i];
         ...
         while (kp + 4 < endkp)
         {
            sum += bp[0]*kp[0] + bp[1]*kp[1] + bp[2]*kp[2] + bp[3]*kp[3] + bp[4]*kp[4];
            bp += 5;
            kp += 5;
         }
...
//      #pragma omp parallel for private( r, rw, i, c, buffer, temp )
      for (r = 0; r < h; r++)
      {
         /* Copy the row into buffer with image at ends replicated for
...
//          for (i = 0; i < w; i++)
//             buffer[halfsize+i] = image[rw+i];

         temp = image[rw+w-1];
         for (i = 0; i < halfsize; i++)
            buffer[i+halfsize+w] = temp;

         conv_buffer_fast(buffer, kernel, w, ksize);
...
//      #pragma omp parallel for private( c, i, r, buffer )
      for (c = 0; c < w; c++)
      {
         ...
         conv_buffer_fast(buffer, kernel, h, ksize);
...
//      #pragma omp parallel for
   ...
   bool extract_rotated_patch( T1* dst, T1* src, int h, int w, T2 ry, T2 rx, int patch_width, T3 rotation_angle )
   {
      ...
      REAL kos = cos( rotation_angle );
      REAL zin = sin( rotation_angle );
...
//    REAL* filter_2d(REAL* &im, int r, int c, REAL* filter, int fr, int fc, bool in_place=false);
   ...
   REAL sigmoid(REAL x, REAL miny, REAL maxy, REAL rate, REAL sym_axis);
   ...
   template<class T> inline
   REAL* angle(T* x, T* y, int lsz)
   {
      REAL* ang = kutility::allocate<REAL>(lsz);
   ...
   template<class T> inline
   REAL* magnitude(T* arrx, T* arry, int lsz)
   {
      REAL* mag = kutility::allocate<REAL>(lsz);
   ...
   /// ones.
   template<class T> inline
   void cartesian2polar(T x, T y, REAL &r, REAL &th)
   {
...
//#pragma omp parallel for
      for( int l=0; l<layer_no; l++ )
      {
         ...
         {
            REAL value = kos * dx[ index ] + zin * dy[ index ];
   ...
   template<class T1, class T2> inline
   REAL bilinear_interpolation(T1* arr, int w, T2 x, T2 y)
   {
      int mnx = (int)floor( x );
      int mny = (int)floor( y );
      int mxx = (int) ceil( x );
      ...
      if( alfa < 0.001 ) return REAL(beta * arr[mnyw+mxx] + (1-beta) * arr[mxyw+mxx]);
      if( alfa > 0.999 ) return REAL(beta * arr[mnyw+mnx] + (1-beta) * arr[mxyw+mnx]);
      if( beta < 0.001 ) return REAL(alfa * arr[mxyw+mnx] + (1-alfa) * arr[mxyw+mxx]);
      if( beta > 0.999 ) return REAL(alfa * arr[mnyw+mnx] + (1-alfa) * arr[mnyw+mxx]);

      return REAL( beta*(alfa * arr[mnyw+mnx] + (1-alfa)*arr[mnyw+mxx] )
   ...
   template<class T1, class T2> inline
   REAL l2norm( T1* a, T2* b, int sz)
   {
      REAL norm=0;
      for( int i=0; i<sz; i++ )
      {
         norm += square( (REAL)a[i] - (REAL)b[i] );
      }
      norm = sqrt( norm );
   ...
   template<class T> inline
   REAL l2norm( T y0, T x0, T y1, T x1 )
   {
      REAL d0 = x0 - x1;
      REAL d1 = y0 - y1;
   ...
   /// multiple of sz.
   template<class T1, class T2> inline
   REAL weighted_l2_norm(T1* a, T1* b, int sz, T2* w=NULL, int reg=-1)
   {
      if( w == NULL )
   ...
   template<class T1, class T2> inline
   REAL mean_absolute_difference( T1* arr1, T2* arr2, int size)
   {
      REAL mad_score=0;

      for( int i=0; i<size; i++ )
      {
         mad_score += fabs( (REAL)arr1[i] - (REAL)arr2[i] );
      }
   ...
   template<class T> inline
   REAL variance(T* data, int dsz)
   {
      REAL var = 0.0;

      REAL mu = kutility::mean(data,dsz);
      REAL x;
   ...
   template<class T> inline
   REAL standart_deviation(T* data, int dsz)
   {
      REAL var = kutility::variance(data, dsz);
      REAL std = sqrt(var);
      return std;
   }
...
/// computes the mean and variance of the array
   template<class T> inline
   void mean_variance(T* data, int dsz, REAL &mean, REAL &variance)
   {
   ...
   template<class T1, class T2> inline
   void rotate( T1 y1, T1 x1, T2& y2, T2& x2, REAL theta, T1 ty, T1 tx )
   {
      REAL kos = cos( theta );
      REAL zin = sin( theta );
      x2 = (T2)( x1*kos - y1*zin );
   ...
   template<class T> inline
   T* rotate( T* imge, int h, int w, REAL theta, REAL ry=0, REAL rx=0 )
   {
      REAL kos = cos(theta);
      REAL zin = sin(theta);
...
//                rimge[(ny*w+nx)] = imge[y*w+x];
   ...
   T* stretch(T* image, int sz, T val, bool in_place=false)
   {
      ...
      REAL s = 255.0/(REAL)(max_inten-min_inten);
