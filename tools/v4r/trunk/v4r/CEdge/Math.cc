/**
 * $Id: Math.cc,v 1.1.1.1 2008/02/29 10:12:02 jp Exp $
 */

// ATTENTION: Hack to avoid static pointer //jp: 23.8.2007
...
//static FILE *rand_file = 0;
void InitMath()
{
  InitLogFactTab();
  srand(2407771);
  //rand_file = fopen("rand.txt", "w");
}
 ...
 * completely outside.
 */
bool ClipLine(int xmin, int ymin, int xmax, int ymax, double *x1, double *y1, double *x2, double *y2)
{
  // left
  if(*x1 >= xmin)    // don't clip point 1
  {
...
{
  return betai(k+1, l-k, (float)p);
}

/**
 * Significance that exaclty k points support an m-parametric model of size l,
 ...
 * Considering a horizontal line of maximal length with phi = 0, the next phi is given as
 * atan(1/width).
 * ATTENTION: Hack to avoid static pointer //jp: 23.8.2007
 */
double AngularResolution()
{
//  return atan(1./StereoCore::ImageWidth());
  return atan(1./640.);
}
