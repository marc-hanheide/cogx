...
/**
 * @brief New image for processing.
 * Call this function for processing vs3 with cast.
 * @param image Image
 */
void VisionCore::NewImage(Image *image)
{
	// initialize BufferVideo without data
// 	video->Init(image->Width(), image->Height(), image->ColorFmt(), 0, false);

	// create new image
  delete img;
  img = new Image(image->Data(), image->Width(), image->Height(),
      image->BytesPerPixel(), image->ColorFmt(), false, image->BytesPerLine());
