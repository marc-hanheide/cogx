...
const float SIFT::PeakThreshInit = 0.04;

const float SIFT::EdgeEigenRatio = 10.0;
...
const float SIFT::MaxIndexVal = 0.2;
...
void SIFT::AssignOriHist(const Image& grad, const Image& ori, float octSize, 
        float octScale, float octRow, float octCol, KeyList& keys)
{
        ...
        float scale, float row, float col)
{
    ...
    for (i = 0; i < VecLength; i++) {
        ...
        assert(intval >= 0);
        key.ivec[i] = (unsigned char) GETMIN<int>(255, intval);
    }
}
...
void SIFT::GaussianBlur(Image& image, float sigma)
{
    ...
    ksize = GETMAX<int>(3, ksize);    /* Kernel must be at least 3. */
        ...
        for (c = 0; c < image.cols; c++) {
            if (image.pixels[r][c] == 2.0)
                ...
                if ((val >= 250) && (val <= 255))
                    ...
                    fputc(GETMAX<int>(0, GETMIN<int>(255, val)), fp);
            } // if - else
        }
}
...
void SIFT::DrawLine(Image& image, int r1, int c1, int r2, int c2)
{
    ...
    if (SIFT::ABS(r2 - r1) < SIFT::ABS(c2 - c1)) {
