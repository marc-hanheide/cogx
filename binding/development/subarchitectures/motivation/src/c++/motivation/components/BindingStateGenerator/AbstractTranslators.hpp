...
template <>
struct MatchesFeatureApplicable<BindingFeatures::RelationLabel> :
  public TranslatorApplicabilityFunctor {
  
  MatchesFeatureApplicable(const BindingFeatures::RelationLabel & _feature) :
    m_feature(_feature,"") {}

  virtual bool operator()(const UnionPtr & _union) const {
    Feature<BindingFeatures::RelationLabel> feature(_union->getFeature<BindingFeatures::RelationLabel>(), "");
    return !strcmp(CORBA::string_dup(m_feature->m_label), CORBA::string_dup(feature->m_label));
  };
  
  virtual ~MatchesFeatureApplicable(){}
  
protected: 
  Feature<BindingFeatures::RelationLabel> m_feature;
};
...
struct BasicUnionFeatureTranslator :
  ...
  virtual void translate(const UnionPtr & _union,
    ...
    const Binding::FeatureSetWithRepetitions & featureSets(_union->featureSetWithRepetitions());
    Binding::FeatureSetWithRepetitions::const_iterator fs = featureSets.find(cast::typeName<FeatureT>());

    if(fs != featureSets.end()) {      
      const Binding::OneTypeOfFeaturesWithRepetitions & features(fs->second);
      for (Binding::OneTypeOfFeaturesWithRepetitions::const_iterator feature = features.begin();
	   feature != features.end(); ++feature) {	
	translateFeature(_union, extractIDLFeature<FeatureT>(*(*feature)), _var, _state);	
      }
