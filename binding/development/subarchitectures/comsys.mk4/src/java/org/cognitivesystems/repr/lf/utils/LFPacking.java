...
public class LFPacking {
	...
	/** 	for (int i=0; i < Lfs.length ; i++) {
			LFUtils.lfToGraph(Lfs[i], "graphs/testLF" +incr, true);
		...
		for (int i=0 ; i < plf.pNodes.length ; i++) {
			...
			for (int j=0; PN1.nomsPePairs.length > 0 && j < PN1.nomsPePairs.length ; j++) {
				...
				while (PN1.nomsPePairs.length > 0 && it.hasNext()) {
					...
					if (PN2==null) {
						System.err.println("PROBLEM: Node "+ PN2Id + " not found in the packing nodes hashtable");
					...
					if (LfIds1.equals(LfIds2) && (LfIds1.equals(LfIds3))) {
						...
						for (Enumeration<PackingEdge> e = PackingEdges.elements(); e.hasMoreElements() ;) {
							...
							if (!peTest.equals(pe)) {
								...
								//			System.err.println ("Warning: there is another edge pointing to the packing node we want to delete");
											additionalVerificationOK = false;
										}
											else {
								//				System.err.println ("OK, false alert");
											}
									}
								}
							}
						...
						if (!hasCycle && nbLoops > 10 && additionalVerificationOK && pe.targets.length == 1) {
