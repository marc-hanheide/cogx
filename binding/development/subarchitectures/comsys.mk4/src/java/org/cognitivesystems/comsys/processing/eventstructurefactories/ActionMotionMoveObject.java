...
/**
	The class <b>ActionMotionMoveObject</b> provides a factory for producing event 
	nuclei for a "MOVE" verbal predicate, acting on objects -- e.g. "move the object to the left." 
	This factory does not produce an event nucleus for another interpretation, namely
	for the case where the addressed actor is the object of moving -- e.g. "move to the left, move to the kitchen, 
	move aside." If the other interpretation is encountered, a <tt>null</tt> result is produced. 
...
public class ActionMotionMoveObject
...

{
	...
	/** 
		...
		If the method finds that there is no patient, then it returns a <tt>null</tt> result. This is
		used to distinguish the object-moving interpretation from the actor-moving interpretation. 
	...
	*/ 	
		...
		if (moveVerb != null) { 
			try { 
				...
				if (hasDependent(moveVerb,"Patient")) { 
					...
					PackedNominal patient = this.getDependentNominal(moveVerb,"Patient",packedNoms);		
	...
	/** 
		...
		event nucleus being returned is "action-motion/move-object".  
		<p>
	...
	*/
	...
	public EventNucleusResults produceEventNucleus (String nomVar, String nucleusId, TreeMap packedNoms) {
		...
		// Create the event
		Event event = createEvent(nucleusId, nomVar, _sort+"/"+_prop+"-object");
		// Create the state
...
} // end class ActionMotionMoveObject
