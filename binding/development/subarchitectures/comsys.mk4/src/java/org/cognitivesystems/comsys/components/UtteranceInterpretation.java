...
import org.cognitivesystems.comsys.processing.ActiveWordLatticeParser;
...
/**
The class <b>UtteranceInterpretation</b> ... 
...
public class UtteranceInterpretation 
	...
	// beam width for incremental chart scoring
     ...
     * @param _id
     */
    public UtteranceInterpretation (String _id) {
        super(_id);
    } // constructor/1
	...
	*/
	...
	public void init () { 
		...
		try { 
			...
			if (params != null)
				props.put("parameterVector", params);

			props.put("beamwidth", new Integer(beamwidth));
	...
	*/ 
    ...
    public void configure(Properties _config) {
        ...
        if (_config.containsKey("--inputParamFile")) {
            String inputParamFile = _config.getProperty("--inputParamFile");
            Vector<String> lines = LearningUtils.readFile(inputParamFile);
			params = LearningUtils.buildParameterVector(lines);
           log("using the parameter vector file " + inputParamFile);
        }
        
        if (_config.containsKey("--beamwidth")) {
        	String beamwidthstr = _config.getProperty("--beamwidth");
        	beamwidth = (new Integer(beamwidthstr)).intValue();
        	 log("beam width for incremental chart scoring: " + beamwidth);
        }
	...
	*/ 
    ...
    public void executeParseTask(ProcessingData pd)
        ...
        // FIRST CASE: we receive a new phonological string
            ...
            if (data != null) {
                ...
                if (phonString.wordSequence.length() != 0) {
                	PackedLFParseResults results = 
                		(PackedLFParseResults) parser.incrParse(phonString);
                	// put the results on the working memory
                	// now write the results to the WM, as a PackedLFs 
                			
                	String[] LFs = LFUtils.plfGetPackingNode(results.plf,results.plf.root).lfIds;
					PhonStringLFPair[] pairs = new PhonStringLFPair[LFs.length];
					
					for (int i = 0 ; i < LFs.length ; i++) {
						PhonStringLFPair pair = new PhonStringLFPair();
						pair.phon = phonString;
						pair.logicalFormId = LFs[i];
						pairs[i] = pair;
					}
        ...
        // SECOND CASE: we receive a word recognition lattice
        ...
        // THIRD CASE: we receive a packed logical form 
        // (representing partial or complete analyses)
            ...
            if (data != null) {
                ...
                // get the input string
                PhonString phonString = plfs.phonStringLFPairs[0].phon;
				...
				// the received analysis is complete	
				} else { 
		...
		/**			LFPacking packingTool = new LFPacking();
					LogicalForm[] lfs = packingTool.unpackPackedLogicalForm(plfs);
					for (int i = 0 ; i < lfs.length; i++) {
						LogicalForm lf = lfs[i];
						LFUtils.lfToGraph(lf, "lf-"+i, true);
					} */ 
