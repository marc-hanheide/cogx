...
public class UtteranceRealization 
	extends ManagedProcess {
		 ...
		 * @param _id
		 */
		public UtteranceRealization (String _id) {
			super(_id);
		} // constructor/1
		...
		private void handleWorkingMemoryChange(WorkingMemoryChange _wmc) {
			try {
				...
				// set the goal
				String taskGoal = ComsysGoals.REALIZATION_TASK;
		...
		throws ComsysException {
			...
			if (pdTypes.contains(CASTUtils.typeName(ProductionLF.class))) {
				CASTData data = pd
                .getByType(CASTUtils.typeName(ProductionLF.class));
		...
		private void addNewPLFToWorkingMemory (LogicalForm planLF, String realization) {
			...
			plf.phonStringLFPairs[0].phon = new PhonString();
			plf.phonStringLFPairs[0].phon.wordSequence = realization;
			plf.type = "production";
