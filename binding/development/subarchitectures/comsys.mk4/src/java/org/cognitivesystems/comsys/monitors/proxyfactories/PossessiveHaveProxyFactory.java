...
public class PossessiveHaveProxyFactory 
	extends AbstractProxyFactory
{
	...
	public PossessiveHaveProxyFactory () { 
		init();
	} // end
	...
	private ProxyFactoryResults createRelations (PackedNominal nom, PackedLogicalForm plf, TreeMap packedNoms) { 	
		...
		while (relsIter.hasNext()) { 
			...
			if (rel.mode.equals("Actor")) { 
				...
				log("Added an Actor for [possessive], based on LF-structure");
				...
				PackedNominal eventNom = (PackedNominal) packedNoms.get(rel.dep);
				String eventSort = getSort(eventNom);
				if (eventSort.equals("ascription")) {
					AscriptionProxyFactory ascriptionPF = new AscriptionProxyFactory();
					PackedNominal eventArg = ascriptionPF.retrieveArgument(eventNom,packedNoms);
					ppr.depNomVar = eventArg.nomVar;
				} 
			...
			while (peIter.hasNext()) { 
				...
				if (packingEdge.mode.equals("Modifier")) { 
					...
					for (Iterator targetIter = targets.iterator(); targetIter.hasNext(); ) { 
						...
						actorPPR.addContentStatus("intentional");				
						pendingRels.add(actorPPR); 
						log("Added an Actor for [cognition], based on PLF-structure");
