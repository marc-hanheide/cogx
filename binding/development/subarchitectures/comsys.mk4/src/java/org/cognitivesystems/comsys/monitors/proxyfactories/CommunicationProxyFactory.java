...
public class CommunicationProxyFactory 
	extends AbstractProxyFactory
{
	...
	public CommunicationProxyFactory () { 
		super();
		logging = false;
		rootSort = "communication";
	} // end
	...
	*/
	...
	{  
		...
		{
			CommunicationTellProxyFactory tellPF = new CommunicationTellProxyFactory();
			tellPF.setDiscRefs(_discRefsAccess);
			ProxyFactoryResults tellResult = tellPF.produceProxies(nom,plf,packedNoms);
			// Fuse with the resulting proxy structure features
			proxyStructure.fuse(tellResult.getProxyStructure());
			// Update the excludes
			excludes.addAll(tellResult.getExcludes());
			// Update the relations
			pendingRels.addAll(tellResult.getPendingProxyRelations());
			proxyStructure.addRelations(tellResult.getPendingProxyRelations().iterator());
			// Create the interrogative structures, if necessary

		} else { 
			System.err.println("Communication Factory is being called for an unknown verb ["+verb+"]");
			// System.exit(0);
		} // end if..else 
