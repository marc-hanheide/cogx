...
public class EventProxyFactory 
	extends AbstractProxyFactory
{
	...
	*/
	...
	{  
		...
		if (mood != null && mood.equals("int") && !verb.equals("context")) { 
		...
		if (verb.equals("happen")
			) 
		{
			EventHappenProxyFactory happenPF = new EventHappenProxyFactory();
			happenPF.setDiscRefs(_discRefsAccess);
			ProxyFactoryResults happenResult = happenPF.produceProxies(nom,plf,packedNoms);
			// Fuse with the resulting proxy structure features
			proxyStructure.fuse(happenResult.getProxyStructure());
			// Set the excludes
			excludes.addAll(happenResult.getExcludes());
			// Add the relations
			pendingRels.addAll(happenResult.getPendingProxyRelations());
			proxyStructure.addRelations(happenResult.getPendingProxyRelations().iterator());		
			result.setProxyStructure(happenResult.getProxyStructure());
		} else if (verb.equals("context") ) { 
			if (!hasDependent(nom,"Subject")) { 
				addBasicContent(nom);
			} else { 
				returnNull = true;
			} 	
		} else { 
			System.err.println("EventProxyFactory is being called for an unknown verb ["+verb+"]");
			System.exit(0);
		} // end if..else 
