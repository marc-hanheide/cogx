...
sub add_entry{
	...
	# advs have a family of type adv.class (ex. adv.direction)
		...
		@entries[0] = $pred; #set first entry to just bare pred
		...
		if(@forms != 1){
     	...
     	$mod_type = substr $class, 2;  #strip of m-
		foreach $entry (@entries) {push(@word_forms, [$entry, "\@s-".$mod_type." ".$compcats] ) };
	...
	# otherwise in dict.list
		...
		@entries[0] = $pred;
		#$macro="\@s-".$mod_type." ".$compcats;
		
		foreach $entry (@entries) {push(@word_forms, [$entry, "\@s-".$mod_type." ".$compcats] ) };
		
		@families=("prep".$of_flag);
	...
	# in dict.list  listed as , e.g. right.prep --> mod-right.prep
		...
		@forms = ();
     	...
     	@forms = split(/ /, @entries[0]);
        shift (@forms); #this removes the first element from forms (which is the stem)
			
		# set up word-forms
		@entries[0] = $pred; # take only stem from first as entry
        foreach $entry (@entries) {push(@word_forms, [$entry, ""])};
		
		#create families (attach mod- to give forms (dir.type)

		foreach $form (@forms) {
			push(@families, "mod-".$form);
		}
