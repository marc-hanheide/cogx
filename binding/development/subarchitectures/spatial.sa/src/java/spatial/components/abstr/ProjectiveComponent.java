...
public abstract class ProjectiveComponent
	...
	private void writeRelationshipsToWorkingMemory()
		...
		//		
		// for (SpatialRelationship rel : m_relationshipsToWrite) {
		// rel.
		// }
	...
	protected void redrawGraphics3D() {
		if (m_relationshipsToWrite != null) {
			for (SpatialRelationship rel : m_relationshipsToWrite) {
				try {
					SpatialLocation landmark = getSpatialLocation(rel.m_landmark);
					SpatialLocation target = getSpatialLocation(rel.m_target);

					drawLine3D(landmark.m_centroid.m_x,
							landmark.m_centroid.m_y, landmark.m_centroid.m_z,
							target.m_centroid.m_x, target.m_centroid.m_y,
							target.m_centroid.m_z, 200, 0, 0, NONE.value);
					
//					drawBox3D(target.m_centroid.m_x, target.m_centroid.m_y,
//							target.m_centroid.m_z, 0.04, 0.04, 0.04, 200, 150, 0,
//							FAT.value);
