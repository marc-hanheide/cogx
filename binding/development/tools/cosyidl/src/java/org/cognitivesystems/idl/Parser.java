/*
 * @(#)Parser.java 1.24 04/06/21 Copyright 2004 Sun Microsystems, Inc.
 ...
 * with IBM Corp. @(#)Parser.java 1.24 04/06/21
 */
...
// integral as well as floating-point literals. No other variations
// allowed.
// -D58058<daz> Set index array type to long rather than array element
// type.
// -D56351<daz> Update computation of RepositoryIDs to CORBA 2.3 (see
// spec.).
// -D57110<daz> Provide method() to set/clear ability for scoped names
// to
// resolve to modules. Allows rep. ids to be assigned to modules.
// -D46094<daz> Prohibit exceptions from appearing wihtin structs,
// unions, exceptions.
// -D46094<daz> Prohibit attributes from appearing as operation
// parameter types,
// operation return types, attribute types.
// -D59067<daz> Prohibit nested value boxes.
// -D59166<daz> Prohibit collisions between keywords and non-escaped
// identifiers.
// -D59809<daz> At Pigeonhole(), add map short name of CORBA types to
// long name
// (e.g., CORBA/StringValue --> org/omg/CORBA/StringValue), which allows
// fully-
// qualified CORBA type names to resolve successfully.
// -F60858.1<daz> Support "-corba" option, level <= 2.2: issue warning
// for
// keyowrd collisions;
// -D60942<daz> Prohibit operations from appearing within parameter
// types.
...
// -D61919<daz> Emit entries for modules originally opened in #include
// files
// appearing at global scope and then reopened in the main IDL file.
// Only
...
import org.cognitivesystems.idl.constExpr.*;
...
class Parser {
     ...
     *
     **/
    Parser(Preprocessor preprocessor, Arguments arguments,
            Hashtable overrides, Hashtable symtab, SymtabFactory stFac,
            ExprFactory exprFac, String[] genKeywords) {
        ...
        overrideNames = (overrides == null) ? new Hashtable()
            : overrides;
    ...
    void parse(String file) throws IOException {
        ...
        // <f46082.51> Remove -stateful feature.
        // scanner = new Scanner (fileEntry, keywords, verbose,
        // parseStateful, emitAll);
        // <f60585.1> Support "-corba [level]" option.
        // scanner = new Scanner (fileEntry, keywords, verbose,
        // emitAll);
        scanner = new Scanner(fileEntry, keywords, verbose, emitAll,
            corbaLevel, arguments.scannerDebugFlag);
        ...
        token = new Token(0);
        tokenHistory.insert(token); // Initialize look back buffer
                                    // <26jul1997daz>.
        try {
            match(0);
        ...
        catch (EOFException exception) // skipToSemicolon MIGHT throw
                                        // this
        {
            // It has already been reported, just end.
        }
    ...
    private void addPrimEntries() {
        ...
        symbolTable.put("long long", stFactory
            .primitiveEntry("long long"));
        symbolTable.put("unsigned short", stFactory
            .primitiveEntry("unsigned short"));
        symbolTable.put("unsigned long", stFactory
            .primitiveEntry("unsigned long"));
        symbolTable.put("unsigned long long", stFactory
            .primitiveEntry("unsigned long long"));
        ...
        // Support fixed type: symbolTable.put ("fixed",
        // stFactory.primitiveEntry ("fixed"));
        ...
        lcSymbolTable.put("long long", stFactory
            .primitiveEntry("long long"));
        lcSymbolTable.put("unsigned short", stFactory
            .primitiveEntry("unsigned short"));
        lcSymbolTable.put("unsigned long", stFactory
            .primitiveEntry("unsigned long"));
        lcSymbolTable.put("unsigned long long", stFactory
            .primitiveEntry("unsigned long long"));
        ...
        // Support fixed type: lcSymbolTable.put ("fixed",
        // stFactory.primitiveEntry ("fixed"));
    ...
    // ModuleEntry is the topLevelModule; add its contained types to the
    // emit list.
    /**
     *
     **/
    private void addToEmitList(ModuleEntry entry) {
        for (Enumeration e = entry.contained().elements(); e
            .hasMoreElements();) {
            SymtabEntry emitEntry = (SymtabEntry) e.nextElement();
            if (emitEntry.emit()) {
                emitList.addElement(emitEntry);

                // <d61919> I think the absence of the following
                // statement was an
                // oversight. If module X.Y.Z first appears in an
                // include file, then is
                // reopened in the main IDL source, this statement
                // guarantees that X.Y.Z
                // definitions within the main IDL source are emitted.
                // /---------------------------------------------------------------------
                // If any of this module's elements should be emitted,
                // add
            ...
            switch (token.type) {
                ...
                case Token.Abstract:
                    ...
                    if (token.type == Token.Interface)
                        ...
                        throw ParseException.syntaxError(scanner,
                            new int[] {
                                Token.Interface, Token.Valuetype
                            }, token.type);
                    break;
                case Token.Custom:
                ...
                default:
                    throw ParseException.syntaxError(scanner,
                        new int[] {
                            Token.Typedef, Token.Struct, Token.Union,
                            Token.Enum, Token.Const, Token.Exception,
                            Token.Interface, Token.Valuetype,
                            Token.Module
                        }, token.type);
            }
     ...
     *
     **/
    private void module(ModuleEntry entry) throws IOException,
            ParseException {
        match(Token.Module);
     ...
     *
     **/
    private void interface2(ModuleEntry module, String name,
            int interfaceType) throws IOException, ParseException {
        if (token.type == Token.Colon || token.type == Token.LeftBrace) {
            ...
            // Comment must immediately precede "[local | abstract]
            // interface" keyword
            entry
                .comment(tokenHistory
                    .lookBack(entry.getInterfaceType() == InterfaceEntry.NORMAL ? 2
                        : 3).comment);
            ...
            ForwardEntry entry = stFactory.forwardEntry(module,
                (IDLID) repIDStack.peek());
            entry.sourceFile(scanner.fileEntry());
            entry.name(name);
            entry.setInterfaceType(interfaceType);
     ...
     *
     **/
    private void interfaceDcl(InterfaceEntry entry) throws IOException,
            ParseException {
        if (token.type != Token.LeftBrace)
            ...
            if (objectEntry == null)
                ...
                ParseException.wrongType(scanner,
                    overrideName("Object"), "interface", objectEntry
                        .typeName());
            else
                ...
                default:
                    throw ParseException.syntaxError(scanner,
                        new int[] {
                            Token.Typedef, Token.Struct, Token.Union,
                            Token.Enum, Token.Const, Token.Exception,
                            Token.Readonly, Token.Attribute,
                            Token.Oneway, Token.Float, Token.Double,
                            Token.Long, Token.Short, Token.Unsigned,
                            Token.Char, Token.Wchar, Token.Boolean,
                            Token.Octet, Token.Any, Token.String,
                            Token.Wstring, Token.Identifier,
                            Token.DoubleColon, Token.Void,
                            Token.ValueBase
                        }, token.type);
            }
            match(Token.Semicolon);
    ...
    private void inheritanceSpec(InterfaceEntry entry)
            throws IOException, ParseException {
        for (match(Token.Colon);; match(Token.Comma)) {
            ...
            if (isInterfaceOnly(realParent)) {
                ...
                if (entry.derivedFrom().contains(realParent))
                    ParseException.alreadyDerived(scanner, realParent
                        .fullName(), entry.fullName());
                else if (!entry.isAbstract()
                    || (((InterfaceType) realParent).getInterfaceType() == InterfaceType.ABSTRACT))
                    entry.derivedFromAddElement(realParent, scanner);
                else
                    ParseException.nonAbstractParent(scanner, entry
                        .fullName(), parent.fullName());
            }
            else if (isForward(realParent)) {
                ParseException.illegalForwardInheritance(scanner, entry
                    .fullName(), parent.fullName());
            }
            else
                ParseException.wrongType(scanner, parent.fullName(),
                    "interface", entryName(parent));

            if ((parent instanceof InterfaceEntry)
                && (((InterfaceEntry) parent).state() != null))
                if (entry.state() == null)
                    entry.initState();
                else
                    throw ParseException.badState(scanner, entry
                        .fullName());
    ...
    // <57110> Member _moduleIsLegalType may be set by any feature to
    // allow
    // method scopedName() and any of its helper methods --
    // qualifiedName(),
    // partlyQualifiedName(), and unqualifiedName() -- to return a
    // ModuleEntry
    // rather than a parse error in the event a name resolves to a
    // module. The
    // flag must be cleared (set to false) to resume normal parsing
    // behavior.
    //
    // Currently, this is used only when preprocessing the ID pragma
    // directive.
    ...
    SymtabEntry scopedName(SymtabEntry container, SymtabEntry expected,
            boolean mustBeReferencable) throws IOException,
            ParseException {
        ...
        if (entry == null)
            ...
            // not be called now, since a semantic exception ocurred,
            // but
            ...
            throw ParseException.illegalIncompleteTypeReference(
                scanner, name);

        return entry;
    } // scopedName

    private void valueProd(ModuleEntry entry, boolean isAbstract)
            throws IOException, ParseException {
     ...
     *
     **/
    private void value2(ModuleEntry module, String name,
            boolean isAbstract, boolean isCustom) throws IOException,
            ParseException {
        repIDStack.push(((IDLID) repIDStack.peek()).clone());
        // The 'actual' repository ID will be calculated at the end of
        // the
        // parsing phase, since it is based on the entire contents of
        // the
        // declaration, and needs to have all forward references
        // resolved:
        ValueEntry entry = stFactory.valueEntry(module,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name(name);
        entry.setInterfaceType(isAbstract ? InterfaceType.ABSTRACT
            : InterfaceType.NORMAL);
        entry.setCustom(isCustom);
        // Comment must immediately precede "[abstract | custom] value"
        // keyword
        entry.comment(tokenHistory
            .lookBack((isAbstract || isCustom) ? 3 : 2).comment);
     ...
     *
     **/
    private void valueDcl(ValueEntry entry) throws IOException,
            ParseException {
        if (token.type == Token.Colon)
            ...
            if (objectEntry == null)
                ...
                ParseException.wrongType(scanner,
                    overrideName("ValueBase"), "value", objectEntry
                        .typeName());
            else
     ...
     *
     **/
    private void valueInheritanceSpec(ValueEntry entry)
            throws IOException, ParseException {
        match(Token.Colon);
        ...
        for (;; match(Token.Comma), isTruncatable = false) {
            ...
            SymtabEntry realParent = typeOf(parent);
            if (isValue(realParent)
                && !(realParent instanceof ValueBoxEntry))
     ...
     *
     **/
    private void valueSupportsSpec(ValueEntry entry)
            throws IOException, ParseException {
    ...
    private void valueElement(ValueEntry entry) throws IOException,
            ParseException {
        if (entry.isAbstract())
            export(entry);
        else
            switch (token.type) {
                ...
                case Token.Public:
                    ...
                    break;
                case Token.Init:
                case Token.Factory: // <d62023> "factory" supplants
                                    // "init" in 2.4RTF
                    initDcl(entry);
                    break;
                case Token.Typedef:
                ...
                default:
                    throw ParseException.syntaxError(scanner,
                        new int[] {
                            Token.Private, Token.Public, Token.Init,
                            Token.ValueBase, Token.Typedef,
                            Token.Struct, Token.Union, Token.Enum,
                            Token.Const, Token.Exception,
                            Token.Readonly, Token.Attribute,
                            Token.Oneway, Token.Float, Token.Double,
                            Token.Long, Token.Short, Token.Unsigned,
                            Token.Char, Token.Wchar, Token.Boolean,
     ...
     *
     **/
    private void valueStateMember(ValueEntry entry) throws IOException,
            ParseException {
        TypedefEntry typedefEntry = stFactory.typedefEntry(entry,
            (IDLID) repIDStack.peek());
        ...
        if (isPublic)
            ...
            match(Token.Private);
        // <f46082.40> Add constructed types declared "inline" to the
        // contained
        // vector of this value entry.
        boolean isConstTypeSpec = (token.type == Token.Struct
            || token.type == Token.Union || token.type == Token.Enum);
        // <f46082.40> Make typedefEntry anonymous. If this line is
        // removed,
    ...
    private void addDeclarators(ValueEntry entry,
            TypedefEntry typedefEntry, boolean isPublic)
            throws IOException, ParseException {
        int modifier = isPublic ? InterfaceState.Public
            : InterfaceState.Private;
        try {
            Vector typedefList = new Vector();
            declarators(typedefEntry, typedefList);
            for (Enumeration e = typedefList.elements(); e
                .hasMoreElements();)
                entry.addStateElement(new InterfaceState(modifier,
                    (TypedefEntry) e.nextElement()), scanner);
        }
        catch (ParseException exception) {
            skipToSemicolon();
        }
     ...
     *
     **/
    private void initDcl(ValueEntry entry) throws IOException,
            ParseException {
        MethodEntry method = stFactory.methodEntry(entry,
            (IDLID) repIDStack.peek());
        ...
        else // <d62023> In 2.4rtf, <init_dcl> ::= "factory"
                // <Indentifier> "(" ...
        {
            match(Token.Factory);
     ...
     *
     **/
    private void initParamDcl(MethodEntry entry) throws IOException,
            ParseException {
        ParameterEntry parmEntry = stFactory.parameterEntry(entry,
            (IDLID) repIDStack.peek());
     ...
     *
     **/
    private void valueBox(ModuleEntry module, String name)
            throws IOException, ParseException {
        repIDStack.push(((IDLID) repIDStack.peek()).clone());
        // Note: The 'actual' repository ID will be calculated at the
        // end of
        // the parsing phase, since it is based on the entire contents
        // of the
        // declaration, and needs to have all forward references
        // resolved:
        ValueEntry entry = stFactory.valueBoxEntry(module,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name(name);
        ...
        // ParseException.badAbstract (scanner, entry.fullName());
        SymtabEntry valueForward = (SymtabEntry) Parser.symbolTable
            .get(entry.fullName());
        if (valueForward != null
            && valueForward instanceof ForwardEntry)
        ...
        typedefEntry.sourceFile(scanner.fileEntry());
        typedefEntry.comment(token.comment);
        // <d50237> Workaround to place typedefEntry in the _contained
        // vector of
        // this value box entry ONLY when <type_spec> is a constructed
        // type declared
        // at this point (i.e., not an identifier that resolves to a
        // constructed
        // type), so that emitters may generate bindings for it. <daz>
        boolean isConstTypeSpec = token.type == Token.Struct
            || token.type == Token.Union || token.type == Token.Enum;
        // <d50618> Make typedefEntry anonymous. If this line is
        // removed, the
        ...
        // typedefEntry.name ("");
        entry.addStateElement(new InterfaceState(InterfaceState.Public,
            typedefEntry), scanner);
        if (isConstTypeSpec)
            entry.addContained(typedefEntry);
        currentModule = module;
        repIDStack.pop();
     ...
     *
     **/
    private void valueForwardDcl(ModuleEntry module, String name,
            boolean isAbstract) throws IOException, ParseException {
        ForwardValueEntry entry = stFactory.forwardValueEntry(module,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name(name);
        entry.setInterfaceType(isAbstract ? InterfaceType.ABSTRACT
            : InterfaceType.NORMAL);
        // Comment must immediately precede "[abstract] value"
        // keyword[s]
        entry
            .comment(tokenHistory.lookBack(isAbstract ? 3 : 2).comment);
        pigeonhole(module, entry);
    } // valueForwardDcl

    private void nativeDcl(SymtabEntry entry) throws IOException,
            ParseException {
        match(Token.Native);
        NativeEntry nativeEntry = stFactory.nativeEntry(entry,
            (IDLID) repIDStack.peek());
     ...
     *
     **/
    private void constDcl(SymtabEntry entry) throws IOException,
            ParseException {
        match(Token.Const);
        ConstEntry constEntry = stFactory.constEntry(entry,
            (IDLID) repIDStack.peek());
     ...
     *
     **/
    private void constType(SymtabEntry entry) throws IOException,
            ParseException {
        switch (token.type) {
            case Token.Octet:
            ...
            case Token.Identifier:
            case Token.DoubleColon:
                entry.type(scopedName(entry.container(), stFactory
                    .primitiveEntry()));
                ...
                if (!((entryType instanceof PrimitiveEntry) || (entryType instanceof StringEntry))) {
                    ParseException.wrongType(scanner, entry.fullName(),
                        "primitive or string", entryName(entry.type()));
                    ...
                    if (entryType.name().equals(any)) {
                        ParseException.wrongType(scanner, entry
                            .fullName(), "primitive or string (except "
                            + any + ')', any);
                        entry.type(qualifiedEntry("long"));
                    }
                }
            ...
            default:
                throw ParseException.syntaxError(scanner, new int[] {
                    Token.Long, Token.Short, Token.Unsigned,
                    Token.Char, Token.Wchar, Token.Boolean,
                    Token.Float, Token.Double, Token.String,
                    Token.Wstring, Token.Identifier, Token.DoubleColon
                }, token.type);
        }
    } // constType
    ...
    private void verifyIntegral(Number n, SymtabEntry t) {
        ...
        // org.cognitivesystems.idl.constExpr package
        ...
        if (t == qualifiedEntry("octet")) {
            ...
            if (n.longValue() > (long) Integer.MAX_VALUE * 2 + 1
                || n.longValue() < 0)
                outOfRange = true;
        }
        else if (t == qualifiedEntry("unsigned short")) {
            if (n.intValue() > (int) Short.MAX_VALUE * 2 + 1
                || n.intValue() < 0)
                outOfRange = true;
        }
        else if (t == qualifiedEntry("long long")) {
            // BigInteger required because value being compared may
            // exceed
            ...
            BigInteger ullMax = BigInteger.valueOf(Long.MAX_VALUE)
                .multiply(BigInteger.valueOf(2)).add(
            ...
            /*
             * if (n instanceof Short) got = "short"; else if (n
             * instanceof Integer) got = "long"; else got = "long long";
             */
            got = "long";
        ...
        if (outOfRange)
            ParseException.outOfRange(scanner, n.toString(), t
                .fullName());
    } // verifyIntegral
    ...
    private void verifyString(Expression e, SymtabEntry t) {
        ...
        if (!(t instanceof StringEntry)) {
            ParseException.wrongExprType(scanner, t.fullName(), e
                .type());
        }
            ...
            try {
                ...
                if (string.length() > max.intValue())
                    ParseException.stringTooLong(scanner, string, max
                        .toString());
            }
    ...
    private void verifyFloat(Number f, SymtabEntry t) {
        ...
        boolean outOfRange = false;
        if (t.name().equals(overrideName("float"))) {
            double absVal = (f.doubleValue() < 0.0) ? f.doubleValue()
                * -1.0 : f.doubleValue();
            if ((absVal != 0.0)
            ...
            // Cannot check range of double until BigDecimal is the
            // basis
            // of all floating-point types. Currently, it is Double. The
            // parser will fail when instantiating a Double with an
            // exception.
        }
        else {
            ParseException.wrongExprType(scanner, t.fullName(),
        ...
        if (outOfRange)
            ParseException.outOfRange(scanner, f.toString(), t
                .fullName());
    } // verifyFloat
     ...
     *
     **/
    Expression constExp(SymtabEntry entry) throws IOException,
            ParseException {
        ...
        try {
            ...
            // <d54042> Coerces integral value to Double if an integer
            // literal
            // was used to initialize a floating-point constant
            // expression.
            if (expr instanceof Terminal
                && expr.value() instanceof BigInteger
                && (overrideName(expr.type()).equals("float") || overrideName(
                    expr.type()).indexOf("double") >= 0)) {
                expr.value(new Double(((BigInteger) expr.value())
                    .doubleValue()));
            }
        }
        catch (EvaluationException exception) {
            ParseException.evaluationError(scanner, exception
                .toString());
        }
        return expr;
    } // constExp
     ...
     *
     **/
    private Expression orExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
     ...
     *
     **/
    private Expression xorExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
     ...
     *
     **/
    private Expression andExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
     ...
     *
     **/
    private Expression shiftExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
     ...
     *
     **/
    private Expression addExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
     ...
     *
     **/
    private Expression multExpr(Expression e, SymtabEntry entry)
            throws IOException, ParseException {
        if (e == null)
            e = unaryExpr(entry);
        else {
     ...
     *
     **/
    private Expression unaryExpr(SymtabEntry entry) throws IOException,
            ParseException {
        if (token.equals(Token.Plus)) {
            match(token.type);
    ...
    private Expression primaryExpr(SymtabEntry entry)
        ...
        if (parsingConditionalExpr) {
            ...
            token = prep.token; // Get the current token from
                                // preprocessor
        }
        else
            switch (token.type) {
                ...
                case Token.DoubleColon:
                    ...
                    expectedC.value(exprFactory.terminal("1",
                        BigInteger.valueOf(1)));
                    SymtabEntry ref = scopedName(entry.container(),
                        expectedC);
                    if (!(ref instanceof ConstEntry)) {
                        ParseException.invalidConst(scanner, ref
                            .fullName());
                        // An error occurred. Just give it some bogus
                        // value. <daz>
                        // primary = exprFactory.terminal ("1", new Long
                        // (1));
                        primary = exprFactory.terminal("1", BigInteger
                            .valueOf(1));
                    }
                    else
                        primary = exprFactory
                            .terminal((ConstEntry) ref);
                    break;
     ...
     *
     **/
    Expression literal(SymtabEntry entry) throws IOException,
            ParseException {
        ...
        switch (token.type) {
            case Token.IntegerLiteral:
                ...
                try {
                    literal = exprFactory.terminal(string,
                        parseString(string));
                    literal.type(entry.typeName());
                }
                catch (NumberFormatException exception) {
                    ParseException.notANumber(scanner, string);
                    literal = exprFactory.terminal("0", BigInteger
                        .valueOf(0));
                }
                break;
            case Token.CharacterLiteral:
                ...
                literal = exprFactory.terminal("'"
                    + string.substring(1) + "'", new Character(string
                    .charAt(0)), isWide);
                break;
            case Token.FloatingPointLiteral:
                match(Token.FloatingPointLiteral);
                try {
                    literal = exprFactory.terminal(string, new Double(
                        string));
                    literal.type(entry.typeName());
                }
     ...
     *
     **/
    private BigInteger parseString(String string)
     ...
     *
     **/
    private Terminal booleanLiteral() throws IOException,
            ParseException {
        Boolean bool = null;
     ...
     *
     **/
    private Expression stringLiteral() throws IOException,
            ParseException {
     ...
     *
     **/
    private Expression positiveIntConst(SymtabEntry entry)
            throws IOException, ParseException {
     ...
     *
     **/
    private SymtabEntry typeDcl(SymtabEntry entry) throws IOException,
            ParseException {
        switch (token.type) {
            case Token.Typedef:
                match(Token.Typedef);
     ...
     *
     **/
    private TypedefEntry typeDeclarator(SymtabEntry entry)
            throws IOException, ParseException {
        TypedefEntry typedefEntry = stFactory.typedefEntry(entry,
            (IDLID) repIDStack.peek());
        typedefEntry.sourceFile(scanner.fileEntry());
     ...
     *
     **/
    private SymtabEntry typeSpec(SymtabEntry entry) throws IOException,
            ParseException {
        return ((token.type == Token.Struct)
            || (token.type == Token.Union) || (token.type == Token.Enum)) ? constrTypeSpec(entry)
    ...
    private SymtabEntry simpleTypeSpec(SymtabEntry entry,
        ...
        if ((token.type == Token.Identifier)
            ...
            SymtabEntry container = ((entry instanceof InterfaceEntry)
                ...
                || (entry instanceof StructEntry) || (entry instanceof UnionEntry)) ? entry
                : entry.container();
            return scopedName(container, stFactory.primitiveEntry(),
                mustBeReferencable);
        }
        return ((token.type == Token.Sequence)
            || (token.type == Token.String) || (token.type == Token.Wstring)) ? templateTypeSpec(entry)
            : baseTypeSpec(entry);
    } // simpleTypeSpec
     ...
     *
     **/
    private SymtabEntry baseTypeSpec(SymtabEntry entry)
            throws IOException, ParseException {
        switch (token.type) {
            case Token.Float:
            case Token.Double:
            ...
            case Token.Any:
                return anyType();
                // NOTE: Object and ValueBase are <base_type_spec>s, but
                // both
                // are processed at simpleTypeSpec(), not here.
                // parmTypeSpec()
                // directly checks for these types. Could make
                // baseTypeSpec() do
                // the same
            default:
                throw ParseException.syntaxError(scanner, new int[] {
                    Token.Float, Token.Double, Token.Long, Token.Short,
                    Token.Unsigned, Token.Char, Token.Wchar,
                    Token.Boolean, Token.Octet, Token.Any
                }, token.type);
        }
    } // baseTypeSpec
     ...
     *
     **/
    private SymtabEntry templateTypeSpec(SymtabEntry entry)
            throws IOException, ParseException {
        switch (token.type) {
            case Token.Sequence:
        ...
        throw ParseException.syntaxError(scanner, new int[] {
            Token.Sequence, Token.String, Token.Wstring
        }, token.type);
    } // templateTypeSpec
     ...
     *
     **/
    private SymtabEntry constrTypeSpec(SymtabEntry entry)
            throws IOException, ParseException {
        switch (token.type) {
            case Token.Struct:
                return structType(entry);
     ...
     *
     **/
    private void declarators(TypedefEntry entry, Vector list)
            throws IOException, ParseException {
        for (;; match(Token.Comma)) {
     ...
     *
     **/
    private void declarator(TypedefEntry entry) throws IOException,
            ParseException {
        entry.name(token.name);
        // If the declarator is commented then override the comment
     ...
     *
     **/
    private PrimitiveEntry floatingPtType() throws IOException,
            ParseException {
        String name = "double";
        if (token.type == Token.Float) {
            ...
            ParseException.syntaxError(scanner, new int[] {
                Token.Float, Token.Double
            }, token.type);
        }
     ...
     *
     **/
    private PrimitiveEntry integerType(SymtabEntry entry)
            throws IOException, ParseException {
        String name = "";
     ...
     *
     **/
    private PrimitiveEntry charType() throws IOException,
            ParseException {
        String tokenName;
        ...
        try {
            ...
            ParseException.undeclaredType(scanner,
                overrideName(tokenName));
        }
        return ret;
    } // charType
     ...
     *
     **/
    private PrimitiveEntry booleanType() throws IOException,
            ParseException {
        PrimitiveEntry ret = null;
        match(Token.Boolean);
        try {
     ...
     *
     **/
    private PrimitiveEntry octetType() throws IOException,
            ParseException {
        PrimitiveEntry ret = null;
        match(Token.Octet);
        try {
     ...
     *
     **/
    private StructEntry structType(SymtabEntry entry)
            throws IOException, ParseException {
        match(Token.Struct);
        String name = token.name;
        match(Token.Identifier);
        ...
        if (token.type == Token.LeftBrace) {
            ...
            throw ParseException.syntaxError(scanner, new int[] {
                Token.Semicolon, Token.LeftBrace
            }, token.type);
        }
        return structEntry;
    } // structType
    ...
    private void member(StructEntry entry) throws IOException {
        TypedefEntry newEntry = stFactory.typedefEntry(entry,
            (IDLID) repIDStack.peek());
        ...
        try {
            ...
            if (newEntry.type() == entry)
                throw ParseException
                    .recursive(scanner, entry.fullName(),
                        (token.name == null) ? "" : token.name);
            // <d46094> Exception cannot appear within a struct, union,
            // or exception
            if (typeOf(newEntry) instanceof ExceptionEntry)
                throw ParseException.illegalException(scanner,
                    entryName(entry));
     ...
     *
     **/
    private UnionEntry unionType(SymtabEntry entry) throws IOException,
            ParseException {
        match(Token.Union);
        String name = token.name;
        match(Token.Identifier);
     ...
     *
     **/
    private SymtabEntry switchTypeSpec(UnionEntry entry)
            throws IOException, ParseException {
        ...
        switch (token.type) {
            ...
            default:
                throw ParseException.syntaxError(scanner, new int[] {
                    Token.Long, Token.Short, Token.Unsigned,
                    Token.Char, Token.Boolean, Token.Enum,
                    Token.Identifier, Token.DoubleColon
                }, token.type);
        }
        return ret;
     ...
     *
     **/
    private void switchBody(UnionEntry entry) throws IOException,
            ParseException {
        caseProd(entry);
        while (!token.equals(Token.RightBrace))
            caseProd(entry);
        entry.defaultBranch((defaultBranch == null) ? null
            : defaultBranch.typedef);
        defaultBranch = null;
    } // switchBody
     ...
     *
     **/
    private void caseProd(UnionEntry entry) throws IOException,
            ParseException {
     ...
     *
     **/
    private void caseLabel(UnionEntry entry, UnionBranch branch)
            throws IOException, ParseException {
        if (token.type == Token.Case) {
            match(Token.Case);
            ConstEntry tmpEntry = stFactory.constEntry(entry,
                (IDLID) repIDStack.peek());
     ...
     *
     **/
    private Expression matchEnum(EnumEntry entry) throws IOException,
            ParseException {
        // Get the symbol table entry for the case label based on the
        // scope of the EnumEntry, NOT the UnionEntry (the union could
        // be
        ...
        // case M::A:
        // case ::M::A:
        SymtabEntry label = scopedName(entry.container(),
            new SymtabEntry());
     ...
     *
     **/
    private void elementSpec(UnionEntry entry, UnionBranch branch)
            throws IOException, ParseException {
        TypedefEntry typedef = stFactory.typedefEntry(entry,
            (IDLID) repIDStack.peek());
        ...
        if (typedef.type() == entry)
            throw ParseException.recursive(scanner, entry.fullName(),
                (token.name == null) ? "" : token.name);
        // <d46094> Exception cannot appear within a struct, union, or
        // exception
     ...
     *
     **/
    private EnumEntry enumType(SymtabEntry entry) throws IOException,
            ParseException {
        match(Token.Enum);
        ...
        if (isntInStringList(enumEntry.elements(), token.name)) {
            ...
            SymtabEntry element = new SymtabEntry(entry,
                (IDLID) repIDStack.peek());
            ...
            // <d50237> Place the SymtabEntry representing this
            // enumeration
            // contant into the SymtabEntry defining its scope (e.g.,
            // InterfaceEntry,
            // ValueEntry, etc.) rather than the SymtabEntry passed in,
            // which
     ...
     *
     **/
    private void enumType2(EnumEntry entry) throws IOException,
            ParseException {
        while (token.type == Token.Comma) {
            ...
            if (isntInStringList(entry.elements(), name)) {
                ...
                SymtabEntry element = new SymtabEntry(
                    entry.container(), (IDLID) repIDStack.peek());
                // if block taken from EnumEntry ctor:
                if (element.module().equals(""))
                    element.module(element.name());
                else if (!element.name().equals(""))
     ...
     *
     **/
    private SequenceEntry sequenceType(SymtabEntry entry)
            throws IOException, ParseException {
        ...
        if (token.type == Token.Comma) {
            match(Token.Comma);
            ConstEntry tmpEntry = stFactory.constEntry(newEntry,
                (IDLID) repIDStack.peek());
            tmpEntry.sourceFile(scanner.fileEntry());
     ...
     *
     **/
    private StringEntry stringType(SymtabEntry entry)
            throws IOException, ParseException {
     ...
     *
     **/
    private Expression stringType2(SymtabEntry entry)
            throws IOException, ParseException {
     ...
     *
     **/
    private void fixedArraySize(TypedefEntry entry) throws IOException,
            ParseException {
        match(Token.LeftBracket);
        ConstEntry tmpEntry = stFactory.constEntry(entry,
            (IDLID) repIDStack.peek());
        tmpEntry.sourceFile(scanner.fileEntry());
        // <d58058> Set type of tmpExpr to "long", which is the array
        // index type.
        // Previously, this type was erroneously set to the array
        // element type.
     ...
     *
     **/
    private void attrDcl(InterfaceEntry entry) throws IOException,
            ParseException {
        AttributeEntry attribute = stFactory.attributeEntry(entry,
            (IDLID) repIDStack.peek());
        attribute.sourceFile(scanner.fileEntry());
        // Comment must immediately precede the "attribute" keyword.
        // Save the
        ...
        // Override declaration comment if attribute identifier is
        // commented
        if (!token.comment.text().equals(""))
            attribute.comment(token.comment);
        ...
        if (!token.comment.text().equals("")) {
            // Create a temporary attribute with declaration comment so
            // cloning in
            // attrdcl2() can use declaration comment as default.
            AttributeEntry attributeClone = (AttributeEntry) attribute
                .clone();
    ...
    private void attrDcl2(InterfaceEntry entry, AttributeEntry clone)
        ...
        while (token.type == Token.Comma) {
            ...
            attribute.name(token.name);
            // Override the declaration comment (i.e., that preceding
            // the
            // "attribute" keyword) if the attribute identifier is
            // commented.
            if (!token.comment.text().equals(""))
                attribute.comment(token.comment);
            entry.methodsAddElement(attribute, scanner);
     ...
     *
     **/
    private void exceptDcl(SymtabEntry entry) throws IOException,
            ParseException {
        match(Token.Exception);
        ExceptionEntry exceptEntry = stFactory.exceptionEntry(entry,
            (IDLID) repIDStack.peek());
        repIDStack.push(((IDLID) repIDStack.peek()).clone());
        ((IDLID) repIDStack.peek()).appendToName(token.name);
        ...
        if (token.equals(Token.LeftBrace)) {
            ...
            throw ParseException.syntaxError(scanner, Token.LeftBrace,
                token.type);
    } // exceptDcl
     ...
     *
     **/
    private void opDcl(InterfaceEntry entry) throws IOException,
            ParseException {
        MethodEntry method = stFactory.methodEntry(entry,
            (IDLID) repIDStack.peek());
        method.sourceFile(scanner.fileEntry());
        // Comment must immediately precede "oneway" keyword or
        // <return_type_spec>
        method.comment(token.comment);
    ...
    private void checkIfOpLegalForOneway(MethodEntry method) {
        ...
        if ((method.type() != null)
            ...
            for (Enumeration e = method.parameters().elements(); e
                .hasMoreElements();) {
     ...
     *
     **/
    private void opDcl2(MethodEntry method) throws IOException,
            ParseException {
     ...
     *
     **/
    private void opDcl3(MethodEntry entry) throws IOException,
            ParseException {
        if (token.type != Token.Semicolon) {
            if (!token.equals(Token.Raises)
                && !token.equals(Token.Context))
                throw ParseException.syntaxError(scanner, new int[] {
                    Token.Raises, Token.Context, Token.Semicolon
                }, token.type);
     ...
     *
     **/
    private SymtabEntry opTypeSpec(SymtabEntry entry)
            throws IOException, ParseException {
        SymtabEntry ret = null;
     ...
     *
     **/
    private void parameterDcls(MethodEntry entry) throws IOException,
            ParseException {
        match(Token.LeftParen);
     ...
     *
     **/
    private void parameterDcls2(MethodEntry entry) throws IOException,
            ParseException {
        if (token.type == Token.RightParen)
            match(Token.RightParen);
        else {
     ...
     *
     **/
    private void paramDcl(MethodEntry entry) throws IOException,
            ParseException {
        ParameterEntry parmEntry = stFactory.parameterEntry(entry,
            (IDLID) repIDStack.peek());
        parmEntry.sourceFile(scanner.fileEntry());
     ...
     *
     **/
    private void paramAttribute(ParameterEntry entry)
            throws IOException, ParseException {
     ...
     *
     **/
    private void raisesExpr(MethodEntry entry) throws IOException,
            ParseException {
        match(Token.Raises);
        match(Token.LeftParen);
        // Comment must immediately precede <scoped_name> for exception
        Comment tempComment = token.comment;
        SymtabEntry exception = scopedName(entry.container(), stFactory
            .exceptionEntry());
        if (typeOf(exception) instanceof ExceptionEntry) {
            // Comment must immediately precede <scoped_name> for
            // exception
     ...
     *
     **/
    private void raisesExpr2(MethodEntry entry) throws IOException,
            ParseException {
        while (token.type == Token.Comma) {
            match(Token.Comma);
            // Comment must immediately precede <scoped_name> of
            // exception
            Comment tempComment = token.comment;
            SymtabEntry exception = scopedName(entry.container(),
                stFactory.exceptionEntry());
            if (typeOf(exception) instanceof ExceptionEntry) {
                // Comment must immediately precede <scoped_name> of
                // exception
                exception.comment(tempComment);
     ...
     *
     **/
    private void contextExpr(MethodEntry entry) throws IOException,
            ParseException {
        match(Token.Context);
        match(Token.LeftParen);
    ...
    private void contextExpr2(MethodEntry entry) throws IOException,
            ParseException {
        while (token.type == Token.Comma) {
            match(Token.Comma);
            String stringLit = (String) stringLiteral().value();
     ...
     *
     **/
    private SymtabEntry paramTypeSpec(SymtabEntry entry)
            throws IOException, ParseException {
        SymtabEntry ret = null;
        switch (token.type) {
            case Token.Float:
            ...
            default:
                throw ParseException.syntaxError(scanner, new int[] {
                    ...
                    Token.DoubleColon, Token.ValueBase
                }, token.type);
        }
        return ret;
    } // paramTypeSpec
    ...
    private void match(int type) throws IOException, ParseException {
        ...
        if (!token.equals(type)) {
            exception = ParseException.syntaxError(scanner, type,
                token.type);
            // Missing a semicolon is a common error. If a semicolon was
            // expected,
            // assume it exists and keep the current token (don't get
            // the next one).
        ...
        // <d59166> Identifiers that collide with keywords are illegal.
        // Note
        // that escaped identifers never collide!
        /*
         * if (token.collidesWithKeyword ()) { // <f60858.1> Issue a
         * warning only if (corbaLevel <= 2.2f)
         * ParseException.keywordCollisionWarning (scanner, token.name);
         * else exception = ParseException.keywordCollision (scanner,
         * token.name); }
         */
        ...
        if (token.equals(Token.Identifier)
            || token.equals(Token.MacroIdentifier)) {
    ...
    private void issueTokenWarnings() {
        ...
        if ((token.equals(Token.Identifier) || token
            .equals(Token.MacroIdentifier))
            && !token.isEscaped()) {
            // Identifier collision with keyword in another release.
            // Identifier collision with keyword in letter, but not in
            // case.
            if (token.collidesWithKeyword())
                ParseException.warning(scanner, Util.getMessage(
                    "Migration.keywordCollision", token.name));
        }
        // Deprecated keyword.
        if (token.isKeyword() && token.isDeprecated())
            ParseException.warning(scanner, Util.getMessage(
                "Deprecated.keyword", token.toString()));
    } // issueTokenWarnings
    ...
    private ModuleEntry newModule(ModuleEntry oldEntry) {
        ModuleEntry entry = stFactory.moduleEntry(oldEntry,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name(token.name);
        // If this named module already exists, just reopen it.
        /* <46082.46.01> if cppModule, always create new module entry */
        SymtabEntry prevEntry = (SymtabEntry) symbolTable.get(entry
            .fullName());
        if (!cppModule && prevEntry != null
            && prevEntry instanceof ModuleEntry) {
            // A module has been reopened, return that ModuleEntry.
            entry = (ModuleEntry) prevEntry;
            if (oldEntry == topLevelModule) {
                // Do a little checking:
                if (!entry.emit())
                    // The entry module is being reopened to put new
                    // stuff into it.
                    // The module itself is not marked as "emit", but
                    // the new stuff
                    // may be, so put the module on the emitList (add it
                    // to topLevelModule).
                    addToContainer(oldEntry, entry);
                else if (!oldEntry.contained().contains(entry))
                    // <d50767> The entry module being reopened is to be
                    // emitted, but
                    // will not be placed on the emitList! I.E., it was
                    // not added to
                    // topLevelModule. Occurs when a generator manually
                    // inserts
                    // ModuleEntrys into the symbol table (e.g., org;
                    // see preParse()
    ...
    private EnumEntry newEnumEntry(SymtabEntry oldEntry) {
        EnumEntry entry = stFactory.enumEntry(oldEntry,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name(token.name);
        pigeonhole(oldEntry, entry);
    ...
    private SequenceEntry newSequenceEntry(SymtabEntry oldEntry) {
        SequenceEntry entry = stFactory.sequenceEntry(oldEntry,
            (IDLID) repIDStack.peek());
        entry.sourceFile(scanner.fileEntry());
        entry.name("");
        pigeonhole(oldEntry, entry);
    ...
    private void updateSymbolTable(String fullName, SymtabEntry entry,
            boolean lcCheck) {
        ...
        // <d59809> Allow fully-qualified CORBA types to be resolved by
        // mapping
        // short name (e.g., CORBA/StringValue) to long name, actual
        // name.
        String omgPrefix = "org/omg/CORBA";
        if (fullName.startsWith(omgPrefix)) {
            overrideNames.put("CORBA"
                + fullName.substring(omgPrefix.length()), fullName);
        }
    }
    ...
    private void pigeonhole(SymtabEntry container, SymtabEntry entry) {
        ...
        if (overrideNames.get(fullName) == null) {
            ...
            SymtabEntry oldEntry = (SymtabEntry) symbolTable
                .get(fullName);
            ...
            if (oldEntry == null) {
                ...
                String repIDPrefix = ((IDLID) entry.repositoryID())
                    .prefix();
                String oldRepIDPrefix = ((IDLID) oldEntry
                    .repositoryID()).prefix();
                ...
                if (oldEntry instanceof ForwardEntry
                    ...
                    if (!(repIDPrefix.equals(oldRepIDPrefix))) {
                        // Disallow multiple ForwardEntry's having same
                        // Repository
                        // ID prefixes (CORBA 2.3).
                        ParseException.badRepIDPrefix(scanner,
                            fullName, oldRepIDPrefix, repIDPrefix);
                    }
                }
            }
                ...
                // Ignore CORBA PIDL types entered at preParse() by
                // generator.
            }
            else if (isForwardable(oldEntry, entry)) {
                // Both oldEntry and entry are structs or unions.
                // Legality depends on isReferencable on the two
                // entries:
                // oldEntry Entry
                ...
                if (entry.isReferencable()) {
                    String firstFile = oldEntry.sourceFile()
                        .absFilename();
                    ...
                    if (!firstFile.equals(defFile))
                        ...
                        try {
                            oldRefList = (List) oldEntry
                                .dynamicVariable(ftlKey);
                        }
                        catch (NoSuchFieldException exc) {
                            throw new IllegalStateException();
                        }
                        ...
                        if (oldRefList != null) {
                            ...
                            while (iter.hasNext()) {
                                SymtabEntry elem = (SymtabEntry) iter
                                    .next();
                                elem.type(entry);
                            }
                        }
    ...
    private boolean isForwardable(SymtabEntry oldEntry,
            SymtabEntry entry) {
        return ((oldEntry instanceof StructEntry) && (entry instanceof StructEntry))
            || ((oldEntry instanceof UnionEntry) && (entry instanceof UnionEntry));
    }
     ...
     *
     **/
    private void pigeonholeMethod(InterfaceEntry container,
            MethodEntry entry) {
        if (entry.name().equals(""))
            entry.name(unknownNamePrefix + ++sequence);
        ...
        if (overrideNames.get(fullName) == null) {
            ...
            symbolTable.put(fullName, entry);
            lcSymbolTable.put(lcFullName, entry);
            // <d59809> Allow fully-qualified CORBA types to be resolved
            // by mapping
            // short name (e.g., CORBA/StringValue) to long name, actual
            // name.
            if (fullName.startsWith("org/omg/CORBA"))
                overrideNames.put("CORBA" + fullName.substring(13),
                    fullName);
        }
    } // pigeonholeMethod
     ...
     *
     **/
    private void addToContainer(SymtabEntry container,
    ...
    SymtabEntry qualifiedEntry(String typeName) {
        ...
        // <d57110> Relax this retriction when parsing ID pragma
        // directive, e.g..
    ...
    SymtabEntry recursiveQualifiedEntry(String typeName) {
        ...
        if (typeName != null && !typeName.equals("void")) {
            ...
            if (index >= 0) {
                // Figure out if the container of this thing exists,
                // converting any typedefs to interfaces if necessary.
                type = recursiveQualifiedEntry(typeName.substring(0,
                    index));
                if (type == null)
                    return null;
                else if (type instanceof TypedefEntry)
                    typeName = typeOf(type).fullName()
                        + typeName.substring(index);
            }

            // If we got this far, a container exists, start over
            // looking
            // for the thing itself (this is the meat of the method):
            type = searchOverrideNames(typeName);
            if (type == null)
                type = (SymtabEntry) symbolTable.get(typeName); // search
                                                                // global
                                                                // scope:
            if (type == null)
     ...
     *
     **/
    SymtabEntry partlyQualifiedEntry(String typeName,
            SymtabEntry container) {
        // This is the simple logic of this method:
        // type = searchModuleScope (typeName.substring (0,
        // typeName.lastIndexOf ('/')), container);
        // type = qualifiedEntry (type.fullName () + typeName.substring
        ...
        if (typeName != null) {
            ...
            type = recursivePQEntry(typeName.substring(0, index),
                container);
            ...
            if (container != null)
                type = searchModuleScope(typeName.substring(0, typeName
                    .lastIndexOf('/')), container);
            if (type == null)
                type = qualifiedEntry(typeName);
            else
                type = qualifiedEntry(type.fullName()
    ...
    SymtabEntry recursivePQEntry(String typeName, SymtabEntry container) {
        ...
        if (typeName != null) {
            ...
            if (index < 0)
                ...
                // If we got this far, a container exists, start over,
                // looking
                // for the thing itself (This is the meat of the
                // method):
                if (container != null)
                    type = searchModuleScope(typeName.substring(0,
                        typeName.lastIndexOf('/')), container);
                if (type == null)
                    ...
                    type = recursiveQualifiedEntry(type.fullName()
                        + typeName.substring(typeName.lastIndexOf('/')));
            }
        }
        return type;
     ...
     *
     **/
    SymtabEntry unqualifiedEntryWMod(String typeName,
            SymtabEntry container) {
        SymtabEntry type = null;
        if ((typeName != null) && !typeName.equals("void")) {
            // Search local scope:
            type = (SymtabEntry) symbolTable.get(container.fullName()
                + '/' + typeName);
            if (type == null)
    ...
    /**
     * Walks up the enclosing scopes until it finds an interface type.
     * Then, searches up that interface inheritance tree for the type
     * definition.
     * 
     * @param name
     *            type name to be searched for.
     * @param ptype
     *            parent type entry.
     */
    SymtabEntry searchParentInheritanceScope(String name,
            SymtabEntry ptype) {
        ...
        if ((ptype == null) || !(ptype instanceof InterfaceEntry)) {
            return null; // could not find an enclosing interface
                            // type - give up.
        }

        // check if the enclosing interface supports the type
        // definition.
    ...
    SymtabEntry searchGlobalInheritanceScope(String name) {
        ...
        if (index >= 0) {
            ...
            ? searchLocalInheritanceScope(name.substring(index + 1),
                entry)
                : null;
        }
        return entry;
     ...
     *
     **/
    SymtabEntry searchLocalInheritanceScope(String name,
            SymtabEntry container) {
        return (container instanceof InterfaceEntry) ? searchDerivedFrom(
            name, (InterfaceEntry) container)
            : null;
    ...
    SymtabEntry searchOverrideNames(String name) {
        ...
        return (overrideName != null) ? (SymtabEntry) symbolTable
            .get(overrideName) : null;
    } // searchOverrideNames
    ...
    SymtabEntry searchModuleScope(String name, SymtabEntry container) {
        ...
        return (type == null) ? (SymtabEntry) symbolTable.get(name)
            : type;
    } // searchModuleScope
    ...
    SymtabEntry searchDerivedFrom(String name, InterfaceEntry i) {
        for (Enumeration e = i.derivedFrom().elements(); e
            .hasMoreElements();) {
            ...
            if (tmp instanceof InterfaceEntry) {
                ...
                SymtabEntry type = (SymtabEntry) symbolTable
                    .get(fullName);
                if (type != null)
                    return type;
            ...
            // else it is a ForwardEntry and nothing can be done at this
            // point.
        }
        return null;
    } // searchDerivedFrom
    ...
    String entryName(SymtabEntry entry) {
        ...
        if (entry instanceof ValueBoxEntry)
            return "value box";
        if (entry instanceof ForwardValueEntry
            || entry instanceof ValueEntry)
            return "value";
    ...
    private boolean isInterface(SymtabEntry entry) {
        return entry instanceof InterfaceEntry
            || (entry instanceof ForwardEntry && !(entry instanceof ForwardValueEntry));
    }

    private boolean isValue(SymtabEntry entry) {
        return entry instanceof ValueEntry; // || entry instanceof
                                            // ForwardValueEntry;
    }
    ...
    private boolean isntInList(Vector list, SymtabEntry entry) {
        ...
        for (Enumeration e = list.elements(); e.hasMoreElements();) {
            ...
            if (entry == eEntry) // && entry.fullName().equals
                                    // (eEntry.fullName()))
            {
                ParseException.alreadyDeclared(scanner, entry
                    .fullName());
                isnt = false;
                break;
            }
    ...
    void forwardEntryCheck() {
        for (Enumeration e = symbolTable.elements(); e
            .hasMoreElements();) {
    ...
    /*
     * void updateRepositoryIds () { for (Enumeration e =
     * symbolTable.elements(); e.hasMoreElements();) { SymtabEntry entry =
     * (SymtabEntry) e.nextElement(); if (entry instanceof ValueEntry)
     * ((ValueEntry) entry).calcRepId(); } } // updateRepositoryIds
     */
    ...
    // <d56351> In CORBA 2.3, an IDL file provides a new scope for
    // Repository IDs.
    // The following methods provide a means for other classes in the
    // framework
    ...
    boolean noWarn;
    Scanner scanner;
    // <f46082.40> No longer necessary due to new valueElement()
    // algorithm.
    ...
    // <d56351> Static field necessary to allow Scanner access to
    // enterind/exiting
    ...
    // Circular buffer containing most recent tokens, including the
    // current token.
