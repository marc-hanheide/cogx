...
namespace cast {
  ...
  bool CASTWorkingMemory::add(const string & _id, 
	...
	m_storage[_id]->setVersion((j->second + 1));
  ...
  bool CASTWorkingMemory::overwrite(const string &  _id, 
    ...
    if(i != m_storage.end()) {
      ...
      _pData->setVersion(i->second->getVersion() + 1);
  ...
  CASTWorkingMemory::remove(const string &  _id) {
    ...
    //check whether the id exists
    WMItemMap::iterator i = m_storage.find(_id);
    shared_ptr<CASTWorkingMemoryItem> pItem; //starts NULL
    ...
    if(i != m_storage.end()) {
      ...
      //store last version
      m_lastVersions[_id] = pItem->getVersion();      
    }
  ...
  CASTWorkingMemory::get(const string &  _id) {
    ...
    //check whether the id exists
    WMItemMap::iterator i = m_storage.find(_id);
    shared_ptr<CASTWorkingMemoryItem> item; //starts NULL
  ...
  int CASTWorkingMemory::getOverwriteCount(const string &  _id) {
    ...
    WMItemMap::iterator i = m_storage.find(_id);
    if(i != m_storage.end()) {
      //cout<<"stored version: "<<_id<<" "<<i->second->getVersion()<<endl;
      count =  i->second->getVersion();
    }
    else {
      VersionMap::const_iterator j = m_lastVersions.find(_id);
      if(j != m_lastVersions.end()) {
  ...
  CASTWorkingMemory::getByType(const string & _type,
			       vector< shared_ptr<CASTWorkingMemoryItem> > & _items) {
