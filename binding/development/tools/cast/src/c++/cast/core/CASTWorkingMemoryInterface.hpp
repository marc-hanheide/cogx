...
namespace cast {
  ...
  public:
    ...
    virtual void getByType(const std::string & _type, 
			   const int & _count,
			   std::vector< boost::shared_ptr<CASTWorkingMemoryItem> > & _items) = 0;
    ...
    virtual bool add(const std::string & _id, 
		     boost::shared_ptr<CASTWorkingMemoryItem> _pData) = 0;
     ...
     */
    virtual boost::shared_ptr<CASTWorkingMemoryItem> remove(const std::string &  _id) = 0;
    ...
    virtual boost::shared_ptr<CASTWorkingMemoryItem> get(const std::string &  _id) = 0;
