...
public class AudienceMember extends ManagedComponent {
	...
	@Override
	public void start() {
		addChangeFilter(ChangeFilterFactory.createGlobalTypeFilter(Joke.class,
	...
	private Reaction generateAudienceReaction(Joke _joke) {
	...
	private void newJokeAdded(WorkingMemoryChange _wmc) {
		...
		try {
			if (_wmc.address.subarchitecture.equals(getSubarchitectureID())) {
				...
				// with its id
				Joke joke = getMemoryEntry(id, Joke.class);
