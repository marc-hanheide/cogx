 ...
 * @see
 */
class TranslationProcess : public cast::ManagedComponent
{
...
private:
  ...
  typedef std::pair<std::string, NavData::NavCommandPtr> tpNavCommandWithId;
	
  // Queue of tasks with mutex
  class tpQueue:
    public std::list<tpNavCommandWithId>,
...
private:
  ...
  void changeNavCmdCompletion(const std::string &id, 
                              const NavData::Completion &completion, 
   ...
   */
  void addTaskToQueue(const std::string &id, const NavData::NavCommandPtr &cmd);
   ...
   */
  bool translateCommand(const NavData::NavCommandPtr &nav, 
