...
(:domain Storage-NetBenefit-OF-ADL)
(:objects
...
(:init
	...
	(inside depot0-1-2 depot0)
	(inside depot1-1-1 depot1)
	(inside depot1-1-2 depot1)
	(inside depot1-1-3 depot1)
...
(:goal (and
	...
	(forall (?h - hoist)
	   (preference p3B (exists (?s - storearea ?d - depot)
			      (and (at ?h ?s) (inside ?s ?d) (not (= ?d depot1))))))
	...
	(forall (?d - depot ?s - storearea ?h - hoist)
	   (preference p1A (imply (and (at ?h ?s) (inside ?s ?d))
				  (forall (?c - crate) (not (=(in ?c) ?d))))))))

(:metric maximize (- 30 (+ (* 1 (is-violated p0A))
		     (* 1 (is-violated p1A))
		     (* 2 (is-violated p2A))
		     (* 2 (is-violated p2B))
		     (* 2 (is-violated p2C))
