 ...
 */
public abstract class SimpleTransferFunction<From extends Ice.ObjectImpl, To extends dBelief>
	 ...
	 */
	public SimpleTransferFunction(ManagedComponent component,
	...
	public boolean transform(WorkingMemoryChange wmc, From from,
		...
		Map<String, FormulaProbPair> mapping;
		try {
			mapping = getFeatureValueMapping(wmc, from);
			if (mapping==null)
				return false;
			for (Entry<String, FormulaProbPair> fvm : mapping.entrySet()) {
				FormulaDistribution fd = FormulaDistribution.create();
				fd.add(fvm.getValue().val, fvm.getValue().prob);
	 ...
	 */
	protected abstract Map<String, FormulaProbPair> getFeatureValueMapping(
