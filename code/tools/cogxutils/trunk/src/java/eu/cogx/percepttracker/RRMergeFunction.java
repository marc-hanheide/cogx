...
public class RRMergeFunction<From extends dBelief, Via extends dBelief, To extends dBelief>
		extends SimpleMergeFunction<From, Via, To> {
	...
	public RRMergeFunction(List<String> _types, PointerMap<?> _map,
			Class<From> _fromCls, Class<Via> _viaCls, Class<To> _toCls) {
		super(_types, _map, _fromCls, _viaCls, _toCls);
		ignoreForPointerPropagationSet.add(SOURCE_BELIEF_ID);
	}

	public RRMergeFunction(List<String> _types, PointerMap<?> _map,
			Collection<String> _ignoreKeys, Class<From> _fromCls,
			Class<Via> _viaCls, Class<To> _toCls) {
		super(_types, _map, _ignoreKeys, _fromCls, _viaCls, _toCls);
		ignoreForPointerPropagationSet.add(SOURCE_BELIEF_ID);
	}
	...
	public To createDest(WorkingMemoryAddress idToCreate, WorkingMemoryChange wmc,
		...
		To create = super.createDest(idToCreate, wmc, from);
        ...
        @Override
              public Via createVia(WorkingMemoryAddress idToCreate, WorkingMemoryChange wmc,
                              From from) throws IncompatibleAssignmentException {
                      Via create = super.createVia(idToCreate, wmc, from);
                      create.estatus = getEpistemicStatus();
                      return create;
              }
