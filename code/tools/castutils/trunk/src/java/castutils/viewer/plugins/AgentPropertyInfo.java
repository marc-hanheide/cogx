...
public class AgentPropertyInfo implements Plugin {
	...
	public Vector<Object> toVector(Ice.ObjectImpl iceObject) {
		...
		if (gpp.distribution != null
			...
			double val = ((FloatValue) ((DiscreteProbabilityDistribution) gpp.distribution).data
					.get(0).value).value;
			extraInfo.add("val=" + val);
		} else if (gpp.distribution != null
				&& ((DiscreteProbabilityDistribution) gpp.distribution).data
						.get(0).value instanceof IntegerValue) {

			long val = ((IntegerValue) ((DiscreteProbabilityDistribution) gpp.distribution).data
					.get(0).value).value;
			extraInfo.add("val=" + val);
		}
