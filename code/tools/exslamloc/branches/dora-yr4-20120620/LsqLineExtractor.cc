...
int
LsqLineExtractor::extract(double *x, double *y, double *r, int nData)
{
#ifdef PRINTLSQSTUFF
  ...
  x_ = new double[nData];
  y_ = new double[nData];
  memcpy(x_, x, sizeof(double)*nData);
  memcpy(y_, y, sizeof(double)*nData);
  ...
  int index=0, startIndex, stopIndex;
  int used[nData];
  int nUsed = 0;
  int nBadOnesInRow = 0;
  ...
  do {
    ...
    if ((useDistAndRatio_ && 
      ...
      CureCERR(60) << "Line (" << nLines() << ") is a candidate\n";
	...
	CureCERR(60) << "Found line (" << nLines() << ") that starts at "
		  << startIndex << ". Determining length...\n";
	...
	fillInLine(nUsed, used, line);
        if (line.h() >= 0.5 * minLength_) {
          CureCERR(60) << "Line (" << nLines() << ") starts at " 
                        << startIndex << " and stops at " << used[nUsed - 1]
                        << " length " << line.h() * 2.0 << std::endl;
          ...
          CureCERR(60) << "Line was only " << 2.0 * line.h() 
                    << " which is not long enough\n";
          startIndex++;
        }

      } else {
	//CureCERR(60) << "Too many bad points in row\n";
	startIndex++;
      }
    } else {
      //CureCERR(60) << "Points too far apart for example\n";
      startIndex++;
    }
...
void
LsqLineExtractor::fillInLine(int nUsed, int* used, RedundantLine2DRep& line)
{
  ...
  while (fabs(angleDiffRad(tmp, approxTheta)) > PiFourth)
    tmp += PiHalf;
  theta = tmp;
  ...
  if (fabs(angleDiffRad(tmp - PiHalf, approxAlpha)) > PiHalf)
