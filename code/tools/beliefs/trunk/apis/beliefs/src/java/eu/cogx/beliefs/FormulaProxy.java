...
public class FormulaProxy<T extends dFormula> {

	private T _formula = null;
	private float _probability;
	private String _instantiated; 
	
	public static FormulaProxy<?> createElementary(String elem, float prob) {
		return new FormulaProxy<ElementaryFormula>(new ElementaryFormula(-1, elem), prob);
	}
	
	public static FormulaProxy<?> createBoolean(boolean elem, float prob) {
		return new FormulaProxy<BooleanFormula>(new BooleanFormula(-1, elem), prob);
	}
	
	public static FormulaProxy<?> createInteger(int elem, float prob) {
		return new FormulaProxy<IntegerFormula>(new IntegerFormula(-1, elem), prob);
	}
	
	public static FormulaProxy<?> createFloat(float elem, float prob) {
		return new FormulaProxy<FloatFormula>(new FloatFormula(-1, elem), prob);
	}
	
	public static FormulaProxy<?> createFormula(dFormula elem, float prob) {
		return new FormulaProxy<dFormula>(elem, prob);
	}
	
	public static FormulaProxy<?> createUnknown(float prob) {
		return new FormulaProxy<UnknownFormula>(new UnknownFormula(-1), prob);
	}
	
	/** Empty constructor. No variables are initialized. */ 
	public FormulaProxy(T d) { 
		_formula = d;
		_formula.id = -1;
		_probability = -1;
		_instantiated = ""; 
	 ...
	 */
	public FormulaProxy (T formula, float prob) 
	{
		_formula = formula;
		_probability = prob;
