					...
					const char *vertex_header_file, const char *fragment_header_file)
{
	...
	if( vertex_header_file && !vhs ){
		sprintf(charbuffer, "[tgShader::tgShader] Error loading header file for vertex shader '%s'.", vertex_header_file);
		throw std::runtime_error(charbuffer);
	}

	if( fragment_header_file && !fhs ){
		sprintf(charbuffer, "[tgShader::tgShader] Error loading header file for fragment shader '%s'.", fragment_header_file);
		throw std::runtime_error(charbuffer);
	}
