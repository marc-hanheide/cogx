...
int CMVision::addOptions(color_info *c) {
    if (color_count >= (CMV_MAX_COLORS - 1))  return -1;
    colors[color_count] = *c;
    setThreshold(color_count,c->y_low, c->y_high, c->u_low, c->u_high, c->v_low, c->v_high);
    color_count++;
    return color_count-1;
}

int CMVision::changeOptions(color_info *c, int color) {
    if (color < color_count) {
	colors[color] = *c;
        setThreshold(color,c->y_low, c->y_high, c->u_low, c->u_high, c->v_low, c->v_high);
        return color;
    } else {
        return addOptions(c);
    }
