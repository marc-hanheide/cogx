...
bool ModelLoader::LoadPly(TrackerModel &model, const char* filename){
	...
	plyfile = ply_open_for_reading((char*)filename, &nelems, &elist, &file_type, &version);
	if(plyfile==0){
		printf("[ModelLoader::LoadPly] Failed to load ply file '%s'\n", filename);
		return false;
	}
...
// 	}
	...
	if(num_textured_faces==model.m_faces.size())
		model.m_fulltextured = true;
...
bool ModelLoader::SavePly(TomGine::tgModel &model, const char* filename){
	...
	printf("[ModelLoader::SavePly(Model)] Warning not implemented, use ModelLoader::SavePly(TrackerModel)\n");
...
bool ModelLoader::SavePly(TrackerModel &model, const char* name){
	...
	if(!pFile){
		char errmsg[128];
		sprintf(errmsg, "[ModelLoader::SavePly] Can not create file '%s'! Path existing?", name);
		throw std::runtime_error(errmsg);
	}
	...
	// Header of vertex
	sprintf(number, "%d", (int)model.m_vertices.size());
	...
	// Header of face
	sprintf(number, "%d", (int)model.m_faces.size());
	...
	// Data of vertex
	for(i=0; i<model.m_vertices.size(); i++){
	...
	// Data of face
	for(i=0; i<model.m_faces.size(); i++){
