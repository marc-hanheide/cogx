...
namespace kutility
{
   ...
   {
      REAL xp = exp(rate*(x-sym_axis));
      return (maxy - miny) * xp / ( xp + 1 ) + miny;
   }

   REAL* gaussian_1d(int fsz, REAL sigma, REAL mean )
   {
      REAL* fltr = new REAL[fsz];

      int sz = (fsz-1)/2;

      int counter=-1;
      REAL sum = 0.0;
      REAL v = 2*sigma*sigma;
      for( int x=-sz; x<=sz; x++ )
      {
         counter++;
   ...
   REAL* gaussian_2d(int fsz, REAL sigma, REAL mn)
   {
      int fltr_size = fsz * fsz;
      REAL* fltr = new REAL[fltr_size];
