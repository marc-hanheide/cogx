...
int main ( int argc, char *argv[] ) {
    ...
    for (unsigned int i = 0; (i < nrOfImages) && (i < nameFilesA.size()) && (i < nameFilesB.size()); i+=every) {
        ...
        std::vector<cv::Point2f> cornersA, cornersB(patternSize.area());
        /// @ToDo replace the cornersA with a matrix of an nx1 matrix width CV_64FC2
        /// cv::Mat imgPoints(cornersA.size(), 1, CV_32FC2, &cornersA[0]);
        bool cornersAOK = findChessboardCorners(imgA, patternSize, cornersA);
        bool cornersBOK = findChessboardCorners(imgB, patternSize, cornersB);
    ...
    if (objectPoints.size() > 0)  {
        ...
        if (imagePoints2.size() == imagePoints1.size()) {
            ...
            if (strOut.empty()) {
                strTargetFolder = std::string("distored");
            } else {
                strTargetFolder = strOut + std::string("/distored");
            }
            ...
            cv::write  ( fs,  F );
            {
                ...
                std::string strTargetFolder;
                if (strOut.empty()) {
                    strTargetFolder = std::string("undistored");
                } else {
                    strTargetFolder = strOut + std::string("/undistored");
                }
                createFolder(strTargetFolder);
                fs.open(strTargetFolder + std::string("/cameraMatrix1.xml"), cv::FileStorage::WRITE);
                cv::write ( fs, cameraMatrix1 );
                ...
                cv::waitKey(10000);
                */
                /// @ToDo OpenCV StereoBM calbration should be started 
            }
        }
