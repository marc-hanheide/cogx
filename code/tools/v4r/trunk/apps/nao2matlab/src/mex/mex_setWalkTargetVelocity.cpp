...
void mexFunction ( int nlhs, mxArray *plhs[], int nrhs,  const mxArray *prhs[] ) {
    ...
    if  ((nrhs > 6) || (nrhs < 2)){
        mexErrMsgTxt ( "usage: setWalkTargetVelocity(opjPtr, x, y, theta, frequency, post)\n" );
    }
    double x = 0;
    double y = 0;
    double theta = 0;
    ...
    if (nrhs > 4) frequency = mxGetScalar ( prhs[4] );
    if (nrhs > 5) post = mxGetScalar ( prhs[5] );

		AK::NaoQi *pNaoQi = (AK::NaoQi *) AK::getMxArray2Ptr( prhs[0] );
    int taskID = pNaoQi->setWalkTargetVelocity  (x, y, theta, frequency, post);
