...
public class SpinAroundExecutor extends NonBlockingActionExecutor {
    ...
    static Logger logger = Logger.getLogger(SpinAroundExecutor.class);

    public SpinAroundExecutor(ManagedComponent _component,
                               final int _steps) {
        m_component = _component;
        m_steps = _steps;
        m_remainingCommands = new Stack<NavCommand>();
        
        logger.info("new SpinAround " + m_steps + " with steps.");
        
        double increment = (2 * Math.PI) / m_steps;
        for (int i = 0; i < m_steps; i++) {
            NavCommand cmd = SpatialActionInterface.newNavCommand();
            cmd.cmd = CommandType.TURN;
            cmd.angle = new double[] { increment };
            cmd.tolerance = new double[] { 10.0 * Math.PI / 180 };
    ...
    protected void triggerTurn()
    {
        try {
            ...
            logger.debug("Caught excpetion " + e.getMessage());
        }
    }
