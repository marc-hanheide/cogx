...

{
	...
	int size, i, j, k, adr, num, rc_unsat, rc_unseen, pargtype;
	Bool fixpoint, assSatInIniState;
	Facts *f;
	NormOperator *no;
	EasyTemplate *t1, *t2;
	NormEffect *ne;
	...
	if ( gcmd_line.display_info == 118 ) {
		printf("starting reachability analysis.\n");
		fflush(stdout);
	}
	...
	for ( i = 0; i < gnum_predicates; i++ ) {
		...
		if ( gcmd_line.display_info == 118 ) {
			printf("\ncalloc for i %d (%s) size %d", i, gpredicates[i], size);
			fflush(stdout);
		}
	...
	/* count replanning conditions */
	for ( i = 0; i < gnum_easy_operators; i++ ) {
		no = geasy_operators[i];
		no->num_replan_conds = 0;
		for ( k = 0; k < no->num_preconds; k++ ) {
			if (no->preconds[k].isReplanCond) {
				no->num_replan_conds++;
			}
		}
	}
	...
	/* do only axiom updates first (necessary for checking satisfied replanning conditions of assertions)
	 */
	fixpoint = FALSE;
	while ( !fixpoint ) {
		fixpoint = TRUE;
		...
		while ( t1 ) {
			...
			/* "no" is an axiom that has become applicable. */
			num = 0;
	...
	if ( gcmd_line.display_info == 217 ) {
		printf("\nreachability analysis after first axiom update:");
	...
	/* sort out assertions with satisfied replanning conditions */
	if (gcmd_line.prediction_level) {
		printf("\nPrinting expandable assertion now (may not be used by planner anymore):");
		printf("\nASSERTIONS START\n");
	}
	t1 = geasy_templates;
	while ( t1 ) {
		...
		rc_unsat = no->num_replan_conds;
		rc_unseen =  no->num_replan_conds;
		assSatInIniState = (no->isAssertion && rc_unsat == 0) ? TRUE : FALSE;

		for ( i = 0; i < no->num_preconds && !assSatInIniState; i++ ) {
			if (!no->preconds[i].isReplanCond)
				continue;
			...
			/* printf("Assertion %s: unsat %d, unseen %d, triggered %d\n", no->operator->name, rc_unsat, rc_unseen, assSatInIniState);*/

			if (rc_unsat == 0) {
				assSatInIniState = TRUE;
				break;
			}
		}
		...
		if (assSatInIniState) {
			if (gcmd_line.prediction_level) {
				printf("!!! assertion %s ", no->operator->name);
				for ( j=0; j < no->num_vars; j++) {
					printf("%s ", gconstants[t1->inst_table[j]]);
				}
				printf("\n");
			}
			t2 = t1->next;
...

{
	...
	for ( i = 0; i < garity[lp]; i++ ) {
		 ...
		 */
		r += b * gmember_nr[largs[minj]][gpredicates_args_type[lp][minj]];/* need number **within type** here! */
		b *= min;
		done[minj] = TRUE;
	}
