...
FrontierExplorerEventListener::FrontierExplorerEventListener(const std::string &n)
{
  m_Name = n;
}
...
FrontierExplorer::FrontierExplorer(Cure::NavController &navCtrl,
                                   LocalGridMap<unsigned char> &lgm)
  :NavControllerEventListener("FrontierExplorer"),
   NavGraphEventListener("FrontierExplorer"),
   m_NumUnreachble(0),
   m_NumGatewaySkipped(0),
   m_NavCtrlTaskId(1000),
   m_NavCtrl(navCtrl),
   m_LGMap(lgm),
   m_MinFrontWidth(0.8),
   m_MaxFrontWidth(2.0),
   m_Active(false),
   m_PassGateways(false),
...
void
FrontierExplorer::addEventListener(FrontierExplorerEventListener *l)
{
...
void
FrontierExplorer::delEventListener(FrontierExplorerEventListener *l)
{
...
void
FrontierExplorer::stopExploration()
{
...
int
FrontierExplorer::startNextExplorationStep(int taskId, int explAreaId)
{
  ...
  CureCERR(30) << "startNextExplorationStep("
               << taskId << ") first="
               << int(firstExplorationStep)
               << std::endl;
  ...
  if (m_ExplorationTaskId < 1) {
    CureCERR(20) << "\n\nWARNING: You should use taskId > 0, not "
                 << taskId << "\n\n";
  }

  CureCERR(60) << "Looking for fronts\n";
  int nf = m_FrontFinder->findFrontiers(m_MinFrontWidth, 
                                        m_MaxFrontWidth,
                                        m_Fronts);
  ...
  for (int x = -m_LGMap.getSize(); x <= m_LGMap.getSize(); x++) {
    for (int y = -m_LGMap.getSize(); y <= m_LGMap.getSize(); y++) {
      if (m_LGMap(x,y) == '1') {
        m_NonFreeSpace.setBit(x + m_LGMap.getSize(), 
                              y + m_LGMap.getSize(), 
                              true);
      }
    }
  ...
  m_NonFreeSpace.growInto(m_PathGrid, 
                          0.5*m_NavCtrl.getRobotWidth()/m_LGMap.getCellSize(), 
                          true);
  ...
  for (int x = -m_LGMap.getSize(); x <= m_LGMap.getSize(); x++) {
    for (int y = -m_LGMap.getSize(); y <= m_LGMap.getSize(); y++) {
      if (m_LGMap(x,y) == '2') {
        m_PathGrid.setBit(x + m_LGMap.getSize(), 
                          y + m_LGMap.getSize(), 
                          true);
      }
    }
  ...
  if(m_PassGateways == false) {
  	NavGraphNode* closest_node = m_NavCtrl.m_Graph.getClosestNode(cp.getX(),cp.getY(),0,3); // 0 us angle and 3 is maximum distance
	long aid;
	if (m_ExplAreaId>=0)
  ...
  if (nf > 0) {
    ...
    for (fi = m_Fronts.begin(); fi != m_Fronts.end(); fi++) {
      ...
      if (m_LGMap.worldCoords2Index(cp.getX(), cp.getY(), r2, c2) == 0 &&
        ...
        double d = (m_PathGrid.path(r1, c1, r2, c2, path, 
                                    20 * m_LGMap.getSize()) * 
                    m_LGMap.getCellSize());
        if (d > 0) {
          
          if (!m_PassGateways && pathIsThroughGateway(path, 
                                                       gatewayList,
                                                       m_NavCtrl.m_Graph)) {
#ifdef FE_PRINT_FRONTS
    ...
    if (m_CurrFront != m_Fronts.end()) {
      ...
      for (int row = rowStep; row < nextPath.Rows; row += rowStep) {
        ...
        m_LGMap.index2WorldCoords(nextPath(row, 0) - m_LGMap.getSize(),
                                  nextPath(row, 1) - m_LGMap.getSize(),
                                  x, y);
        n.setX(x);
        n.setY(y);
        n.setTheta(0);
      ...
      CureCERR(40) << "Created path with " << navPath.size()
                   << " nodes\n";
...
bool 
FrontierExplorer::pathIsThroughGateway(Cure::ShortMatrix &path, 
                                       std::list<Cure::NavGraphGateway*> &gws,
                                       Cure::NavGraph &graph) 
{
  if (gws.empty() || path.Rows <= 2) return false;
  ...
  CureCERR(50) << "Closest is node " << closest->getId()
  ...
  double xW, yW;
  m_LGMap.index2WorldCoords(r - m_LGMap.getSize(),
                            c - m_LGMap.getSize(),
                            xW, yW);    
...
bool 
FrontierExplorer::pathIsThroughGateway2(Cure::ShortMatrix &path, 
                                        std::list<Cure::NavGraphGateway*> &gws,
                                        Cure::NavGraph &graph) 
{
  if (gws.empty() || path.Rows <= 2) return false;
  ...
  double xS, yS;
  m_LGMap.index2WorldCoords(path(0, 0) - m_LGMap.getSize(),
                            path(0, 1) - m_LGMap.getSize(),
                            xS, yS);

  for (int row = 1; row < path.Rows; row++) {
    double xE, yE;
    m_LGMap.index2WorldCoords(path(row, 0) - m_LGMap.getSize(),
                              path(row, 1) - m_LGMap.getSize(),
                              xE, yE);
...
bool 
FrontierExplorer::frontierUnreachable(Cure::FrontierPt &f) {
  for (std::list<Cure::FrontierPt>::iterator fi = m_Unreachable.begin();
       fi != m_Unreachable.end(); fi++) {
...
void 
FrontierExplorer::doneTask(int taskID)
{
  ...
  if (m_NavCtrlTaskId == taskID) {
    ...
    CureCERR(30) << "TaskID did not match for doneTask "
                 << taskID << "!=" << m_NavCtrlTaskId 
                 << " we are no longer in exploration mode!!!\n";
    stopExploration();
  }
}

void 
FrontierExplorer::abortTask(int taskID)
{
  if (m_Active == false) return;
  ...
  if (m_NavCtrlTaskId == taskID) {
    ...
    CureCERR(30) << "TaskID did not match for abortTask "
                 << taskID << "!=" << m_NavCtrlTaskId 
                 << " we are no longer in exploration mode!!!\n";
    stopExploration();
  }  
...
void 
FrontierExplorer::failTask(int taskID, int error)
{
  if (m_Active == false) return;
  ...
  if (m_NavCtrlTaskId == taskID) {
    ...
    CureCERR(30) << "TaskID did not match for failTask "
                 << taskID << "!=" << m_NavCtrlTaskId 
                 << " we are no longer in exploration mode!!!\n";
    stopExploration();
  }
...
void 
FrontierExplorer::newGateway(NavGraphGateway &gw)
{
  if (m_Active == false) return;
...
void
FrontierExplorer::reportExplorationDone(int taskId, int status)
{
  CureCERR(30) << "Reporting exploration task " << taskId << " done\n";

  if (m_ExplorationTaskId != taskId) {
    CureCERR(20) << "\nWARNING: task ids do not match "
                 << m_ExplorationTaskId << "!="
                 << taskId << "!!!!!\n\n";
  }
