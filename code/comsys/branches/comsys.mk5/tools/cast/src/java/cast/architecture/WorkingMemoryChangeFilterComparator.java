...
public class WorkingMemoryChangeFilterComparator implements
	...
	public static final boolean allowsChange(WorkingMemoryChangeFilter _filter,
		...
		if (_filter.type.length() > 0) {
			...
			boolean allowsTypeChange = false;

			// direct comparison
			if (_filter.type.equals(_change.type)) {
				// System.out.println("allowing based on exact match: " +
				// _filter.type);
				...
				for (int i = 0; i < _change.superTypes.length - 1; ++i) {
					// if the filter matches against the supertype
					if (_filter.type.equals(_change.superTypes[i])) {
						// System.out.println("allowing based on supertypes: " +
						// _filter.type);
						allowsTypeChange = true;
						break;
					}
