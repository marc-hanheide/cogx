 ...
 * @version     $Revision: 1.56 $, $Date: 2005/11/29 21:45:12 $
 */
public class Chart
{
    ...
    // new best time limit, in ms., or as a pct from first (from prefs)
    private int newBestTimeLimit = NO_TIME_LIMIT;
    private double newBestTimeLimitPct = NO_TIME_LIMIT;
    
    // pruning value (from prefs)
    private int pruningValue = NO_PRUNING;
    
    // flag for whether to collect category combos
    private boolean collectCombos = true;

    // flag for whether to use packing
    private boolean usePacking = false;

    // flag for whether to do unpacking
    private boolean doUnpacking = true;
     ...
     * search is done.
     * In the packing case, the search always continues until done, and then 
     * unpacking is performed according to the preference setting.
     * In the anytime case, the search continues either until done, 
     ...
     * or until the new best time limit (if any) beyond the first complete realization 
     * is exceeded.
     * If the collect combos option is set, then the combinatory rules are only invoked 
     * when an edge with a new category is moved to the chart, in which case 
    ...
    public void combine(int timeLimitMS, boolean waitForCompleteEdge) {
        ...
        while (!agenda.isEmpty()) {
            ...
            // check for timeout, in anytime case
            if (!usePacking) {
                long currentTime = System.currentTimeMillis();
    ...
    // unpack complete edges
	private void doUnpacking() {
	    @SuppressWarnings("unchecked")
        Set<Edge> unpacked = new THashSet(new TObjectIdentityHashingStrategy());
        // unpack each complete edge, updating best edge 
        for (Edge edge : edges) {
    ...
    private void unpack(Edge edge, Set<Edge> unpacked) {
        ...
        List prunedEdges = pruningStrategy.pruneEdges(mergedList);
    ...
    private void unpackAlt(Edge alt, Set<Edge> unpacked, EdgeHash merged) {
        ...
        for (Sign[] combo : altCombos) {
            Sign sign = (edgeFactory.hasLfAltsOrOpts) 
                ? Sign.createDerivedSignWithNewLF(resultCat, combo, history.getRule())
                : Sign.createDerivedSign(resultCat, combo, history.getRule());
    ...
    // whose sign is equivalent (up to surface words) and which has equal 
    ...
    // if the edge replaces an (essentially) equivalent edge of higher 
    ...
    private static void addSorted(List<Edge> list, Edge edge) {
        // do binary search
        int index = Collections.binarySearch(list, edge, edgeComparator);
        // check if search found an edge with the same score
        if (index >= 0) {
            // if so, advance the index past same scoring edges
