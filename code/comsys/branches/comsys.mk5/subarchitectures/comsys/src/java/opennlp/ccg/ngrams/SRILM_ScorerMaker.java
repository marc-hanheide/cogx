...
import opennlp.ccg.realize.SignScorer;
...
public class SRILM_ScorerMaker implements ScorerMaker {
     ...
     */
    public void prepScorer(File tmpDir, int foldNum, URL trainURL, URL testURL) throws IOException {
        // write training/test files, if not already present
        writeTrainingAndTestFiles(tmpDir, foldNum, trainURL, testURL);
    ...
    protected void writeTrainingAndTestFiles(File tmpDir, int foldNum, URL trainURL, URL testURL) throws IOException {
        File trainingFile = new File(tmpDir, trainingfile(foldNum)); 
        if (!trainingFile.exists()) {
            String trainingPath = trainingFile.getCanonicalPath();
            writeTargets(trainURL, trainingPath);
        }
        File testFile = new File(tmpDir, testfile(foldNum)); 
        if (!testFile.exists()) {
            String testPath = testFile.getCanonicalPath();
            writeTargets(testURL, testPath); 
        }
    }
    ...
    protected void writeTargets(URL tbUrl, String textfile) throws IOException {
        if (!useSemClasses) 
            cvr.tester.writeTargets(tbUrl, textfile); 
        else 
            cvr.tester.writeTargetsSC(tbUrl, textfile); 
    }
     ...
     */
    public SignScorer loadScorer(File tmpDir, int foldNum, URL trainURL) throws IOException {
        String lmfile = filename(foldNum);
