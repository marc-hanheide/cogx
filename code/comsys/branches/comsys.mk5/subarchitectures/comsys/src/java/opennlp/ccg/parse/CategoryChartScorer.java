...
implements ChartScorer
{
			...
			while (signsIter.hasNext()) { 
				...
				if (catClass.indexOf("AtomCat") != -1) {
					...
					if (((AtomCat)target).getType().equals("s")) { 
						log("Looking at S target");

						signsToRemove.addElement(sign);

						/**
						if (catFS.hasAttribute("MOOD")) {
							String value = ((SimpleType)catFS.getValue("MOOD")).getName();
							if (value.startsWith("s-ind-ell") || value.startsWith("s-minor")) { 
								log("Category ["+target.toString()+"] is indicative elided");
								signsToRemove.addElement(sign);
							} else { 
								log("MOOD feature equals ["+catFS.getValue("MOOD")+"] so don't remove");
								ordering.add(sign);
							} // end 
						} else if (catFS.hasAttribute("VFORM")) { 
							String value = ((SimpleType)catFS.getValue("VFORM")).getName();
							if (value.startsWith("fin-ctxt")) { 
								log("Category ["+target.toString()+"] is finite context, to be removed");
								signsToRemove.addElement(sign);
							}							

						} else { 
							System.out.println("Target category doesn't have MOOD: "+target.toString());
							ordering.add(sign);
					...
					// ADDED BY PLISON
						...
						Sign[] signs = sign.getDerivationHistory().getInputs();
						if (signs != null) {
							for (int i = 0 ; i < signs.length ; i++) {
								Sign subsign = signs[i];
								Category subcat = subsign.getCategory();
								TargetCat subtarget = subcat.getTarget();
								FeatureStructure subcatFS = subtarget.getFeatureStructure();	
							/**	if (subcatFS.hasAttribute("MOOD")) {
									String value = ((SimpleType)subcatFS.getValue("MOOD")).getName();
	...
	public Chart score (Chart chart, int stringPos) { 
		...
		Object[] pruningResults = this.score(result.get(0,stringPos), stringPos, (stringPos == (chart._size-1))); 
