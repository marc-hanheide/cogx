 ...
 * @version     $Revision: 1.10 $, $Date: 2005/09/14 22:06:35 $
 **/
public class SatOp extends HyloFormula {
    ...
    public SatOp(Element e) {
        ...
        if (nom != null) {
            _nominal = new NominalAtom(HyloHelper.prefix(nom), HyloHelper.type(nom));
        } else {
            nom = e.getAttributeValue("nomvar");
            if (nom != null) {
                _nominal = new NominalVar(HyloHelper.prefix(nom), HyloHelper.type(nom));
            } else {
