...
// 3D height of "standard" box
static double DEFAULT_OBJ_SIZE = 0.30;
...
//Get parameters for setupFERNS from the config file
  ...
  string cfgGroup = "BlackVision";

  // Read cmd line options
  map<string,string>::const_iterator it1 = _config.find("--config");
  string configFile1;
  if (it1!= _config.end()) 
  {
    configFile1 = it1->second;
  }
  ...
  {
    if (!cf1.read(configFile1))
      throw runtime_error(exceptionMessage(__HERE__, "Unable to load configuration file '%s'!", configFile1.c_str()));
  }

  // Get configuration from the file
  try
  {
    maximum_number_of_points_on_model=cf1.getIntValue(cfgGroup, "maximum_number_of_points_on_model", 40); //400
    number_of_generated_images_to_find_stable_points=cf1.getIntValue(cfgGroup, "number_of_generated_images_to_find_stable_points", 500); //5000
    minimum_number_of_views_rate=cf1.getDoubleValue(cfgGroup, "minimum_number_of_views_rate", 0.0);
    patch_size=cf1.getIntValue(cfgGroup, "patch_size", 32);
    yape_radius=cf1.getIntValue(cfgGroup, "yape_radius", 7);
    number_of_octaves=cf1.getIntValue(cfgGroup, "number_of_octaves", 4);
    number_of_ferns=cf1.getIntValue(cfgGroup, "number_of_ferns", 30);
    number_of_tests_per_fern=cf1.getIntValue(cfgGroup, "number_of_tests_per_fern", 12);
    number_of_samples_for_refinement=cf1.getIntValue(cfgGroup, "number_of_samples_for_refinement", 1000); //10000
    number_of_samples_for_test=cf1.getIntValue(cfgGroup, "number_of_samples_for_test", 200);
  }
  catch(bad_lexical_cast &)
  {
    throw runtime_error(exceptionMessage(__HERE__, "Incorrect item value in the config file '%s'!", configFile1.c_str() ));
  }

  // Print the configuration
  log("Configuration:");
  log("-> maximum_number_of_points_on_model: %d", maximum_number_of_points_on_model);
  log("-> number_of_generated_images_to_find_stable_points: %d", number_of_generated_images_to_find_stable_points);
  log("-> number_of_samples_for_refinement: %d", number_of_samples_for_refinement);


  setupFERNS(maximum_number_of_points_on_model, number_of_generated_images_to_find_stable_points, minimum_number_of_views_rate, patch_size, yape_radius, number_of_octaves, number_of_ferns, number_of_tests_per_fern, number_of_samples_for_refinement, number_of_samples_for_test);
}
...
void ObjectDetectorFERNS::setupFERNS(int maximum_number_of_points_on_model, int number_of_generated_images_to_find_stable_points, double minimum_number_of_views_rate, int patch_size, int yape_radius, int number_of_octaves, int number_of_ferns, int number_of_tests_per_fern, int number_of_samples_for_refinement, int number_of_samples_for_test) throw(runtime_error)
{
  ...
  for(size_t i = 0; i < detectors.size(); i++)
  {
    detectors[i] = planar_pattern_detector_builder::build_with_cache(
                     ...
                     number_of_generated_images_to_find_stable_points,//5000
    ...
    if (!trackers[i]->load(trackerfn.c_str()))
    {
      ...
      trackers[i]->learn(detectors[i]->model_image,
         ...
         1000//10000 // number of training samples
         );
