...
void
CureHWUtils::convScan2dToCureNarrow(const Laser::Scan2d &sIn,
                                    Cure::LaserScan2d &sOut, double angle)
{
  if (sIn.ranges.empty()) {
  ...
  int samples  = static_cast<int>(angle / sIn.angleStep);


  double r[sIn.ranges.size()-2*samples];
  for (unsigned int i = samples; i < sIn.ranges.size()-samples; i++) r[i-samples] = sIn.ranges[i];

  sOut.import(Cure::Timestamp(sIn.time.s, sIn.time.us),
             sIn.ranges.size()-2*samples,
             r,
             sIn.angleStep,
             sIn.startAngle+sIn.angleStep*samples,
             sIn.maxRange,
             sIn.rangeRes);
}
