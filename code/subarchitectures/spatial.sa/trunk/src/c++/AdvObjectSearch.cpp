...
void AdvObjectSearch::SavePlaneMap() {
...
void AdvObjectSearch::configure(
...
void AdvObjectSearch::start() {
	...
	addChangeFilter(createLocalTypeFilter<SpatialData::PlanePoints> (
			cdl::OVERWRITE), new MemberFunctionChangeReceiver<AdvObjectSearch> (this,
			&AdvObjectSearch::newPlanePointCloud));
	...
	addChangeFilter(createLocalTypeFilter<SpatialData::NavCommand> (
			cdl::OVERWRITE), new MemberFunctionChangeReceiver<AdvObjectSearch> (this,
...
void AdvObjectSearch::newVisualObject(
...
void AdvObjectSearch::owtRecognizer3DCommand(
		const cast::cdl::WorkingMemoryChange &objID) {
	if (isWaitingForDetection) {
		try {
...
void AdvObjectSearch::owtNavCommand(const cast::cdl::WorkingMemoryChange &objID) {
...
void AdvObjectSearch::owtTiltAngleRequest(
		const cast::cdl::WorkingMemoryChange &objID) {
	try {
		log("got tilt angles");
...
void AdvObjectSearch::owtGridMapDouble(
		const cast::cdl::WorkingMemoryChange &objID) {
	//    try {
...
void AdvObjectSearch::runComponent() {
...
void AdvObjectSearch::Recognize() {
	...
	for (unsigned int i = 0; i < VPDistribution.size(); i++) {
		...
		d2 = sqrt(pow(p1.x - VPDistribution[i].x, 2) + pow(p1.y
				- VPDistribution[i].y, 2));
		tilt += atan(d1 / d2);
	}
...
void AdvObjectSearch::MovePanTilt(double pan, double tilt, double tolerance) {
	if (m_usePTZ) {
...
void AdvObjectSearch::AskForDistribution(std::vector<std::string> objectlist,
		double probSum) {
	log("asking for distribution: %s , %s", m_SearchMode.c_str(),
			m_CurrentTarget.c_str());
...
void AdvObjectSearch::ReadPlaneMap() {
	int length;
	char * buffer;
	// m_Mutex.lock();
...
void AdvObjectSearch::receiveScan2d(const Laser::Scan2d &castScan) {
...
void AdvObjectSearch::newRobotPose(const cast::cdl::WorkingMemoryChange &objID) {
...
int AdvObjectSearch::NextBestView() {
...
void AdvObjectSearch::newPlanePointCloud(
		const cast::cdl::WorkingMemoryChange &objID) {
...
void AdvObjectSearch::DisplayPDFinPB(double xoffset, double yoffset,
		std::string name) {
	...
	for (int x = -m_lgm->getSize(); x <= m_lgm->getSize(); x++) {
		for (int y = -m_lgm->getSize(); y < m_lgm->getSize(); y++) {
			...
			linecloudp.add_line(xW2 + xoffset, yW2 + yoffset, (*m_pdf)(x, y).prob
					* multiplier1, xW3 + xoffset, yW3 + yoffset, (*m_pdf)(x, y + 1).prob
					* multiplier1);
		}
	}
	...
	for (int x = -m_lgm->getSize(); x < m_lgm->getSize(); x++) {
		for (int y = -m_lgm->getSize(); y <= m_lgm->getSize(); y++) {
			...
			linecloudp.add_line(xW2 + xoffset, yW2 + yoffset, (*m_pdf)(x, y).prob
					* multiplier1, xW3 + xoffset, yW3 + yoffset, (*m_pdf)(x + 1, y).prob
					* multiplier1);
		}
	}
	/* Display pdfIn in as line cloud PB END */
...
double AdvObjectSearch::ActionProbabilityPerCell(int x, int y,
...
void AdvObjectSearch::MeasurementUpdate(bool result) {
	...
	/* DEBUG */
	log("current vp index %d , cones size %d", m_CurrentViewPointIndex,
			m_VCones.size());
	double denomsum = 0.0;
	for (int x = -m_lgm->getSize(); x <= m_lgm->getSize(); x++) {
		for (int y = -m_lgm->getSize(); y <= m_lgm->getSize(); y++) {
...
void AdvObjectSearch::PlaneObservationUpdate(
		std::set<std::pair<int, int> > NewPlanePoints) {
...
bool isCircleFree(const Cure::LocalGridMap<unsigned int> &map, double xW,
...
void AdvObjectSearch::SampleGrid() {
	srand ( time(NULL));
	...
	for (int x = -m_lgm->getSize(); x <= m_lgm->getSize(); x++) {
		for (int y = -m_lgm->getSize(); y <= m_lgm->getSize(); y++) {
			if ((*m_lgm)(x, y) == 1 || (*m_lgm)(x, y) == 3) { // occupied OR Plane
				m_NonFreeSpace.setBit(x + m_lgm->getSize(), y + m_lgm->getSize(), true);
			}
		}
	}
	...
	while (i < m_samplesize) {
		...
		if (!haspoint && (*m_lgm)(randx, randy) == 0 && isCircleFree(*m_lgm, xW,
				yW, 0.4)) {
			/*if reachable*/
			...
			if (m_lgm->worldCoords2Index(lastRobotPose->x, lastRobotPose->y, rS, cS)
				...
				double d =
						(m_PathGrid.path(rS, cS, rE, cE, path, 20 * m_lgm->getSize())
								* m_lgm->getCellSize());
...
std::vector<std::vector<int> > AdvObjectSearch::GetViewCones() {
...
std::vector<int> AdvObjectSearch::GetInsideViewCone(XVector3D &a, bool addall) {
	std::vector<int> tpoints;
	XVector3D b, c, p;
...
void AdvObjectSearch::CalculateViewCone(XVector3D a, double direction,
...
void AdvObjectSearch::FindBoundingRectangle(XVector3D a, XVector3D b,
...
bool AdvObjectSearch::isPointInsideTriangle(XVector3D p, XVector3D a,
...
bool AdvObjectSearch::isPointSameSide(XVector3D p1, XVector3D p2, XVector3D a,
		XVector3D b) {
...
void AdvObjectSearch::PostNavCommand(Cure::Pose3D position,
...
void AdvObjectSearch::addRecognizer3DCommand(
