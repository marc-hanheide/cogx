...
LocalMapManager::LocalMapManager() :
	m_planeProcessingCooldown(true) {
	m_placeInterface = 0;
...
LocalMapManager::~LocalMapManager() {
	...
	delete m_Glrt1;
	delete m_Glrt2;
	for (std::vector<CharMap *>::iterator it = m_planeObstacleMaps.begin(); it
			!= m_planeObstacleMaps.end(); it++) {
		delete *it;
	}

	for (map<int, CharMap *>::iterator it = m_nodeGridMaps.begin(); it
			!= m_nodeGridMaps.end(); it++) {
		delete it->second;
	}
	delete m_lgm2;
	...
	if (m_isUsingSeparateGridMaps) {
		delete m_Glrt1_alt;
		delete m_Glrt2_alt;
		for (map<int, CharMap *>::iterator it = m_nodeGridMapsAlt.begin(); it
				!= m_nodeGridMapsAlt.end(); it++) {
			delete it->second;
		}
		delete m_lgm2_alt;
	}
	...
	if (!m_bNoPlanes) {
		for (vector<GridObjectFinder*>::iterator it = m_planeObjectFinders.begin(); it
				!= m_planeObjectFinders.end(); it++) {
			delete *it;
		}
	}
}
...
void LocalMapManager::configure(const map<string, string>& _config) {
	...
	m_isUsingSeparateGridMaps = m_MaxLaserRangeForPlaceholders
			!= m_MaxLaserRangeForCombinedMaps;
	...
	if (!m_bNoPlanes) {
		...
		for (unsigned int i = 0; i < m_maxNumberOfClusters; i++) {
			m_planeObstacleMaps.push_back(new CharMap(MapSize, CellSize, 0,
					CharMap::MAP1));
		}
	...
	FrontierInterface::HypothesisEvaluatorPtr servant =
			new EvaluationServer(this);
	registerIceServer<FrontierInterface::HypothesisEvaluator,
			FrontierInterface::HypothesisEvaluator> (servant);

	FrontierInterface::LocalMapInterfacePtr mapservant = new LocalMapServer(this);
	registerIceServer<FrontierInterface::LocalMapInterface,
			FrontierInterface::LocalMapInterface> (mapservant);
...
void LocalMapManager::start() {
	...
	if (!m_bNoPlaces) {
		m_placeInterface = getIceServer<FrontierInterface::PlaceInterface> (
				"place.manager");
...
void LocalMapManager::runComponent() {
	...
	log("the number of cells from the center to the edge of the LGM is: %u",
			m_lgm2->getSize());
		...
		if (curNode != 0) {
			if (curNode->nodeId != prevNode) { /* The robot is in some new node! */
				...
				if (m_nodeGridMaps.find(curNode->nodeId) == m_nodeGridMaps.end()) { /* search on nodeId */
					...
					m_lgm2 = new CharMap(m_mapsize, m_cellsize, '2', CharMap::MAP1,
							curNode->x, curNode->y);
					...
					if (m_isUsingSeparateGridMaps) {
						...
						m_lgm2_alt = new CharMap(m_mapsize, m_cellsize, '2', CharMap::MAP1,
								curNode->x, curNode->y);
					}
				...
				if (m_Displaylgm1) {
					m_Displaylgm1
							= new Cure::XDisplayLocalGridMap<unsigned char>(*m_lgm1);
...
void LocalMapManager::LoadNodeGridMaps(std::string filename) {
	...
	while (getline(file, line)) {
		...
		getline(file, line);
		int count = 0;
		CharMap* newMap = new CharMap(m_mapsize, m_cellsize, '2', CharMap::MAP1,
				cx, cy);
...
void LocalMapManager::newRobotPose(const cdl::WorkingMemoryChange &objID) {
	...
	double deltaT = lastRobotPose->time.s - oldTime.s + (lastRobotPose->time.us
			- oldTime.us) * 0.000001;
...
void LocalMapManager::receiveScan2d(const Laser::Scan2d &castScan) // <--- Laser::Scan2d @ Laser.ice
{
	...
	if (m_TOPP.isTransformDefined()) {
		...
		if (m_TOPP.getPoseAtTime(cureScan.getTime(), scanPose) == 0) {
			...
			m_lgm1->setValueInsideCircle(m_SlamRobotPose.getX(),
					m_SlamRobotPose.getY(), 0.55 * 0.40, '0');
			m_lgm2->setValueInsideCircle(m_SlamRobotPose.getX(),
					m_SlamRobotPose.getY(), 0.55 * 0.45, '0');
			if (m_isUsingSeparateGridMaps) {
			...
			if (m_bDetectDoors) {
				...
				list<HSS::RedundantLine2DRep> detDoors = m_doorExtractor.doors();
				...
				for (list<HSS::RedundantLine2DRep>::iterator it = detDoors.begin(); it
						!= detDoors.end(); it++) {
					bool found = false;
					double doorX = odomNew[0] + cos(odomNew[2]) * it->xC() - sin(
							odomNew[2]) * it->yC();
					double doorY = odomNew[1] + sin(odomNew[2]) * it->xC() + cos(
							odomNew[2]) * it->yC();
					...
					for (map<string, FrontierInterface::DoorHypothesisPtr>::iterator it2 =
						...
						if (dist < 1.0) {
							it2->second->x = (it2->second->x * m_detectedDoorsNum[it2->first]
									+ doorX) / (m_detectedDoorsNum[it2->first] + 1);
							it2->second->y = (it2->second->y * m_detectedDoorsNum[it2->first]
									+ doorY) / (m_detectedDoorsNum[it2->first] + 1);
							it2->second->width = (it2->second->width
									* m_detectedDoorsNum[it2->first] + doorWidth)
									/ (m_detectedDoorsNum[it2->first] + 1);
							it2->second->theta = (it2->second->theta
									* m_detectedDoorsNum[it2->first] + doorTheta)
									/ (m_detectedDoorsNum[it2->first] + 1);
							if (it2->second->theta < -M_PI)
								it2->second->theta += 2 * M_PI;
							if (it2->second->theta > M_PI)
								it2->second->theta -= 2 * M_PI;

							m_detectedDoorsNum[it2->first] = m_detectedDoorsNum[it2->first]
									+ 1;
							if (m_bShowDoorsInPB) {
								HSS::displayDoorMeas(m_HSSGroupProxy, odomNew, xsR,
										m_doorExtractor, it2->first);
							}
...
NavData::FNodePtr LocalMapManager::getCurrentNavNode() {
...
FrontierInterface::HypothesisEvaluation LocalMapManager::getHypothesisEvaluation(
		int hypID) {
	...
	double relevantX;
	double relevantY;
	for (vector<SpatialData::NodeHypothesisPtr>::iterator it = hyps.begin(); it
			!= hyps.end(); it++) {
		try {
			if ((*it)->hypID == hypID) {
	...
	if (originPlaceID != -1) {
		...
		CharMap *propertyLGM = m_isUsingSeparateGridMaps ? m_lgm1_alt : m_lgm1;
		...
		for (int yi = -propertyLGM->getSize(); yi < propertyLGM->getSize() + 1; yi++) {
			...
			for (int xi = -propertyLGM->getSize(); xi < propertyLGM->getSize() + 1; xi++) {
				...
				if ((*propertyLGM)(xi, yi) == '0') {
					totalFreeCells++;
					//Cell is free
					double relevantDistSq = (x - relevantX) * (x - relevantX) + (y
							- relevantY) * (y - relevantY);
					// Check if some other hypothesis from this Place is closer to this
					// grid cell

					bool process = true;
					for (vector<SpatialData::NodeHypothesisPtr>::iterator it =
							relevantHypotheses.begin(); process && it
							!= relevantHypotheses.end(); it++) {
						if ((*it)->hypID != hypID) {
							double distSq = (x - (*it)->x) * (x - (*it)->x) + (y - (*it)->y)
									* (y - (*it)->y);
					...
					if (process) {
						...
						// other node
						for (vector<NavData::FNodePtr>::iterator it2 = nodes.begin(); it2
								!= nodes.end(); it2++) {
							try {
								double distSq = (x - (*it2)->x) * (x - (*it2)->x) + (y
										- (*it2)->y) * (y - (*it2)->y);
								if (distSq < 2.0 * 2.0)
									process = false;
						...
						if (process) {
							...
							for (int yi2 = yi - 1; yi2 <= yi + 1; yi2++) {
								for (int xi2 = xi - 1; xi2 <= xi + 1; xi2++) {
									if (xi2 < -propertyLGM->getSize() || xi2
											> propertyLGM->getSize() || yi2 < -propertyLGM->getSize()
		...
		log("Total free cells: %i; total processed %i", totalFreeCells,
				totalProcessed);
...
FrontierInterface::HypothesisEvaluation LocalMapManager::EvaluationServer::getHypothesisEvaluation(
...
SpatialData::LocalGridMap LocalMapManager::LocalMapServer::getCombinedGridMap(
	...
	if (m_pOwner->m_placeInterface) {
		for (SpatialData::PlaceIDSeq::const_iterator it = places.begin(); it
				!= places.end(); it++) {

			//NOTE: May block
			NavData::FNodePtr node = m_pOwner->m_placeInterface->getNodeFromPlaceID(
					*it);
			if (node != 0) {
...
void LocalMapManager::getCombinedGridMap(SpatialData::LocalGridMap &map,
		const vector<NavData::FNodePtr> &nodes) {
	vector<const CharMap *> maps;

	for (vector<NavData::FNodePtr>::const_iterator it = nodes.begin(); it
			!= nodes.end(); it++) {
	...
	for (vector<const CharMap *>::iterator it = maps.begin(); it != maps.end(); it++) {
		double mapminx = (*it)->getCentXW() - (*it)->getSize()
				* (*it)->getCellSize();
		double mapmaxx = (*it)->getCentXW() + (*it)->getSize()
				* (*it)->getCellSize();
		double mapminy = (*it)->getCentYW() - (*it)->getSize()
				* (*it)->getCellSize();
		double mapmaxy = (*it)->getCentYW() + (*it)->getSize()
				* (*it)->getCellSize();
	...
	CharMap newMap(newSize, cellSize, '2', CharMap::MAP1, cx, cy);
	...
	SpatialData::MapInterfacePrx mapPrx(getIceServer<SpatialData::MapInterface> (
			"spatial.control"));
	SpatialData::LocalGridMap obstacleMap = mapPrx->getBoundedMap(minx, maxx,
			miny, maxy);

	Cure::LocalGridMap<unsigned char> cureObstacleMap(obstacleMap.size,
			obstacleMap.cellSize, '2', Cure::LocalGridMap<unsigned char>::MAP1,
			obstacleMap.xCenter, obstacleMap.yCenter);
	...
	CharMap newMap2(newSize, cellSize, '2', CharMap::MAP1, cx, cy);
