...
double RelationEvaluator::evaluateInness(const Object *objectC,
		const Object *objectO) {
	if (objectC->type == OBJECT_BOX || objectC->type == OBJECT_PLANE
			|| objectC->type == OBJECT_SPHERE || objectC->type == OBJECT_CYLINDER) {
		return 0;
	}
	...
	if (objectO->type == OBJECT_PLANE) {
		...
		if (plane->shape == PLANE_OBJECT_RECTANGLE) {
			...
			for (int i = 0; i < N - 1; i++) {
				polyO.vertices.push_back(vector3(radius * cos(i * step), radius * sin(i
						* step), 0));
			...
			entireVolume = planeThickness * N * radius * cos(M_PI / N)
					* sin(M_PI / N);
		}
	...
	if (objectC->type == OBJECT_HOLLOW_BOX) {
		...
		if (objectO->type == OBJECT_BOX || objectO->type == OBJECT_HOLLOW_BOX) {
			...
			return containedVolume / entireVolume * exp(-penetrationPenalty
					/ distanceFalloffInside * 0.3010129996);
		} else {
