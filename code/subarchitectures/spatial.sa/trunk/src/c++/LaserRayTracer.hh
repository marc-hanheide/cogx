...
public:
  ...
  void addScanStationarySensor(const Laser::Scan2d &castScan,
      vector<double> LaserPose, GDIsObstacle &checkobstacle,
      GDMakeFree &makeempty, GDMakeObstacle &makeobstacle);
private:
...
template<class MapData> int LaserRayTracer<MapData>::isOutside() const {
  if ((m_Xi < 0 || (mapSize.first < m_Xi) || (m_Yi < 0 || (mapSize.second
      < m_Yi))))
    return true;
  else
    return false;
...
template<class MapData> double LaserRayTracer<MapData>::getDistToStart() const {
...
template<class MapData> int LaserRayTracer<MapData>::stepRay() {
  //printf("stepping ray \n");
  return (this->*drawLineFcn)();
}

template<class MapData> int LaserRayTracer<MapData>::drawLineXp() {
  m_Xi++;
  m_Xf++;
  m_Yf += secondaryStep;
  ...
  if ((m_Xi < -mapSize.first) || (mapSize.first < m_Xi) || (m_Yi
      < -mapSize.second) || (mapSize.second < m_Yi))
    return true;
  else
    return false;
...
template<class MapData> int LaserRayTracer<MapData>::drawLineXn() {
  m_Xi--;
  m_Xf--;
  m_Yf -= secondaryStep;
  m_Yi = (0 < m_Yf ? int(m_Yf + 0.5) : int(m_Yf - 0.5));
...
template<class MapData> int LaserRayTracer<MapData>::drawLineYp() {
  m_Yi++;
  m_Yf++;
  m_Xf += secondaryStep;
...
template<class MapData> int LaserRayTracer<MapData>::drawLineYn() {
  m_Yi--;
  m_Yf--;
  m_Xf -= secondaryStep;
  m_Xi = (0 < m_Xf ? int(m_Xf + 0.5) : int(m_Xf - 0.5));
...
void LaserRayTracer<MapData>::addScanStationarySensor(
    const Laser::Scan2d &castScan, vector<double> LaserPose,
    GDIsObstacle &checkobstacle, GDMakeFree &makeempty,
    GDMakeObstacle &makeobstacle) {
  if (LaserPose.size() != 4) {
    printf(
        "Not adding scan, laser pose is incomplete! (should be [x,y,z,theta] \n");
    return;
  }
  ...
  for (unsigned int i = 0; i < castScan.ranges.size(); i++) {
    ...
    setRayStart(LaserPose[0], LaserPose[1], LaserPose[3] + castScan.startAngle
        + castScan.angleStep * i);
    while (!isOutside() && getDistToStart() < castScan.ranges[i]
      ...
      for (unsigned int i = 0; i < column.size(); i++) {
        ...
        if (LaserPose[2] < column[i].celing) {
          ...
          if (checkobstacle(column[i].data)) {
            ...
            m_map->boxSubColumnModifier(m_Xi, m_Yi, LaserPose[2],
                m_map->getMinBloxelHeight(), makeempty);
          }
          break;
        }
      }
      ...
      for (unsigned int i = 0; i < colsize; i++) {
        ...
        if (LaserPose[2] < (*m_map)(m_Xi, m_Yi)[i].celing) {
          ...
          if (!checkobstacle((*m_map)(m_Xi, m_Yi)[i].data)) {
            ...
            //if this is an obstacle carve an empty box in it of minbloxel size
            m_map->boxSubColumnModifier(m_Xi, m_Yi, LaserPose[2],
                m_map->getMinBloxelHeight(), makeobstacle);
          }
        }
        break;
