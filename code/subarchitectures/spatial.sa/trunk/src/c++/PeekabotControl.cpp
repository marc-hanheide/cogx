...
void PeekabotControl::runComponent() {
  ...
  actionZone.add(controlmodule, "action-zone", peekabot::REPLACE_ON_CONFLICT);
  ...
  searchhere.add(controlmodule, "search-here", peekabot::REPLACE_ON_CONFLICT);
  ...
  startspot.add(controlmodule, "start-spot ", peekabot::REPLACE_ON_CONFLICT);
  ...
  if (m_PeekabotClient.is_connected()) {
    ...
    while (isRunning()) {
        ...
        if (has1 and has2 and has3) {
          ...
          for (unsigned int h = 0; h < fnodeseq.size(); h++) {
            ...
            if (hypot(fnodeseq[h]->y - r.get_result().m_c[1], fnodeseq[h]->x
              ...
              SpatialData::PlacePtr place = agg->getPlaceFromNodeID(
                  fnodeseq[h]->nodeId);
      ...
      if (r.succeeded()) {
        ...
        if (hypot(yNoA - r.get_result().m_c[1], xNoA - r.get_result().m_c[0])
            < radius) {
