...
void ConnectivityWriter::start() {
	...
	addChangeFilter(
			createLocalTypeFilter<SpatialProperties::GatewayPlaceProperty> (
					cdl::WILDCARD),
			new MemberFunctionChangeReceiver<ConnectivityWriter> (this,
					&ConnectivityWriter::changedGateway));
	m_marshaller = Marshalling::MarshallerPrx(getIceServer<
			Marshalling::Marshaller> ("proxy.marshaller"));
}

void ConnectivityWriter::stop() {
}

void ConnectivityWriter::runComponent() {
}

void ConnectivityWriter::configure(
		const std::map<std::string, std::string>& _config) {
}

void ConnectivityWriter::newConnectivity(const cdl::WorkingMemoryChange &wmc) {
	SpatialProperties::ConnectivityPathPropertyPtr prop = getMemoryEntry<
			SpatialProperties::ConnectivityPathProperty> (wmc.address);
	if (prop != 0) {
		...
		log("Adding connectivity relation between Place %s and %s", uid.c_str(),
				uid2.c_str());
		while (!m_marshaller->addRelation("connectivity", relationUID, "place",
				uid, "place", uid2, 1.0, origin)) {
		...
		feature->alternativeValues.push_back(
				new binder::autogen::featvalues::BooleanValue(1, getCASTTime(), true));
...
void ConnectivityWriter::changedGateway(const cdl::WorkingMemoryChange &wmc) {
	SpatialProperties::GatewayPlacePropertyPtr prop = getMemoryEntry<
			SpatialProperties::GatewayPlaceProperty> (wmc.address);
	if (prop != 0) {
		...
		if (wmc.operation == cdl::ADD) {
			...
			feature->featlabel = "gateway";
			feature->alternativeValues.push_back(
					new binder::autogen::featvalues::StringValue(1, getCASTTime(),
							"gateway"));
			m_marshaller->addFeature(type, uid, feature);
			m_marshaller->commitFeatures(type, uid);
