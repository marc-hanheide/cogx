...
GridObjectFinder::GridObjectFinder(IplImage *objectGrid, int objectXCenter,
		int objectYCenter, ObjectFinderSymmetry symmetry, double angStep) :
	m_angStep(angStep) {
	double maxAngle;
	...
	int objRadius = (int) ceil(sqrt(2) * max(max(objectGrid->height
			- objectYCenter, objectYCenter - 1), max(objectGrid->width
			- objectXCenter, objectXCenter - 1)));
...
GridObjectFinder::~GridObjectFinder() {
	for (std::vector<IplImage *>::iterator it = m_aspectImages.begin(); it
			!= m_aspectImages.end(); it++) {
		cvReleaseImage(&(*it));
	}
}
...
createTableFinder(double width, double height, double cellSize) {
	...
	GridObjectFinder *newFinder = new GridObjectFinder(objIm, size.width / 2 + 1,
			size.height / 2 + 1, HALF_SYMMETRY);
	cvReleaseImage(&objIm);
	return newFinder;
}

void GridObjectFinder::findObject(IplImage *image, int *outX, int *outY,
...
void GridObjectFinder::findObject(Cure::LocalGridMap<unsigned char> &lgm,
		int *outX, int *outY, double *outAngle, double *outConfidence) {
	...
	for (int x = -lgmSize; x <= lgmSize; x++) {
		for (int y = -lgmSize; y <= lgmSize; y++) {
			cvSet2D(lgmImage, x + lgmSize, y + lgmSize, CV_RGB(lgm[lp], lgm[lp],
					lgm[lp]));
			lp++;
		}
	}
