...
PlaceMapper::PlaceMapper() :
  m_placeIDCounter(0), m_hypIDCounter(0) {
}

PlaceManager::PlaceManager() :
  PlaceMapper(), m_robotInitialPoseReceived(false), m_initialMovementThreshold(
      0.05), m_isPathFollowing(false), m_startNodeForCurrentPath(-1),
      m_goalPlaceForCurrentPath(-1), m_currentNodeOnPath(-1),
      m_firstMovementRegistered(false), m_foundNodeThisMovement(false) {
 ...
 */
void PlaceManager::configure(const std::map<std::string, std::string>& _config) {
  ...
  registerIceServer<FrontierInterface::PlaceInterface,
      FrontierInterface::PlaceInterface> (servant);
}

/*
 * Visualizes forbidden zone and creates the working memory listeners
 */
void PlaceManager::start() {
  if (m_usePeekabot) {
    while (!m_PeekabotClient.is_connected() && (m_RetryDelay > -1)) {
      sleepComponent( m_RetryDelay);
      connectPeekabot();
    }
    m_ProxyForbiddenMap.add(m_PeekabotClient, "pm_forbidden",
  ...
  addChangeFilter(createLocalTypeFilter<NavData::EndPlaceTransitionCommand> (
      cdl::ADD), new MemberFunctionChangeReceiver<PlaceManager> (this,
  ...
  addChangeFilter(createLocalTypeFilter<NavData::ObjData> (cdl::OVERWRITE),
      new MemberFunctionChangeReceiver<PlaceManager> (this,
          &PlaceManager::newObject));
  addChangeFilter(createLocalTypeFilter<FrontierInterface::DoorHypothesis> (
      cdl::ADD), new MemberFunctionChangeReceiver<PlaceManager> (this,
  ...
  m_mapInterface = MapInterfacePrx(getIceServer<MapInterface> (
      "spatial.control"));
  if (m_useLocalMaps) {
    hypothesisEvaluator = FrontierInterface::HypothesisEvaluatorPrx(
        getIceServer<FrontierInterface::HypothesisEvaluator> ("map.manager"));
  } else {
...
void PlaceMapper::SavePlaces() {
  ...
  for (vector<PlaceMapEntry>::iterator it = entries.begin(); it
      != entries.end(); it++) {
...
void PlaceManager::runComponent() {
 ...
 */
void PlaceManager::newNavNode(const cast::cdl::WorkingMemoryChange &objID) {
  try {
    NavData::FNodePtr oobj = getMemoryEntry<NavData::FNode> (objID.address);
 ...
 */
void PlaceManager::newEndPlaceTransitionCommand(
  ...
  try {
    NavData::EndPlaceTransitionCommandPtr obj = getMemoryEntry<
        NavData::EndPlaceTransitionCommand> (objID.address);
    ...
    overwriteWorkingMemory<NavData::EndPlaceTransitionCommand> (objID.address,
        obj);
  } catch (DoesNotExistOnWMException) {
...
void PlaceManager::modifiedNavNode(const cast::cdl::WorkingMemoryChange &objID) {
  ...
  try {
    ...
    if (oobj != 0) {
      ...
      if (prev == 0) {
        ...
        log(
            "Did not find the node from before, have to assume that we did not start early enough to catch it, will treat it as new");
        newNavNode(objID);
...
void PlaceManager::deletedNavNode(const cast::cdl::WorkingMemoryChange &objID) {
  ...
  if (oobj != 0) {
    ...
    PlaceID correspondingPlaceID = _getPlaceIDForNode(oobj->getData()->nodeId);
...
void PlaceManager::checkUnassignedEdges(int newNodeID) {
  for (set<NavData::AEdgePtr>::iterator it = m_unprocessedEdges.begin(); it
      != m_unprocessedEdges.end(); it++) {
    try {
      if ((*it)->startNodeId == newNodeID || (*it)->endNodeId == newNodeID) {
        log("Processing deferred edge between nodes %i and %i",
            (*it)->startNodeId, (*it)->endNodeId);
 ...
 */
void PlaceManager::processEdge(NavData::AEdgePtr oobj) {
  try {
    ...
    if (startPlace >= 0 && endPlace >= 0) {
      ...
      wrn("Warning: New edge was detected between %i and %i, but connected Places were missing!",
          startPlace, endPlace);
 ...
 */
void PlaceManager::newEdge(const cast::cdl::WorkingMemoryChange &objID) {
  log("newEdge called (%s)", objID.address.id.c_str());
  try {
    NavData::AEdgePtr oobj = getMemoryEntry<NavData::AEdge> (objID.address);
...
void PlaceManager::modifiedEdge(const cast::cdl::WorkingMemoryChange &objID) {
...
void PlaceManager::newObject(const cast::cdl::WorkingMemoryChange &objID) {
 ...
 */
void PlaceManager::newDoorHypothesis(
    const cast::cdl::WorkingMemoryChange &objID) {
  log("newDoorHypothesis called  (%s)", objID.address.id.c_str());
  try {
    FrontierInterface::DoorHypothesisPtr doorHyp = getMemoryEntry<
        FrontierInterface::DoorHypothesis> (objID.address);
    ...
    if (doorHyp != 0) {
      ...
      for (vector<NodeHypothesisPtr>::iterator it = hyps.begin(); it
          != hyps.end(); it++) {
...
void PlaceManager::updateReachablePlaceholderProperties(int placeID) {
  ...
  for (vector<NodeHypothesisPtr>::iterator hypIt = hypotheses.begin(); hypIt
      != hypotheses.end(); hypIt++) {
    try {
      if ((*hypIt)->originPlaceID == placeID) {
        ...
        if (placeholderID >= 0) {
...

          {
            ...
            SpatialProperties::ValueProbabilityPair pair = { freespacevalue,
                1.0 };
            SpatialProperties::ValueProbabilityPairs pairs;
            ...
            map<int, string>::iterator foundFSIt = m_freeSpaceProperties.find(
                placeholderID);
          ...
          /* Frontier length property */
          {
            ...
            SpatialProperties::ValueProbabilityPair pair = { bordervalue, 1.0 };
            SpatialProperties::ValueProbabilityPairs pairs;
            pairs.push_back(pair);
            SpatialProperties::DiscreteProbabilityDistributionPtr discDistr =
                new SpatialProperties::DiscreteProbabilityDistribution;
            ...
            map<int, string>::iterator foundUnexpIt = m_borderProperties.find(
                placeholderID);
...
void PlaceManager::evaluateUnexploredPaths() {
  ...
  if (!m_bNoPlaceholders) {
        ...
        try {
          ...
          for (vector<NodeHypothesisPtr>::iterator extantHypIt =
            ...
            if (extantHyp->hypID == nodeHyp->hypID) {
              ...
              for (vector<ForbiddenZone>::iterator fbIt =
                  m_forbiddenZones.begin(); fbIt != m_forbiddenZones.end(); fbIt++) {
                //        log("checking forbidden zone: %.02g, %.02g, %.02g, %.02g,",
                //            fbIt->minX, fbIt->minY, fbIt->maxX, fbIt->maxY);
                //        log("checking against: %.02g, %.02g", extantHyp->x,
                //            extantHyp->y);
              ...
              if (extantHyp->gateway) {
                int closestNodeId = m_mapInterface->findClosestNode(
                    extantHyp->x, extantHyp->y);
                int pid = _getPlaceIDForNode(closestNodeId);
                if (pid != -1) {
                  set<int> curPlaceConnectivities = m_connectivities[pid];
                  for (set<int>::iterator it1 = curPlaceConnectivities.begin(); it1
                      != curPlaceConnectivities.end(); it1++) {
                    set<int> placeConnectivities = m_connectivities[(*it1)];
                    set<int>::iterator it2 = placeConnectivities.find(
                        _getPlaceIDForNode(extantHyp->originNodeID));
              ...
              if (exists) {
                log("move overlapped placeholder %d", _getPlaceIDForHyp(
                    nodeHyp->hypID));
                nodeHyp->x = extantHyp->x;
                nodeHyp->y = extantHyp->y;
                if (extantHyp->originPlaceID == -1)
                  extantHyp->originPlaceID = _getPlaceIDForNode(
                      extantHyp->originNodeID);
                //Check if there's a place on this side of the door
                NavData::FNodePtr originNode = _getNodeForPlace(
                    extantHyp->originPlaceID);
                if (originNode->gateway) {
                  set<int> placeConnectivities =
                      m_connectivities[extantHyp->originPlaceID];

                  double minDist = 100;
                  int minDistPlaceID = -1;

                  for (set<int>::iterator it1 = placeConnectivities.begin(); it1
                      != placeConnectivities.end(); it1++) {
                    NavData::FNodePtr testNode = _getNodeForPlace((*it1));
                    if (testNode != 0) {
                      bool side1 = (extantHyp->y < originNode->y + tan(
                          originNode->theta) * (extantHyp->x - originNode->x));
                      bool side2 = (testNode->y < originNode->y + tan(
                          originNode->theta) * (testNode->x - originNode->x));
                      if (side1 == side2) {
                        // Both node are on the same side - connect nodes
                        double dist = (extantHyp->y - testNode->y)
                            * (extantHyp->y - testNode->y) + (extantHyp->x
                            - testNode->x) * (extantHyp->x - testNode->x);
                        if (dist < minDist) {
                          minDist = dist;
                          minDistPlaceID = (*it1);
                        }
                ...
                if (nodeHyp->originPlaceID != extantHyp->originPlaceID) {
                  ...
                  log("Deleting connectivity between %i and %i",
                      nodeHyp->originPlaceID, place->id);
                  deleteConnectivityProperty(nodeHyp->originPlaceID, place->id);
                  for (list<std::pair<int, int> >::iterator hit =
                      m_hypotheticalConnectivities.begin(); hit
                      != m_hypotheticalConnectivities.end(); hit++) {
                    if (hit->first == nodeHyp->originPlaceID && hit->second
                        == place->id) {
                  ...
                  log("Adding connectivity between %i and %i",
    ...
    for (vector<NodeHypothesisPtr>::iterator extantHypIt = hypotheses.begin(); extantHypIt
        != hypotheses.end(); extantHypIt++) {
      NodeHypothesisPtr newHyp = *extantHypIt;
      if (newHyp->hypID == -1) {
        log("create new placeholder");
        //        newHyp->hypID = m_hypIDCounter;
        if (newHyp->originPlaceID == -1)
          newHyp->originPlaceID = _getPlaceIDForNode(newHyp->originNodeID);
        //Check if there's a place on this side of the door
        NavData::FNodePtr originNode = _getNodeForPlace(newHyp->originPlaceID);
        if (originNode->gateway) {
          set<int> placeConnectivities =
              m_connectivities[newHyp->originPlaceID];
          double minDist = 100;
          int minDistPlaceID = -1;
          for (set<int>::iterator it1 = placeConnectivities.begin(); it1
              != placeConnectivities.end(); it1++) {
            NavData::FNodePtr testNode = _getNodeForPlace((*it1));
            if (testNode != 0) {
              bool side1 = (newHyp->y < originNode->y + tan(originNode->theta)
                  * (newHyp->x - originNode->x));
              bool side2 = (testNode->y < originNode->y
                  + tan(originNode->theta) * (testNode->x - originNode->x));
              if (side1 == side2) {
                // Both node are on the same side - connect nodes

                double dist = (newHyp->y - testNode->y) * (newHyp->y
                    - testNode->y) + (newHyp->x - testNode->x) * (newHyp->x
                    - testNode->x);
                if (dist < minDist) {
                  minDist = dist;
                  minDistPlaceID = (*it1);
        ...
        for (vector<ForbiddenZone>::iterator fbIt = m_forbiddenZones.begin(); fbIt
          ...
          if (newHyp->x <= fbIt->maxX && newHyp->x >= fbIt->minX && newHyp->y
              <= fbIt->maxY && newHyp->y >= fbIt->minY) {
            log("Placeholder in forbidden zone excluded");
            excluded = true;
            break;
          }
        ...
        for (vector<pair<int, NodeHypothesisPtr> >::iterator rejectedHypIt =
            m_rejectedHypotheses.begin(); rejectedHypIt
            != m_rejectedHypotheses.end(); rejectedHypIt++) {
          if ((*rejectedHypIt).first >= maxTimesRejected) {
            double distanceSq = ((*rejectedHypIt).second->x - newHyp->x)
                * ((*rejectedHypIt).second->x - newHyp->x)
                + ((*rejectedHypIt).second->y - newHyp->y)
                    * ((*rejectedHypIt).second->y - newHyp->y);
            if (distanceSq < minDistanceSq) {
              excluded = true;
              break;
        ...
        if (!excluded) {
          ...
          log("Added new hypothesis at (%f, %f) with ID %i and PlaceID %d",
              newHyp->x, newHyp->y, newHyp->hypID, newPlaceID);
  ...
  if (curPlaceID >= 0) {
    ...
    while (swapped) {
      ...
      for (size_t k = 0; k < gr.size() - j; k++) {
        double dist1 = (gr[k].second->x - curNode->x) * (gr[k].second->x
            - curNode->x) + (gr[k].second->y - curNode->y) * (gr[k].second->y
            - curNode->y);
        double dist2 = (gr[k + 1].second->x - curNode->x)
            * (gr[k + 1].second->x - curNode->x) + (gr[k + 1].second->y
            - curNode->y) * (gr[k + 1].second->y - curNode->y);
        if (dist2 < dist1) {
          tmp = gr[k];
          gr[k] = gr[k + 1];
          gr[k + 1] = tmp;
    ...
    for (vector<pair<PlaceID, NavData::FNodePtr> >::iterator it = gr.begin(); it
        != gr.end(); it++) {
      if ((it->second->nodeId != curNode->nodeId)
          && (curPlaceConnectivities.find(it->first)
        ...
        for (set<int>::iterator it1 = curPlaceConnectivities.begin(); it1
          ...
          if (link != 0) {
            ...
            for (set<int>::iterator it3 = placeConnectivities.begin(); it3
                != placeConnectivities.end(); it3++) {
        ...
        if ((max_dist == 1.5) && ((curNode->gateway == 1)
            || (it->second->gateway == 1))) {
          continue;
        }
        ...
        if (dist2 < max_dist * max_dist) {
          ...
          if (path_dist / sqrt(dist2) > 0 && path_dist / sqrt(dist2) < 22) {
            ...
            if ((it->second->gateway == 0) && (curNode->gateway == 0)) {
              for (vector<FrontierInterface::DoorHypothesisPtr>::iterator
                  itDoor = doorHyps.begin(); itDoor != doorHyps.end(); itDoor++) {
                ...
                if (((n2x * ny - n2y * nx) != 0)
                    && ((n2y * nx - n2x * ny) != 0)) {
                  double s = (ny * x1 - nx * y1 - dx * ny + dy * nx) / (n2x
                      * ny - n2y * nx);
                  double t = (dx * n2y - dy * n2x - x1 * n2y + y1 * n2x) / (n2y
                      * nx - n2x * ny);
                  //                  log("dx %f dy %f theta %f width %f s %f t %f", dx, dy, theta,
                  //                      width, s, t);
...
void PlaceManager::setOrUpgradePlaceholderGatewayProperty(int hypothesisID,
    double value) {
  log("setOrUpgradePlaceholderGatewayProperty(%i,%f) called", hypothesisID,
      value);
  m_PlacePropsMutex.lock();
  ...
  SpatialProperties::ValueProbabilityPair pair = { gatewaynessValue, value };
  SpatialProperties::ValueProbabilityPairs pairs;
  pairs.push_back(pair);
  ...
  SpatialProperties::ValueProbabilityPair pair2 = { noGatewaynessValue, (1
      - value) };
  ...
  map<int, string>::iterator foundFSIt = m_placeholderGatewayProperties.find(
      placeholderID);
  if (foundFSIt != m_placeholderGatewayProperties.end()) {
    try {
      lockEntry(foundFSIt->second, cdl::LOCKEDOD);
      SpatialProperties::GatewayPlaceholderPropertyPtr gwProp = getMemoryEntry<
          SpatialProperties::GatewayPlaceholderProperty> (foundFSIt->second);
    ...
    addToWorkingMemory<SpatialProperties::GatewayPlaceholderProperty> (newID,
        gwProp);
...
double PlaceManager::getGatewayness(double x, double y) {
  vector<FrontierInterface::DoorHypothesisPtr> doorHyps;
  getMemoryEntries<FrontierInterface::DoorHypothesis> (doorHyps);
...
NavData::FNodePtr PlaceManager::getCurrentNavNode() {
  ...
  for (vector<NavData::FNodePtr>::iterator it = nodes.begin(); it
    ...
    try {
      ...
      double distance = (x - robotX) * (x - robotX) + (y - robotY) * (y
          - robotY);
      if (distance < minDistance) {
        ret = *it;
        minDistance = distance;
...
FrontierInterface::PlaceMembership PlaceManager::getPlaceMembership(double inX,
    double inY) {
  ...
  if (closestNodeID == -1) {
    error("Error! Found no place membership on %i, assigning to place 0!",
        __LINE__);
    placeID = 0;
  } else {
...
void PlaceManager::beginPlaceTransition(int goalPlaceID) {
  ...
  if (curNode != 0) {
    ...
    error("Error! Could not find current Nav node!");
  }
...
void PlaceManager::processNewNode(NavData::FNodePtr node) {
  try {
  ...
  try {
    ...
    if (wasExploring && !m_foundNodeThisMovement) {
      ...
      for (vector<pair<int, NodeHypothesisPtr> >::iterator rejectedHypIt =
          m_rejectedHypotheses.begin(); rejectedHypIt
          != m_rejectedHypotheses.end(); rejectedHypIt++) {
        double distanceSq = ((*rejectedHypIt).second->x - goalHyp->x)
            * ((*rejectedHypIt).second->x - goalHyp->x)
            + ((*rejectedHypIt).second->y - goalHyp->y)
                * ((*rejectedHypIt).second->y - goalHyp->y);
        if (distanceSq < minDistanceSq) {
          (*rejectedHypIt).second->x = ((*rejectedHypIt).second->x
              * (*rejectedHypIt).first + goalHyp->x) / ((*rejectedHypIt).first
              + 1);
          (*rejectedHypIt).second->y = ((*rejectedHypIt).second->y
              * (*rejectedHypIt).first + goalHyp->y) / ((*rejectedHypIt).first
              + 1);
...
NodeHypothesisPtr PlaceManager::PlaceServer::getHypFromPlaceID(int placeID,
...
NavData::FNodePtr PlaceManager::PlaceServer::getNodeFromPlaceID(int placeID,
    const Ice::Current &_context) {
  //  m_pOwner->lockComponent();
...
PlacePtr PlaceManager::PlaceServer::getPlaceFromNodeID(int nodeID,
    const Ice::Current &_context) {
  //  m_pOwner->lockComponent();
...
void PlaceManager::PlaceServer::beginPlaceTransition(int goalPlaceID,
    const Ice::Current &_context) {
  //  m_pOwner->lockComponent();
...
void PlaceManager::robotMoved(const cast::cdl::WorkingMemoryChange &objID) {
  ...
  if (!m_firstMovementRegistered) {
    if (!m_robotInitialPoseReceived) {
      ...
      if (robotPoses.size() != 0) {
        double robotX = robotPoses[0]->x;
        double robotY = robotPoses[0]->y;
        double distance_squared = (m_robotInitialX - robotX) * (m_robotInitialX
            - robotX) + (m_robotInitialY - robotY) * (m_robotInitialY - robotY);
        log("Squared distance from robot's initial position is %f",
            distance_squared);
...
void PlaceManager::deletePlaceProperties(int placeID) {
  ...
  deletePlaceholderProperties(placeID);
  {
    ...
    if (it != m_gatewayProperties.end()) {
      try {
        ...
        wrn("Gateway property could not be deleted; already missing!");
      }
...
void PlaceManager::deletePlaceholderProperties(int placeID) {
  ...
  m_PlacePropsMutex.lock();
  {
    ...
    if (it != m_freeSpaceProperties.end()) {
      try {
        ...
        wrn("Free space property could not be deleted; already missing!");
      }
  ...
  }
  {
    ...
    if (it != m_borderProperties.end()) {
      try {
        ...
        wrn("Border property could not be deleted; already missing!");
      }
  ...
  }
  {
    ...
    map<int, string>::iterator it =
        m_placeholderGatewayProperties.find(placeID);
    if (it != m_placeholderGatewayProperties.end()) {
      try {
        deleteFromWorkingMemory(it->second);
      } catch (...) {
        wrn(
            "Gateway placeholder property could not be deleted; already missing!");
      }
...
void PlaceManager::addNewGatewayProperty(int placeID) {
  ...
  SpatialProperties::ValueProbabilityPair pair1 = { trueValue, 0.9 };
  SpatialProperties::ValueProbabilityPair pair2 = { falseValue, 0.1 };
...
PlacePtr PlaceManager::PlaceServer::getCurrentPlace(
    const Ice::Current &_context) {
  //  m_pOwner->lockComponent();
...
FrontierInterface::PlaceMembership PlaceManager::PlaceServer::getPlaceMembership(
    double x, double y, const Ice::Current &_context) {
  //  m_pOwner->lockComponent();
  FrontierInterface::PlaceMembership membership = m_pOwner->getPlaceMembership(
      x, y);
  //  m_pOwner->unlockComponent();
  return membership;
}

int PlaceManager::PlaceServer::updatePlaceholderEdge(int placeholderId,
    const Ice::Current &_context) {
  int ret;
...
FrontierInterface::AdjacencyLists PlaceManager::PlaceServer::getAdjacencyLists(
    const Ice::Current &_context) {
  return m_pOwner->getAdjacencyLists();
}

void PlaceManager::upgradePlaceholder(int placeID, NavData::FNodePtr newNode) {
  log("Upgrading Place %d from Placeholder status; associating with node %d",
      placeID, newNode->nodeId);
  ...
  // real ones (from the node map)
  {
    for (list<pair<int, int> >::iterator it =
        m_hypotheticalConnectivities.begin(); it
        != m_hypotheticalConnectivities.end(); it++) {
...
void PlaceManager::createConnectivityProperty(double cost, int place1ID,
    int place2ID) {
  ...
  if (place1Connectivities.find(place2ID) == place1Connectivities.end()) {
    ...
    SpatialProperties::ValueProbabilityPair pair1 = { costValue1, 0.9 };
    SpatialProperties::ValueProbabilityPair pair2 = { costValue2, 0.1 };
    ...
    addToWorkingMemory<SpatialProperties::ConnectivityPathProperty> (newID,
        connectivityProp1);
    set<int> & place1Connectivities = m_connectivities[place1ID];
...
bool PlaceManager::deleteConnectivityProperty(int place1ID, int place2ID) {
  ...
  if (place1Connectivities.find(place2ID) != place1Connectivities.end()) {
    string placeIDstr = concatenatePlaceIDs(place1ID, place2ID);
    map<string, string>::iterator it = m_placeIDsToConnectivityWMID.find(
        placeIDstr);
...
void PlaceManager::mapLoadStatusOverwritten(const cdl::WorkingMemoryChange &wmc) {
...
void PlaceMapper::_checkConsistency(int line) {
#ifdef DO_CONSISTENCY_CHECK
  IceUtil::Mutex::Lock lock(m_mutex);
  for (vector<PlaceMapEntry>::const_iterator it = entries.begin(); it
      != entries.end(); it++) {
    ...
    if (entry.place == 0) {
      ...
      if (entry.placeWMID == "") {
        ...
        if (entry.place->id != placeEntry->id || entry.place->status
            != placeEntry->status) {
          error(
              "Consistency check on %i failed: local and WM places not equal!",
              line);
        }
      }
    }
    if (entry.node == 0) {
      if (entry.hyp == 0) {
        ...
        if (entry.hypWMID == "") {
          ...
          NodeHypothesisPtr hypEntry = getMemoryEntry<NodeHypothesis> (
              entry.hypWMID);
          if (entry.hyp->x != hypEntry->x || entry.hyp->y != hypEntry->y
              || entry.hyp->hypID != hypEntry->hypID
              || entry.hyp->originPlaceID != hypEntry->originPlaceID) {
            error(
                "Consistency check on %i failed: local and WM hyps not equal!",
                line);
            error("(%f, %f); (%f, %f); (%i, %i); (%i, %i)", entry.hyp->x,
                hypEntry->x, entry.hyp->y, hypEntry->y, entry.hyp->hypID,
                hypEntry->hypID, entry.hyp->originPlaceID,
                hypEntry->originPlaceID);
          }
        }
      }
    } else {
...
const PlacePtr PlaceMapper::_getPlace(PlaceID id) {
...
PlaceID PlaceMapper::_getPlaceIDForNode(NodeID id) {
...
const NavData::FNodePtr PlaceMapper::_getNodeForPlace(PlaceID id) {
...
PlaceID PlaceMapper::_getPlaceIDForHyp(HypID id) {
...
const NodeHypothesisPtr PlaceMapper::_getHypForPlace(PlaceID id) {
...
string PlaceMapper::_getPlaceWMIDForPlace(PlaceID id) {
...
string PlaceMapper::_getHypWMIDForPlace(PlaceID id) {
...
void PlaceMapper::_getPlaceholders(std::vector<PlaceID> &ret) {
...
// Note: once you get this list, it's not guaranteed to remain correct!
void PlaceMapper::_getTruePlaces(std::vector<PlaceID> &ret) {
...
void PlaceMapper::_overwriteHypForPlace(PlaceID placeID, NodeHypothesisPtr hyp) {
...
void PlaceMapper::_updateNodeForPlace(PlaceID placeID, NavData::FNodePtr node) {
...
void PlaceMapper::_deletePlace(PlaceID id) {
...
void PlaceMapper::_upgradePlaceholderMappings(PlaceID placeID, PlacePtr place,
...
PlaceID PlaceMapper::_addPlaceWithNode(PlacePtr _place, NavData::FNodePtr _node)//, std::string _nodeWMID)
{
  ...
  int newPlaceID;
  {
    ...
    if (it != m_NodeIDToPlaceIDMap.end()) {
      ...
      if (newPlaceID > m_placeIDCounter)
        m_placeIDCounter = newPlaceID;
    } else {
...
PlaceID PlaceMapper::_addPlaceWithHyp(PlacePtr _place, NodeHypothesisPtr _hyp) {
  ...
  int newPlaceID;
  {
    ...
    //TODO: maybe no need to lock this - will only be a problem if 
    //another thread is overwriting this
    log("Adding (%s) place %llu, based on hypothesis %i", newWMID.c_str(),
