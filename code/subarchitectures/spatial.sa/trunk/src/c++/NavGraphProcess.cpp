...
NavGraphProcess::~NavGraphProcess() {
  if (!m_DontWriteFiles)
    saveGraphToFile( m_cureNavGraphFile);
}
...
void NavGraphProcess::saveGraphToFile(const std::string &filename) {
  if (filename != "") {
    ...
    if (fs.is_open()) {
      ...
      for (std::map<long, FNodeHolder>::iterator iter = m_FreeNodes.begin(); iter
          != m_FreeNodes.end(); iter++) {

        char buf[128];
        if (iter->second.m_data->type.empty()) {
          sprintf(buf, "Area00");
        } else {
          sprintf(buf, "%s%02ld", iter->second.m_data->type[0].name.c_str(),
              (long) iter->second.m_data->type[0].id);
        }

        fs << iter->second.m_data->gateway << " "
            << iter->second.m_data->nodeId << " " << iter->second.m_data->x
            << " " << iter->second.m_data->y << " "
            << iter->second.m_data->theta << " " << buf << " "
      ...
      for (std::map<long, AEdgeHolder>::iterator iter = m_AccessEdges.begin(); iter
          != m_AccessEdges.end(); iter++) {
      ...
      for (std::map<long, ObjDataHolder>::iterator iter = m_Objects.begin(); iter
          != m_Objects.end(); iter++) {
        fs << iter->second.m_data->x << " " << iter->second.m_data->y << " "
            << iter->second.m_data->z << " " << iter->second.m_data->areaId
            << " " << iter->second.m_data->category << " "
            << iter->second.m_data->probability << " "
            << iter->second.m_data->objectId << " "
            << iter->second.m_data->camX << " " << iter->second.m_data->camY
            << std::endl;
      }

      // Write AREAS
      fs << "AREAS " << m_Areas.size() << std::endl;
      for (std::list<NavGraphProcess::Area>::iterator ai = m_Areas.begin(); ai
          != m_Areas.end(); ai++) {
        ...
        if (ai->m_data->type.empty()) {
          ...
          fs << ai->m_data->type[0].name << " " << ai->m_data->type[0].id
              << std::endl;
        }
      }
    }
...
void NavGraphProcess::configure(const map<string, string>& _config) {
  ...
  registerIceServer<NavData::NavGraphInterface, NavData::NavGraphInterface> (
      mapservant);
  ...
  if (it != _config.end()) {
    ...
    if (f.is_open()) {
      ...
      if (!m_DontWriteFiles) {
        ...
        std::ofstream(backup.c_str()) << std::ifstream(
            m_cureNavGraphFile.c_str()).rdbuf();
      }
...
double NavGraphProcess::NavGraphServer::getPathLength(double xS, double yS,
    double aS, double xG, double yG, double aG, const Ice::Current &_context) {
...
int NavGraphProcess::NavGraphServer::getAreaId(double x, double y, double a,
    double maxDist, const Ice::Current &_context) {
  int id;
...
int NavGraphProcess::NavGraphServer::getClosestNodeId(double x, double y,
    double a, double maxDist, const Ice::Current &_context) {
  int id;
...
void NavGraphProcess::loadGraphFromFile(const std::string &filename) {
  ...
  if (is <= 0) {
    CureCERR(20) << "Failed to open graph file \"" << filename << "\"\n";
    return;
  }
  ...
  if (getline(is, line)) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (istr >> type >> id >> x >> y >> a >> areaclass >> aid) {
        ...
        int areaclassno = 0;
        if (areaclass.size() >= 2) {
          sscanf(areaclass.substr(areaclass.size() - 2, 2).c_str(), "%d",
              &areaclassno);
        ...
        if (type == Cure::NavGraphNode::NODETYPE_GATEWAY) {
          ...
          addFreeNode(id, x, y, 0.0, a, aid, areaclass, areaclassno, 1, // a gateway
              2.0, // max speed
              width, // door width
              "-"); // name of node
          ...
          addFreeNode(id, x, y, 0.0, a, aid, areaclass, areaclassno, 0, // not gateway
              2.0, // max speed
              0.0, // door width
              "-");// name of node
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (istr >> id1 >> id2) {
        ...
        if (m_cureNavGraph.addEdgeToEdgeList(id1, id2) == 0) {
          ...
          CureCERR(20) << "WARNING: Problems with edge on line " << lineno
              << "\n";
        }

      } else {
        CureCERR(20) << "WARNING: Failed to read edge data on line " << lineno
            << "\n";
      }
    }
  }
  ...
  if (getline(is, line)) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (istr >> x >> y >> z >> areaId >> category >> probability >> objId
          >> robX >> robY) {
        addObject(x, y, z, robX, robY, areaId, category, objId, probability);
        log("object added: %s: at area %i with probability %f",
            category.c_str(), areaId, probability);
      }
    }
  }
  ...
  if (getline(is, line)) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (istr >> newArea.m_data->id >> newArea.m_data->type[0].name
...
void NavGraphProcess::start() {
  ...
  addChangeFilter(createLocalTypeFilter<FrontierInterface::DoorHypothesis> (
      cdl::ADD), new MemberFunctionChangeReceiver<NavGraphProcess> (this,
      &NavGraphProcess::newDoorHypothesis));

  log("NavGraphProcess started");
}

void NavGraphProcess::checkAndAddArea(int aid, const std::string &areaClass,
    long areaClassNo) {
  ...
  if (aIter != m_Areas.end()) {
    ...
    if (areaClass != "") {
      ...
      if (!aIter->m_data->type.empty() && (aIter->m_data->type[0].name
          == areaClass)) {

        // We found an exiting areas so we do not have to add anything
        log(
            "Area %d already exist and class is already %s, no need to update WM",
            aid, areaClass.c_str());
        ...
        overwriteWorkingMemory<NavData::Area> (aIter->m_WMid, aIter->m_data);
    ...
    addToWorkingMemory<NavData::Area> (newArea.m_WMid, newArea.m_data);
...
void NavGraphProcess::areaIdConflict(Cure::NavGraphNode &currNode,
  ...
  log(
      "Area conflict detected for node %d in area %d, door with id %d may not be  door",
      currNode.getId(), currNode.getAreaId(), gw.getId());
}
...
void NavGraphProcess::changedCurrentNode(int fromId, int toId) {
  ...
  while (m_AccessEdges.size() + m_NewAEdges.size()
    ...
    if ((makeEdgeId(m_cureNavGraph.m_Edges.back().getNodeId1(),
        m_cureNavGraph.m_Edges.back().getNodeId2()) == makeEdgeId(fromId, toId))
        && (m_AccessEdges.find(makeEdgeId(
            m_cureNavGraph.m_Edges.back().getNodeId1(),
            m_cureNavGraph.m_Edges.back().getNodeId2())) == m_AccessEdges.end())) {
      ...
      debug(
          "Have to do a brute force search for the new edge since it was not the last one added");

      // We have to do a brute force search for the missing node
      std::list<Cure::NavGraphEdge>::iterator ei;
      for (ei = m_cureNavGraph.m_Edges.begin(); ei
          != m_cureNavGraph.m_Edges.end(); ei++) {
        ...
        for (aei = m_AccessEdges.begin(); aei != m_AccessEdges.end(); aei++) {
          ...
          if (makeEdgeId(ei->getNodeId1(), ei->getNodeId2()) == makeEdgeId(
              aei->second.m_data->startNodeId, aei->second.m_data->endNodeId)) {
            foundMatch = true;
            break;
          }
        }
  ...
  if (nEdgesAdded > 1) {
    log(
        "WARNING: More than one edge added at a time, might get edge without both edns in WM");
  }

  if (publishChangesNow)
    writeGraphToWorkingMemory();
}
...
void NavGraphProcess::changedNodeType(int id) {
  ...
  for (std::list<FNodeHolder>::iterator fni = m_NewFNodes.begin(); fni
    ...
    if (node->getType() == Cure::NavGraphNode::NODETYPE_GATEWAY) {
      ...
      fni->m_data->width[0] = ((Cure::NavGraphGateway*) &(*node))->getWidth();
    }

    if (fni->m_WMid != "") {
      overwriteWorkingMemory<NavData::FNode> (fni->m_WMid, fni->m_data);
    }
  }

  for (std::map<long, FNodeHolder>::iterator fni = m_FreeNodes.begin(); fni
      != m_FreeNodes.end(); fni++) {

    if (fni->second.m_data->nodeId != id)
      continue;

    if (((node->getType() == Cure::NavGraphNode::NODETYPE_GATEWAY)
        && (fni->second.m_data->gateway == 1)) || ((node->getType()
        != Cure::NavGraphNode::NODETYPE_GATEWAY)
        && (fni->second.m_data->gateway == 0)))
      break;
...
void NavGraphProcess::writeGraphToWorkingMemory(bool forceWrite) {
  ...
  for (std::list<FNodeHolder>::iterator fni = m_NewFNodes.begin(); fni
    ...
    if (fni->m_data->gateway || fni->m_data->type.empty()) {
      ...
      checkAndAddArea(fni->m_data->areaId, fni->m_data->type[0].name,
    ...
    addToWorkingMemory<NavData::FNode> (fni->m_WMid, fni->m_data);

    log("Writing free node %d of type %d in area %d to WM",
        (int) fni->m_data->nodeId, fni->m_data->gateway,
  ...
  while (!m_ChangedNodes.empty()) {
    ...
    overwriteWorkingMemory<NavData::FNode> ((*i)->m_WMid, (*i)->m_data);

    // Remove all instances of this so that we do not send out more
    // than one message about it
    for (std::list<FNodeHolder*>::iterator j = m_ChangedNodes.begin(); j
        != m_ChangedNodes.end();) {
      if (*i == *j) {
  ...
  if (!m_AreasFromFile.empty()) {
    for (std::list<Area>::iterator ai = m_AreasFromFile.begin(); ai
        != m_AreasFromFile.end(); ai++) {
      if (ai->m_data->type.empty()) {
        checkAndAddArea(ai->m_data->id);
      } else {
        checkAndAddArea(ai->m_data->id, ai->m_data->type[0].name,
            ai->m_data->type[0].id);
      }
    }
  ...
  for (std::list<AEdgeHolder>::iterator aei = m_NewAEdges.begin(); aei
      != m_NewAEdges.end();) {

    log("Writing access edge, connecting nodes %d and %d to WM",
        (int) aei->m_data->startNodeId, (int) aei->m_data->endNodeId);

    aei->m_WMid = newDataID();
    addToWorkingMemory<NavData::AEdge> (aei->m_WMid, aei->m_data);

    m_AccessEdges.insert(std::make_pair(makeEdgeId(aei->m_data->startNodeId,
        aei->m_data->endNodeId), *aei));
  ...
  for (std::list<ObjDataHolder>::iterator oni = m_NewObjData.begin(); oni
      != m_NewObjData.end();) {
    oni->m_WMid = newDataID();
    addToWorkingMemory<NavData::ObjData> (oni->m_WMid, oni->m_data);
  ...
  while (!m_ChangedObjects.empty()) {
    ...
    try {
      ...
      overwriteWorkingMemory<NavData::ObjData> ((*i)->m_WMid, (*i)->m_data);
    ...
    // Remove all instances of this so that we do not send out more
    // than one message about it
    for (std::list<ObjDataHolder*>::iterator j = m_ChangedObjects.begin(); j
        != m_ChangedObjects.end();) {
      if (*i == *j) {
  ...
  if ((wroteAnyPart || forceWrite) && (!m_FreeNodes.empty()
      || !m_Objects.empty())) {
    ...
    if (m_NavGraphWMid == "") {
      ...
      addToWorkingMemory<NavData::NavGraph> (m_NavGraphWMid, graph); // sync!

      log("Added composite NavGraph to working memory");
    } else {
      debug("Write Graph to WM");
      overwriteWorkingMemory<NavData::NavGraph> (m_NavGraphWMid, graph);
    }

    debug(
        "Wrote composite NavGraph with %d nodes, %d edges and %d objects to WM",
        graph->fNodes.size(), graph->aEdges.size(), graph->objects.size());
...
void NavGraphProcess::addFreeNode(long nodeId, double x, double y, double z,
    double theta, long areaId, const string &areaType, long areaTypeNo,
    short gateway, double maxSpeed, double width, const std::string &name,
  ...
  log(
      "Added free node %d of type %d in area %d at pose (x=%.2f y=%.2f a=%.4f) (local repr)",
      (int) fnode.m_data->nodeId, fnode.m_data->gateway,
      (int) fnode.m_data->areaId, fnode.m_data->x, fnode.m_data->y,
      fnode.m_data->theta);
}

void NavGraphProcess::addObject(double x, double y, double z, double camX,
    double camY, long areaId, const string &category, long objectId,
    double probability) {
  // Add the object
  ...
  if (m_UniqueObjects) {
    ...
    // the position of it
    for (std::map<long, ObjDataHolder>::iterator i = m_Objects.begin(); i
        != m_Objects.end(); i++) {
      if (i->second.m_data->category == category) {
        *(i->second.m_data) = *(onode.m_data);

        log(
            "Adding pointer to object 0x%X with category %s and areaId %d objectId %d",
    ...
    // working memory
    for (std::list<ObjDataHolder>::iterator i = m_NewObjData.begin(); i
        != m_NewObjData.end(); i++) {
      if (i->m_data->category == category) {
        i->m_data = onode.m_data;
        println(
            "Updated new object of category %s and id %d in area %d (local)",
            category.c_str(), objectId, areaId);
        return;
      }
    }
    ...
    println(
        "WARNING: Not dealing with non-unique objects. Will add a new instance for every observation");
  }
...
void NavGraphProcess::writeTopologicalPosToWorkingMemory(int aid) {
  if (!m_TopRobPos)
    m_TopRobPos = new NavData::TopologicalRobotPos;
...
void NavGraphProcess::runComponent() {
  ...
  if (m_WriteFirstGraph) {
    SpatialData::MapLoadStatusPtr statusStruct = new SpatialData::MapLoadStatus;
    ...
    addToWorkingMemory<SpatialData::MapLoadStatus> (m_MapLoadStatusWM,
        statusStruct);
  }
...
void NavGraphProcess::newRobotPose(const cdl::WorkingMemoryChange &objID) {
  shared_ptr<CASTData<NavData::RobotPose2d> > oobj = getWorkingMemoryEntry<
      NavData::RobotPose2d> (objID.address);

  Cure::Pose3D cp;
  cp.setTime(Cure::Timestamp(oobj->getData()->time.s, oobj->getData()->time.us));
  ...
  debug("newRobotPose called with x=%.3f y=%.3f theta=%.3f t=%.6f", cp.getX(),
      cp.getY(), cp.getTheta(), cp.getTime().getDouble());
...
void NavGraphProcess::newVisualObject(
    const cast::cdl::WorkingMemoryChange & wmChange) {
  IceUtil::Mutex::Lock lock(m_GraphMutex);

  m_graphEventQueue.push_back(pair<GraphEventType,
      cast::cdl::WorkingMemoryAddress> (NEW_VISUAL_OBJECT, wmChange.address));
}

void NavGraphProcess::processNewVisualObject(
    const cast::cdl::WorkingMemoryAddress & addr) {
  VisionData::VisualObjectPtr visualObjectPtr = getMemoryEntry<
      VisionData::VisualObject> (addr);

  string category = visualObjectPtr->identLabels[0];
  double probability = visualObjectPtr->identDistrib[0];
  log("Observed object of category \"%s\" with probability %f",
      category.c_str(), probability);
  ...
  long areaId = 0;
  if (m_TopRobPos)
    areaId = m_TopRobPos->areaId;

  {
    IceUtil::Mutex::Lock lock(m_GraphMutex);

    addObject(objPw.getX(), objPw.getY(), objPw.getZ(), rp.getX(), rp.getY(),
...
void NavGraphProcess::newObjObs(const cdl::WorkingMemoryChange &objID) {
  ...
  m_graphEventQueue.push_back(pair<GraphEventType,
      const cast::cdl::WorkingMemoryAddress> (NEW_OBJ_OBS, objID.address));
}

void NavGraphProcess::processNewObjObs(const cdl::WorkingMemoryAddress &addr) {
  shared_ptr<CASTData<NavData::ObjObs> > oobj = getWorkingMemoryEntry<
      NavData::ObjObs> (addr);
  ...
  // The default distance is 1m
  double d = 1.0;
  if (!oobj->getData()->dist.empty())
    d = oobj->getData()->dist[0];
  ...
  long objId = -1;
  if (!oobj->getData()->id.empty())
    objId = oobj->getData()->id[0];
...

  {
    IceUtil::Mutex::Lock lock(m_GraphMutex);
    addObject(objPw.getX(), objPw.getY(), objPw.getZ(), rp.getX(), rp.getY(),
        areaId, oobj->getData()->category, objId, 1.0);
...
void NavGraphProcess::processScan(Cure::LaserScan2d &cureScan) {
  ...
  if (m_WriteFirstGraph) {
    ...
    try {
      ...
      SpatialData::MapLoadStatusPtr loadStatus = getMemoryEntry<
          SpatialData::MapLoadStatus> (m_MapLoadStatusWM);
      loadStatus->nodesWritten = true;
      overwriteWorkingMemory<SpatialData::MapLoadStatus> (m_MapLoadStatusWM,
          loadStatus);
  ...
  if (status) {
    ...
    debug(
        "Wanted robot pose at t=%.6f (last odometry t=%.6f, robot pose t=%.6f)",
...

  {
    ...
    int ret = m_cureNavGraph.maybeAddNewNodeAt(cp.getX(), cp.getY(),
        cp.getTheta());
  ...
  if (m_MotionDetector) {
    ...
    if (status) {
      ...
      m_MotionDetector->checkForMotion(cureScan.getTime().getDouble(),
          cureScan.getNPts(), cureScan.getRanges(), cureScan.getStartAngle(),
          cureScan.getAngleStep(), m_LaserPoseR.getX(), m_LaserPoseR.getY(),
          m_LaserPoseR.getTheta(), scanOdomPose.getX(), scanOdomPose.getY(),
  ...
  if (hypot(m_LastDoorCheckPose.getX() - cp.getX(), m_LastDoorCheckPose.getY()
      - cp.getY()) > minMoveDistToCheck) {
    if (m_InDoor) {
      // Check if we have moved enough since adding the door to
      // say that we are no longer in a door
      const double minDoorDist = 1.0;
      if (hypot(m_LastDoorPose.getX() - cp.getX(), m_LastDoorPose.getY()
          - cp.getY()) > minDoorDist) {
        m_InDoor = false;
      }
    }
    ...
    if (!m_InDoor) {
      ...
      if (m_MotionDetector && !m_MotionDetector->m_Movements.empty()) {
      ...
      if (m_DoorDetector.inDoorOpening(doorScan, m_MinDoorWidth, m_MaxDoorWidth)) {
        double minDistSq = DBL_MAX;
        if (m_UseDoorHypotheses) {
          for (vector<pair<double, double> >::iterator it =
              m_doorHypPositions.begin(); it != m_doorHypPositions.end(); it++) {
        ...
        if (!m_UseDoorHypotheses || minDistSq < 4.0) {
          ...
          log("Door is detected angR=%.2fdeg angL=%.2fdeg",
              m_DoorDetector.m_AngleR * 180 / M_PI, m_DoorDetector.m_AngleL
                  * 180 / M_PI);
          ...
          double xR = lpW.getX() + (m_DoorDetector.m_RangeR * cos(
              lpW.getTheta() + m_DoorDetector.m_AngleR));
          double yR = lpW.getY() + (m_DoorDetector.m_RangeR * sin(
              lpW.getTheta() + m_DoorDetector.m_AngleR));
          double xL = lpW.getX() + (m_DoorDetector.m_RangeL * cos(
              lpW.getTheta() + m_DoorDetector.m_AngleL));
          double yL = lpW.getY() + (m_DoorDetector.m_RangeL * sin(
              lpW.getTheta() + m_DoorDetector.m_AngleL));
          ...
          log(
              "Estimated door pose to x=%.2f y=%.2f theta=%fdeg lpW.x=%.2f lpW.y=%.2f lpW.theta=%.2fdeg rR=%.2f rL=%.2f",
              x, y, dir * 180 / M_PI, lpW.getX(), lpW.getY(), lpW.getTheta()
                  * 180.0 / M_PI, m_DoorDetector.m_RangeR,
              m_DoorDetector.m_RangeL);

          {
            IceUtil::Mutex::Lock lock(m_GraphMutex);
            //long lastid = m_cureNavGraph.m_Nodes.back()->getId();
            unsigned long lastsize = m_cureNavGraph.m_Nodes.size();
            m_cureNavGraph.addGatewayNode(x, y, dir, m_DoorDetector.m_Width);
            if (m_cureNavGraph.m_Nodes.size() == lastsize) {
              checkForNodeChanges();
            } else {
              addFreeNode(
                  m_cureNavGraph.m_Nodes.back()->getId(),
                  //lpW.getX(), lpW.getY(), 0.0, cp.getTheta()-M_PI_2,
                  x, y, 0.0, dir, m_cureNavGraph.m_Nodes.back()->getAreaId(),
                  "Area", 0, 1, m_cureNavGraph.m_Nodes.back()->getMaxSpeed(),
                  m_DoorDetector.m_Width, "-", true); // ask for the area class!!
              writeGraphToWorkingMemory();
            }
          }
  ...
  if (m_WriteFirstTopologicalPose) {
    debug("Writing topological position: %d", m_cureNavGraph.getCurrentAreaId());
    writeTopologicalPosToWorkingMemory(m_cureNavGraph.getCurrentAreaId());
    m_WriteFirstTopologicalPose = false;
  }
}

int NavGraphProcess::processNewCureNavGraphNode(bool knownToBeGateway, double w) {
  debug("processNewCureNavGraphNode");
  Cure::NavGraphNode &lastNode = *(m_cureNavGraph.m_Nodes.back());

  if (!m_FreeNodes.empty() && m_FreeNodes.find(lastNode.getId())
      != m_FreeNodes.end()) {
  ...
  short gateway = 0;
  double width = 1.0;
  if (knownToBeGateway || (lastNode.getType()
      == Cure::NavGraphNode::NODETYPE_GATEWAY)) {
    width = w;
    gateway = 1;
  }
  ...
  debug(
      "Cure added new node id=%d (aid=%d, g=%d, w=%.1f) at x=%.2f y=%.2f a=%.4f",
      lastNode.getId(), lastNode.getAreaId(), gateway, width, lastNode.getX(),
      lastNode.getY(), lastNode.getTheta());

  addFreeNode(lastNode.getId(), lastNode.getX(), lastNode.getY(), 0.0,
      lastNode.getTheta(), lastNode.getAreaId(), "Area", 0, gateway,
      lastNode.getMaxSpeed(), width, lastNode.getName(), true);
...
void NavGraphProcess::checkForNodeChanges() {
  ...
  for (ni = m_cureNavGraph.m_Nodes.begin(); ni != m_cureNavGraph.m_Nodes.end(); ni++) {
    ...
    for (fni = m_FreeNodes.begin(); fni != m_FreeNodes.end(); fni++) {
      ...
      if ((*ni)->getId() == fni->second.m_data->nodeId) {
        ...
        if ((*ni)->getType() != fni->second.m_data->gateway) {
          ...
          log("Node %d changed type from %d to %d", (*ni)->getId(),
  ...
  if (changesMade)
    writeGraphToWorkingMemory(true);
}

void NavGraphProcess::newDoorHypothesis(
    const cast::cdl::WorkingMemoryChange &objID) {
  IceUtil::Mutex::Lock lock(m_eventQueueMutex);

  m_graphEventQueue.push_back(pair<GraphEventType,
      const cdl::WorkingMemoryAddress> (NEW_DOOR_HYPTOHESIS, objID.address));
}

void NavGraphProcess::processNewDoorHypothesis(
    const cast::cdl::WorkingMemoryAddress &addr) {
  try {
    FrontierInterface::DoorHypothesisPtr doorHyp = getMemoryEntry<
        FrontierInterface::DoorHypothesis> (addr);
