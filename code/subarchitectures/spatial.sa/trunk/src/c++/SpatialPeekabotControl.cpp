...
SpatialPeekabotControl::SpatialPeekabotControl() :
  m_maxPlaces(0), m_pendingQueryReceiver(this), gadget_y(10), gadget_ystep(2),
      gadgetLineLength(20), m_maxPlaceholderID(0), m_hideGadget(false) {
  m_CtrlAction = 0;
  m_doPathQuery = false;
}
...
void SpatialPeekabotControl::start() {
  if (!m_hideGadget) {
    addChangeFilter(cast::createLocalTypeFilter<SpatialData::Place>(
        cast::cdl::ADD), new cast::MemberFunctionChangeReceiver<
        SpatialPeekabotControl>(this, &SpatialPeekabotControl::newPlace));

    addChangeFilter(cast::createLocalTypeFilter<SpatialData::Place>(
        cast::cdl::DELETE), new cast::MemberFunctionChangeReceiver<
        SpatialPeekabotControl>(this, &SpatialPeekabotControl::deletedPlace));
  }

  //Only for Year1 visualization!
  addChangeFilter(
      cast::createLocalTypeFilter<FrontierInterface::NodeHypothesis>(
          cast::cdl::ADD), new cast::MemberFunctionChangeReceiver<
          SpatialPeekabotControl>(this,
...
void SpatialPeekabotControl::runComponent() {
  ...
  if (!m_hideGadget) {
    ...
    startspot.add(m_controlmodule, "start-spot ", peekabot::REPLACE_ON_CONFLICT);
    ...
    if (m_PeekabotClient.is_connected()) {
      ...
      while (isRunning()) {
        ...
        if (r.succeeded()) {
          dir = atan2((r.get_result().m_c[1] - oldposy), (r.get_result().m_c[0]
              - oldposx)) * 180 / M_PI;
          if (dir < 0)
            dir += 360;
          ...
          if (has1 and has2 and has3) {
            ...
            for (unsigned int h = 0; h < fnodeseq.size(); h++) {
              ...
              if (hypot(fnodeseq[h]->y - r.get_result().m_c[1], fnodeseq[h]->x
                  - r.get_result().m_c[0]) < radius) {
                // seems we are asked to search this node
                //log("fnodeid %i",fnodeseq[h]->nodeId);
                SpatialData::PlacePtr place =
                    m_placeInterface->getPlaceFromNodeID(fnodeseq[h]->nodeId);
                ...
                if (!isadded) {
                  ...
                  log("placeid : %lli (fnodeid: %lli) added to search plan!",
                      place->id, fnodeseq[h]->nodeId);
                }
              }
            }
          ...
          if (r.succeeded() && hypot(searchy - r.get_result().m_c[1], searchx
              - r.get_result().m_c[0]) < radius && !placeseq.empty()
              && !sentplancommand) {
        ...
        if (r.succeeded()) {
          ...
          if (hypot(gadget_y - r.get_result().m_c[1], xNoA
              - r.get_result().m_c[0]) < radius) {
            ...
            if (r.succeeded()) {
              ...
              if (wasInCtrl && (xT == r.get_result().m_c[0]) && (yT
                  == r.get_result().m_c[1])) {
                sendCmd = false;
              }
              ...
              if (sendCmd) {
                ...
                std::string id = "";
                if (yT - (gadget_y - 1) < 0.5 && yT - (gadget_y - 1) > -(0.5
                    + gadget_ystep * (m_maxPlaces / gadgetLineLength))) { //fabs(yT-(gadget_y-1)) < 0.5) {

                  // Get the place id
                  cmd->destId.resize(1);
                  int ypos = (int) (((gadget_y - 1 + 0.5) - yT) / gadget_ystep);
                  cmd->destId[0] = long(xT + 0.5) + ypos * gadgetLineLength;
                  log("Reading command: %i, %i, %i", ypos, long(xT + 0.5),
                      (int) cmd->destId[0]);
                  if (cmd->destId[0] < 0)
                    cmd->destId[0] = 0;
                  ...
                  if (m_doPathQuery) {
                    ...
                    addChangeFilter(cast::createIDFilter(queryID,
                        cast::cdl::OVERWRITE), &m_pendingQueryReceiver);
                  ...
                  id = "gotoxy-" + newDataID();
                  log(
                      "Sending robot to new target position (%.2f, %.2f) task id: %s",
                      xT, yT, id.c_str());
...
void SpatialPeekabotControl::PendingQueryReceiver::workingMemoryChanged(
  ...
  if (m_dependentCommand != 0) {
    ...
    SpatialData::PathTransitionProbRequestPtr query = m_parent->getMemoryEntry<
        SpatialData::PathTransitionProbRequest> (_wmc.address);
    if (query && query->status == SpatialData::QUERYCOMPLETED) {
      if (query->successProb > 0.0) {
        ...
        m_parent->addToWorkingMemory<SpatialData::NavCommand> (id,
            m_dependentCommand);
      } else {
...
void SpatialPeekabotControl::newPlace(
    const cast::cdl::WorkingMemoryChange &_wmc) {
...
void SpatialPeekabotControl::deletedPlace(
    const cast::cdl::WorkingMemoryChange &_wmc) {
  updatePeekabotGadget();
}
...
void SpatialPeekabotControl::updatePeekabotGadget() {
  ...
  int i = 0;
  for (vector<SpatialData::PlacePtr>::iterator it = places.begin(); it
      != places.end(); it++) {
    char identifier[100];
    for (; i < (*it)->id; i++) {
    ...
    lp.set_pose(1.0 * (i % gadgetLineLength), gadget_y - 0.5 - (gadget_ystep
        * (i / gadgetLineLength)), 0, 0, 0, 0);
    ...
    cp.add(m_controlmodule, identifier, peekabot::REPLACE_ON_CONFLICT);
    cp.set_scale(0.04);
    cp.set_pose(1.0 * (i % gadgetLineLength), gadget_y - 1 - (gadget_ystep * (i
        / gadgetLineLength)), 0, 0, 0, 0);
    i++;
  }
  for (; i < m_maxPlaces; i++) {
    char identifier[100];
...
NavData::FNodePtr SpatialPeekabotControl::getCurrentNavNode() {
...
void SpatialPeekabotControl::newNodeHypothesis(
    const cast::cdl::WorkingMemoryChange &_wmc) {
  //  try {
...
void SpatialPeekabotControl::deletedNodeHypothesis(
    const cast::cdl::WorkingMemoryChange &_wmc) {
  updatePlaceholderVisualization();
}

void SpatialPeekabotControl::updatePlaceholderVisualization() {
  ...
  for (it = hypotheses.begin(); it != hypotheses.end(); it++) {
    int hypID = (*it)->hypID;
    m_maxPlaceholderID = m_maxPlaceholderID > hypID ? m_maxPlaceholderID
        : hypID;
  }
