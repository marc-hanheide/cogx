...
bool ConeSlicer::startColumn(double _x, double _y) 
{
  ...
  if (minSqDist > 0.0 && 
      (x-origin.x)*(x-origin.x)+(y-origin.y)*(y-origin.y) < minSqDist) {
    return false;
  }
  ...
  do {
    ...
    switch(getNextPlane(topPoint.z, z1,z2,z3,z4)) {
      case 1: topPoint.z = z1; break;
      case 2: topPoint.z = z2; break;
      case 3: topPoint.z = z3; break;
      case 4: topPoint.z = z4; break;
      default: topPoint.z = zMin; break;
    }
  }
...
void ConeSlicer::getNextBoundary(
  ...
  if (highestPlane == 5) {
    ...
    // going out or in
    bool clipUpper = 
      farClipPlanes[currentRayX][currentRayY].normal.z > 0.0;
    ...
    if (clipPlane.normal.z < epsilon && 
	clipPlane.normal.z > -epsilon) {
...
inline void ConeSlicer::optMinus(Vector3 & res, const Vector3 & arg1, const Vector3 & arg2){
    res.x = arg1.x - arg2.x;
    res.y = arg1.y - arg2.y;
    res.z = arg1.z - arg2.z;
}
