...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::setStart(double x,
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::getGridCoords(
		int& x, int& y) const {
	x = m_Xi;
	y = m_Yi;
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::getWorldCoords(
...
template<class MAPDATA> double ObjGridLineRayTracer<MAPDATA>::getDistToStart() const {
	return hypot(m_Yf - m_Ys, m_Xf - m_Xs) * m_CellSize;
}

template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::isOutside() const {
	if ((m_Xi < -m_Size) || (m_Size < m_Xi) || (m_Yi < -m_Size)
			|| (m_Size < m_Yi))
		return true;
	else
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::stepRay() {
	return (this->*drawLineFcn)();
}

template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::drawLineXp() {
	m_Xi++;
	m_Xf++;
	m_Yf += m_K;
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::drawLineXn() {
	m_Xi--;
	m_Xf--;
	m_Yf -= m_K;
	m_Yi = (0 < m_Yf ? int(m_Yf + 0.5) : int(m_Yf - 0.5));
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::drawLineYp() {
	m_Yi++;
	m_Yf++;
	m_Xf += m_K;
...
template<class MAPDATA> int ObjGridLineRayTracer<MAPDATA>::drawLineYn() {
	m_Yi--;
	m_Yf--;
	m_Xf -= m_K;
	m_Xi = (0 < m_Xf ? int(m_Xf + 0.5) : int(m_Xf - 0.5));
...
void ObjGridLineRayTracer<MAPDATA>::addScan(Cure::SICKScan &scan,
	...
	for (int i = 0; i < scan.getNPts(); i++) {
		...
		if (i == 0) {
			...
			if ((scan.getRange(i) - scan.getRange(i - 1) > maxDiff)
					&& (scan.getRange(i) - scan.getRange(i + 1) > maxDiff))
				continue;
		}
