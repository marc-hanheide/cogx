...
NewNavGraph::NewNavGraph(double nodeDist) :
	m_NodeDist(nodeDist), m_MaxDist(5), m_CurrNode(0), m_NextNodeId(0),
			m_MaxSpeed(1), m_LastAreaId(-1), m_AssignedNewAreaId(false),
...
void NewNavGraph::configRLStars(double sizeCurrent, double sizeNormal,
		double sizeNamedGoal, double sizeGateway, double sizeGatewayBlocked) {
	if (sizeCurrent > 0)
		m_RLStarSizeCurrent = sizeCurrent;
	if (sizeNormal > 0)
		m_RLStarSizeNormal = sizeNormal;
	if (sizeNamedGoal > 0)
		m_RLStarSizeNamedGoal = sizeNamedGoal;
	if (sizeGateway > 0)
		m_RLStarSizeGateway = sizeGateway;
	if (sizeGatewayBlocked > 0)
		m_RLStarSizeGatewayBlocked = sizeGatewayBlocked;
}

int NewNavGraph::clear() {
	for (std::list<NavGraphNode*>::iterator ni = m_Nodes.begin(); ni
			!= m_Nodes.end(); ni++) {
		delete *ni;
	}
	m_Nodes.clear();
...
int NewNavGraph::getCurrentNodeId() const {
	if (m_CurrNode)
		return m_CurrNode->getId();
	else
		return -1;
}

int NewNavGraph::setCurrentNodeId(int id) {
...
int NewNavGraph::getCurrentNodeType() const {
	if (m_CurrNode)
		return m_CurrNode->getType();
	else
		return -1;
}

int NewNavGraph::getCurrentNodeAreaClass() const {
	if (m_CurrNode)
		return m_CurrNode->getAreaClass();
	else
		return -1;
}

int NewNavGraph::setCurrentNodeAreaClass(int c) {
	if (m_CurrNode) {
...
long NewNavGraph::getCurrentAreaId() const {
	if (m_CurrNode)
		return m_CurrNode->getAreaId();
	else
		return -1;
}

int NewNavGraph::addFreePose(double x, double y, double a) {
	maybeAddNewNodeAt(x, y, a, false, NavGraphNode::NODETYPE_NODE);
	return 0;
}

int NewNavGraph::maybeAddNewNodeAt(double x, double y, double a) {
...
int NewNavGraph::maybeAddNewNodeAt(double x, double y, double a,
	...
	if (m_CurrNode == 0) {
		...
		if (type == NavGraphNode::NODETYPE_GATEWAY) {
			NavGraphGateway *g = new NavGraphGateway("-", m_NextNodeId, x, y, a,
					m_MaxSpeed, 1.0);
			m_Nodes.push_back(g);
			m_Gateways.push_back(g);
		} else {
			m_Nodes.push_back(
					new NavGraphNode("-", m_NextNodeId, x, y, a, m_MaxSpeed));
		}
	...
	if (!forceInNode) {
		...
		double cost = hypot(m_CurrNode->getY() - nc->getY(), m_CurrNode->getX()
				- nc->getX());
		if (nc != m_CurrNode && dc < d && dc <= m_NodeDist && cost < 1.5
				* m_NodeDist) {
			...
			if (m_CurrNode->getAreaId() != nc->getAreaId() && m_CurrNode->getType()
					!= NavGraphNode::NODETYPE_GATEWAY && nc->getType()
					!= NavGraphNode::NODETYPE_GATEWAY) {
	...
	if (type == NavGraphNode::NODETYPE_GATEWAY) {
		...
		m_Nodes.push_back(new NavGraphNode("-", m_NextNodeId, x, y, a, m_MaxSpeed));
	}
	...
	if (n->getType() != NavGraphNode::NODETYPE_GATEWAY) {
		if (m_CurrNode->getType() != NavGraphNode::NODETYPE_GATEWAY) {
			...
			for (std::list<NavGraphGateway*>::iterator gi = m_Gateways.begin(); gi
					!= m_Gateways.end(); gi++) {
				// Loop over all edges
				for (std::list<NavGraphEdge*>::iterator ei = (*gi)->m_Edges.begin(); ei
						!= (*gi)->m_Edges.end(); ei++) {
					...
					if (next) {
						if (next->getId() == m_CurrNode->getId()) {
							...
							bool side1 = (m_CurrNode->getY() < (*gi)->getY() + tan(
									(*gi)->getTheta()) * (m_CurrNode->getX() - (*gi)->getX()));
							bool side2 = (n->getY() < (*gi)->getY() + tan((*gi)->getTheta())
									* (n->getX() - (*gi)->getX()));
							if (side1 == side2) {
			...
			for (std::list<NavGraphEdge*>::iterator ei = (*gi)->m_Edges.begin(); ei
				...
				if (next) {
					// The node we are moving from is connected to a door, check if both nodes are on the same side of the door
					bool side1 = (next->getY() < (*gi)->getY() + tan((*gi)->getTheta())
							* (next->getX() - (*gi)->getX()));
					bool side2 = (n->getY() < (*gi)->getY() + tan((*gi)->getTheta())
							* (n->getX() - (*gi)->getX()));
					if (side1 == side2) {
	...
	CureCERR(40) << "Added new node " << *n << " when leaving " << *m_CurrNode
			<< std::endl;
	m_AssignedNewAreaId = true;
	m_NextNodeId++;
...
int NewNavGraph::findPath(NavGraphNode *from, NavGraphNode *to, std::list<
		NavGraphNode> &path, double *cost) {
	if (from == 0) {
	...
	if (!from->findPath(to, c)) {
		CureCERR(30) << "Found no path from " << *from << " to " << *to
				<< std::endl;
		return -1;
	}
...
int NewNavGraph::findPath(NavGraphNode *from, int toNodeId, std::list<
		NavGraphNode> &path, double *cost) {
	NavGraphNode *to = getNode(toNodeId);
	if (to == 0) {
		CureCERR(20) << "findPath(" << from->getId() << "," << toNodeId
				<< ") Could not find to node\n";
		return -2;
	}
...
int NewNavGraph::findPath(int fromNodeId, NavGraphNode *to, std::list<
		NavGraphNode> &path, double *cost) {
	NavGraphNode *from = getNode(fromNodeId);
	if (from == 0) {
		CureCERR(20) << "findPath(" << fromNodeId << "," << to->getId()
				<< ") Could not find from node\n";
		return -2;
	}
...
int NewNavGraph::findPath(int fromNodeId, int toNodeId,
		std::list<NavGraphNode> &path, double *cost) {
	NavGraphNode *from = getNode(fromNodeId);
	if (from == 0) {
		CureCERR(20) << "findPath(" << fromNodeId << "," << toNodeId
				<< ") Could not find from node\n";
		return -2;
	}
...
int NewNavGraph::findPath(NavGraphNode *from, double xG, double yG, double aG,
		std::list<NavGraphNode> &path, double *cost) {
	NavGraphNode *to = getClosestNode(xG, yG, aG, m_MaxDist);
	if (to == 0) {
		CureCERR(30) << "No node within " << m_MaxDist << "m from desired goal (x="
	...
	if (hypot(path.back().getX() - xG, path.back().getY() - yG) > 0.001) {
		...
		NavGraphNode tempGoal("TEMPGOAL", -42, xG, yG, aG,
...
int NewNavGraph::findPath(double xS, double yS, double aS, NavGraphNode *to,
		std::list<NavGraphNode> &path, double *cost) {
	NavGraphNode *from = getClosestNode(xS, yS, aS, m_MaxDist);
	if (from == 0) {
		CureCERR(30) << "No node within " << m_MaxDist
				<< "m from desired start (x=" << xS << " y=" << yS
				<< "), failed findPath\n";
		return -2;
	}
...
int NewNavGraph::findPath(double xS, double yS, double aS, double xG,
		double yG, double aG, std::list<NavGraphNode> &path, double *cost) {
	NavGraphNode *from = getClosestNode(xS, yS, aS, m_MaxDist);
	if (from == 0) {
		CureCERR(30) << "No node within " << m_MaxDist
				<< "m from desired start (x=" << xS << " y=" << yS
...
int NewNavGraph::deleteNode(int id) {
	return deleteNode(getNode(id));
}

int NewNavGraph::deleteNode(const std::string &name) {
	return deleteNode(getNode(name));
}

int NewNavGraph::deleteNode(NavGraphNode *node) {
	if (node == 0) {
	...
	for (std::list<NavGraphEdge*>::iterator nei = node->m_Edges.begin(); nei
			!= node->m_Edges.end(); nei++) {
		NavGraphNode *neighbor = (*nei)->getNext(node);
		if (neighbor) {
			neighbor->disconnectFrom(node);

			// Now also remove the edge from the list
			for (std::list<NavGraphEdge>::iterator ei = m_Edges.begin(); ei
					!= m_Edges.end(); ei++) {
	...
	if (!removed) {
		CureCERR(20) << "WARNING: Could not find node to remove " << *node
				<< std::endl;
	}
...
int NewNavGraph::addGatewayNode(double x, double y, double a, double width) {
	bool added = false;
	int ret = 0;
	...
	if (m_CurrNode != 0) {
		if (m_CurrNode->getType() == NavGraphNode::NODETYPE_GATEWAY) {
			...
			// node
			for (std::list<NavGraphEdge*>::iterator ei = m_CurrNode->m_Edges.begin(); !added
					&& ei != m_CurrNode->m_Edges.end(); ei++) {
				NavGraphNode *next = (*ei)->getNext(m_CurrNode);
				if (next != 0) {
					if (next->getType() == NavGraphNode::NODETYPE_GATEWAY && hypot(y
							- next->getY(), x - next->getX()) < 2.0 * m_NodeDist) {
						CureCERR(40) << "Already close to gateway node " << *next << "!!\n";
						return 1;
					}
				}
			}
			...
			if (m_CurrNode->getName() == "-" && m_CurrNode->getType()
					== NavGraphNode::NODETYPE_NODE && hypot(y - m_CurrNode->getY(), x
					- m_CurrNode->getX()) < 0.5 * m_NodeDist) {
			...
			for (std::list<NavGraphEdge*>::iterator ei = m_CurrNode->m_Edges.begin(); ei
					!= m_CurrNode->m_Edges.end(); ei++) {
				NavGraphNode *next = (*ei)->getNext(m_CurrNode);
...
int NewNavGraph::turnGatewayIntoNode(int id, bool uniteAreaIds, long idToKeep) {
	...
	if (gi == m_Gateways.end()) {
		CureCERR(20) << "WARNING: Did not find gateway with id " << g->getId()
				<< std::endl;
		return 1;
	}
	...
	if (idToKeep >= 0) {
		if (idToKeep == aid1) {
			...
			CureCERR(20) << "WARNING: idToKeep=" << idToKeep
					<< " does not match any of the two ids " << aid1 << " " << aid2
					<< std::endl;
		}
	} else {
...
int NewNavGraph::replaceNode(NavGraphNode *origNode, NavGraphNode *newNode) {
	if (!origNode) {
	...
	if (origNode == newNode) {
		CureCERR(30) << "No need to replace nodes, they are the same " << *newNode
				<< std::endl;
		return 0;
	}
	...
	for (std::list<NavGraphEdge*>::iterator nei = origNode->m_Edges.begin(); nei
			!= origNode->m_Edges.end(); nei++) {
	...
	for (std::list<NavGraphNode*>::iterator ni = neighbors.begin(); ni
			!= neighbors.end(); ni++) {
		connectNodes(newNode->getId(), (*ni)->getId(), hypot(newNode->getY()
				- (*ni)->getY(), newNode->getX() - (*ni)->getX()));
	}

	return 0;
}

int NewNavGraph::makeGatewayOutOfNode(NavGraphNode *node, double x, double y,
	...
	CureCERR(40) << "\nShifting existing node " << *node << " and turning it "
			<< "into gateway node at " << "x=" << x << " y=" << y << " a=" << a
			<< "\n";
	...
	if (node->m_Edges.size() > 1) {
		...
		int nBehind = 0;
		for (std::list<NavGraphEdge*>::iterator ei = node->m_Edges.begin(); ei
				!= node->m_Edges.end(); ei++) {
			...
			if (next) {
				...
				if (da > M_PI_2) {
					CureCERR(50) << "Behind[" << nBehind << "] " << *next << std::endl;
					behind[nBehind++] = next;
				}
			}
		}
		...
		if (nBehind > 0) {
			...
			for (int i = 0; i < nBehind; i++) {
				...
				for (std::list<NavGraphEdge*>::iterator ei = behind[i]->m_Edges.begin(); ei
						!= behind[i]->m_Edges.end(); ei++) {
	...
	NavGraphGateway *g = new NavGraphGateway(node->getName(), node->getId(), x,
			y, a, node->getMaxSpeed(), width);
...
bool NewNavGraph::foundPathToNode(NavGraphNode *goal, NavGraphNode *node) {
	if (node == 0)
		return false;
	...
	for (std::list<NavGraphEdge*>::iterator ei = node->m_Edges.begin(); ei
			!= node->m_Edges.end(); ei++) {
		NavGraphNode *next = (*ei)->getNext(node);
		if (next && node->costG == 0 && next->getType()
				!= NavGraphNode::NODETYPE_GATEWAY) {
			if (next == goal)
				return true;
			else
				return foundPathToNode(goal, next);
		}
	}
...
void NewNavGraph::spreadNewAreaId(long areaId, NavGraphNode *node) {
	if (node == 0)
		return;
...
int NewNavGraph::makeNodeOutOfGateway(NavGraphGateway *g, double x, double y,
		double a) {
	if (g == 0) {
	...
	CureCERR(40) << "\nTurning gateway " << *g << " into normal node at " << "x="
			<< x << " y=" << y << " a=" << a << "\n";

	NavGraphNode *node = new NavGraphNode(g->getName(), g->getId(), x, y, a,
			g->getMaxSpeed());
...
std::list<NavGraphGateway*>::iterator NewNavGraph::findConnectingGateway(
		long aid1, long aid2) {
	// Loop over all gateways
	for (std::list<NavGraphGateway*>::iterator gi = m_Gateways.begin(); gi
			!= m_Gateways.end(); gi++) {
...
int NewNavGraph::replaceGatewaysConnecting(long aid1, long aid2) {
	std::list<NavGraphGateway*>::iterator gi = findConnectingGateway(aid1, aid2);
	if (gi == m_Gateways.end()) {
		CureCERR(20) << "WARNING: Did not find gateway connecting " << aid1
				<< " and " << aid2 << std::endl;
		return 1;
	}
...
int NewNavGraph::mergeAreaIds(long aid1, long aid2) {
	for (std::list<NavGraphNode*>::iterator ni = m_Nodes.begin(); ni
			!= m_Nodes.end(); ni++) {
		if ((*ni)->getAreaId() == aid2)
			(*ni)->setAreaId(aid1);
	}
...
int NewNavGraph::connectNodes(int id1, int id2, double cost) {
	NavGraphNode *n1 = getNode(id1);
	NavGraphNode *n2 = getNode(id2);

	if (n1 == 0 || n2 == 0) {
		CureCERR(20) << "Cannot connect nodes==0, n1=" << n1 << " n2=" << n2
				<< "\n";
		return 1;
	}
	...
	for (std::list<NavGraphEdge>::iterator ei = m_Edges.begin(); ei
			!= m_Edges.end(); ei++) {
		if ((ei->getNodeId1() == n1->getId() && ei->getNodeId2() == n2->getId())
				|| (ei->getNodeId1() == n2->getId() && ei->getNodeId2() == n1->getId())) {
	...
	m_Edges.push_back(NavGraphEdge(n1, n2, hypot(n1->getY() - n2->getY(),
			n1->getX() - n2->getX())));
...
int NewNavGraph::markPose(double x, double y, double a, const std::string &tag) {
	if (tag == "") {
...
int NewNavGraph::loadGraph(const std::string &filename) {
	std::fstream fs;
...
int NewNavGraph::saveGraph(const std::string &filename) {
	std::fstream fs;
...
int NewNavGraph::addNodeToNodeList(int id, int areaId, double x, double y,
		double a, const std::string &name, double maxspeed) {
	m_Nodes.push_back(new NavGraphNode(name, id, x, y, a, maxspeed));
	m_Nodes.back()->setAreaId(areaId);

	if (id >= m_NextNodeId)
		m_NextNodeId = id + 1;
	if (areaId > m_LastAreaId)
		m_LastAreaId = areaId;

	return 0;
}

int NewNavGraph::addDoorToNodeList(int id, int areaId, double x, double y,
		double a, double width, const std::string &name, double maxspeed) {
	NavGraphGateway *g = new NavGraphGateway(name, id, x, y, a, maxspeed, width);
	m_Nodes.push_back(g);
	m_Gateways.push_back(g);
	m_Nodes.back()->setAreaId(areaId);
...
int NewNavGraph::addEdgeToEdgeList(int id1, int id2) {
	if (getEdge(id1, id2)) {
		return 0;
	}
...
int NewNavGraph::readFromStream(std::istream &is) {
	std::string line, key;
	int n;
		...
		for (int i = 0; i < n; i++) {
			...
			if (istr >> type >> id >> x >> y >> a) {
				...
				if (type == NavGraphNode::NODETYPE_GATEWAY) {
					...
					if (!(istr >> width)) {
						width = 1.0;
						CureCERR(30) << "No width for gateway id=" << id << " assuming "
								<< width << "m\n";
					}
...
int NewNavGraph::writeToStream(std::ostream &os) {
	// Write NODES
	os << "NODES " << m_Nodes.size() << std::endl;
	for (std::list<NavGraphNode*>::iterator ni = m_Nodes.begin(); ni
			!= m_Nodes.end(); ni++) {
		os << (*ni)->getType() << " " << (*ni)->getId() << " " << (*ni)->getX()
				<< " " << (*ni)->getY() << " " << (*ni)->getTheta() << " "
				<< (*ni)->getName() << " " << (*ni)->getAreaId();
	...
	for (std::list<NavGraphEdge>::iterator ei = m_Edges.begin(); ei
			!= m_Edges.end(); ei++) {
		os << ei->getNodeId1() << " " << ei->getNodeId2() << " " << ei->getCost()
				<< std::endl;
	}
...
std::list<NavGraphGateway*> NewNavGraph::getGatewaysInArea(const long aid) {
	std::list<NavGraphGateway*> ret;

	for (std::list<Cure::NavGraphGateway*>::iterator gi = m_Gateways.begin(); gi
			!= m_Gateways.end(); gi++) {
		...
		if (!inThisArea) {
			for (std::list<Cure::NavGraphEdge*>::iterator ei = n->m_Edges.begin(); ei
					!= n->m_Edges.end(); ei++) {
...
void NewNavGraph::displayRL(RoboLookProxy *rlp, int env, bool clearStars,
		bool clearLines, bool showDoorFrames) {
	if (!rlp)
		return;
...
void NewNavGraph::addEventListener(NavGraphEventListener *l) {
...
void NewNavGraph::delEventListener(NavGraphEventListener *l) {
	std::list<NavGraphEventListener*>::iterator li;
...
void NewNavGraph::setAutoFix(bool autofix) {
	m_AutoFixGateway = autofix;
}

bool NewNavGraph::setAreaName(int areaId, const std::string &name) {
...
bool NewNavGraph::setAreaClass(int areaId, int c) {
	std::map<int, NavArea>::iterator ai = m_Areas.find(areaId);
	if (ai != m_Areas.end()) {
...
std::string NewNavGraph::getAreaName(int areaId) const {
...
int NewNavGraph::getAreaClass(int areaId) {
	...
	for (std::list<NavGraphNode*>::iterator ni = m_Nodes.begin(); ni
		...
		if ((*ni)->getAreaId() == areaId) {
			if (minClass <= (*ni)->getAreaClass() && (*ni)->getAreaClass()
					<= maxClass) {
	...
	if (maxVotes > 0) {
		CureCERR(30) << "Best vote for area " << areaId << " is " << bestClass
				<< std::endl;
	} else {
...
bool NewNavGraph::setGatewayStatesToUnknown() {
	for (std::list<NavGraphGateway*>::iterator gi = m_Gateways.begin(); gi
			!= m_Gateways.end(); gi++) {
...
void NewNavGraph::notifyChangedArea(long areaId) {
...
void NewNavGraph::notifyChangedCurrentNode(long fromId, long toId) {
	CureCERR(60) << "Notifying that the current noded changed from " << fromId
			<< " to " << toId << std::endl;
	std::list<NavGraphEventListener*>::iterator li;
	for (li = m_EventListeners.begin(); li != m_EventListeners.end(); li++) {
...
operator<<(std::ostream &os, const NewNavGraph &a) {
	...
	os << "=================== Nodes =======================\n";
	for (std::list<NavGraphNode*>::const_iterator ni = a.m_Nodes.begin(); ni
			!= a.m_Nodes.end(); ni++) {
		os << **ni << std::endl;
	}
	os << "=================== Edges =======================\n";
	for (std::list<NavGraphEdge>::const_iterator ei = a.m_Edges.begin(); ei
			!= a.m_Edges.end(); ei++) {
		os << *ei << std::endl;
	}
	return os;
