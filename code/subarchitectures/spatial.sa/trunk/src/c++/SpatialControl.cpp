...
double SpatialControl::MapServer::getPathLength(double x1, double y1,
    double x2, double y2, const Ice::Current &_context) {
  return m_pOwner->getPathLength(x1, y1, x2, y2);
}

int SpatialControl::MapServer::findClosestNode(double x, double y,
    const Ice::Current &_context) {
  return m_pOwner->findClosestNode(x, y);
}

SpatialData::NodeHypothesisSeq SpatialControl::MapServer::refreshNodeHypothesis(
    const Ice::Current &_context) {
  return m_pOwner->refreshNodeHypothesis();
}

void SpatialControl::MapServer::addConnection(int node1id, int node2id,
    const Ice::Current &_context) {
  return m_pOwner->addConnection(node1id, node2id);
}
void SpatialControl::MapServer::removeConnection(int node1id, int node2id,
    const Ice::Current &_context) {
  return m_pOwner->removeConnection(node1id, node2id);
}

SpatialData::HeightMap SpatialControl::MapServer::getHeightMap(
    const Ice::Current &_context) {
...
SpatialControl::SpatialControl() :
  NewNavController(m_NavGraph, m_LMap, this), NewNavControllerEventListener(
      "SpatialControl") {
  m_CurrPerson = -1;
  m_CurrPersonWMid = "";
...
void SpatialControl::connectPeekabot() {
  try {
    ...
    if (m_usePeekabot) {
      ...
      m_OdomPoseProxy.add(m_PeekabotClient, "Odom_pose_SC",
...
void SpatialControl::CreateGridMap() {
  ...
  if (m_showExpandedMap) {
    ...
    m_ProxyGridMapExpanded2.add(m_PeekabotClient, "grid_map_expanded2",
        cellSize, peekabot::REPLACE_ON_CONFLICT);
...
void SpatialControl::moveSimulatedPTZ(double pan) {
  // Instantly "move PTZ"
 ...
 */
void SpatialControl::startMovePanTilt(double pan, double tilt, double tolerance) {
 ...
 */
void SpatialControl::overwrittenPanTiltCommand(
 ...
 */
void SpatialControl::checkPTZPose(const ptz::PTZPose &pose) {
  ...
  if (pose.tilt < -M_PI / 4 + 0.05 && pose.tilt > -M_PI / 4 - 0.05 && pose.pan
      < 0.05 && pose.pan > -0.05) {
...
void SpatialControl::LoadGridMap(std::string filename) {
  ...
  m_lgm = new Cure::LocalGridMap<unsigned char>(sz, 0.05, '2',
...
void SpatialControl::LoadHeightMap(std::string filename) {
  ...
  m_lgmKH = new Cure::LocalGridMap<double>(sz, 0.05, FLT_MAX,
...
void SpatialControl::configure(const map<string, string>& _config) {
  ...
  if (_config.find("--pcserver") != _config.end()) {
    ...
    map<string, string>::const_iterator it = _config.find(
        "--min-obstacle-height");
    if (it != _config.end()) {
  ...
  if (_config.find("--show-binary-map") != _config.end()) {
    m_displayBinaryMap = new Cure::XDisplayLocalGridMap<unsigned char>(
        *m_binaryMap);
  } else {
  ...
  registerIceServer<SpatialData::MapInterface, SpatialData::MapInterface> (
      new MapServer(this));
}
...
void SpatialControl::start() {
  ...
  addChangeFilter(createLocalTypeFilter<NavData::MapUpdateStatusChange> (
      cdl::ADD), new MemberFunctionChangeReceiver<SpatialControl> (this,
  ...
  addChangeFilter(createLocalTypeFilter<NavData::VisualExplorationCommand> (
      cdl::ADD), new MemberFunctionChangeReceiver<SpatialControl> (this,
      &SpatialControl::newVisualExplorationCommand));

  addChangeFilter(createLocalTypeFilter<NavData::InhibitNavControl> (
      cdl::DELETE), new MemberFunctionChangeReceiver<SpatialControl> (this,
  ...
  addChangeFilter(createGlobalTypeFilter<ptz::SetPTZPoseCommand> (
      cdl::OVERWRITE), new MemberFunctionChangeReceiver<SpatialControl> (this,
  ...
  addChangeFilter(
      createLocalTypeFilter<NavData::InternalNavCommand> (cdl::ADD),
      new MemberFunctionChangeReceiver<SpatialControl> (this,
...
void SpatialControl::changeCurrentCommandCompletion(
    const NavData::Completion &value, const NavData::StatusError &status) {
  ...
  overwriteWorkingMemory<NavData::InternalNavCommand> (m_CurrentCmdAddress.id,
      cmd);
 ...
 */
void SpatialControl::blitHeightMap(Cure::LocalGridMap<unsigned char>& lgm,
    Cure::LocalGridMap<double>* heightMap, int minX, int maxX, int minY,
    int maxY, double obstacleMinHeight, double obstacleMaxHeight) {
  int xi, yi;
  ...
  for (yi = minY; yi <= maxY; yi++) {
    for (xi = minX; xi <= maxX; xi++) {
      if ((*heightMap)(xi, yi) > obstacleMinHeight && (*heightMap)(xi, yi)
          < obstacleMaxHeight) {
        lgm(xi, yi) = '1';
  ...
  for (yi = minY + 1; yi < maxY; yi++) {
    for (xi = minX + 1; xi < maxX; xi++) {
      if (lgm(xi, yi) == '1' && lgm(xi - 1, yi) != '1' && lgm(xi + 1, yi)
          != '1' && lgm(xi - 1, yi - 1) != '1' && lgm(xi + 1, yi - 1) != '1'
          && lgm(xi - 1, yi + 1) != '1' && lgm(xi + 1, yi + 1) != '1' && lgm(
          xi, yi - 1) != '1' && lgm(xi, yi + 1) != '1') {
        lgm(xi, yi) = '0';
      }
    }
  }
...
class ComparePoints {
public:
  bool operator()(const PointCloud::SurfacePoint& lhs,
      const PointCloud::SurfacePoint& rhs) const {
...
void SpatialControl::updateGridMaps() {
  ...
  while (!m_LScanQueue.empty()) {
    ...
    if (status == 0) {
      ...
      if (m_usePeekabot) {
        ...
        m_ProxyScan.set_pose(lpW.getX(), lpW.getY(), lpW.getZ(),
            lpW.getTheta(), 0, 0);
      }
      {
        //        SCOPED_TIME_LOG;
        tmp_glrt.addScan(scan, lpW, m_MaxExplorationRange);
        tmp_lgm->setValueInsideCircle(LscanPose.getX(), LscanPose.getY(), 0.55
            * Cure::NewNavController::getRobotWidth(), '0');
      }
  ...
  if (laserMinX == INT_MAX || laserMinY == INT_MAX || laserMaxX == INT_MIN
      || laserMaxY == INT_MIN) {
    delete tmp_lgm;
    return;
  }
  ...
  if (m_lgm->worldCoords2Index(laserMinX, laserMinY, laserMinXi, laserMinYi)
      != 0) {
    laserMinXi = -m_lgm->getSize() + 1;
    laserMinYi = -m_lgm->getSize() + 1;
  }
  if (m_lgm->worldCoords2Index(laserMaxX, laserMaxY, laserMaxXi, laserMaxYi)
      != 0) {
  ...
  if (m_UsePointCloud) {
    ...
    double lastptztime = m_lastPtzNavPoseCompletion.s
        + m_lastPtzNavPoseCompletion.us / 1000000.0;
    bool hasScanPose;
    {
      IceUtil::Mutex::Lock lock(m_PPMutex);

      hasScanPose = m_TOPP.getPoseAtTime(Cure::Timestamp(frameTime.s,
          frameTime.us), scanPose) == 0;
    }
    ...
    if (hasScanPose && m_ptzInMappingPose && time - lastptztime
      ...
      }
      {
        ...
        for (PointCloud::SurfacePointSeq::iterator it = points.begin(); it
          ...
          if (m_lgmKH->worldCoords2Index(pX, pY, xi, yi) == 0) {
            ...
            bool oldObstacle = (lgmKH(xi, yi) > m_obstacleMinHeight && lgmKH(
                xi, yi) < m_obstacleMaxHeight);
            bool newObstacle = (pZ > m_obstacleMinHeight && pZ
                < m_obstacleMaxHeight);

            bool updateHeightMap = true;

            double alpha = m_currentPTZPose.tilt + vFOV / 2;
            double fovZ = kinectZ + (it->p.x * cos(m_currentPTZPose.pan)
                + it->p.y * sin(m_currentPTZPose.pan)) * tan(alpha);
            ...
            double nx = it->p.x * cos(-m_currentPTZPose.pan) - it->p.y * sin(
                -m_currentPTZPose.pan);
            double ny = it->p.x * sin(-m_currentPTZPose.pan) + it->p.y * cos(
                -m_currentPTZPose.pan);
            if ((nx < m_minKinectX) || (nx > m_maxKinectX) || (m_KinectK * nx
                - ny < 0) || (-m_KinectK * nx - ny > 0))
              updateHeightMap = false;
    ...
    if (m_usePeekabot) {
      ...
      if (m_PBDisplayMutex.tryLock()) {
        if (!m_bDoPBSync) {
          ...
          for (int yi = min(pointcloudMinYi, laserMinYi) + 1; yi < max(
              pointcloudMaxYi, laserMaxYi); yi++) {
            for (int xi = min(pointcloudMinXi, laserMinXi) + 1; xi < max(
                pointcloudMaxXi, laserMaxXi); xi++) {
              double xr, yr;
              m_lgm->index2WorldCoords(xi, yi, xr, yr);
              if ((*tmp_lgm)(xi, yi) == '0')
                cells.add(xr, yr, 0);
          ...
          if (m_show3Dobstacles) {
            ...
            for (int yi = pointcloudMinYi; yi <= pointcloudMaxYi; yi++) {
              for (int xi = pointcloudMinXi; xi <= pointcloudMaxXi; xi++) {
                ...
                if ((*tmp_lgm)(xi, yi) == '1') {
                  if ((*m_lgmKH)(xi, yi) != FLT_MAX) {
                    for (double zi = 0; zi <= floor((*m_lgmKH)(xi, yi) / 0.05)
                        * 0.05; zi += 0.05) {
                      cells1.set_cell(xr, yr, zi, 1);
                    }
                  }
    ...
    for (int yi = minY; yi <= maxY; yi++) {
      for (int xi = minX; xi <= maxX; xi++) {
        ...
        if ((nx > m_maxKinectX) || (m_KinectK * nx - ny < 0) || (-m_KinectK
            * nx - ny > 0)) {
          if ((*tmp_lgm)(xi, yi) != '1')
            (*tmp_lgm)(xi, yi) = '2';
        } else {
  ...
  /* Update the nav map */
  {
    ...
    //    SCOPED_TIME_LOG;
    m_LMap.clearMap();
    tmp_lgm->setValueInsideCircle(currPose.getX(), currPose.getY(), 0.55
        * Cure::NewNavController::getRobotWidth(), '0');
...
void SpatialControl::runComponent() {
  ...
  while (isRunning()) {
    ...
    if (m_visualExplorationOngoing && m_waitingForPTZCommandID == "") {
      ...
      debug("last Point cloud time: %f", m_lastPointCloudTime.s
          + m_lastPointCloudTime.us * 1e-6);
      debug("last PTU time: %f", m_lastPtzNavPoseCompletion.s
          + m_lastPtzNavPoseCompletion.us * 1e-6);
      if (m_lastPointCloudTime > m_lastPtzNavPoseCompletion) {
        ...
        if (m_visualExplorationPhase == 2) {
          ...
          try {
            NavData::VisualExplorationCommandPtr cmd = getMemoryEntry<
                NavData::VisualExplorationCommand> (m_visualExplorationCommand);
...
void SpatialControl::newNavGraph(const cdl::WorkingMemoryChange &objID) {
  NavData::NavGraphPtr oobj = getMemoryEntry<NavData::NavGraph> (objID.address);
  ...
  if (ng != 0) {
    ...
    for (unsigned int i = 0; i < ng->fNodes.size(); i++) {
      ...
      if (ng->fNodes[i]->gateway) { // add gateway node
        ...
        m_NavGraph.addDoorToNodeList(nodeId, areaId, x, y, theta, width,
            areaType, maxSpeed);
      }

      else { // add ordinary node

        m_NavGraph.addNodeToNodeList(nodeId, areaId, x, y, theta, areaType,
            maxSpeed);
    ...
    if (m_ready)
      debug("m_ready is true");
    else
      debug("m_ready is false");
  }
  //  m_Mutex.unlock();
}
...
void SpatialControl::deletePersonData(const cdl::WorkingMemoryChange &objID) {
  ...
  for (std::vector<SpatialControl::PersonData>::iterator pi = m_People.begin(); pi
      != m_People.end(); pi++, i++) {
 ...
 */
void SpatialControl::newVisualExplorationCommand(
    const cdl::WorkingMemoryChange &objID) {
  log("Received new VisualExplorationCommand");
  try {
    NavData::VisualExplorationCommandPtr obj = getMemoryEntry<
        NavData::VisualExplorationCommand> (objID.address);
...
void SpatialControl::newRobotPose(const cdl::WorkingMemoryChange &objID) {
  ...
  //FIXME
  m_SlamRobotPose.setTime(Cure::Timestamp(oobj->time.s, oobj->time.us));
...
void SpatialControl::newNavCtrlCommand(const cdl::WorkingMemoryChange &objID) {
  ...
  if (oobj != 0) {
    ...
    ExplorationConfinedByGateways
        = oobj->getData()->SetExplorerConfinedByGateways;
    ...
    } else if (m_commandType == NavData::lGOTOXY || m_commandType
    ...
    if (m_commandType == NavData::lFOLLOWPERSON) {
      ...
      if (m_CurrPerson >= 0) {
        ...
        sprintf(buf, "Found person to follow at xW=%.2fm yW=%.2fm (d=%.2fm)\n",
            m_People[m_CurrPerson].m_data->y, m_People[m_CurrPerson].m_data->x,
            minD);
        log(buf);
      } else {
...
void SpatialControl::receiveOdometry(const Robotbase::Odometry &castOdom) {
  ...
  //the new RobotPose arrives
  {
    ...
    if (m_usePeekabot) {
      m_OdomPoseProxy.set_pose(m_CurrPose.getX(), m_CurrPose.getY(), 2.3,
          m_CurrPose.getTheta(), 0, 0);
    }
...
void SpatialControl::processOdometry(Cure::Pose3D cureOdom) {
...

  {
    ...
    long int diff = (newTime.s - oldTime.s) * 1000000l + (newTime.us
        - oldTime.us);
    oldTime = newTime;
    if (diff > 1500000) {
  ...
  if (m_ready || m_bNoNavGraph) { // have to get a first nav graph 
...

    {
      ...
      if (m_taskStatus == TaskFinished) {
      ...
      } else if (m_taskStatus == NewTask && (m_commandType
          == NavData::lSTOPROBOT || m_waitingForPTZCommandID == "")) {
        ...
        if (m_waitingForMapSave) {
          ...
          if ((m_commandType == NavData::lGOTOXYA)) {
            ...
            ret = Cure::NewNavController::gotoXYA(currentTaskId, m_commandX,
                m_commandY, m_commandTheta);
          }
          ...
          // GOTO_XY_ROUGH
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::gotoXY(currentTaskId, m_commandX,
                m_commandY);
          }
          ...
          // GOTO_POLAR
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::gotoPolar(currentTaskId,
                m_commandTheta, m_commandR);
          }
          ...
          // GOTO_AREA
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::gotoArea(currentTaskId,
                m_commandAreaId);
          }
          ...
          // GOTO_NODE
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::gotoNode(currentTaskId,
                m_commandNodeId);
          }
          ...
          // ROTATE_REL
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::rotateRel(currentTaskId,
                m_commandTheta);
          ...
          // ROTATE_ABS
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::rotateAbs(currentTaskId,
                m_commandTheta);
          ...
          // BACK_OFF
            ...
            Cure::NewNavController::setPositionToleranceFinal( m_TolPos);
            Cure::NewNavController::setOrientationTolerance( m_TolRot);
            ret = Cure::NewNavController::backOff(currentTaskId,
                m_commandDistance);
          ...
          if (m_commandType == NavData::lSTOPROBOT) {
            ...
            changeCurrentCommandCompletion(NavData::SUCCEEDED, NavData::UNKNOWN);
            m_taskStatus = NothingToDo;
            ...
            if ((m_commandType == NavData::lFOLLOWPERSON)) {
              ...
              if (m_CurrPerson < 0 || m_CurrPerson > (int) m_People.size() - 1) {
                ...
                ret = Cure::NewNavController::followPerson(currentTaskId,
                    ...
                    m_People[m_CurrPerson].m_data->speed, 0);
 ...
 */
void SpatialControl::mapUpdaterLoop() {
  while (isRunning()) {
    {
      ...
      if (m_saveLgm) {
        ...
        if (m_taskStatus == NothingToDo && m_mapUpdatesSinceLastSave >= 60) {
...
void SpatialControl::execCtrl(Cure::MotionAlgorithm::MotionCmd &cureCmd) {
  ...
  debug("execCtrl sending (v=%fm/s,w=%frad/s) to RobotServer", cmd.speed,
      cmd.rotspeed);
...
void SpatialControl::getExpandedBinaryMap(
    const Cure::LocalGridMap<unsigned char>* gridmap, Cure::BinaryMatrix &map,
    double k, double k2) {
  ...
  for (int x = -gridmapSize; x < gridmapSize; ++x) {
    for (int y = -gridmapSize; y < gridmapSize; ++y) {
      if ((*gridmap)(x, y) == '1') {
        ...
        ungrown_map.setBit(x + gridmapSize, y + gridmapSize, false);
    }
  }
  // Grow each occupied cell to account for the size of the robot.
  ungrown_map.growInto(map, k * 0.5 * Cure::NewNavController::getRobotWidth()
      / m_lgm->getCellSize());
  ...
  map.growInto(ungrown_map, k2 * 0.5 * Cure::NewNavController::getRobotWidth()
      / m_lgm->getCellSize());
  map = ungrown_map;
  //  if(lockMapsMutex) {
  //    m_MapsMutex.unlock();
...
double SpatialControl::getPathLength(double x1, double y1, double x2, double y2) {
  ...
  double dist = map.path(x1i + m_lgm->getSize(), y1i + m_lgm->getSize(), x2i
      + m_lgm->getSize(), y2i + m_lgm->getSize(), path, maxDist);
...
int SpatialControl::findClosestNode(double x, double y) {
  ...
  while (closestNodeId == -1) {
    for (vector<NavData::FNodePtr>::iterator nodeIt = nodes.begin(); nodeIt
        != nodes.end(); ++nodeIt) {
      try {

        int nodexi, nodeyi;
        if (m_lgm->worldCoords2Index((*nodeIt)->x, (*nodeIt)->y, nodexi, nodeyi)
            != 0)
          continue;
 ...
 */
bool SpatialControl::check_point(int x, int y,
    vector<NavData::FNodePtr> &nodes,
    vector<SpatialData::NodeHypothesisPtr> &non_overlapped_hypotheses,
    Cure::BinaryMatrix& map, Cure::BinaryMatrix& map1, int &closestNodeId) {
  closestNodeId = -1;
  if (map(x + m_lgm->getSize(), y + m_lgm->getSize()) == false) {
    ...
    for (vector<SpatialData::NodeHypothesisPtr>::iterator extantHypIt =
        non_overlapped_hypotheses.end(); extantHypIt
        != non_overlapped_hypotheses.begin();) {
      SpatialData::NodeHypothesisPtr extantHyp = *(--extantHypIt);
      double dist2sq = (xr - extantHyp->x) * (xr - extantHyp->x) + (yr
          - extantHyp->y) * (yr - extantHyp->y);
      if (dist2sq < 1 * 1) {
        counter1++;
        return false;
      }
    ...
    while (closestNodeId == -1) {
      ...
      for (vector<NavData::FNodePtr>::iterator nodeIt = nodes.begin(); nodeIt
          != nodes.end(); ++nodeIt) {
        try {
          double dist2sq = (xr - (*nodeIt)->x) * (xr - (*nodeIt)->x) + (yr
              - (*nodeIt)->y) * (yr - (*nodeIt)->y);
          if (dist2sq < m_min_sep_dist * m_min_sep_dist) {
            ...
            int nodexi, nodeyi;
            if (m_lgm->worldCoords2Index((*nodeIt)->x, (*nodeIt)->y, nodexi,
                nodeyi) != 0)
              continue;
...
SpatialData::NodeHypothesisSeq SpatialControl::refreshNodeHypothesis() {
  ...
  getMemoryEntries<SpatialData::NodeHypothesis> (hypotheses);
  {
    ...
    for (vector<SpatialData::NodeHypothesisPtr>::iterator extantHypIt =
      ...
      //CHECK IF OVERLAPPED
      int hypxi, hypyi;
      if (m_lgm->worldCoords2Index(extantHyp->x, extantHyp->y, hypxi, hypyi)
          != 0)
        continue;
      if (map(hypxi + m_lgm->getSize(), hypyi + m_lgm->getSize()) == true)
        overlapped = true;

      for (vector<NavData::FNodePtr>::iterator nodeIt = nodes.begin(); (nodeIt
          != nodes.end()) && !overlapped; ++nodeIt) {
        try {
          double dist2sq = (extantHyp->x - (*nodeIt)->x) * (extantHyp->x
              - (*nodeIt)->x) + (extantHyp->y - (*nodeIt)->y) * (extantHyp->y
              - (*nodeIt)->y);
          if (dist2sq < m_min_sep_dist * m_min_sep_dist) {
            if ((!extantHyp->gateway) || (extantHyp->gateway
                && (*nodeIt)->gateway)) {
              overlapped = true;
  ...
  }
  {
    ...
    for (vector<SpatialData::NodeHypothesisPtr>::iterator extantHypIt =
        ...
        != overlapped_hypotheses.end(); extantHypIt++) {
      SpatialData::NodeHypothesisPtr extantHyp = *extantHypIt;
      int hypxi;
      int hypyi;
      if (m_lgm->worldCoords2Index(extantHyp->x, extantHyp->y, hypxi, hypyi)
      ...
      for (int i = 0; i < 30; i++) {
        ...
        int r = round(m_maxMovePlaceholderRadius / 30 * i
            / m_lgm->getCellSize());
        ...
        for (int j = 0; j < 36; j++) {
          ...
          if (check_point(x, y, nodes, ret, map, map1, originNodeID)) {
            SpatialData::NodeHypothesisPtr new_nh =
  ...
  //2. LOOP POINTS AROUND ROBOT
  {
    ...
    m_lgm->worldCoords2Index(currPose.getX(), currPose.getY(), robotxi, robotyi);
    {
      ...
      for (int i = 0; i < 40; i++) {
        double theta = (rand() % 360) * M_PI / 180;
        int r = round((m_maxNewPlaceholderRadius - m_minNewPlaceholderRadius)
            / 40 * i / m_lgm->getCellSize()) + round(m_minNewPlaceholderRadius
            / m_lgm->getCellSize());
 ...
 */
void SpatialControl::getBoundedMap(SpatialData::LocalGridMap &map,
    const Cure::LocalGridMap<unsigned char>* gridmap, double minx, double maxx,
  ...
  for (int x = minxi; x <= maxxi; ++x) {
    for (int y = minyi; y <= maxyi; ++y) {
      ...
      if (x < -sizeX + xiCenter || x > sizeX + xiCenter || y < -sizeY
          + yiCenter || y > sizeY + yiCenter || x < -lgmsize || x > lgmsize
          || y < -lgmsize || y > lgmsize) {
...
void SpatialControl::displayHelperLoop() {
