...
ObjectRelationManager::~ObjectRelationManager() {
  for (map<string, spatial::Object*>::iterator it = m_objectModels.begin(); it
      != m_objectModels.end(); it++) {
    delete it->second;
  }
}
...
void ObjectRelationManager::start() {
  ...
  addChangeFilter(createGlobalTypeFilter<VisionData::VisualObject> (
      cdl::OVERWRITE),
      new MemberFunctionChangeReceiver<ObjectRelationManager> (this,
          &ObjectRelationManager::newObject));

  addChangeFilter(
      createLocalTypeFilter<FrontierInterface::ObservedPlaneObject> (cdl::ADD),
      new MemberFunctionChangeReceiver<ObjectRelationManager> (this,
          &ObjectRelationManager::newPlaneObject));

  addChangeFilter(
      createLocalTypeFilter<FrontierInterface::ObservedPlaneObject> (
          cdl::OVERWRITE), new MemberFunctionChangeReceiver<
          ObjectRelationManager> (this, &ObjectRelationManager::newPlaneObject));
  ...
  addChangeFilter(
      createGlobalTypeFilter<FrontierInterface::ObjectPriorRequest> (cdl::ADD),
      new MemberFunctionChangeReceiver<ObjectRelationManager> (this,
  ...
  if (m_bDisplayPlaneObjectsInPB || m_bDisplayVisualObjectsInPB) {
    ...
    if (m_bDisplayPlaneObjectsInPB) {
      m_planeProxies.add(m_PeekabotClient, "plane_objects",
          peekabot::REPLACE_ON_CONFLICT);
    }
    if (m_bDisplayVisualObjectsInPB) {
      m_objectProxies.add(m_PeekabotClient, "visual_objects",
          peekabot::REPLACE_ON_CONFLICT);
    }
...
void ObjectRelationManager::newRobotPose(const cdl::WorkingMemoryChange &objID) {
  ...
  double deltaT = lastRobotPose->time.s + lastRobotPose->time.us * 0.000001
      - oldTime;
  ...
  if (momVel > m_standingStillThreshold) {
    ...
    if (m_bRecognitionIssuedThisStop) {
      ...
      for (std::set<string>::iterator it = m_lookForObjects.begin(); it
          != m_lookForObjects.end(); it++) {
        addTrackerCommand(VisionData::REMOVEMODEL, it->c_str());
      }
      for (std::map<std::string, SpatialObjectPtr>::iterator it =
    ...
    double diff = lastRobotPose->time.s + lastRobotPose->time.us * 0.000001
        - oldTime;
  ...
  while (!infile.eof()) {
    ...
    if (token == "nRooms") {
      ...
      while (strlen(line) > 0 && !infile.eof()) {
        ...
        is2 >> tmp;
        is2 >> b;
        if (a < 0 || a >= nRooms || b < 0 || b >= nRoomCategories || tmp
            != "is") {
...
void ObjectRelationManager::runComponent() {
  ...
  def.pdf = 0.0;
  SpatialGridMap::GridMap<SpatialGridMap::GridMapData> pdfMap(100, 100, 0.05,
      0.05, 0, 2.0, 0, 0, 0, def);
  ...
  while (isRunning()) {
    ...
    if (m_bDetectObjects) {
      ...
      if (!m_bRecognitionIssuedThisStop && m_timeSinceLastMoved
          > m_recognitionTimeThreshold) {
    ...
    if (m_bDetectObjects) {
      lockComponent();
      if (m_objectOnnessValues.size() > 0 || m_objectInnessValues.size() > 0) {
...
void ObjectRelationManager::newObject(const cast::cdl::WorkingMemoryChange &wmc) {
  m_updatedObjects.insert(wmc.address);
  debug("newObject called ");
}

void ObjectRelationManager::newObject(
    const cdl::WorkingMemoryAddress &wmAddress) {
  VisionData::VisualObjectPtr observedObject;
  string wmId = wmAddress.id;
  try {
    observedObject = getMemoryEntry<VisionData::VisualObject> (wmAddress);
  ...
  debug("Got VisualObject: %s (%f,%f,%f)", obsLabel.c_str(),
      observedObject->pose.pos.x, observedObject->pose.pos.y,
      observedObject->pose.pos.z);

  Pose3 pose = observedObject->pose;

  if (pose.pos.x > 100 || pose.pos.x < -100 || pose.pos.y > 100 || pose.pos.y
      < -100 || pose.pos.z > 100 || pose.pos.z < -100) {
    log(
        "Warning! Junk coordinates in visual object %s! (%f, %f, %f); Ignoring",
        observedObject->identLabels[0].c_str(), pose.pos.x, pose.pos.y,
        pose.pos.z);
    return;
  }
  //Get robot pose
  ...
  map<std::string, SpatialObjectPtr>::iterator it = m_objects.find(
      visualObjectID);
  ...
  if (m_objectModels.find(visualObjectID) == m_objectModels.end()) {
    ...
    if (observedObject->model != 0 && observedObject->model->vertices.size()
        == 1) {
  ...
  if (diff > 0.1 || bNewObject) {
    ...
    if (m_objectWMIDs.find(visualObjectID) == m_objectWMIDs.end()) {
      log("Writing new spatial object to WM: %s",
          m_objects[visualObjectID]->label.c_str());
      string newID = newDataID();

      addToWorkingMemory<SpatialData::SpatialObject> (newID,
          m_objects[visualObjectID]);
      m_objectWMIDs[visualObjectID] = newID;
    } else {

      try {
        debug("Overwriting spatial object to WM: %s",
            m_objects[visualObjectID]->label.c_str());
        overwriteWorkingMemory<SpatialData::SpatialObject> (
            m_objectWMIDs[visualObjectID], m_objects[visualObjectID]);
      } catch (DoesNotExistOnWMException) {
    ...
    if (m_bDisplayVisualObjectsInPB && m_objectProxies.is_assigned()) {
      ...
      if (obsObject->type == OBJECT_BOX) {
        ...
        theobjectproxy.add(m_objectProxies, m_objects[visualObjectID]->label,
            peekabot::REPLACE_ON_CONFLICT);
        theobjectproxy.translate(pose.pos.x, pose.pos.y, pose.pos.z);
        double angle;
        Vector3 axis;
        toAngleAxis(pose.rot, angle, axis);
        log("x = %f y = %f z = %f   angle = %f axis=(%f,%f,%f)", pose.pos.x,
            pose.pos.y, pose.pos.z, angle, axis.x, axis.y, axis.z);
    ...
    if (0) {
      ...
      if (m_objectOnnessValues.size() == 6 && m_objectInnessValues.size() == 6) {
        ...
        if (inferRelationsThreeObjects(inferred, BOnA, AOnB, BOnT, AOnT, BInA,
            AInB, BInT, AInT)) {
...
void ObjectRelationManager::runInference() {
  ...
  for (map<StrPair, double>::iterator it = m_objectOnnessValues.begin(); it
      != m_objectOnnessValues.end(); it++) {
    ...
    if (baseObjects.count(label1) == 0) {
      log("Adding ON relation: %s, %s, %f", label1.c_str(), label2.c_str(),
          it->second);
      orgg.addOnRelationObservation(label1, label2, it->second);
    }
  }

  for (map<StrPair, double>::iterator it = m_objectInnessValues.begin(); it
      != m_objectInnessValues.end(); it++) {
    string label1 = it->first.first;
    string label2 = it->first.second;
    ...
    if (baseObjects.count(label1) == 0) {
      log("Adding IN relation: %s, %s, %f", label1.c_str(), label2.c_str(),
          it->second);
  ...
  int i = 0;
  for (rocs::ml::FactorGraph::ConstVariableIterator it = fg.variableBegin(); it
      != fg.variableEnd(); ++it) {
...
void ObjectRelationManager::objectChanged(
    const cast::cdl::WorkingMemoryChange &wmc) {
  try {
    VisionData::VisualObjectPtr observedObject = getMemoryEntry<
        VisionData::VisualObject> (wmc.address);
  } catch (DoesNotExistOnWMException) {
    log("Error! overwritten Object missing on WM!");
  }
}

void ObjectRelationManager::newPlaneObject(
    const cast::cdl::WorkingMemoryChange &wmc) {
  try {
    FrontierInterface::ObservedPlaneObjectPtr observedObject = getMemoryEntry<
        FrontierInterface::ObservedPlaneObject> (wmc.address);

    log("Read object %s at %f, %f, %f", observedObject->label.c_str(),
        observedObject->pos.x, observedObject->pos.y, observedObject->pos.z);
    ...
    double diff = length(m_planeObjects[obsPlaneLabel]->pos
        - observedObject->pos);
    double angDiff = m_planeObjects[obsPlaneLabel]->angle
        - observedObject->angle;
    if (angDiff > M_PI)
      angDiff -= 2 * M_PI;
    if (angDiff < -M_PI)
      angDiff += 2 * M_PI;
    ...
    if (diff > 0.01 || isNew) {
      ...
      if (m_bDisplayPlaneObjectsInPB) {
        if (m_PeekabotClient.is_connected()) {
          ...
          PlaneObject &planeObject = m_planeObjectModels[obsPlaneLabel];
          char identifier[100];
...
void ObjectRelationManager::setContainmentProperty(
    const string &visualObjectID, int placeID, double confidence) {
  ...
  if (it == m_containmentProperties.end()) {
    ...
    containmentProp = new SpatialProperties::PlaceContainmentObjectProperty;
  ...
  SpatialProperties::ValueProbabilityPair pair1 = { placeValue1, confidence };
  SpatialProperties::ValueProbabilityPair pair2 = { placeValue2, 1.0
      - confidence };
...
void ObjectRelationManager::connectPeekabot() {
  try {
...
void ObjectRelationManager::recomputeOnnessForObject(
    const string &visualObjectID) {
  if (m_objectModels.find(visualObjectID) == m_objectModels.end()) {
  ...
  for (map<string, FrontierInterface::ObservedPlaneObjectPtr>::iterator it =
      m_planeObjects.begin(); it != m_planeObjects.end(); it++) {
    PlaneObject po;
    ...
    if (m_planeObjectModels.find(it->second->label)
        != m_planeObjectModels.end()) {
  ...
  for (map<string, SpatialObjectPtr>::iterator it = m_objects.begin(); it
      != m_objects.end(); it++) {
    ...
    if (supportObjectID != visualObjectID) {
      ...
      m_objectModels[supportObjectID]->pose = m_objects[supportObjectID]->pose;

      double onness = m_evaluator.evaluateOnness(
          m_objectModels[supportObjectID], obj);

      log("Object %s(%s) on object %s(%s) is %f", visualObjectID.c_str(),
          visualObjectLabel.c_str(), it->first.c_str(),
...
void ObjectRelationManager::recomputeInnessForObject(
    const string &visualObjectID) {
  if (m_objectModels.find(visualObjectID) == m_objectModels.end()) {
    log("Error! Object model was missing!");
    return;
  ...
  for (map<string, SpatialObjectPtr>::iterator it = m_objects.begin(); it
    ...
    if (containerObjectID != visualObjectID) {
      ...
      double inness = m_evaluator.evaluateInness(
          m_objectModels[containerObjectID], obj);
...
void ObjectRelationManager::recomputeOnnessForPlane(const string &planeLabel) {
  ...
  fromAngleAxis(po.pose.rot, m_planeObjects[planeLabel]->angle, vector3(0.0,
      0.0, 1.0));
  ...
  for (map<string, SpatialData::SpatialObjectPtr>::iterator it =
      m_objects.begin(); it != m_objects.end(); it++) {
    ...
    log("Evaluating object %s on object %s", visualObjectID.c_str(),
        planeLabel.c_str());
    double onness = m_evaluator.evaluateOnness(&po,
        m_objectModels[visualObjectID]);
    log("Object %s on object %s is %f", visualObjectID.c_str(),
...
void ObjectRelationManager::addRecognizer3DCommand(
...
void ObjectRelationManager::addTrackerCommand(
    VisionData::TrackingCommandType cmd, std::string visualObjectID) {
...
void ObjectRelationManager::readPlaneModelsFromFile() {
...
void ObjectRelationManager::newPriorRequest(const cdl::WorkingMemoryChange &wmc) {
  try {
    FrontierInterface::ObjectPriorRequestPtr request = getMemoryEntry<
        FrontierInterface::ObjectPriorRequest> (wmc.address);

    processPriorRequest(request);

    overwriteWorkingMemory<FrontierInterface::ObjectPriorRequest> (wmc.address,
        request);
...
void ObjectRelationManager::processPriorRequest(
  ...
  if (m_planeObjectModels.find(supportObjectID) != m_planeObjectModels.end()) {
    ...
    for (map<string, FrontierInterface::ObservedPlaneObjectPtr>::iterator it =
      ...
      if (it->second->label == supportObjectID) {
        ...
        fromAngleAxis(supportObject->pose.rot, it->second->angle, vector3(0.0,
            0.0, 1.0));
      }
    }
  } else {
    ...
    if (m_objects.find(supportObjectID) == m_objects.end()) {
      ...
      log(
          "Support object was unknown; computing distribution with object in origin");
  ...
  FrontierInterface::RelationSeq::iterator rit = request->relationTypes.begin();
  for (vector<string>::iterator it = request->objects.begin(); it
      != request->objects.end(); it++, rit++) {
  ...
  SampleCloud cloud;
  m_sampler. sampleBinaryRelationSystematically(relations, objectChain,
  ...
  cloud.makePointCloud(request->outCloud->center, request->outCloud->interval,
      request->outCloud->xExtent, request->outCloud->yExtent,
      request->outCloud->zExtent, request->outCloud->values);
