...
int main(void) {
  ...
  sampler.sampleBinaryRelationSystematically(relations, objects, oris1, labels,
      pdf.getCellSize(), testCloud);

  testCloud.KernelDensityEstimation2D(pdf, objects.back()->pose.pos,
      sampler.m_kernelWidthFactor, total, 1.0);
}
