...
NewWrappedSLAM::NewWrappedSLAM(short numberOfSensors, bool usefuse)
  :DataFilter(4, 0, false),
    m_MapBank(10),
  m_Map(&m_MapBank),
  m_Dead(200),
   m_Hough(),
  m_SlotScans(10, SICKSCAN_TYPE),
  m_SlotMeas(10, MEASUREMENTSET_TYPE),
  m_ScanDisp(0)
{
...
NewWrappedSLAM::NewWrappedSLAM(PoseErrorModel *modelA, 
                         Timestamp & period, 
  ...
  :DataFilter(4, 0, false),
   m_MapBank(10),
   m_Map(&m_MapBank),
   m_Dead(modelA, period,modelB,fuser,userotate,inputBuffer,outputbuffer),
   m_Hough(),
   m_SlotScans( 10, SICKSCAN_TYPE),
   m_SlotMeas( 10, MEASUREMENTSET_TYPE),
   m_ScanDisp(0)
...
void
NewWrappedSLAM::constructorInit()
{
...
{
  if(m_OffsetSlot) delete[]m_OffsetSlot;
  m_OffsetSlot=0;
  if (m_ScanDisp) delete m_ScanDisp;
  if (m_ModelA) delete m_ModelA;
  if (m_ModelB) delete m_ModelB;
  if (m_Fuser) delete m_Fuser;
  m_ModelB = 0;
  m_ModelA = 0;
  m_Fuser = 0;
  m_ScanDisp=0;
...
int
NewWrappedSLAM::resetRobotPose(const Pose3D &p, bool useCov)
{
...
Cure::Pose3D
NewWrappedSLAM::getPosePrediction()
{
  return PoseProvider::getPredictedWorldPose(m_SLAM->PoseOut,m_Dead.m_SlotInA);
}

int 
NewWrappedSLAM::config(const std::string &cfgFile)
{
  ConfigFileReader cfg;
  if (cfg.init(cfgFile.c_str())) return -1;
  ...
  if (m_Map.config(cfg, m_SLAM->MSet)) return -1;
  if (m_ConfigModels&1){
    int odType;
    std::string odParams;
    ...
    if (m_ModelA->config(odParams)) return -1;
  }
  if (m_ConfigModels&2){
    int gyroType;
    std::string gyroParams;
    if (cfg.getInertialModel(gyroType, gyroParams)){      
      CureCERR(20) << "WARNING inertialType=" << gyroType
		   << " not handled\n";
    }else{
	...
	return -1;
      ...
      if (m_ModelB->config(gyroParams)) return -1;
      m_Dead.setErrorModel(m_ModelB, 1);
      m_Dead.setFuser(m_Fuser,true);
    }
  }
  if (m_SLAM->setup(cfg)) return -1;
  if ( useScanlines){
...
void 
NewWrappedSLAM::loosenMatching(double factor, int type)
{
  m_SLAM->loosenMatching(factor,type);
}

void 
NewWrappedSLAM::setGain(double gain)
{
    m_SLAM->setGain(gain);
}

void 
NewWrappedSLAM::increaseMinCovariance(double factor)
{
 if ( useScanlines)
  m_Hough.increaseMinCovariance(factor);
}
void 
NewWrappedSLAM::increaseLineQuality(double factor)
{
 if ( useScanlines)
   m_Hough.increaseLineQuality(factor);
}
void 
NewWrappedSLAM::removeClutter(double factor)
{
 if ( useScanlines)
...
int
NewWrappedSLAM::addOdometry(const Pose3D &odo)
{
  struct timeval tv;
  gettimeofday(&tv, NULL);
  char buf[128];
  sprintf(buf, "1 0 %ld.%06ld %ld.%06ld", odo.Time.Seconds, odo.Time.Microsec, tv.tv_sec, tv.tv_usec);
...
int
NewWrappedSLAM::addInertial(const Pose3D &inert)
{
...
int 
NewWrappedSLAM::addScan(const SICKScan &scan)
{
...
int 
NewWrappedSLAM::addMeasurementSet(MeasurementSet &measSet)
{
   struct timeval tv;
  gettimeofday(&tv, NULL);
  char buf[128];
  ...
  sprintf(buf, "2 %d %ld.%06ld %ld.%06ld", err, measSet.Time.Seconds, measSet.Time.Microsec, tv.tv_sec, tv.tv_usec);
  m_walkclocktimefile << buf << std::endl;
  if (err) {
    return -err;
  }
...
unsigned short 
NewWrappedSLAM::write(Cure::TimestampedData& p, const int port)
{
  if (port == 0) {
    ...
    CureCERR(20) << "Can only write to port 0,1  and 2 for \""
                 << FilterName << "\"\n";
...
int 
NewWrappedSLAM::saveMap(const std::string &filename)
{
...
int 
NewWrappedSLAM::saveTrackKeys(const std::string &filename)
{
  CureCERR(30) << "Saving TrackKeys \"" << filename << "\"\n";
  std::fstream fs;
  fs.open(filename.c_str(),std::ios::out);
  if (!fs.good()) {
    CureCERR(20) << "Failed to save hash tables to file \""
                 << filename << "\"\n";  
    return 1;
  }
