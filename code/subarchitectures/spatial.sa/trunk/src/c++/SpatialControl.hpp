...
private:
  ...
  private:
    virtual bool isCircleObstacleFree(double x, double y, double radius,
    ...
    virtual SpatialData::LocalGridMap getBoundedMap(double minx, double maxx,
        double miny, double maxy, const Ice::Current &_context) {
    ...
    virtual SpatialData::DoubleOpt getGridmapRaytrace(double startAngle,
        double angleStep, int beamCount, const Ice::Current &_context) {
    ...
    MapServer(SpatialControl *owner) :
      m_pOwner(owner) {
    }
...
protected:
  ...
  bool check_point(int x, int y, vector<NavData::FNodePtr> &nodes, vector<
      SpatialData::NodeHypothesisPtr> &non_overlapped_hypotheses,
  ...
  void getBoundedMap(SpatialData::LocalGridMap &map, const Cure::LocalGridMap<
      unsigned char> *gridmap, double minx, double maxx, double miny,
      double maxy) const;
  ...
  void blitHeightMap(Cure::LocalGridMap<unsigned char>& lgm,
  ...
  enum TaskStatus {
    ...
    TaskFinished
  // nav ctrl command finished, completion needs posting
  };
