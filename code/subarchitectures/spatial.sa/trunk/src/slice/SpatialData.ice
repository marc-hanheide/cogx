...
class MapLoadStatus {
  bool nodesWritten;		
  // NavGraphProcess signals all nav nodes (and edges) have been written to WM
  bool placesWritten;		
  // PlaceManager signals all Places (including placeholders) have been written to WM
  bool roomsWritten;		
  // Coma signals all ComaRooms have been written to WM
  bool categoryDataWritten;	
  // Categorical.SA signals all category place/placeholder properties have been written to WM
  bool obstacleMapsLoaded;	
  // SpatialControl signals all grid map data has been loaded
  bool localMapsLoaded;		
  // LocalMapManager signals all grid map data has been loaded
};
  ...
  interface MapInterface {
    ...
    void addConnection(int node1id,int node2id);
    void removeConnection(int node1id,int node2id);
