...
namespace SpatialGridMap {
  ...
  class GDAddKDE {
    ...
    public:
      ...
      GDAddKDE(double radius,
	  const vector<vector<double> > &_zPositions,
	  const vector<vector<double> > &_values,
	  const vector<double> &_xySqDistances,
	  double _multiplier = 1.0) : 
	kernelRadius(radius),
	zPositions(_zPositions), 
	values(_values),
  ...
  class GDMergeKDE {
	...
	for (; i < bloxelCount; i++) {
	  ...
	  double currCeling = celing > (*cookieCutter)[i].celing + zOffset ?
	    (*cookieCutter)[i].celing +zOffset : celing;
