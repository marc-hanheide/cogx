...
#define BLOXEL_SIZE_EPS (1e-8)
    ...
    template <class MapData>
        struct Bloxel {
            ...
            Bloxel(){};
            Bloxel(MapData data, double celing) : data(data), celing(celing){};
        };
        ...
        class GridMap {
            ...
            */
            ...
            template <class BloxelFunctor> 
                void boxSubColumnModifier(int i, int j, double zPos, double zSize, BloxelFunctor & functor, bool autoMerge = true);	    
 ...
 /**
	    ...
	    template <class BloxelFunctor>
             ...
             */
            template <class BloxelFunctor>
                void alignedBoxQuery(int xMin, int xMax, int yMin, int yMax, double zMin, double zMax, BloxelFunctor & query, bool autoMerge = true);
            // Deprecated, use query method for everything
            template <class BloxelFunctor>
                void alignedBoxModifier(int xMin, int xMax, int yMin, int yMax, double zMin, double zMax, BloxelFunctor & modifier, bool autoMerge = true);
             ...
             */ 
            template <class BloxelFunctor>
                void columnQuery(double xPos, double yPos, double zPos, double zSize, BloxelFunctor & functor, bool autoMerge = true);
             ...
             */ 
            template <class BloxelFunctor>
                void boxQuery(double xPos, double yPos, double zPos, double xSize, double ySize, double zSize, double rotation, BloxelFunctor & functor);
            // Deprecated, use query method for everything
            template <class BloxelFunctor>
                void boxModifier(double xPos, double yPos, double zPos, double xSize, double ySize, double zSize, double rotation, BloxelFunctor & functor);
            ...
            template <class PointFunctor, class ClearFunctor>
                void laserQuery(double xPos, double yPos, double zPos, int n, double startAngle, double angleStep, double maxRange, double *scans, PointFunctor & pointQuery, ClearFunctor & clearQuery);
            // Deprecated, use query method for everything
            template <class PointFunctor, class ClearFunctor>
                void laserModifier(double xPos, double yPos, double zPos, int n, double startAngle, double angleStep, double maxRange, double *scans, PointFunctor & pointModifier, ClearFunctor & clearModifier);
            ...
            template <class ObstacleFunctor, class PointFunctor, class ClearFunctor>
                void coneQuery(double xPos, double yPos, double zPos, double panAngle, double tiltAngle, double horizSizeAngle, double vertSizeAngle, double maxRange, int widthInRays, int heightInRays, ObstacleFunctor & obstacle, PointFunctor & pointQuery, ClearFunctor & clearQuery, double minDistance = 0.0);

            // Deprecated, use query method for everything
            template <class ObstacleFunctor, class PointFunctor, class ClearFunctor>
                void coneModifier(double xPos, double yPos, double zPos, double panAngle, double tiltAngle, double horizSizeAngle, double vertSizeAngle, double maxRange, int widthInRays, int heightInRays, ObstacleFunctor & obstacle, PointFunctor & pointModifier, ClearFunctor & clearModifier, double minDistance = 0.0);
            ...
            template <class BloxelFunctor>
                inline void colQuery(int i, int j, BloxelFunctor & functor, double zMin, double zMax, bool autoMerge = true);

            // Helper functions for modification of columns
            template <class BloxelFunctor>
                inline bool modifySubcolumn(int i, int j, 
                        BloxelFunctor &modifier, double zMin, double zMax,
			bool autoMerge);
            // Helper function modifies bloxels before query call
            Range carveHelper(FunctorType type,
                    vector<Bloxel<MapData> > & column, double zMin, double zMax);

            // Helper function for queries
            template <class BloxelFunctor>
                inline void iteratorHelper(int xPos, int yPos, BloxelFunctor & functor, double zMin, double zMax);

            // Helper for box rotation
            pair<double, double> boxRotHelper(double xCent, double yCent, double x, double y, double rot) const;
    ...
    //Constructor
    template <class MapData>
        GridMap<MapData>::GridMap(int xGrid, int yGrid, double cellSize, double minBloxel, double zMin, double zMax, double mapCenterX, double mapCenterY, double mapRotation, MapData defaultValue) : xGrid(xGrid), yGrid(yGrid), cellSize(cellSize), minBloxel(minBloxel), mapZMin(zMin), mapZMax(zMax), mapCenterX(mapCenterX), mapCenterY(mapCenterY), mapRotation(mapRotation), defaultValue(defaultValue){
    ...
    template <class MapData>
        double GridMap<MapData>::getCellSize() { return cellSize; }
    template <class MapData>
        double GridMap<MapData>::getMinBloxelHeight(){ return minBloxel; }
    template <class MapData>
        double GridMap<MapData>::getRotation() { return mapRotation; }
    template <class MapData>
        MapData GridMap<MapData>::getDefaultValue() { return defaultValue; }
    ...
    template <class MapData>
        void GridMap<MapData>::setRotation(double rotation) { mapRotation = rotation; }
    template <class MapData>
        void GridMap<MapData>::setDefaultValue(MapData value) { defaultValue = value; }
    ...
    template <class MapData>
        const vector<Bloxel<MapData> > & GridMap<MapData>::operator() (int xPos, int yPos) const {
            return getCol(xPos,yPos);
        }

    // Point retrieval
    template <class MapData>
        const MapData & GridMap<MapData>::valueAtPoint(double xPos, double yPos, double zPos) const {
    ...
    template <class MapData>
        template <class BloxelFunctor>
        inline void GridMap<MapData>::colQuery(int i, int j, BloxelFunctor & functor, double zMin, double zMax, bool autoMerge){
            // This is a good place for central input validation
            if(zMax - zMin < minAcceptedBloxel() || zMax < this->mapZMin || zMin > this->mapZMax){
...
//                    << " RANGE: " << zMin << "-" << zMax << endl;
    ...
    template <class MapData>
		...
		bool autoMerge) {
            ...
            Range indices = carveHelper((FunctorType)modifier.type(),
                        getCol(i,j), zMin, zMax); 
		...
		getCol(i,j);
            ...
            for (int zi = indices.first; zi <= indices.second; zi++) {
                ...
                if (ceiling - floor < minAcceptedBloxel()) {
                    cerr << "Oh noes! ("<<i<<","<<j<<") is ["<<floor<<","<<ceiling<<"]!\n";
                }
                //Update bloxel
    ...
    template <class MapData>
        ...
        {
            vector<Bloxel<MapData> >&column =
                getCol(i,j);

            for (typename vector<Bloxel<MapData> >::iterator it = column.begin();
                    it != column.end();) {
    ...
    template <class MapData>
        template <class BloxelFunctor>
        inline void GridMap<MapData>::iteratorHelper(int xPos, int yPos, BloxelFunctor & functor, double zMin, double zMax){
    ...
    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::boxSubColumnModifier(int i, int j, double zPos, double zSize, BloxelFunctor & functor, bool autoMerge) {
            colQuery(i, j, functor, zPos-zSize/2, zPos+zSize/2, autoMerge);
        }

    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::alignedBoxQuery(int xMin, int xMax, int yMin, int yMax, double zMin, double zMax, BloxelFunctor & query, bool autoMerge){
    ...
    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::alignedBoxModifier(int xMin, int xMax, int yMin, int yMax, double zMin, double zMax, BloxelFunctor & modifier, bool autoMerge){
            alignedBoxQuery(xMin, xMax, yMin, yMax, zMin, zMax, modifier, autoMerge);
        }
            
    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::columnQuery(double xPos, double yPos, double zPos, double zSize, BloxelFunctor & functor, bool autoMerge){
            pair<int,int> coords = worldToGridCoords(xPos, yPos);
            colQuery(coords.first, coords.second, functor, zPos-zSize/2, zPos+zSize/2, autoMerge);
        }
    ...
    template <class MapData>
        pair<double, double> GridMap<MapData>::boxRotHelper(double xCent, double yCent, double x, double y, double rot) const {
            //Pos relative center position
            x -= mapCenterX;
            y -= mapCenterY;
            xCent -= mapCenterX;
    ...
    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::boxQuery(double xPos, double yPos, double zPos, double xSize, double ySize, double zSize, double rotation, BloxelFunctor & functor){
            //Validate input
            if(xSize < this->cellSize){
                cerr << "GridMap warning: Box is to thin to work, setting xSize to cellSize" << endl;
                xSize = this->cellSize;
            }
            if(ySize < this->cellSize){
                cerr << "GridMap warning: Box is to thin to work, setting ySize to cellSize" << endl;
                ySize = this->cellSize;
            }
            ...
            corners[0] = boxRotHelper(xPos, yPos, xPos + xSize / 2, yPos + ySize / 2, rotation);
            corners[1] = boxRotHelper(xPos, yPos, xPos + xSize / 2, yPos - ySize / 2, rotation);
            corners[2] = boxRotHelper(xPos, yPos, xPos - xSize / 2, yPos - ySize / 2, rotation);
            corners[3] = boxRotHelper(xPos, yPos, xPos - xSize / 2, yPos + ySize / 2, rotation);
            ...
            for(int i=xMinI; i<=xMaxI; i++){
                ...
                for(int j=0; j<4; j++){
                    ...
                    if(fabs(x3 - x4) < 1e-8){
                        ...
                        intersect[j] = ((x1*y2 - y1*x2)*(y3 - y4) - (y1 - y2)*(x3*y4 - y3*x4))
    ...
    template <class MapData>
        template <class BloxelFunctor>
        void GridMap<MapData>::boxModifier(double xPos, double yPos, double zPos, double xSize, double ySize, double zSize, double rotation, BloxelFunctor & functor) {
            boxQuery(xPos, yPos, zPos, xSize, ySize, zSize, rotation, functor);
        }

    template <class MapData>
        template <class PointFunctor, class ClearFunctor>
        void GridMap<MapData>::laserQuery(double xPos, double yPos, double zPos, int n, double startAngle, double angleStep, double maxRange, double *scans, PointFunctor & pointQuery, ClearFunctor & clearQuery){};

    template <class MapData>
        template <class PointFunctor, class ClearFunctor>
        void GridMap<MapData>::laserModifier(double xPos, double yPos, double zPos, int n, double startAngle, double angleStep, double maxRange, double *scans, PointFunctor & pointModifier, ClearFunctor & clearModifier){};

    // Helper for cone modification
    template <class MapData>
        template <class ObstacleFunctor>
        inline double GridMap<MapData>::rayCollisionDistance(cv::Mat_<double>origin,
                cv::Mat_<double>direction, double maxDistance, const ObstacleFunctor &obstacle) const
        {
            pair<int, int> originIndices = worldToGridCoords(origin(0,0), origin(1,0));
            pair<double, double> originCellMiddle = 
                gridToWorldCoords(originIndices.first, originIndices.second);
	    ...
	    bool noYStep = false;
	    bool noXStep = false;
            if (fabs(depthPerY) > 1000.0) noYStep = true;
	    if(fabs(depthPerX) > 1000.0) noXStep = true;

	    if (noYStep && noXStep) return maxDistance;
	      ...
	      if (i+originIndices.first >= 0 && i+originIndices.first < xGrid &&
		  j + originIndices.second >= 0 && j + originIndices.second < yGrid)
	      {
		const vector<Bloxel<MapData> > &column = 

		  getCol(i + originIndices.first,
		      j + originIndices.second);

		for (currentBloxelID = 0; currentBloxelID < (int)column.size(); 
		    currentBloxelID++) {
		  if (column[currentBloxelID].celing > currentZ) {
		    ...
		    break;
                ...
                double nextX = originCellMiddle.first + i*cellSize + 0.5*cellSize*stepX;
                double nextY = originCellMiddle.second + j*cellSize + 0.5*cellSize*stepY;
                double nextZ;
                double nextDepth;
		    		  ...
		    		  j + originIndices.second >= 0 && j + originIndices.second < yGrid)
		{
                    ...
                    if (upwards) {
                        for (int id = currentBloxelID; id < (int)column.size()-1; id++) {
                            if (column[id].celing > currentZ &&
                                    column[id].celing < nextZ &&
                                    obstacle(column[id+1].data)) {
                                return currentDepth + 
                                    depthStep * (column[id].celing - currentZ)/(nextZ - currentZ);
                            }
    ...
    template <class MapData>
        template <class ObstacleFunctor, class PointFunctor, class ClearFunctor>
        void GridMap<MapData>::coneQuery(double xPos, double yPos, double zPos, double panAngle, double tiltAngle, double horizSizeAngle, double vertSizeAngle, double maxRange, int widthInRays, int heightInRays, ObstacleFunctor & obstacle, PointFunctor & pointQuery, ClearFunctor & clearQuery, double minDistance){

            //will be treated as perfectly vertical.
            ConeSlicer slicer(xPos, yPos, zPos, 
                    panAngle, tiltAngle,
                    horizSizeAngle, vertSizeAngle,
                    maxRange,
                    widthInRays, heightInRays, 
                    obstacle,
                    *this,
		    minDistance);
            pair<double, double> lowerCorner = slicer.getBoundsLower();
            pair<double, double> upperCorner = slicer.getBoundsUpper();
            pair<int, int> lowerCornerIndices = 
                worldToGridCoords(lowerCorner.first, lowerCorner.second);
            pair<int, int> upperCornerIndices =
                worldToGridCoords(upperCorner.first, upperCorner.second);
            pair<double, double> lowerCornerCenter = 
                gridToWorldCoords(lowerCornerIndices.first, lowerCornerIndices.second);

            double x = lowerCornerCenter.first;
            for(int i=lowerCornerIndices.first; i<=upperCornerIndices.first; 
                    i++, x += cellSize){
                double y = lowerCornerCenter.second;
                for(int j=lowerCornerIndices.second; j<=upperCornerIndices.second; 
                        j++, y += cellSize){
    ...
    template <class MapData>
        template <class ObstacleFunctor, class PointFunctor, class ClearFunctor>
        void GridMap<MapData>::coneModifier(double xPos, double yPos, double zPos, double panAngle, double tiltAngle, double horizSizeAngle, double vertSizeAngle, double maxRange, int widthInRays, int heightInRays, ObstacleFunctor & obstacle, PointFunctor & pointModifier, ClearFunctor & clearModifier, double minDistance){
            coneQuery(xPos, yPos, zPos, panAngle, tiltAngle, horizSizeAngle, vertSizeAngle, maxRange, widthInRays, heightInRays, obstacle, pointModifier, clearModifier, minDistance);
        };

    template <class MapData>
        inline std::ostream& operator << (std::ostream& stream, const GridMap<MapData>& map)
        {
