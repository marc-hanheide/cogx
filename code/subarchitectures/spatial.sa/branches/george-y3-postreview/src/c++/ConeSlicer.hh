...
class ConeSlicer {
  public:
    ...
    inline bool planeZPos(double & res, const Vector3 & point, const Vector3 & norm, double x, double y) const;
  ...
  private:
    ...
    inline void optMinus(Vector3 & res, const Vector3 & arg1, const Vector3 & arg2);
  ...
  maxRange(maxRange), widthInRays(widthInRays), heightInRays(heightInRays)
{
  ...
  for (int horizRayX = -widthInRays; horizRayX <= widthInRays; horizRayX++) {
    ...
    for (int vertRayY = -heightInRays; vertRayY <= heightInRays; vertRayY++) {
      ...
      double rayLength = 
	owner.rayCollisionDistance(originCV, directionCV, maxRange, obstacle);
...
inline bool ConeSlicer::planeZPos(double & res, const Vector3 & point, const Vector3 & norm, double x, double y) const {
  // Plane is vertical, no z-coordinate
  if(fabs(norm.z) < 1e-8) 
    return false;

  //z = (zn*z0 - nx(x-x0) + ny(y-y0)) / nz
  res = (norm.z*point.z - norm.x*(x-point.x) - norm.y*(y-point.y)) / norm.z;
  return true;
}
