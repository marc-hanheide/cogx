...
void ConceptualQueryMaker::start() {
	log("I have started");
	addChangeFilter(
			createGlobalTypeFilter<VisionData::VisualObject> (cdl::ADD),
	 ...
	 addChangeFilter(cast::createGlobalTypeFilter<PerceptBelief>(cast::cdl::ADD),
	            new cast::MemberFunctionChangeReceiver<ConceptualQueryMaker>(this, &ConceptualQueryMaker::newPerceptBelief));

	 addChangeFilter(cast::createGlobalTypeFilter<GroundedBelief>(cast::cdl::ADD),
		            new cast::MemberFunctionChangeReceiver<ConceptualQueryMaker>(this, &ConceptualQueryMaker::newGroundedBelief));

	 m_visualObjects.push_back("table");
		m_queryHandlerServerInterfacePrx =
				getIceServer<ConceptualData::QueryHandlerServerInterface>(m_queryHandlerName);
}
...
void ConceptualQueryMaker::ObjectQuery(){
	...
	log("querying over %d objects %d rooms", m_visualObjects.size(), m_roomIds.size());
	string querystring;
	string plus = "p(+";
	string closebracket = ")";
	for (unsigned int i =0; i < m_visualObjects.size(); i++){
		...
		for (unsigned int j=0; j<m_roomIds.size(); j++){
			querystring = plus + m_namegenerator.getUnexploredObjectVarName(atoi(m_roomIds[j].c_str()),
																		   m_visualObjects[i],SpatialData::INROOM, "", "").c_str() + closebracket;
			log("querystring: %s", querystring.c_str());
			SpatialProbabilities::ProbabilityDistribution probDistrib = m_queryHandlerServerInterfacePrx->query(querystring);
			for (unsigned int k=0; k < probDistrib.massFunction.size(); k++){
				for (unsigned int l=0; l < probDistrib.massFunction[k].variableValues.size(); l++){
					log("here");
					StringRandomVariableValuePtr stringVariable = new StringRandomVariableValue;
					stringVariable = StringRandomVariableValuePtr::dynamicCast(probDistrib.massFunction[k].variableValues[l]);
...
void ConceptualQueryMaker::newPerceptBelief(
	    ...
	    log("Got new percept belief: id: %s, type: %s", belief->id.c_str(), belief->type.c_str());
}
...
void ConceptualQueryMaker::newGroundedBelief(
	    ...
	    if(belief->type == "ComaRoom"){
	    	...
	    	basicDistrib = BasicProbDistributionPtr::dynamicCast(condIndDistrib->distribs["RoomId"]);
	    	formulaValues = FormulaValuesPtr::dynamicCast(basicDistrib->values);
	    	integerformula = IntegerFormulaPtr::dynamicCast(formulaValues->values[0].val);
...
string roomid = lexical_cast<string>(integerformula->val);
	    ...
	    log("Got new grounded belief: id: %s, type: %s", belief->id.c_str(), belief->type.c_str());
}
