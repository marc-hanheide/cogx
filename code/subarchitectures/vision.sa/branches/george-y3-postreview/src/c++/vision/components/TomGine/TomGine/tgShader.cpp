...
tgShader::tgShader(const char *vertex_file, const char *fragment_file, const char *header)
{
	...
	if( vertex_file && !vs )
		throw std::runtime_error("[tgShader::tgShader] Error loading file for vertex shader");
	
	if( fragment_file && !fs )
		throw std::runtime_error("[tgShader::tgShader] Error loading file for fragment shader");
