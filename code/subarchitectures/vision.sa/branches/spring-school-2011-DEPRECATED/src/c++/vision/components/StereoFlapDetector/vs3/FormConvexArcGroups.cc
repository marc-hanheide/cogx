...
: GestaltPrinciple(vc), arcs(1000)
{
  ...
  // COCURV is the default of: COCURV, YUEN, ELL
  core->GetConfig().AddItem("CONVEXARCS_GROUP_CRITERION", "COCURV");
}
...
void FormConvexArcGroups::InformNewGestalt(Gestalt::Type type, unsigned idx)
{
  ...
  if(core->GetConfig().GetValueString("CONVEXARCS_GROUP_CRITERION")
      == "COCURV")
    GROUP_CRITERION = CRIT_COCURV;
  else if(core->GetConfig().GetValueString("CONVEXARCS_GROUP_CRITERION")
      == "YUEN")
  ...
  if(core->GetConfig().GetValueString("CONVEXARCS_SEARCH_METHOD")
      == "EXHAUSTIVE")
    SEARCH_METHOD = SEARCH_EXHAUSTIVE;
  else if(core->GetConfig().GetValueString("CONVEXARCS_SEARCH_METHOD")
      == "GREEDY")
...
double FormConvexArcGroups::Evaluate(Array<Arc*> &arcs, unsigned l,
 ...
 */
unsigned FormConvexArcGroups::Hash(Array<Arc*> &arcs, unsigned l,
    unsigned u)
{
