...
for i= 1:N
   ...
   if mtdTSc
      ...
      cUpd=lf2sfa(rC,ANSyes);
      cAsk1=lf2sfa(rC,[ANSpy ANSpn]);
      cKnown=lf2sfa(rC,[ANSyes ANSno ANSpy ANSpn]);
      cAsk2=cgt(find(~ismember(cgt,cKnown)))';
      cAsk=union(cAsk1,cAsk2);
      cAnsYes=cAsk(ismember(cAsk,cgt));
      c=union(cUpd,cAnsYes)';
   ...
   if mtdTSl
      rC=qnt2ql(MTDrec(f,mCtsc,mFStsc),THRs);
      cUpd=lf2sfa(rC,[ANSyes ANSpy]);
      cKnown=lf2sfa(rC,[ANSyes ANSno ANSpy ANSpn]);
      cAsk=cgt(find(~ismember(cgt,cKnown)))';
      cAnsYes=cAsk(ismember(cAsk,cgt));
      c=union(cUpd,cAnsYes)';
