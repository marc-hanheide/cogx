...
% Hell = internalHellingerJointSupportnD( pdf_cmprs, pdf, useWeightedHellinger ) ;
 Hell = uCostModel( otherClasses.pdf, pdf, pdf_cmprs, otherClasses.priors, approximateCost ) ;
...
%         sub_pdf.w = sub_pdf.w/ sum(sub_pdf.w) ;
        ...
        Hell1 = uCostModel( otherClasses.pdf, pdf, pdf_compr,  otherClasses.priors, approximateCost ) ;
    end
...
%                 sub_pdf.w = sub_pdf.w/ sum(sub_pdf.w) ;
    ...
    selection = ones(1,length(pdf.w)) ;        
        ...
        Hell2 = uCostModel( otherClasses.pdf, pdf, pdf_compr,  otherClasses.priors, approximateCost ) ;
