 ...
 */
Patch3D::Patch3D(cv::Mat_<cv::Vec4f> _p, cv::Mat_<cv::Vec4f> _h_p) : Gestalt3D(Gestalt3D::PATCH)
{
  points = _p;
  hull_points = _h_p;
  
printf("Patch3D::Patch3D: Create Histogram\n");
  int nr_bins = 10;
  hist = new ColorHistogram(nr_bins, points);
printf("Patch3D::Patch3D: Create Histogram done\n");
}
...
{
  printf("\Patch3D: %u\n", id);
  printf(" points (rows/cols): %u/%u\n", points.rows, points.cols);
  
  for(unsigned row=0; row<points.rows; row++)
    for(unsigned col=0; col<points.cols; col++)
      printf(" point: %4.3f / %4.3f / %4.3f\n", points(row, col)[0], points(row, col)[1], points(row, col)[2]);
}
