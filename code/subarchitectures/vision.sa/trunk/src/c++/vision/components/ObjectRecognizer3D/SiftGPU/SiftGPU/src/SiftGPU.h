...
public:
	...
	//in order to minimize the upload time, you should use GL_LUMINANCE unsigned char image data.
	SIFTGPU_EXPORT virtual int  RunSIFT(int width, int height, const unsigned char * data, unsigned int gl_format);
	SIFTGPU_EXPORT virtual int  RunSIFT(int width, int height, const float* data, unsigned int gl_format);
...
public:
	...
	//compiler setting of /MT and /MTd). Without the overloaded this operator
...
///matcher export
//This is a cg-based sift match implementation. 
//Be ware that using CUDA for this is faster than cg
class SiftMatchCG;
class SiftMatchGPU
{
private:
	SiftMatchCG * matcher;
public:
	//OpenGL Context creation/verfification, initialization is done automatically inside
	SIFTGPU_EXPORT virtual int  CreateContextGL();
	SIFTGPU_EXPORT virtual int  VerifyContextGL();
