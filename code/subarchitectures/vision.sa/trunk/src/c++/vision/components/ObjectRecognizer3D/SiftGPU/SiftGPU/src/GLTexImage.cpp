...
int GLTexImage::DownSamplePixelData(int gl_format, int width, int height, int ds, 
									const unsigned char * pin, unsigned char * pout)	
{
	...
	int ws = width/ds;
	int hs = height/ds;
	const unsigned char * line = pin, * p;
...
int GLTexImage::DownSamplePixelData(int gl_format, int width, int height, int ds, const float * pin, float * pout)	
{
	int step, linestep;
	int i, j; 
	int ws = width/ds;
...
int GLTexImage::SetImageData( int width,  int height, const unsigned char * data, 
							 unsigned int gl_format,  int& ds )
{
	...
	if(GlobalUtil::_verbose)
	{
		...
		if(ds >0) 	std::cout<<"Down sample to \t"<<ws<<"x"<<hs<<"\n";
	}
...
int GLTexImage::SetImageData( int width,  int height, const float * data, 
							 unsigned int gl_format,  int& ds )
{
