...
//   learner->WriteResults2File();
  ...
  svmFileCreator->Process(/*planePopout, */kcore, camPars[2].fx, camPars[2].fy, camPars[2].cx, camPars[2].cy);
      ...
      for(unsigned i=0; i< sois.size(); i++)
      {
        ...
        //     color.float_value = s[3];
          ...
          for(unsigned j=0; j < bottom; j++)
          {
            ...
            cv::Vec4f s = soi_hull[j];
            cv::Vec4f e = soi_hull[i];
            tgRenderer->AddLine3D(s[0], s[1], s[2], e[0], e[1], e[2], 255, 255, 255, 0);
          }
          tgRenderer->Update();
        }
      }
