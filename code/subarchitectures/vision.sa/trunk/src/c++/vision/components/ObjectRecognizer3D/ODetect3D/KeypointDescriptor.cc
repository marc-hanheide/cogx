...
void KeypointDescriptor::Draw(IplImage *img, KeypointDescriptor &k, CvScalar col)
{
  ...
  float angle = ScaleAngle_0_2pi(acos(V(1,1)))*180./M_PI;
  SDraw::DrawEllipse(img, k.X(), k.Y(), D(1,1), D(2,2),angle, col);
  P::Vector2 v = P::Vector2(D(2,2),0);
  v = P::Rotate(v,-k.Angle());
  v = P::Vector2(v.x+k.X(), v.y+k.Y());
