...
int  SiftGPU::RunSIFT( int width,  int height, const unsigned char * data, unsigned int gl_format)
{
...
int  SiftGPU::RunSIFT( int width,  int height, const float * data, unsigned int gl_format)
{
...
//	<<"-fbo1/-fbos    : Use single/Multiple FBOs for whole work\n"
	...
	<<"-dcv <int>     : Which version of descriptor generation code to use: \n" 
	<<"                 1(gp4fp), 4, 8, 16(default/fastest), 32(gp4gp)\n"
	<<"-fs <int>      : Block Size for freature storage <default : 4>\n"
	<<"-glsl          : Use incomplete GLSL SIFTGPU instead of CG (default : CG)\n"
	...
	<<"-pack          : Use new packed implementation (Limitation: dcv must be 16) \n"
	<<"-di            : Use dynamic array indexing in shader if available (defualt : no)\n"
	<<"                 This could make orientaiton computation faster on cards like GTX 280\n"
	<<"\n";
}
void SiftGPU::ParseParam(int argc, char **argv)
{
	...
	for( i = 0; i< argc; i++)
	{
		...
		}else if(_stricmp(opt, "m")==0 || _stricmp(opt, "mo")==0)
		{
			...
			GlobalUtil::_MaxOrientation = min(max(2, mo), 4);
		...
		}else if(_stricmp(opt, "s") ==0)
		{
			...
			//at least two orientation
			GlobalUtil::_SubpixelLocalization = min(max(1, sp),5);
		}else if(_stricmp(opt, "exds") ==0)
		{
		...
		}else if(_stricmp(opt, "dcv")==0)
		{
			int dcv;
			sscanf(param, "%d", &dcv);
			if(dcv >= 1)
			{
				if(dcv == 1 || dcv == 4 || dcv == 8 || dcv == 16 || dcv == 32)
					GlobalUtil::_DescriptorPPT = dcv;
				i++;
			}
...
int SiftGPU::VerifyContextGL()
{
	...
	InitSiftGPU();
	return GlobalUtil::_HaveGlSupport + GlobalUtil::_FullSupported;
}

int SiftGPU::IsFullSupported()
{
	return GlobalUtil::_HaveGlSupport==1 &&  GlobalUtil::_FullSupported;
}
