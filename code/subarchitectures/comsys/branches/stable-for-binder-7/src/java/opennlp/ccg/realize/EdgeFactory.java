 ...
 * @version     $Revision: 1.69 $, $Date: 2008/08/26 02:45:34 $
 */
public class EdgeFactory
{
    ...
    /** The sign scorer. */
    public final opennlp.ccg.synsem.SignScorer signScorer;
    ...
    /** Constructor. */
    public EdgeFactory(Grammar grammar, List<SatOp> preds, opennlp.ccg.synsem.SignScorer signScorer) {
    	this(grammar, preds, signScorer, null);
    }
    
    /** Constructor with hypertagger. */
    public EdgeFactory(Grammar grammar, List<SatOp> preds, opennlp.ccg.synsem.SignScorer signScorer, Hypertagger hypertagger) {
    ...
    private List<Edge> createNewEdges(Edge edgeA, Edge edgeB, boolean collectCombos, boolean bothDirections) {
        ...
        // A B combos
        List<Sign> results = generalRules.applyBinaryRules(edgeA.sign, edgeB.sign);
        binaryRuleApps++; 
        ...
        if (bothDirections) {
            reversedResults = generalRules.applyBinaryRules(edgeB.sign, edgeA.sign);
            binaryRuleApps++; 
        }
