...
Operator::Operator(istream &in, const vector<Variable *> &variables) {
  ...
  in >> count; // number of replan conditions
  for(int i = 0; i < count; i++) {
    int varNo, val;
    in >> varNo >> val;
    replan.push_back(Prevail(variables[varNo], val));
  }
...
void Operator::dump() const {
  ...
  for(int i = 0; i < replan.size(); i++)
    cout << "  " << replan[i].var->get_name() << " := " << replan[i].prev;
  cout << endl;
  cout << "pre-post:";
...
void Operator::generate_cpp_input(ofstream &outfile) const {
  ...
  outfile << replan.size() << endl;
  for(int i = 0; i < replan.size(); i++) {
    assert(replan[i].var->get_level() != -1);
    if(replan[i].var->get_level() != -1)
      outfile << replan[i].var->get_level() << " "<< replan[i].prev << endl;
  }
