...
//    stn.dump();
    ...
    stn.solve();
    vector<Happening> happenings = stn.getHappenings();
    ...
    for(int i = 0; i < happenings.size(); ++i) {
        if(instant_plan[happenings[i].second].type == start_action) {
            ...
            plan[instant_plan[happenings[i].second].correspondingPlanStep].start_time = happenings[i].first;
        }
    }
...
//                        cout << endl;
        ...
        for(int j = 0; j < step.effects.size(); ++j) {
            for(int k = i-1; k >= 0; --k) {
                InstantPlanStep &temp_step = instant_plan[k];
                for(int l = 0; l < temp_step.effects.size(); ++l) {
                    if(temp_step.effects[l].var == step.effects[j].var && temp_step.effects[l].post != step.effects[j].post &&
                            step.op != temp_step.op) {
                        partial_order.insert(make_pair(k,i));
                    }
                }
...
int PartialOrderLifter::getIndexOfPlanStep(const ScheduledOperator& op, double timestamp) {
    for(int i = 0; i < plan.size(); ++i) {
        if(plan[i].op->get_name().compare(op.get_name()) == 0 && plan[i].start_time == timestamp) {
            return i;
        }
    }
    assert(false);
...
void PartialOrderLifter::buildInstantPlan() {
    ...
    map<double, vector<const ScheduledOperator*> > actionsEndingAtGivenTime;
    map<double, list<InstantPlanStep*> > instantActionsStartingAtGivenTime;
...
//        instant_plan[0].effects = effects;
        ...
        if(stateBeforeHappening->operators.size()
            ...
            if(!(double_equals(stateBeforeHappening->timestamp, stateAfterHappening->timestamp))) {
                cout << stateBeforeHappening->timestamp << ", " << stateAfterHappening->timestamp << endl;
                assert(false);
            }
            ...
            actionsEndingAtGivenTime[currentTimeStamp + new_op.time_increment].push_back(
                    &new_op);
            instant_plan.push_back(InstantPlanStep(start_action,
                                currentTimeStamp, stateBeforeHappening->state[new_op.get_duration_var()], &new_op));
            ...
            instant_plan.back().correspondingPlanStep = getIndexOfPlanStep(new_op,currentTimeStamp);
//            assert(instant_plan.size() >= 3);
            instantActionsStartingAtGivenTime[currentTimeStamp].push_back(
                    &(instant_plan.back()));
...
//            assert(instant_plan[0].type == dummy_start_action);
            ...
            assert(stateBeforeHappening->timestamp < stateAfterHappening->timestamp);
            InstantPlanStep *lastEndingAction = NULL;
            for(int j = 0; j
                    < actionsEndingAtGivenTime[currentTimeStamp].size(); ++j) {
                instant_plan.push_back(InstantPlanStep(end_action,
                        currentTimeStamp, -1,
                        actionsEndingAtGivenTime[currentTimeStamp][j]));
                vector<Prevail> preconditions;
                findPreconditions((*actionsEndingAtGivenTime[currentTimeStamp][j]), preconditions, end_action);
                set<int> effect_cond_vars;
                findAllEffectCondVars((*actionsEndingAtGivenTime[currentTimeStamp][j]),effect_cond_vars,end_action);
                instant_plan.back().effects = effects;
                instant_plan.back().effect_cond_vars = effect_cond_vars;
                instant_plan.back().preconditions = preconditions;
                instant_plan.back().overall_conds = (*actionsEndingAtGivenTime[currentTimeStamp][j]).get_prevail_overall();
                double startingTime = currentTimeStamp - actionsEndingAtGivenTime[currentTimeStamp][j]->time_increment;
                for(it = instantActionsStartingAtGivenTime[startingTime].begin();
                         it != instantActionsStartingAtGivenTime[startingTime].end(); ++it) {
