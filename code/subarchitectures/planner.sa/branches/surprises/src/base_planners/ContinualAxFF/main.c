...
struct tms gsylvie_start, gsylvie_end;
...

{
	...
	if ( gcmd_line.display_info >= 1 ) {
		printf("\nff: parsing domain file %s", ops_file);
	} 
	 ...
	 */  
	if ( gcmd_line.display_info >= 1 ) {
		printf(" ... done.\nff: parsing problem file %s", fct_file); 
	}
...
void make_exit( int status ) 

{

	char name[MAX_LENGTH];

	if ( gcmd_line.sylvie_file_name != NULL ) {
		sprintf( name, "%s", gcmd_line.sylvie_file_name );
		if ( (out = fopen( name, "a")) == NULL ) {
			printf("\n\nCan't open Sylvie output file!\n\n");
			exit( 1 );
		}    
		switch ( status ) {
		case 0:
			...
			times( &lend );
			fprintf(out, "%.2f %.2f %d\n", 
					((float) (lend.tms_utime - lstart.tms_utime + 
							  lend.tms_stime - lstart.tms_stime)) / 100.0,
					gsearch_time, gnum_plan_ops );
			break;
		case 1:
			/* proved unsolvable */
			times( &lend );
...

{
	...
	printf("-S <str>    Sylvie's output file name (``totaltime searchtime planlength\\n'' appended)\n");
	printf("-T <num>    Sylvie's time out (secs)\n\n");
...

{
	...
	gcmd_line.sylvie_file_name = NULL;

	gcmd_line.sylvie_time = -1;
	gcmd_line.prediction_level = 0;
		...
		default:
			if ( --argc && ++argv ) {
				...
				case 'f':
					...
					break;
				case 'S':
					gcmd_line.sylvie_file_name = ( char * ) calloc( MAX_LENGTH, sizeof( char ) );
					strncpy( gcmd_line.sylvie_file_name, *argv, MAX_LENGTH );
					break;
				case 'O':
					gcmd_line.plan_output_fn = ( char * ) calloc( MAX_LENGTH, sizeof( char ) );
					strncpy( gcmd_line.plan_output_fn, *argv, MAX_LENGTH );
					break;
				case 'T':
					sscanf( *argv, "%f", &gcmd_line.sylvie_time );
					if ( gcmd_line.sylvie_time <= 0 ) {
						printf("\n\ntime %f out <= 0??", gcmd_line.sylvie_time);
						return FALSE;
					}
					break;
				case 'i':
