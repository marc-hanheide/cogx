...
void FFHeuristic::relaxed_exploration() {
	...
	for(int i = 0; i < triggered_operators.size(); i++) {
        ...
        if (unary_op->unsatisfied_preconditions < 0) {
            /*cout << "neagtive("<< unary_op->unsatisfied_preconditions << ") pre op:" << unary_op->op->get_name() << endl;
            for(int i=0; i < unary_op->precondition.size(); ++i) {
                cout << unary_op->precondition[i]->id << ", ";
            }
            cout << endl;*/
        }
	    assert(unary_op->unsatisfied_preconditions >= 0);
        //cout << "op:" << unary_op->op->get_name() << " has now " << unary_op->unsatisfied_preconditions << " unsat. preconds." << endl;
	    if(unary_op->unsatisfied_preconditions == 0) {
		...
		if(effect->h_add_cost == -1) {
            ...
            //cout << "pushing " << effect->id << "(" << effect->h_add_cost<< ")" << endl;
            queue.push(effect);
