(define (domain dora-avs-iros11)
  ...
  (:functions
   ...
   (p-obj_exists ?l - label ?r - room) - number
  ...
  (:dtrule object_existence_room
           :parameters (?l - label ?r - room ?c - category)
           :precondition (and (= (category ?r) ?c)
                              (defined (p-obj_exists ?l ?r)))
           :effect (probabilistic (p-obj_exists ?l ?r) (and (assign (obj_exists ?l ?r) true)))
           )
