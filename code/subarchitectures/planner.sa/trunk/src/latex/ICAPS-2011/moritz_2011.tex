...
add one action $\actions(p_i;(T_i))$ to the problem for each element,
$p_i (T_i)$, of each probabilistic term from $(:init)$. The physics
of these actions is as follows. $\actions(p_i;(T_i))$ can be executed
if no $\actions(p_j;(T_j))$, $j \neq i$, has been executed from the
same probabilistic term, and, either
$(\pp{probabilistic}~..p_i~(T_i)..)$ is in the root conjunct, or it
occurs in $T_k$ for some executed $\actions(p_k;(T_k))$.
%%
Executing $\actions(p_i;(T_i))$ in $s$ effects a transition to a
successor state $s^v$ with probability $p_i$, and $s^\bot$ with
probability $1 - p_i$. Here, $s^v$ is characterised by the union of
$s$ and atomic terms from $T_i$, and $s^\bot$ is a sink state -- I.e.,
...
preconditions that are not necessarily true in the belief-state, we
must make two further adjustments to our sequential continual
planning. First, if $\action\in\actions$ has a precondition that is
not true with probability $1$ in the current state, then before it is
executed blindly, continual planning switches to a decision-theoretic
mode, that shall be discussed in detail below. Therefore, by
scheduling $\action$ the sequential planner makes a naive assumption
about the possibility of the agent eventually holding strong beliefs
about the truth values of specific state propositions. Second, if the
truth value of $\prop$ is not known, and no assumption is made about
it in the current abstract state, then if $\prop \in \poss(\action)$
we have that action $\action$ is not applicable.

Finally, we come to the issue of optimisation criteria. Here, we
propose the value of an $N$ horizon trace ---something we expect the
sequential planner to maximise--- corresponds to the maximal value of
that trace. In detail, where $p_i$ is the probability that the
$i^{th}$ execution is not a transition to $s^\bot$, we value a trace
according to:
...
received on the first transition to a goal state,
Eq~\ref{eq:tracevalue} corresponds exactly to the criteria of the
infamous degenerate hindsight system
\system{FFR$_a$}~\cite{yoon:etal:2007}. Moreover, form the perspective
of gradient-based reinforcement learning systems, such as William's
REINFORCE and GPOMDP, our optimal trace yields a maximal gradient
contribution under reasonable assumptions.
