...
\subseteq \stochSenses$. A precept is active, written $\stochSense \in
...
its environment. Technically, a belief-state $\bstate$ is a
probability distribution over process states. We write
$\bstate(\state)$ to denote the probability that the process is in
$\state$ according to $\bstate$, and $\bstate_0$ when discussing the
starting configuration. We suppose the latter is given in a factored
tree format.
