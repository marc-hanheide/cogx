...
according to a recursive note of {\em visiting} terms. Here, an atom
is visited if its conjunctive parent is visited, and a conjunctive
term is visited if all its immediate subterms are visited. A
probabilistic term is visited if exactly one of its subterms, $T_i$,
is visited. Each valid visitation of the root term according to this
recursive definition encapsulates a starting state, along with the
probability it occurs. The former corresponds to the union of all
visited atoms, and the latter corresponds to the product of $p_i$
entries on the visited subterms of probabilistic elements. Making
these ideas concrete, our example yields the following flat state
distribution:
...
RICHARD, I HAVE STOLEN A FEW LINES FROM YOUR HIPPO THREAD TO SKETCH
THIS STUFF. I HOPE IT'S TAKEN AS FLATTERY :)
...
POMDP described in DTPDDL.  Our approach uses sequential planning to
compute a linear plan, and one trace of that plan's execution. The
...
until the validation of an assumption about a runtime state
...
by the robot to test the validity of the assumptions so far.

The problem posed to the decision-theoretic system is an abstraction
of the problem at hand. It is induced according to the executing
sequential plan. Specifically, the abstract process is derived by
first omitting all problem variables that are not mentioned in the
linear trace, and then refining that preliminary model, by
progressively adding more problem variables while the resulting state
space remains sufficiently small. Refinement is a simple and
...
\subsection{Sequential Planning in POMDP Determinisations}
...
\subsection{Abstractions for Decision-Theoretic Planner}
