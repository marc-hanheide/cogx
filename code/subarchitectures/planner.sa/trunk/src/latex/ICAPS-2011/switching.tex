...
third plan step. These are called {\em applicability} assumptions if
...
$\assumptiveS{k}$. Also, a distinct assumption about a proposition
...
the union of $\state$ with atomic terms from $T_i$ and the proposition
denoting the undefined fact is deleted. For example, if the action
adds $(=(\pp{is-in}~\pp{box})~\pp{office})$, then it must delete
$(=(\pp{is-in}~\pp{box})~\#)$. State $\state^\bot$ is an added sink,
where $\forall\action$ $\reward(\state^\bot, \action) = 0$ and
...
switches to a contingent session, discussed in the following section.
