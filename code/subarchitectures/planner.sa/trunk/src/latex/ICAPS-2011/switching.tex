...
$\pp{R2D2}$ $s_0 \equiv \{(= (is-in R2D2) kitchen)\}$.
...
In the decision-theoretic mode, the switching planner solves a sensing
...
the achievement of utility according to the given objectives. The
process in this mode has a different reward model that reflects the
value of performing sensing actions in the context of a trace proposed
for the preceding sequential deliberation. In detail, for each
$\assumptiveS{i}$ action scheduled in the current sequential plan, we
have a {\em dual} $\assumptiveDT{i}$ so that
...
rewards the agent for correctly jettisoning a false assumption, and
otherwise penalises the agent. Propositionally these dual actions have
no preconditions or effects -- I.e., $\forall\state\in\states$ $\poss(\assumptiveDT{i})
\subseteq \state$ and $\assumptiveDT{i}(\state) \equiv \state$.  Additionally, for the action,
$\action_i$, whose scheduled execution triggered the switch to the
decision-theoretic mode, we include an action
$\actions.\poss(\action_i)$ with the reward property:
...
\noindent Execution of either a $\assumptiveDT{i}$
action, or $\actions.\poss(\action_i)$, returns control to the
sequential planner. If a dual is executed, then the sequential planner
must replan with $\bstate_0$ equal to the current belief. Otherwise,
if $\actions.\poss(\action_i)$ is executed, then the current
sequential plan is executed until further sensing is scheduled, or to
completion. In our project the rewards, $\$:2^\propositions\to\Re$,
for the POMDP posed to decision-theoretic planning is sourced from a
motivational component of our robotic architecture.
...
current sequential plan. For example, if $\pp{R2D2}$'s plan is:
...
\noindent At this point, decision-theoretic planning would proceed in a cupless
abstraction of the environment, where the inclusion (exclusion) of
POMDP actions is the same as we defined it for the sequential mode.
