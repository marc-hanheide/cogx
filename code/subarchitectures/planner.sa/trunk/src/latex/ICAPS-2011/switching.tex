...
actions in the current sequential plan. Therefore, this mode uses a
different utility model, that reflects the value of performing sensing
actions.

We construct a {\em relaxed} $(\pp{:init})$ block by first
...
\[
\begin{array}{c}
\actions(.8,(=(\pp{is-in}~\pp{box})\pp{office}));\\
(pp{reportlook-for-object}~\pp{R2D2}~\pp{box}~\pp{office});\\
...
In the second step, we iteratively add terms from the original
$(\pp{:init})$ block while the resulting state space remains of a
practicable size. Of course, it is expensive to measure size exactly,
thus in practice we estimate it to be $2^{|\propositions^\#|}$, where
$\propositions^\#$ is the set of propositions in the abstraction
induced by the proposed tree. Terms are proposed for addition, because
they are probabilistic, and because they were omitted in the {\em
relaxed} visitation characterised by {\em assumptive} actions in the
sequential plan. Each iteration, we add one candidate term with the
largest number of parent terms.

Terms a proposed sequentially, with preference given to
terms that are closest to, or which otherwise are, the conjunctive
leaves.
