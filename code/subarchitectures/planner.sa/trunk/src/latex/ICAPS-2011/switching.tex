...
\begin{figure}[h!]
  \centering
  \tikzstyle{tree} = [sibling distance=4mm]
  \tikzstyle{toplevel} = [grow'=right, sibling distance=18mm]
  \tikzstyle{seclevel} = [sibling distance=8mm]
  \tikzstyle{pnode} = [diamond, draw=black, minimum size=2mm]
  \tikzstyle{cnode} = [circle, draw=black, minimum size=2.5mm]
  ...
  \end{tikzpicture}}
\qquad
  \subfloat[Remove facts that are not part of an assumption.]{
  ...
  \end{tikzpicture}}
\qquad
  \subfloat[Re-add relevant facts until the belief space limit is reached.]{
...
would proceed in an abstraction of the environment where that does not
contain $\pp{place2}$, the $\pp{cup}$ or a $\pp{kitchen}$. 
...
number of abstract states in $\bstate_0$ that occur with non-zero probability
...
corresponding proposition.  Loosely speaking, the lower the {\em
  conditional entropy} of an atom is, the more information about the
assumptions can the contingent planner expect to derive from it. Thus,
