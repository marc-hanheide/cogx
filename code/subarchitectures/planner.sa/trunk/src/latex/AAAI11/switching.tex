...
abstract problem is created by selecting only those propositions that
are \emph{relevant} to the subproblem the DT session is asked to
solve.
...
To the resulting abstract model we
...
DT session is terminated, and control is returned to the
sequential planner.
...
belief-states by performing belief revision. For that, we transform
the $(\pp{:init})$ declaration into a tree-shaped bayesian network
which gets updated whenever an actions is performed or an observation
is received. That belief-state representation is used:
(1) as the source of candidate determinisations for sequential
planning, (2) in determining when to switch to a DT session, and (3)
as a mechanism to guide construction of an abstract process for DT
sessions.
...
starting state corresponds to an assumption (usually unique) which
...
When an action that requires sensing is encountered, the planner
switches to a decision theoretic session. That session plans in a {\em
  small} abstract processes defined by the action that triggered the
DT session, the assumptive actions in the proceeding trace and the
...
POMDP are retained. Additionally, for each relevant assumptive action
...
assumption. A disconfirm action can only be executed once, and except in the
applicability of that dual corresponds to a self-transformation
%%
If \switchAction\ is the action that switched the system to a DT
session, then an assumption $\assumptiveS{i}$ is {\em relevant} if it
is directly or indirectly required for \switchAction\ to be
applicable. Continuing our simplified example, if the trace is:
...
relevant for this session, and therefore exclude it from the
POMDP posed to the DT session. 

Given \switchAction, we also include another self-transition action
$\actions.\poss(\switchAction)$, the \emph{goal action} with the
reward property:
...
Execution of either a disconfirmation or the goal action returns
control to a sequential session, which then continues from the
underlying belief-state. Turning to the detail of rewards, in our
integrated system these are sourced from a motivational subsystem. In
this paper, for $\assumptiveDT{i}$ actions we set $\$(x)$ to be a
small positive constant, and have $\hat\$(x)= - \$(x)(1 - \prob) /
...
As are aiming to keep the decision theoretic session fast, we try to
keep the state space of the abstract problem as small as possible. In
a deterministic-actio POMDP, the state space size is determined by two
factors: the number of non-zero states in the initial belief-state and
the number of executable actions. We thus want to keep the number of
uncertain facts as low as possible without losing important
information about the sensing goal and keep the number of actions down
without crippling the ability of the DT planner to gather the required
information.

We perform the abstraction of the belief space by first constructing
an $(\pp{:init})$ declaration which only contains relevant assumptions
from the trace -- e.g., (Fig.\ref{fig:abstraction-b}) gives an
example, where diamonds are probabilistic terms, and circles are
atomic and/or conjunctive.
%%
...
We then proceed to limit the actions by only including those that are
possibly required to a) reach the goal or b) to directly observe an
atom that is part of the abstract belief state.

TODO: relaxed planning graph
