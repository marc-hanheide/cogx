...
import manipulation.runner.cogx.ExemplarySolutionRunner;
 ...
 * 
 */
public class ExemplarySolutionLocalMapConnector implements MapConnector {
	 ...
	 *            current manipulator
	 */
	public ExemplarySolutionLocalMapConnector(Manipulator manipulator) {
		this.manipulator = manipulator;

		try {
			localMapInterface = ((ExemplarySolutionRunner) manipulator
					.getRunner()).getIceServer("map.manager",
					LocalMapInterface.class, LocalMapInterfacePrx.class);
	...
	public double getDataFromIndex(Vector2D index) {
		...
		return Double.parseDouble(Character.toString(map.getData()[(int) index
				.getY() + (int) (index.getX()) * map.getySize()]));
	}
	...
	public Vector2D getIndexFromWorldCoordinates(Vector3D data) {
		return getIndexFromWorldCoordinates(new Vector2D(data.getX(),
				data.getY()));
	}
	...
	public void updateMap() throws ExternalMemoryException {
		try {
			...
			allPlaces = ((ExemplarySolutionRunner) manipulator.getRunner())
