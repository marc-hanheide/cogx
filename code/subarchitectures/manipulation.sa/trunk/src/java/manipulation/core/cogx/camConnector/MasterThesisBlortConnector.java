 ...
 * 
 */
public class MasterThesisBlortConnector implements CamConnector {
	 ...
	 *            current manipulator
	 */
	public MasterThesisBlortConnector(Manipulator manipulator) {
	...
	public void visualObjectChanged(WorkingMemoryChange _wmc) {
		try {
			...
			VisualObject changedObject = ((MasterThesisRunner) manipulator
	...
	public void resetTracker() throws ExternalMemoryException {
		...
		String id2 = ((MasterThesisRunner) manipulator.getRunner()).newDataID();

		try {
			((MasterThesisRunner) manipulator.getRunner()).addToWorkingMemory(
					id2, recognizeCommand);
