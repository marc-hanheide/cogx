...
fbase = file("data/WER_NBest5_SeSyAC_R_NP.txt", 'r')
f = file("data/WER_NBest5_SeSyAC_R_b80.txt", 'r')
textbase = fbase.read()
text = f.read()
linesbase = textbase.split("\n")
lines = text.split("\n")
...
for l in linesbase:
	...
	if len(split) == 2:
		...
		for o in recog.split(" "):
			...
			transf2 = transf2 + " "
		recog = transf2.strip()
		examplesbase.append([orig,recog])
...
totaldistancebase = 0
totalnbofwordsbase = 0

WERResulstbase = []
for e in examplesbase[0:len(examples)]:
	if e[1] != "":
		dist = distance(e[0],e[1])
	else:
		dist = 	len(e[0].split(" "))
	totaldistancebase = totaldistancebase + dist
	totalnbofwordsbase = totalnbofwordsbase + len(e[0].split(" "))
	percent = (float(dist) / len(e[0].split(" "))) * 100
	WERResulstbase.append(percent)
	#print "WER result for: \"%s\": %f %%"%(e[0],percent)

finalresultbase = (float(totaldistancebase) / totalnbofwordsbase) * 100
...
for e in examples:
	...
	percent = (float(dist) / len(e[0].split(" "))) * 100
	WERResults.append(percent)
	if dist >2:
		print "Warning: bad WER result for: \"%s\": %f %%"%(e[0],percent)

finalresult = (float(totaldistance) / totalnbofwords) * 100

print
print ">> DIFFERENCES between our approach and the baseline:" 
print "----------------"
for i in range(0,len(examples)):
	if examplesbase[i][1] != examples[i][1]:
		print "Initial: " + examples[i][0]
		print "Base: " + examplesbase[i][1]
		print "Ours: " + examples[i][1]
		print "-------"
print

print "Current number of examples: %i"%len(examples)
print ">>>>> WER partial results for baseline: %f %%"%finalresultbase
print ">>>>> WER partial results for our approach: %f %%"%finalresult
print
