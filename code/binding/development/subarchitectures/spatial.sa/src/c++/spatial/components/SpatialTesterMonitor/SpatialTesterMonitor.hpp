#ifndef SPATIAL_TESTER_MONITOR_H_
#define SPATIAL_TESTER_MONITOR_H_
...
public:
  SpatialTesterMonitor(const std::string &_id);
...
protected:
  ...
  void successExit() {
    const_cast<SpatialTesterMonitor&>(*this).sleepProcess(100); // sleep for a while and test completeness again (10 times)
    if(m_retest++ < 10) {
    ...
    const_cast<SpatialTesterMonitor&>(*this).sleepProcess(1000); // sleep for a while to allow dotviewer to finish
    std::cout << "\nSUCCESS" << std::endl;
