...
public class ActionGetProxyFactory 
	...
	*/ 	
	...
	private ProxyFactoryResults createRelations (PackedNominal nom, PackedLogicalForm plf, TreeMap packedNoms) { 	
		...
		while (relsIter.hasNext()) { 
			...
			} else if (rel.mode.equals("Recipient")) { 
				PendingProxyRelation recipientPPR = new PendingProxyRelation();
				recipientPPR.headNomVar = nom.nomVar;
				recipientPPR.relMode = "Recipient";
				recipientPPR.depNomVar = rel.dep;
				recipientPPR.addContentStatus("intentional");				
				pendingRels.add(recipientPPR); 
