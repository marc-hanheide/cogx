...
package org.cognitivesystems.comsys.processing.examplegeneration;
...
public class GenerationUtils {
	...
	public static void weightHeuristics (CFG cfg) {
		for (Iterator<String> e = cfg.Rules.keySet().iterator() ; e.hasNext() ; ) {
			String key = e.next();
			for (Iterator<Rule> f = cfg.Rules.get(key).iterator() ; f.hasNext() ;) {
				Rule rule = f.next();
	...
	static public String modifyCase (String input) {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (token.trim().equals("hans"))
				token = token.replace("hans", "Hans");
			if (token.trim().equals("dennis"))
				token = token.replace("dennis", "Dennis");
