...
import org.cognitivesystems.comsys.autogen.ComsysEssentials.SpatioTemporalRepresentation;
...
import org.cognitivesystems.comsys.swing.PackedLFVisualizerGUI;
	...
	extends PackedLFProcessorComponent
{
	...
	public PackedLFVisualizer (String _id) { 
		super(_id);
	} 
	...
	*/ 
    ...
    synchronized private void processSpatioTemporalRepresentation (WorkingMemoryChange _wmc) {
    	try {
    		...
    		// with its id
    		CASTData strWM = new CASTData(id,
		...
		if (pdTypes.contains(CASTUtils.typeName(PackedLFs.class))) { 
            ...
            if (data != null) {
				...
				if (incrementalSeries) {
					...
					gui.add(filename,plfs.phonStringLFPairs[0].phon.wordSequence);
