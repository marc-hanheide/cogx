...
import org.cognitivesystems.comsys.util.uttplan.GREHandler;
import org.cognitivesystems.comsys.util.uttplan.UtterancePlanner;
import org.cognitivesystems.comsys.util.uttplan.UtterancePlanningGrammar;
...
public class ContentPlanning 
	...
	implements GREHandler 	{
		 ...
		 * @param _id
		 */
		public ContentPlanning (String _id) {
			super(_id);
		} // constructor/1
		...
		throws ComsysException {
			...
			if (pdTypes.contains(CASTUtils.typeName(ContentPlanningGoal.class))) {
				...
				if (data != null) {
					...
					LogicalForm goalLF = contentPlanningGoal.lf;
					...
					*/ 
					...
					try { 
						ProductionLF plf = ComsysUtils.newProductionLF();
						plf.lf = reduxLF;
		...
		*/ 
		...
		public String produceRFX (Object referent) { 
			...
			greTask.m_intendedReferentProxy = new WorkingMemoryPointer(CASTUtils.typeName(BindingProxy.class), new WorkingMemoryAddress (referentProxy, "unknown.sa"));
			greTask.m_resultLF = "";
			greTask.m_done = false;
			// Set the flag that we are not done yet
			boolean done = false;
			// Add the struct to working memory, make it blocking
			try { 
				String greTaskId = newDataID();
				addToWorkingMemory(greTaskId, greTask, OperationMode.BLOCKING); 
				// Try to retrieve the struct, do that till it's done
				while (!done) { 
					lockEntry(greTaskId, WorkingMemoryPermissions.LOCKED_ODR);
