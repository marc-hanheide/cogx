package org.cognitivesystems.comsys.processing.parseselection;
...
import org.cognitivesystems.repr.lf.autogen.LFPacking.PackedFeature;
...
import org.cognitivesystems.comsys.processing.saliency.*;
...
public class FeatureExtractionFromPLF extends FeatureExtraction {
	...
	public Hashtable<String,FeatureVector> extractAcousticFeatures() {
		...
		for (int i= 0; i < plf.phonStringLFPairs.length; i++) {		
			...
			fv.addFeature(featName1, pair.phon.confidenceValue);
			String featName2 = getNLConfidenceValueFeatName();
			fv.addFeature(featName2, pair.phon.NLconfidenceValue);
			String featName3 = getRankFeatName();
			fv.addFeature(featName3, pair.phon.rank);	
			if(pair.phon.confidenceValue > 50.0) {
				String featName4 = getConfidenceValueHigherThan50FeatName();
				fv.addFeature(featName4, 1.0);
			}
			if (pair.phon.confidenceValue > highestConfValue) {
				highestConfValue = pair.phon.confidenceValue;
				lfIdWithHighestConfValue = lfId;
			}		
			else if (pair.phon.confidenceValue > secondHighestConfValue) {
				secondHighestConfValue = pair.phon.confidenceValue;
			}
		}
		for (int i = 0; i < plf.nonParsablePhonStrings.length; i++) {
	...
	//		log("Contextually activated word added: " + entity.getConcept());
		...
		for (int i= 0; i < plf.phonStringLFPairs.length; i++) {
			...
			String lfId = pair.logicalFormId;
			FeatureVector fv = featureVectors.get(lfId);
			StringTokenizer tokenizer = new StringTokenizer(pair.phon.wordSequence);
