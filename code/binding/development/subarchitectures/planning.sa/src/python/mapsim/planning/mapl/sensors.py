...
#     def dump_pddl(self, stream, keep_assertions):
#         actions = [self.build_sensing_action(keep_assertions, negative=False)]
#         if self.negcondition:
#             actions.append(self.build_sensing_action(keep_assertions, negative=True))
#         print >>stream, "\n".join(actions)

#     def build_sensing_action(self, keep_assertions, negative=False):
#         sensorpref = PDDL_SENSOR_PREFIX
#         name = self.name
#         paramdecl = " ".join(map(types.pddl_str, self.params))
#         replan = ""
#         if keep_assertions:
#             replan = self.replan_condition.pddl_str(indent="    ")
#         precond = "\n" + self.precondition.pddl_str(indent="    ")
#         names = [a.name for a in self.agents]
#         eff = conditions.ModalLiteral(DIRECT_K_PREFIX, names, self.svar).pddl_str()
#         if negative:
#             negpref = PDDL_NEG_SENSOR_PREFIX
#             negcond = "\n" + self.negcondition.pddl_str(indent="    ")
#             template = neg_sensor_template
#         else:
#             negpref = ""
#             negcond = ""
#             template = sensor_template
#         s = template.substitute(locals())
#         return s
    ...
    def dump_pddl(self, stream, keep_assertions):
        ...
        for action in actions:
            action.dump_pddl(stream, keep_assertions)
        
    def build_sensing_action(self, negative):
        pref = PDDL_SENSOR_PREFIX
        precondition = self.precondition.make_conjunction()
        names = [a.name for a in self.agents]
        #eff = effects.Effect([], conditions.Truth(), conditions.ModalLiteral(DIRECT_K_PREFIX, names, self.svar))
        #eff_str = conditions.ModalLiteral(K_PREFIX, names, self.svar).mapl_str()
        eff_str = "(K %s (%s))" % (" ".join(names), self.svar)
