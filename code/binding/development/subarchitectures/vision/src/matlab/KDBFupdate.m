      ...
      if newFb==oldFb %best feature still the same
         ...
         if ismember(mC1(i).name,C(:,1)) %current feature represents this concept
            ...
            if Fns(i)<=2
               ...
               %pdf1 = updateKDE(mC(i), 0, F(newFb), Fvars(i,newFb), Fns(i), [], [], C_sensor) ;
               if Fns(i)<4 %smooth first few variances
                  bw=getSilvermanBWfromGaussian(Fvars(i,newFb),Fns(i));
                  if bw==0
                     bw=1e-6;
                  end;
                  numg=length(pdf1.weights);
                  pdf1.covariances=ones(numg,1)*bw;
                  pdf1.weights=ones(1,numg)/numg;
               end;
            end
...
%             pdf1.components=mC(i).components;
         ...
         %disp(['!!!!! i=' num2str(i) ' C=' num2str(mC1(i).name) ' Fb=' num2str(newFb) ' var=' num2str(Fvars(i,newFb)) ' comp=' num2str(Fns(i))]);
  ...
  %    pdf1.covariances(isnan(pdf1.covariances))=.01;
   %   pdf1.covariances(isinf(pdf1.covariances))=.01;
      
%       if max(max(isnan(pdf1.covariances)))
%    1
% end

      
%       %COMPRESS if necessary
%       if ( COMP && length(mC1(i).weights) > nMaxComponents )
