...
//    fprintf(modelfl,"SVM-light Version %s\n",VERSION);
  ...
  fwrite(&(model->A), sizeof(double),1,modelfl);
...
#if DENSE
  fwrite(&model->maxNormalized,sizeof(bool), 1, modelfl);  
  for (j=0; j < model->totwords; ++j) {
	  fwrite(&(model->maxNormalizer[j]),sizeof(FVAL),1,modelfl);
   }
#else
...
{// {{{
  ...
  MODEL *model= new MODEL(); //(MODEL *)my_malloc(sizeof(MODEL));
...
//    }
  ...
  if(version >= 201) {
#else
  if(version >= 101 && version < 200) {
#endif 
  	fread(&(model->A),sizeof(double),1,modelfl);
  ...
  if(model->kernel_parm.kernel_type == SVMLight_ns::LINEAR) { /* linear kernel */
  ...
  if(version >= 202) {	  
	  fread(&model->maxNormalized, sizeof(bool), 1, modelfl);	  
	 
	  for (j=0; j < model->totwords; ++j) {
		  fread(&(model->maxNormalizer[j]), sizeof(FVAL),1,modelfl);
	  }    
  } else {	 
	  for (j=0; j < model->totwords; ++j) {
		  model->maxNormalizer[j] = 0;
	  }
	  model->maxNormalized = false;
  } 
...
void lin_model_vec(MODEL * model, std::vector<float> & vref){
  if((model->kernel_parm.kernel_type == 0) && (vref.size() == model->totwords + 1)) {
  // vector<vector <float> > = (2, vector<float> (2,2));
    for(int i=0; i<model->totwords; i++)
      vref.at(i) = static_cast<float>(model->lin_weights[i]);
    vref.at(model->totwords) = static_cast<float>(model->b); 
  } else {
    fprintf(stderr,"ERROR: linear SVM model expected (typeid is %d) and properly sized vector ref\n ", model->kernel_parm.kernel_type);
    exit(1);
  }
}
/*void lin_model_vec(MODEL * model,  std::vector<double> & vref){
  if((model->kernel_parm.kernel_type == 0) && (vref.size() == model->totwords + 1)) {
    // vector<vector <float> > = (2, vector<float> (2,2));
    for(int i=0; i<model->totwords; i++)
       vref.at(i) = static_cast<double>(model->lin_weights[i]);
    vref.at(model->totwords) = static_cast<double>(model->b);
  } else {
     fprintf(stderr,"ERROR: linear SVM model expected (typeid is %d) and properly sized vector ref\n ", model->kernel_parm.kernel_type);
     exit(1);
  }
