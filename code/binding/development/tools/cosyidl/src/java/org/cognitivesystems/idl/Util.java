/*
 * @(#)Util.java 1.13 03/12/19 Copyright 2004 Sun Microsystems, Inc. All
 * rights reserved. SUN PROPRIETARY/CONFIDENTIAL. Use is subject to
 * license terms.
 */
/*
 ...
 * with IBM Corp. @(#)Util.java 1.13 03/12/19
 */
...
import org.cognitivesystems.idl.som.cff.FileLocator;
...
public class Util {
     ...
     * Framework from the appropriate properties file.
     * 
     * @return the version number contained within the appropriate
     *         properties file, which indicates the build of this IDL
     *         Parser Framework.
     */
    ...
    /**
     * Fetch the version number of this build of the IDL Parser
     * Framework. This method may be called before or after the
     * framework has been initialized. If the framework is
     * inititialized, the version information is extracted from the
     * message properties object; otherwise, it is extracted from the
     * indicated messages file.
     * 
    ...
    public static String getVersion(String filename) {
        ...
        if (messages == null) // Use supplied file
        {
            ...
            if (filename == null || filename.equals(""))
                filename = "org/cognitivesystems/idl/idl.prp";
            filename = filename.replace('/', File.separatorChar);
            registerMessageFile(filename);
            version = getMessage("Version.product",
    ...
    public static boolean isPrimitive(String name, Hashtable symbolTable) {
        // Distinguish "string" because the name could be something
        // like:
        // string(25 + 1)
        SymtabEntry entry = (SymtabEntry) symbolTable.get(name);
        if (entry == null) {
            ...
            if (parenIndex >= 0)
                entry = (SymtabEntry) symbolTable.get(name.substring(0,
                    parenIndex));
        }
    ...
    public static String getMessage(String key, String fill) {
        ...
        if (message == null)
            ...
            if (index >= 0)
                message = message.substring(0, index) + fill
    ...
    public static String getMessage(String key, String[] fill) {
        ...
        if (message == null)
            ...
            for (int i = 0; i < fill.length; ++i) {
                ...
                if (index >= 0)
                    message = message.substring(0, index) + fill[i]
                        + message.substring(index + 2);
            }
        return message;
    } // getMessage
     ...
     * by Larry Raper of the Shasta team. static File findFile (String
     * name) throws FileNotFoundException { String classpath =
     * System.getProperty ("java.class.path"); String separator =
     * System.getProperty ("path.separator"); int end =
     * -separator.length (); // so the first pass classpath == original
     * classpath File file; do { classpath = classpath.substring (end +
     * separator.length ()); end = classpath.indexOf (separator); if
     * (end < 0) end = classpath.length (); file = new File
     * (classpath.substring (0, end) + File.separator + "com" +
     * File.separator + "ibm" + File.separator + "idl" + File.separator +
     * name); } while (!file.exists () && end != classpath.length ());
     * if (!file.exists ()) throw new FileNotFoundException (); return
     * file; } // findFile
     */
    ...
    private static void readMessages() {
        ...
        while (fileList.hasMoreElements()) {
            ...
            try {
                ...
                stream = FileLocator
                    .locateLocaleSpecificFileInClassPath(file);
    ...
    /**
     * Register a message file. This file will be searched for in the
     * CLASSPATH.
     */
    public static void registerMessageFile(String filename) {
        if (filename != null)
            if (messages == null)
                ...
                try {
                    DataInputStream stream = FileLocator
                        .locateLocaleSpecificFileInClassPath(filename);
                    messages.load(stream);
                }
                catch (IOException e) {}
    } // registerMessageFile
    ...
    static {
        msgFiles.addElement("org/cognitivesystems/idl/idl.prp");
    }
     ...
     * a given short file name and returns its absolute file
     * specification.
     * 
     * @return Absolute file name of a given short filename
     * @throws FileNotFoundException
     *             The file does not exist in the current user or
     *             specified directories.
     * @see java.io.File.getAbsolutePath
     */
    public static String getAbsolutePath(String filename,
            Vector includePaths) throws FileNotFoundException {
        ...
        if (file.canRead())
            ...
            while (!file.canRead() && pathList.hasMoreElements()) {
                fullname = (String) pathList.nextElement()
    ...
    // <d62023>
    /**
     * Compute the absolute value of the difference between two
     * floating-point numbers having single precision.
     * 
