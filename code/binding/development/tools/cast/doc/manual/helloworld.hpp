...
<code>hello-world/config/helloworld.cast</code> and enter the following lines:
...
terminals and change directory to $TUTORIAL_ROOT in each of them. In the
first terminal, run the <code>cast-server</code> executable. This should give you some
...
Next we need to create an instance of <code>Announce</code> to write
...
under <code>src/java/helloworld</code>. Next, edit <code>HelloWriter.java</code> to contain:
