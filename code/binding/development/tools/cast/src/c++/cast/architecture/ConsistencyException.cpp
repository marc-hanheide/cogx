...
#include "ConsistencyException.hpp"
#include <cstdarg>
 ...
 */
ConsistencyException::ConsistencyException(const cast::cdl::WorkingMemoryAddress& _wma, 
					   const char *file, 
					   const char *function, 
					   int line,
  ...
  : WMException(_wma)
{
  ...
  va_end(arg_list);
  snprintf(msg, 1024, "ConsistencyException: %s:%s:%d: %s", file, function, line, what);
  _what = msg;
}
