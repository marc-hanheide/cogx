...
#include "DoesNotExistOnWMException.hpp"
#include <cstdarg>
 ...
 */
DoesNotExistOnWMException::DoesNotExistOnWMException(const cast::cdl::WorkingMemoryAddress& _wma, 
						     const char *file, 
						     const char *function, 
						     int line,
  ...
  : WMException(_wma)
{
  ...
  va_end(arg_list);
  snprintf(msg, 1024, "DoesNotExistOnWMException: %s:%s:%d: %s", file, function, line, what);
  _what = msg;
}
