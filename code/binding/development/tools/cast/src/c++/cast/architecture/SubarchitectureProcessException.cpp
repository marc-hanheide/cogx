 ...
 */
SubarchitectureProcessException::SubarchitectureProcessException(const char *file, 
								 const char *function, 
								 int line,
  ...
  va_end(arg_list);
  snprintf(msg, 1024, "SubarchitectureProcessException: %s:%s:%d: %s", file, function, line, what);
  _what = msg;
}
