...
#include "CASTWorkingMemoryInterface.hpp"
...
namespace cast {
  ...
  typedef StringMap<boost::shared_ptr<CASTWorkingMemoryItem> >::map WMItemMap;
  ...
  public:
    ...
    virtual void getByType(const std::string & _type, 
			   const int & _count,
			   std::vector< boost::shared_ptr<CASTWorkingMemoryItem> > & _items);
     ...
     */
    virtual boost::shared_ptr<CASTWorkingMemoryItem> remove(const std::string &  _id);
    ...
    virtual boost::shared_ptr<CASTWorkingMemoryItem> get(const std::string &  _id);
