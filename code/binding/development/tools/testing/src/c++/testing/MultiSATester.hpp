...
class MultiSATester : public PrivilegedManagedProcess, public WorkingMemoryChangeReceiver
{
  public:
    MultiSATester(const std::string &_id);
  ...
  protected:
    ...
    void successExit() const {
      const_cast<MultiSATester&>(*this).sleepProcess(2000); // sleep for a while to allow dotviewer to finish
      log("\nSUCCESS\n");
