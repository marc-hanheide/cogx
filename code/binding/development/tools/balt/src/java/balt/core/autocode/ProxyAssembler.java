...
class ProxyAssembler {
    ...
    public static String getSig(Class t) {
        if (t == null) {
            ...
            if (false) {
                ...
                Class a = java.lang.reflect.Array.newInstance(t, 0)
                    .getClass();
    ...
    AMember addMember(int mods, String sig, String name) {
    ...
    public Object addMember(int mods, Class rtype, String name,
            Class ptypes[]) {
        AMember m = addMember(mods, getSig(rtype, ptypes), name);
    ...
    public Object addMember(int mods, Class type, String name) {
        return addMember(mods, type, name, null);
    }
    ...
    private final static int opc_iconst_0 = 3, opc_bipush = 16,
            opc_sipush = 17, opc_ldc = 18, opc_ldc_w = 19,
            opc_ldc2_w = 20, opc_aaload = 50, opc_aastore = 83,
            opc_dup = 89, opc_getfield = 180, field_put = 1,
            field_static = -2, opc_invokevirtual = 182,
            opc_invokespecial = 183, opc_invokestatic = 184,
            opc_invokeinterface = 185, opc_new = 187,
            opc_newarray = 188, opc_anewarray = 189, opc_aload = 25,
            opc_aload_0 = 42, opc_wide = 196, opc_areturn = 176,
            opc_return = 177, opc_checkcast = 192, kind_a = 0,
            kind_i = -4, kind_l = -3, kind_f = -2, kind_d = -1,
            kind_b = 1, kind_c = 2, kind_s = 3;
    ...
    public byte[] internalGetCode() throws IOException {
        ...
        for (int i = 0; i < cv.size(); i++) {
            ...
            if (x == null) {
                ...
                short nt = getNTIndex(m.getName(), getSig(m.getType()));
                short data[] = {
                    CONSTANT_FIELD, ci, nt
                };
                x = data;
            }
            else if (x instanceof Constructor) {
                Constructor m = (Constructor) x;
                short ci = getIndex(m.getDeclaringClass());
                short nt = getNTIndex("<init>", getSig(Void.TYPE, m
                    .getParameterTypes()));
                short data[] = {
                    CONSTANT_METHOD, ci, nt
                };
                x = data;
            }
                ...
                short kind = c.isInterface() ? CONSTANT_INTERFACEMETHOD
                    : CONSTANT_METHOD;
                short ci = getIndex(c);
                short nt = getNTIndex(m.getName(), getSig(m
                ...
                short data[] = {
                    CONSTANT_NAMEANDTYPE, nt.name, nt.sig
                };
                x = data;
            }
    ...
    private byte[] getMethodCode(AMember m, ByteArrayOutputStream code)
            throws IOException {
        if (code.size() == 0) {
        ...
        ByteArrayOutputStream bytes = new ByteArrayOutputStream(code
            .size() + 30);
        DataOutputStream ds = new DataOutputStream(bytes);
        int slop = 10; // ??
    ...
    private void writeAttrs(DataOutputStream ds, Vector attrs)
            throws IOException {
    ...
    private static final short CONSTANT_UTF8 = 1, CONSTANT_UNICODE = 2,
            CONSTANT_INTEGER = 3, CONSTANT_FLOAT = 4,
            CONSTANT_LONG = 5, CONSTANT_DOUBLE = 6, CONSTANT_CLASS = 7,
            CONSTANT_STRING = 8, CONSTANT_FIELD = 9,
            CONSTANT_METHOD = 10, CONSTANT_INTERFACEMETHOD = 11,
