...
NavData::FNodePtr
ExplorationTester::getCurrentNavNode()
{
...
ExplorationTester::runComponent()
{
  ...
  rv->addChangeFilter(
      createLocalTypeFilter<NavData::RobotPose2d>(cdl::ADD)); // local
  rv->wait();
  ...
  while(isRunning()){
    ...
    lockComponent();
    for (map<string, SpatialData::PlacePtr>::iterator it = 
	m_places.begin(); it != m_places.end(); it++) {
	...
	if (predecessor[placeID] >= 0 && !fromSearched[placeID]) {
	  for (map<int, SpatialData::PlacePtr>::iterator it2 = places.begin();
	      !found && it2 != places.end(); it2++) {
		...
		req = doTransitionQuery(placeID, toPlaceID);
	      if (req) {
	...
	if (successor[toPlaceID] >= 0 && !toSearched[toPlaceID]) {
	  for (map<int, SpatialData::PlacePtr>::iterator it2 = places.begin();
	    ...
	    if (predecessor[fromPlaceID] >= 0 && !fromSearched[fromPlaceID]) {
	      ...
	      SpatialData::PathTransitionProbRequestPtr
		req = doTransitionQuery(fromPlaceID, toPlaceID);
	      if (req) {
		if (req->status == SpatialData::QUERYCOMPLETED) {
		  if (req->successProb > 0.5) {
	...
	for (list<int>::iterator it = pathIDs.begin();
	    it != pathIDs.end(); it++) {
	  char buffer[256];
	  sprintf(buffer, " %i", *it);
...
void
ExplorationTester::PlaceChanged(const cdl::WorkingMemoryChange &objID)
{
  SpatialData::PlacePtr place = getMemoryEntry<SpatialData::Place>(objID.address);
...
void
ExplorationTester::placeDeleted(const cdl::WorkingMemoryChange &objID)
{
...
SpatialData::PathTransitionProbRequestPtr
ExplorationTester::doTransitionQuery(int startPlaceID, int goalPlaceID)
{
  Rendezvous *rv = new Rendezvous(*this);
