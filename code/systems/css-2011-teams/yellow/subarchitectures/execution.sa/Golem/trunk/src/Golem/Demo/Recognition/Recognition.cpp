 ...
 * 
 * Program demonstrates 3D hierarchical recognition algorithm.
 * 
...
protected:
	...
	virtual void run(int argc, char *argv[]) {
		...
		U32 n = 0;
		PerfTimer t0;
		golem::RetinaOut retinaOut;
		golem::RecognitionOut recognitionOut;
		...
		while (!universe()->interrupted()) {
			...
			SecTmReal timeStamp;
			if (!robot->read(recognitionOut, retinaOut, timeStamp))
				continue;
