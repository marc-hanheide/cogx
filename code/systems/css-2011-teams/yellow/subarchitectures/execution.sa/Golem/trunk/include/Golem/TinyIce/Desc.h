...
inline void setToDefault(GenWorkspaceCoord& c) {
	setToDefault(c.pos);
	setToDefault(c.vel);
}
CLASS_INIT_IMPL(GenWorkspaceCoord)

inline void setToDefault(GenWorkspaceState& s) {
	setToDefault(s.pos);
	setToDefault(s.vel);
	s.t = (::Ice::Double)0.;
}
CLASS_INIT_IMPL(GenWorkspaceState)

inline void setToDefault(ConfigspaceCoord& c) {
	c.c.clear();
}
CLASS_INIT_IMPL(ConfigspaceCoord)

inline void setToDefault(GenConfigspaceCoord& c) {
	setToDefault(c.pos);
	setToDefault(c.vel);
}
CLASS_INIT_IMPL(GenConfigspaceCoord)

inline void setToDefault(GenConfigspaceState& s) {
	setToDefault(s.pos);
	setToDefault(s.vel);
	s.t = (::Ice::Double)0.;
}
CLASS_INIT_IMPL(GenConfigspaceState)
...
inline void setToDefault(ArmDesc& desc) {
	setToDefault((ActorDesc&)desc);
}
CLASS_INIT_IMPL(ArmDesc)
...
inline void setToDefault(KatanaArmDesc& desc) {
	setToDefault((ArmDesc&)desc);
	desc.bGripper = false;
