...
KatanaJoint::KatanaJoint(Arm* arm) : pArm(arm), pContext(&arm->getContext()) {
}
	
bool KatanaJoint::create(const Desc& desc) {
	if (!desc.isValid())
		throw MsgJointInvalidDesc(Message::LEVEL_CRIT, "KatanaJoint::create(): Invalid description");
...
KatanaArm::KatanaArm(Context* context) : pContext(context) {
}

KatanaArm::~KatanaArm() {
}

bool KatanaArm::create(const Desc& desc) {
	if (!desc.isValid())
		throw MsgArmInvalidDesc(Message::LEVEL_CRIT, "KatanaArm::create(): Invalid description");
...
void KatanaArm::forwardTransform(Mat34& trn, const ConfigspaceCoord& cc) {
...
void KatanaArm::velocitySpatial(Twist& v, const ConfigspaceCoord& cc, const ConfigspaceCoord& dcc) {
...
void KatanaArm::jacobianSpatial(Jacobian& jac, const ConfigspaceCoord& cc) {
	Real s1, c1, s2, c2, s3, c3, s4, c4, s5, c5;
...
bool KatanaGripper::create(const Desc& desc) {
	if (!desc.isValid())
		throw MsgArmInvalidDesc(Message::LEVEL_CRIT, "KatanaGripper::create(): Invalid description");
