...
bool BoundingPlane::create(const Desc& desc) {
	if (!desc.isValid())
		return false;
...
bool BoundingPlane::create(const Vec3& p0, const Vec3& p1, const Vec3& p2) {
...
void BoundingPlane::transform() {
	pose.R.multiply(normal, normalBase);
...
bool BoundingSphere::create(const Desc &desc) {
	if (!desc.isValid())
		return false;
...
bool BoundingSphere::intersect(const Vec3& p) const {
	if (getPosition().distanceSquared(p) < radius*radius)
		return true;
	return false;
}
...
bool BoundingCylinder::Desc::createTriangleMesh(TriangleMesh& mesh) const {
	if (!isValid())
		return false;
...
bool BoundingCylinder::create(const Desc &desc) {
	if (!desc.isValid())
		return false;
	...
	if (!desc.createTriangleMesh(data[0]))
		return false;
...
bool BoundingCylinder::intersect(const Vec3& p) const {
	...
	if (tmp.magnitudeSquared() >= radius*radius)
		return false;
...
Real BoundingCylinder::getSurfaceDistance(const Vec3& p) const {
	...
	d = tmp.magnitudeSquared() - radius*radius;
	if (distance < d)
		distance = d;
	d = dn - distance;
	if (distance < d)
...
void BoundingBox::create() {
	...
	normals[0].set(REAL_ONE, REAL_ZERO, REAL_ZERO);
	pose.R.multiply(normals[0], normals[0]);
	...
	normals[1].set(REAL_ZERO, REAL_ONE, REAL_ZERO);
	pose.R.multiply(normals[1], normals[1]);
	...
	normals[2].set(REAL_ZERO, REAL_ZERO, REAL_ONE);
	pose.R.multiply(normals[2], normals[2]);
	...
	edges[0].multiplyAdd( - dimensions.v1, normals[0], pose.p);
	edges[2] = edges[0];
	edges[0].multiplyAdd( - dimensions.v2, normals[1], edges[0]);
	edges[2].multiplyAdd( + dimensions.v2, normals[1], edges[2]);		
	edges[4] = edges[0];
	edges[0].multiplyAdd( - dimensions.v3, normals[2], edges[0]); // 0 ---
	edges[4].multiplyAdd( + dimensions.v3, normals[2], edges[4]); // 4 --+
	edges[6] = edges[2];
	edges[2].multiplyAdd( - dimensions.v3, normals[2], edges[2]); // 2 -+-
	edges[6].multiplyAdd( + dimensions.v3, normals[2], edges[6]); // 6 -++

	edges[1].multiplyAdd( + dimensions.v1, normals[0], pose.p);
	edges[3] = edges[1];
	edges[1].multiplyAdd( - dimensions.v2, normals[1], edges[1]);
	edges[3].multiplyAdd( + dimensions.v2, normals[1], edges[3]);
	edges[5] = edges[1];
	edges[1].multiplyAdd( - dimensions.v3, normals[2], edges[1]); // 1 +--
	edges[5].multiplyAdd( + dimensions.v3, normals[2], edges[5]); // 5 +-+
	edges[7] = edges[3];
	edges[3].multiplyAdd( - dimensions.v3, normals[2], edges[3]); // 3 ++-
	edges[7].multiplyAdd( + dimensions.v3, normals[2], edges[7]); // 7 +++
}

void BoundingBox::createFrame(Vec3& xb, Vec3& yb, const Vec3& zb) const {
	// base frame 'a'
	const Vec3 xa(REAL_ONE, REAL_ZERO, REAL_ZERO);
	const Vec3 ya(REAL_ZERO, REAL_ONE, REAL_ZERO);
	//const Vec3 za(REAL_ZERO, REAL_ZERO, REAL_ONE);	
...
bool BoundingBox::create(const Desc &desc) {
	if (!desc.isValid())
		return false;
...
void BoundingBox::combine(const BoundingConvexMesh& mesh) {
	combine(&mesh.getVertices().front(), mesh.getVertices().size());
}
...
void BoundingBox::set(const BoundingCylinder& cylinder) {
	...
	// translation vector
	pose.p.multiplyAdd(REAL_HALF*cylinder.getLength(), cylinder.getNormal(), cylinder.getPosition());
	// rotation matrix
	Vec3 xb, yb;
	createFrame(xb, yb, cylinder.getNormal());
	pose.R.fromAxes(xb, yb, cylinder.getNormal());
...
bool BoundingConvexMesh::create(const Desc& desc) {
	if (!desc.isValid())
		return false;
	if (!data[0].create(desc))
		return false;
...
#endif
	for (U32 i = 0; i < 8; i++)
		if (plane.getNormal().dot(box.getEdges()[i]) < plane.getDistance())
			return true;
...
bool Bounds::intersect(const BoundingCylinder& cylinder, const BoundingBox& box) const {
	...
	return Bounds::intersect(
		&cylinder.getVertices().front(), (U32)cylinder.getVertices().size(), centre, box.getEdges(), 8, box.getPose().p
	);
}
...
bool Bounds::intersect(const BoundingBox& box0, const BoundingBox& box1) const {
	...
	return Bounds::intersect(
		box0.getEdges(), 8, box0.getPose().p, box1.getEdges(), 8, box1.getPose().p
	);
}
...
bool Bounds::intersect(const BoundingBox& box, const BoundingConvexMesh& mesh) const {
	...
	return Bounds::intersect(
		box.getEdges(), 8, box.getPose().p, &mesh.getVertices().front(), VERTICES, centre
	);
}
