...
GOLEM_LIBRARY_DECLDIR void loadArmDesc(void* pContext, const std::string& path, void* pArmDesc) {
	loadArmDesc<golem::SixAxisSimArm::Desc>((Context*)pContext, path, (Arm::Desc::Ptr*)pArmDesc);
}

//------------------------------------------------------------------------------

SixAxisSimArm::SixAxisSimArm(golem::Context& context) : BufCtrlSimArm(context) {
}

SixAxisSimArm::~SixAxisSimArm() {
}

bool SixAxisSimArm::create(const Desc& desc) {
	L0 = desc.L0;
	L1 = desc.L1;
	L2 = desc.L2;
	L3 = desc.L3;
...
void SixAxisSimArm::forwardTransform(Mat34& trn, const ConfigspaceCoord& cc) const {
	if (!customKinematics) {
...
void SixAxisSimArm::velocitySpatial(Twist& v, const ConfigspaceCoord& cc, const ConfigspaceCoord& dcc) const {
	if (!customKinematics) {
...
void SixAxisSimArm::jacobianSpatial(Jacobian& jac, const ConfigspaceCoord& cc) const {
	if (!customKinematics) {
