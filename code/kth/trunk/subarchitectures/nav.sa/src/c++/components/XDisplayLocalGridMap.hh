...
#ifndef XDisplayLocalGridMap_hh
#define XDisplayLocalGridMap_hh
...
public:
     ...
     * world
     */
    XDisplayLocalGridMap(XLocalGridMap<MAPDATA> &lm);

    ~XDisplayLocalGridMap();
     ...
     */
    void updateDisplay(Cure::Pose3D *robPose = 0,
                       NavGraph *graph = 0,
                       std::list<FrontierPt> *fPts = 0, int m_samplesize = 0, int* samples = 0,
                       std::vector<int> tpoints = 0,std::vector<std::vector<int> > ViewConePts = 0,
...
template <class MAPDATA>
XDisplayLocalGridMap<MAPDATA>::XDisplayLocalGridMap(XLocalGridMap<MAPDATA> &lm)
        :m_Map(lm) {
    ...
    hint.x = 200;
    hint.y = 300;
    hint.width =  2*lm.getSize()+1;
    hint.height = 2*lm.getSize()+1;
...
template <class MAPDATA>
XDisplayLocalGridMap<MAPDATA>::~XDisplayLocalGridMap() {
...
XDisplayLocalGridMap<MAPDATA>::setScreenCenter(double xc, double yc)
{ m_Xc = xc; m_Yc = yc; }
 
template <class MAPDATA> void
XDisplayLocalGridMap<MAPDATA>::getScreenCenter(double &xc, double &yc) const
{ xc = m_Xc; yc = m_Yc; }
*/

template <class MAPDATA>
void
XDisplayLocalGridMap<MAPDATA>::setRobotPose(double x, double y, double a) {
    Cure::Pose3D p;
    p.Position.X[0] = x;
    p.Position.X[1] = y;
    p.setTheta(a);
...
template <class MAPDATA>
void
XDisplayLocalGridMap<MAPDATA>::setRobotPose(const Pose3D &p) {
    m_Xr = p;
    ...
    for (int i = 0; i < 50; i++) {
        ...
        m_Map.worldCoords2Index(m_Xr.Position.X[0] + 0.2 * cos(aStep * i),
                                ...
                                xi, yi);
        m_RobPts[i].x = xi + m_Map.getSize();
        m_RobPts[i].y = m_Map.getSize() - yi;
    }
}

template <class MAPDATA>
void
XDisplayLocalGridMap<MAPDATA>::updateCoverageDisplay() {
...
template <class MAPDATA>
void
XDisplayLocalGridMap<MAPDATA>::updateDisplay(Pose3D *robPose,
        NavGraph *graph,
        std::list<FrontierPt> *fPts, int m_samplesize, int* samples,
        std::vector<int> tpoints,std::vector<std::vector<int> > ViewConePts,
        int highestVCindex)
{
    ...
    for (int i= 0; i < m_samplesize; i++) {
        gridPt.x = samples[2*i] + m_Map.getSize();
        gridPt.y = 2 * m_Map.getSize() - (samples[2*i +1] + m_Map.getSize());
    ...
    /*for (int i=0; i< int(tpoints.size()/2); i++){
    	gridPt.x = tpoints.at(2*i) + m_Map.getSize();
      gridPt.y = 2 * m_Map.getSize() - (tpoints.at(2*i +1) + m_Map.getSize());
      XDrawPoints(disp, pixmap, gcGreen, &gridPt, 1, CoordModeOrigin);
}*/
    ...
    if (graph) {
        int r = int(0.4 / m_Map.getCellSize() + 0.5);

        for (std::list<NavGraphEdge>::iterator ei = graph->
                m_Edges.begin();
                ei != graph->m_Edges.end();
                ei++) {
            NavGraphNode *n1 = ei->getNode1();
            NavGraphNode *n2 = ei->getNode2();
            int i1, i2, j1, j2;
            if (n1 != 0 && n2 != 0 &&
                    m_Map.worldCoords2Index(n1->getX(), n1->getY(), i1, j1) == 0 &&
                    m_Map.worldCoords2Index(n2->getX(), n2->getY(), i2, j2) == 0) {
                i1 = i1 + m_Map.getSize();
                j1 = m_Map.getSize() - j1;
                i2 = i2 + m_Map.getSize();
                j2 = m_Map.getSize() - j2;
        ...
        for (std::list<NavGraphNode*>
                ::iterator ni = graph->m_Nodes.begin();
                ni != graph->m_Nodes.end();
                ni++) {
            int i, j;
            if (m_Map.worldCoords2Index((*ni)->getX(), (*ni)->getY(), i, j) == 0) {
                ...
                if ((*ni)->getType() == Cure::NavGraphNode::NODETYPE_GATEWAY) {
                    ...
                    XFillArc(disp, pixmap, gcGreen, i-r/2, j-r/2, r, r, 0, 64*360);
                }
            }
    ...
    if (fPts) {
        int r = int(0.8 / m_Map.getCellSize() + 0.5);
        for (std::list<FrontierPt>::iterator fi = fPts->
                begin();
                fi != fPts->end();
                fi++) {
            int i, j;
            if (m_Map.worldCoords2Index(fi->getX(), fi->getY(), i, j) == 0) {
                ...
                XFillArc(disp, pixmap, *gc, i-r/2, j-r/2, r, r,
                         int(64*180*(fi->getTheta()-M_PI_2)/M_PI), 64*360/2);
                XDrawLine(disp, pixmap, *gc,
                          i + int(cos(fi->getTheta()-M_PI_2) * halfLen),
                          j - int(sin(fi->getTheta()-M_PI_2) * halfLen),
                          i + int(cos(fi->getTheta()+M_PI_2) * halfLen),
                          j - int(sin(fi->getTheta()+M_PI_2) * halfLen));
            }
        }
    ...
    for (int i = 0; i < ViewConePts.size(); i++) {
        //printf("Displaying triangle with size %i\n", (int)ViewConePts[i].size());
        for (int j=0; j< int(ViewConePts[i].size()/2); j++) {
            gridPt.x = ViewConePts[i][2*j] + m_Map.getSize();
            gridPt.y = 2 * m_Map.getSize() - (ViewConePts[i][2*j + 1] + m_Map.getSize());
            if (i != highestVCindex) {
                //XDrawPoints(disp, pixmap, gcGreen, &gridPt, 1, CoordModeOrigin);
            }
        }
    ...
    int x,y;
    for (int i = 0; i < ViewConePts.size(); i++) {
        gridPt.x = ViewConePts[i][0] + m_Map.getSize();
        gridPt.y = 2 * m_Map.getSize() - (ViewConePts[i][1] + m_Map.getSize());
        x = gridPt.x-3;
        y = gridPt.y-3;
         if (i == highestVCindex) {
